...
public class BackupInstallerFiles extends SimpleInstallerListener {
	...
	public void beforePacks(AutomatedInstallData idata, Integer npacks, AbstractUIProgressHandler handler)
		...
		copyConfigFiles(cobisHome);
		// copy files infrasstructure .xml
		copyPlanConfig(cobisHome);
	}

	private void copyPlanConfig(String cobisHome) {
	...
	private void copyConfigFiles(String cobisHome) {
	...
	private void configureLog(String cobisHome) {
		...
		if (inputStream != null) {
			...
			properties.put("log4j.appender.file.File", cobisHome + File.separator + "cobis-installer.log");
