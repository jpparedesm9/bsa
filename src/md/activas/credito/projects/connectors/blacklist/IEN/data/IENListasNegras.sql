...
/* 2017/11/09				 Paúl Clavijo	   Debe habilitar compresión        */
/********************************************************************************/
...
begin
  print 'YA EXISTE LA CONFIGURACION DEL SERVICIO >> IRegulatoryComplianceProcess << IN'
end
else
begin
	...
	VALUES (@w_sf_id, 'com.cobis.cloud.sofom.riskcompliance.regulationscompliance.batch.process', 'IRegulatoryComplianceProcess', 'transactionType=LSTNG', 'PROCESAMIENTO DE LISTAS NEGRAS', 'LSTNG', 'LSTNG', NULL, 'IN')
end
...
begin
	...
	VALUES ( @w_obj_id + 1, 'idList', '[ns1:cobiscorp.ecobis.cloud.sofom.riskcompliance.regulationscompliance.batch.dto][RegulatoryCompliance%ns1.RegulatoryComplianceResponseDTO%idList]', 'REGULATORYCOMPLIANCERESPONSE' )
...
select @w_cftd_id = isnull(siguiente,0)
 ...
 select @w_cftd_order =1
	...
	VALUES (@w_cftd_id + 1, NULL, @w_ftd_id, 'ID_PERSONA', 'ID PERSONA QUIEN ES QUIEN', 'STRING', 25, ' ', 'NONE', NULL, 1, 'DETAIL', 1, NULL, '[ns1:cobiscorp.ecobis.cloud.sofom.riskcompliance.regulationscompliance.batch.dto][RegulatoryCompliance%ns1.RegulatoryComplianceResponseDTO%idList]', 1, 0 )
...
begin
	...
	VALUES ( @w_jo_id, @w_co_id, 'Descarga Listas Negras', 'DOWNLOAD', '0 30 9 ? * MON-FRI *', NULL, 'JOB', 1, 'B', 'IN', 'LSTNG' )  -- 0 0 18 2 * ? *
	
	select @w_jo_id = @w_jo_id +1
	INSERT INTO cob_ien..ree_ien_jobs ( jo_id, co_id, jo_description, jo_type, jo_cron_expression, jo_service_id, jo_type_cron, jo_has_template, jo_status, jo_in_out, jo_transaction_type ) 
	VALUES (@w_jo_id, @w_co_id, 'Procesamiento Listas Negras', 'PROCESS', NULL, NULL, 'ENTITY', 1, 'B', 'IN', 'LSTNG' )
...
begin
	...
	(
		...
		@w_ente, NULL, 'send', 'receive', 0, 
		1, '0 40 9 ? * MON-FRI *', 'SofomSantander.txt.zip', 'IN', 'B', -- 0 10 18 2 * ? *
		'CFP=3', 0, 0, 'LSTNG', 0, 
		0, 0, NULL, NULL, NULL, 
		'FTP', 'LSTNG', 0, NULL, 0
	)
