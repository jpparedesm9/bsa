...
public class COBISSecurityManager {
    ...
    public static synchronized <T extends CTSServiceResponseTO> boolean initializeSession(
            ICTSServiceIntegration ctsServiceIntegration,
            SessionSecurityKey secKey, T response, String decodedAuth) {
        try {
            ...
            if (secKey.getLogin() != null && secKey.getApplicationId() != null) {
                ...
                if (!isUserAuthenticated(decodedAuth, secKey.getLogin(), secKey.getPassword(), response)) {
                    return false;
                }
...
/*
                try {
					...
					if (logger.isWarningEnabled()) {
						logger.logWarning(
								"Environment variable [ALGN_FILE] not found.",
								e);
					}
				}
...
*/
                ...
                if (!isUserAuthenticated(decodedAuth, externalUserId, password, response)) {
                    return false;
                }
            }
    ...
    private static <T extends CTSServiceResponseTO> Boolean isUserAuthenticated(String decodedAuth, String user, String password, T response) {
        String authenticationInfo[] = decodedAuth.split(":");
        if (user.contentEquals(authenticationInfo[0]) && password.contentEquals(authenticationInfo[1])) {
            return true;
        } else {
            response.setSuccess(false);
            MessageTO errorMessage = new MessageTO();
            errorMessage.setMessage("Authentication error");
            MessageTO[] messages = {errorMessage};
            response.setMessages(messages);
            return false;
        }
