...
public class SpUtil {
	...
	protected static void initialize(IProcedureRequest aProcedureRequest, Request aParameter, String databaseName, String spName, String targetId) {
			...
			for (int i = 1; i <= rs.getMetaData().getColumnsNumber(); i++) {
				...
				if (rs.getData().getRow(j).getRowData(i).getValue() == null || "".equals(rs.getData().getRow(j).getRowData(i).getValue())) {
					...
					if (type == ICTSTypes.SQLDATETIME || type == ICTSTypes.SQLDATETIME4) {
						/*
						 * if you want in code order to custom database date
						 * parser use key:cobiscloudburo.databaseDateFormat
						 */
					...
					if (type == ICTSTypes.SQLMONEY || type == ICTSTypes.SQLMONEY4) {
						...
						// TODO fabad temporal, se debe resolver formateo, por
						// envío de corrección a Bolivia
	...
	protected static void evaluateResponseStatusSP(IProcedureResponse aProcedureResponse) {
		...
		int wMessageNumber;
		// no se necesita evaluar null en responseBlocks ya que el objeto
		// ProcedureResponse siempre los construye en el
		// constructor
	...
	public static Long diffInDayOfDates(Date startDate, Date endDate) {

		if (logger.isDebugEnabled()) {
			logger.logDebug("Start diffDates");
		}
		try {
			if (startDate != null && endDate != null) {
				Calendar starCalendar = Calendar.getInstance();
				starCalendar.setTime(startDate);
				starCalendar.set(Calendar.HOUR_OF_DAY, 0);
				starCalendar.set(Calendar.MINUTE, 0);
				starCalendar.set(Calendar.SECOND, 0);
				...
				Long timeDiff = endCalendar.getTime().getTime() - starCalendar.getTime().getTime();

				return TimeUnit.DAYS.convert(timeDiff, TimeUnit.MILLISECONDS);
			}
		} finally {
			logger.logDebug("Finish diffDates");
		}
		return 0L;
	}
