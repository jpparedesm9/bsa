...
public abstract class BaseReportServlet extends HttpServlet {
	...
	public void generate(HttpServletRequest request, HttpServletResponse response) throws JRException {
		try {
			...
			String tittleReport = request.getParameter("titTleReport");
			logger.logDebug("tittleReport11334 " + tittleReport);
			...
			if (tittleReport != null && !"".equals(tittleReport.trim())) {
				response.setHeader("Content-disposition", "filename=" + tittleReport + ".pdf");
			}
