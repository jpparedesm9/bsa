...
public class CurrencySymbolCatalog extends BaseEvent implements ILoadCatalog {
	
	private static final ILogger logger = LogFactory.getLogger(CurrencySymbolCatalog.class);

	public CurrencySymbolCatalog(ICTSServiceIntegration serviceIntegration) {
		...
		if (response.isResult()) {
			...
			if (resultado.isSuccess()) {
				...
				for (Currency item : currency) {
					...
					itemMoneda.setValue(item.getSimbolo().trim());
