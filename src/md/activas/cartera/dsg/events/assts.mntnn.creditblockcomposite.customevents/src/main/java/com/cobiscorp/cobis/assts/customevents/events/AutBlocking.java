...
public class AutBlocking extends BaseEvent implements IExecuteCommand{

	private static final ILogger logger = LogFactory.getLogger();
	
	public AutBlocking(ICTSServiceIntegration serviceIntegration) {
	...
	public void executeCommand(DynamicRequest entities, IExecuteCommandEventArgs args) {
		...
		BlockCreditService blockCredit = new BlockCreditService(this.getServiceIntegration());
		
		ServiceResponse serviceResponse;
		serviceResponse = blockCredit.authorizeBlocking(loanBankId);
		
		if(serviceResponse != null && serviceResponse.isResult()) {
			args.setSuccess(true);
			args.getMessageManager().showSuccessMsg("Se autorizó existosamente");
		}else {
