...
public class CatalogProductPaymentModal extends BaseEvent implements
		ILoadCatalog {

	private static final ILogger logger = LogFactory
			.getLogger(CatalogProductPaymentModal.class);

	public CatalogProductPaymentModal(ICTSServiceIntegration serviceIntegration) {
	...
	public List<?> executeDataEvent(DynamicRequest entities,
		...
		productListRequest.setLineaCre((String) catalogDataEventArgs
				.getParameters().getCustomParameters().get("creditType"));
		...
		if (serviceResponse.isResult()) {
			...
			if (serviceResponseTO.isSuccess()) {
				Product[] products = (Product[]) serviceResponseTO
