...
@Properties({ @Property(name = ICOBISReportResourcesService.REPORTING_MODULE_FILTER, value = "cartera"), @Property(name = ICOBISReportResourcesService.REPORTING_NAME_FILTER, value = "solicitudCreditoIndividualRevolvente") })
public class SolicitudCreditoIndividualRevolvente implements ICOBISReportResourcesService<JRBeanCollectionDataSource> {
	
	private static final ILogger logger = LogFactory.getLogger(SolicitudCreditoIndividualRevolvente.class);
	...
	public JRBeanCollectionDataSource getDatasourceReport(Map<String, Object> params) {
		logger.logError("----->>>> Reporte Solicitud Credito Individual Revolvente Inicio Sub");
		...
		logger.logError("----->>>> Reporte Solicitud Credito Individual Revolvente - Principal Sub - application:-" + application + "--");
		...
		try {
			...
			logger.logError("----->>>> Reporte Solicitud Credito Individual Revolvente - Principal Sub - loanData:" + loanData);
			if (loanData != null) {
				logger.logDebug("----->>>> Reporte Solicitud Credito Individual Revolvente - Principal Sub - loanData.getOperationId():" + loanData.getOperationId());
				reportResponsePrincipal = service.getDisbursementOrderDetail(sessionId, application, "3", 3, serviceIntegration);
	...
	public Map<String, Object> getParamsReport(Map<String, Object> params) {
		...
		try {
			...
			logger.logDebug("----->>>> Reporte Solicitud Credito Individual Revolvente - Principal - Error:", e);
			throw new RuntimeException("Reporte Solicitud Credito Individual Revolvente - Principal - Error:", e);
		}
