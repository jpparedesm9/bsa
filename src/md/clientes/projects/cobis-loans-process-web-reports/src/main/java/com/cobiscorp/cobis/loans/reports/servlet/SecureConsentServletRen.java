...
@Properties({ @Property(name = ICOBISReportResourcesService.REPORTING_MODULE_FILTER, value = "cartera"),
		@Property(name = ICOBISReportResourcesService.REPORTING_NAME_FILTER, value = "SecureConsentRen") })
public class SecureConsentServletRen implements ICOBISReportResourcesService<JRBeanCollectionDataSource>, AssembleDataBean {

	private static final ILogger logger = LogFactory.getLogger(SecureConsentServletRen.class);
	...
	public String getReportLocation(Map<String, Object> params) {
		return getFileLocationReport(ConstantValue.JasperPath.PATH_SECURE_CONSENT_PRU_REN);
	}
	...
	private String getFileLocationReport(String jasperFile) {
		...
		return ConstantValue.JasperPath.PATH_SECURE_CONSENT_PRU_REN;
	}
	...
	public JRBeanCollectionDataSource getDatasourceReport(Map<String, Object> params) {
		try {
			if (logger.isDebugEnabled()) {
				logger.logError("----->>>> Reporte getDatasourceReport consentimiento seguro ren");
				logger.logDebug("START getDatasourceReport SECURECONSENTSERVLET ren");
			}
			...
			if (logger.isDebugEnabled()) {
				logger.logDebug("SecureConsentServlet ren-getDatasourceReport>>>>>sessionId:" + sessionId);
				logger.logDebug("SecureConsentServlet ren-getDatasourceReport>>>>>identificationclient:" + identificationclient);
				logger.logDebug("SecureConsentServlet ren-getDatasourceReport>>>>idTramite:" + idTramite);
				logger.logDebug("SecureConsentServlet ren-getDatasourceReport>>>>>reportNemonic:" + reportNemonic);
				logger.logDebug("SecureConsentServlet ren-getDatasourceReport>>>>>operation:" + operation);
			}
			...
			for (String clientReport : consultingClient(idTramite, sessionId)) {
				if (logger.isDebugEnabled()) {
					logger.logDebug("SecureConsentServlet ren-getDatasourceReport>>>>Cliente: " + clientReport);
				}
			...
			if (logger.isDebugEnabled()) {
				logger.logDebug("SecureConsentServlet ren-getDatasourceReport>>>>TOTAL register basic" + jasperDataBeanList.size());
			}
			if (jasperDataBeanList.isEmpty()) {
				jasperDataBeanList.add(returnDataEmptyReport());
				logger.logDebug("SecureConsentServlet ren-getDatasourceReport>>>>Register Empty" + jasperDataBeanList.size());
				return new JRBeanCollectionDataSource(jasperDataBeanList, false);
			}
			return new JRBeanCollectionDataSource(jasperDataBeanList, false);
		} catch (Exception e) {
			logger.logError("----->>>> Error Reporte SecureConsentServlet ren-getDatasourceReport>>>>:", e);
	...
	public Map<String, Object> getParamsReport(Map<String, Object> params) {
		logger.logDebug("----->>>> SecureConsentServletRen -getParamsReport ");
		Method method = new Method();
		method.setPrmMain(params);
		try {
			params.put("urlPathWhatsapp", ConstantValue.Params.URLIMAG_WHATSAPP);
		} catch (Exception e) {
			logger.logDebug("Error SecureConsentServletRen -getParamsReport:", e);
			throw new RuntimeException("Error: SecureConsentServletRen -getParamsReport", e);
		}
