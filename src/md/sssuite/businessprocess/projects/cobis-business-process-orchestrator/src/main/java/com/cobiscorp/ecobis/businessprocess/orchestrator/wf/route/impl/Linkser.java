...
import com.cobiscorp.ecobis.businessprocess.orchestrator.wf.route.ILinkser;


@Component(name = "Linkser", immediate = false)
@Service({ com.cobiscorp.ecobis.businessprocess.orchestrator.wf.route.ILinkser.class })
@Properties({ 
	@Property(name = "service.description", value = "Linkser"), 
	@Property(name = "service.vendor", value = "COBISCORP"),
	@Property(name = "service.version", value = "1.0.0"), 
	@Property(name = "service.identifier", value = "Linkser") })

public class Linkser implements ILinkser {
	
	private static final ILogger LOGGER = LogFactory.getLogger(Linkser.class);
	
	private static final String ORCHESTRATOR_NAME = "cob_procesador..sp_autoriza_ltc";
	...
	@Override
	public boolean linkser(String processInstanceIdentifier, String field1, String field2, Integer field3, String field4) throws CTSServiceException, CTSInfrastructureException
	{ 
		if (LOGGER.isDebugEnabled()) {
		 ...
		 if (LOGGER.isDebugEnabled()) {
				LOGGER.logDebug("ProcedureResponse from LinkserOrchestration " + CommonCharacter.convertKParameter(procedureResponse.getProcedureResponseAsString()));
			    }
