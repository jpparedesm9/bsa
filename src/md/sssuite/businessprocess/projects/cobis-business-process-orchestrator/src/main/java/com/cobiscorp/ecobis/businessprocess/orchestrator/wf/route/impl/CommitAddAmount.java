...
import com.cobiscorp.ecobis.businessprocess.orchestrator.wf.route.ICommitAddAmount;
import com.cobiscorp.ecobis.businessprocess.orchestrator.wf.route.ICommitDisbursementLoan;

@Component(name = "CommitAddAmount", immediate = false)
@Service({ com.cobiscorp.ecobis.businessprocess.orchestrator.wf.route.ICommitAddAmount.class })
@Properties({ 
	@Property(name = "service.description", value = "commitAddAmount"), 
	@Property(name = "service.vendor", value = "COBISCORP"),
	@Property(name = "service.version", value = "1.0.0"), 
	@Property(name = "service.identifier", value = "commitAddAmount") })
public class CommitAddAmount implements ICommitAddAmount {

	private static final ILogger LOGGER = LogFactory.getLogger(CommitAddAmount.class);

	private static final String ORCHESTRATOR_NAME = "cob_procesador..commitAddAmount";
	...
	@Override
	public boolean commitAddAmount(String processInstanceIdentifier, String field1, String field2, Integer field3, String field4) throws CTSServiceException, CTSInfrastructureException {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.logDebug("ProcedureResponse from CommitAddAmountOrchestrator " + CommonCharacter.convertKParameter(procedureResponse.getProcedureResponseAsString()));
		    }
