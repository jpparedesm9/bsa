...
public class LoadCatalogEntities extends BaseEvent implements ILoadCatalog {
	private static final ILogger LOGGER = LogFactory.getLogger(LoadCatalogEntities.class);

	public LoadCatalogEntities(ICTSServiceIntegration serviceIntegration) {
	...
	public List<?> executeDataEvent(DynamicRequest arg0, ILoadCatalogDataEventArgs arg1) {
		...
		try {
			...
			for (CatalogDto catalogDto : queryStoredProcedureManagement.getEntities(arg1, new BehaviorOption(true))) {
			...
			ExceptionUtils.showError(ExceptionMessage.BussinessProcess.CATALOG_ENTITIES, e, arg1, LOGGER);
		}
