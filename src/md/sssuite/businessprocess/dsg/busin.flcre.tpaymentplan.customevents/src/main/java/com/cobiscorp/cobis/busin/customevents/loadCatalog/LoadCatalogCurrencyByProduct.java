...
public class LoadCatalogCurrencyByProduct extends BaseEvent implements
		ILoadCatalog {
	private static final ILogger LOGGER = LogFactory
			.getLogger(LoadCatalogCurrencyByProduct.class);

	public LoadCatalogCurrencyByProduct(
	...
	public List executeDataEvent(DynamicRequest entities,
		...
		BankingProductInformationByProduct bankingProductManager = new BankingProductInformationByProduct(
		...
		DataEntity loanHeader = entities.getEntity(LoanHeader.ENTITY_NAME);// en
																			// la
																			// etapa
																			// de
																			// formas
		...
		if (originalHeader != null
			...
			bankingProductID = originalHeader.get(OriginalHeader.PRODUCTTYPE) == null ? ""
					: originalHeader.get(OriginalHeader.PRODUCTTYPE);
		} else if (loanHeader != null
				&& loanHeader.get(LoanHeader.PRODUCTTYPE) != null) {
			bankingProductID = loanHeader.get(LoanHeader.PRODUCTTYPE) == null ? ""
					: loanHeader.get(LoanHeader.PRODUCTTYPE);
		}
		...
		try {
			  ...
			  LOGGER.logDebug(
			  "---->Recupero las monedas configuaradas en el  catalogo");
			  List<ItemDTO> listItemCurrencies = (List<ItemDTO>)
			  catalogManagement.getCurrencyByProduct("", args);
			  
			  LOGGER.logDebug("---->Recupero las monedas configuaradas en el  APF"
			  ); LOGGER.logDebug("---->getCurrencybyProduct------>" +
			  bankingProductID);
			 
			List<CurrencyProductHistory> bankingProducthistoryCurrencies = bankingProductManager
					.getCurrencybyProduct(args, bankingProductID);
			...
			for (ItemDTO currencies : listItemCurrencies) {
				for (CurrencyProductHistory currencyProductHistory : bankingProducthistoryCurrencies) {
					if (currencies
							...
							.trim()
							.equalsIgnoreCase(
									currencyProductHistory.getCurrencyId()
											.trim())) {
			...
			ExceptionUtils.showError(ExceptionMessage.BussinessProcess.PAYMENTPLAN_CATALOG_CURRENCY, e, args, LOGGER);
		}
