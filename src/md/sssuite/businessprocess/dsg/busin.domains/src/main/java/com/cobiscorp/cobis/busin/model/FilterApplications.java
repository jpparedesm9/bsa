package com.cobiscorp.cobis.busin.model;

import java.util.ArrayList;
import java.util.List;
import com.cobiscorp.designer.api.Property;

public class FilterApplications {

	public static final String EN_FLTRAPICS513 = "EN_FLTRAPICS513";
	
	public static final String VERSION = "1.0.0";
	
	public static final String ENTITY_NAME = "FilterApplications";
	
	
	public static final Property<String> OFFICIAL = new Property<String>("Official", String.class, false);
	
	public static final Property<String> FLOWTYPE = new Property<String>("FlowType", String.class, false);
	
	public static final Property<String> ASSIGNINGOFFICIAL = new Property<String>("AssigningOfficial", String.class, false);
	
	public static final Property<Integer> OFFICE = new Property<Integer>("Office", Integer.class, false);
	
	public static final Property<Integer> SUBSIDIARY = new Property<Integer>("Subsidiary", Integer.class, false);
	
	public static final List<Property<?>> getPks() {
		List<Property<?>> pks = new ArrayList<Property<?>>();
		return pks;
	}

}
