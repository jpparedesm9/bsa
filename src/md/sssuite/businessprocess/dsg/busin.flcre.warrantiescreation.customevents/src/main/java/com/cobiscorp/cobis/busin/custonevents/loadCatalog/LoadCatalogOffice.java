...
public class LoadCatalogOffice extends BaseEvent implements ILoadCatalog {

	private static final ILogger LOGGER = LogFactory.getLogger(LoadCatalogOffice.class);

	public LoadCatalogOffice(ICTSServiceIntegration serviceIntegration) {
	...
	public List<?> executeDataEvent(DynamicRequest arg0, ILoadCatalogDataEventArgs arg1) {
		...
		try {
			...
			for (CatalogDto catalogDto : queryStoredProcedureManagement.getOffices(arg1, new BehaviorOption(true))) {
			...
			ExceptionUtils.showError(ExceptionMessage.BussinessProcess.WARRANTIES_CATALOG_OFFICE, e, arg1, LOGGER);
		}
