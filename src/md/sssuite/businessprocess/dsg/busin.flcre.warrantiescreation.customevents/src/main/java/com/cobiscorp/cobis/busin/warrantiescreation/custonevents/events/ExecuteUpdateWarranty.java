...
public class ExecuteUpdateWarranty extends BaseEvent implements IExecuteCommand {
private static ILogger LOGGER = LogFactory.getLogger(ExecuteUpdateWarranty.class);

	
	public ExecuteUpdateWarranty(ICTSServiceIntegration serviceIntegration) {
	...
	public void executeCommand(DynamicRequest entities, IExecuteCommandEventArgs arg1) {
		if (LOGGER.isDebugEnabled())
			LOGGER.logDebug("Inicio ExecuteUpdateWarranty -> updateWarranty....");
		...
		try {
			...
			if (wCustomersList == null || wCustomersList.size() == 0) {
				if (LOGGER.isDebugEnabled())
					LOGGER.logDebug("Error -> ExecuteUpdateWarranty : La garant�a no tiene clientes asociados");
			...
			if (wGeneralData.get(WarrantyGeneral.TRAMITNUMBER) != null){
				numeroTramite = Integer.parseInt(wGeneralData.get(WarrantyGeneral.TRAMITNUMBER));
			}
			
			if (wGeneralData.get(WarrantyGeneral.GUARANTOR) != null){
			...
			if (saveisOk) {   
				int contador = 0;
				if (wCustomersList.size() == 1)
					contador = 1;
				//Asociar clientes a la garant�a
				for (DataEntity wCustomer : wCustomersList) {
					if (LOGGER.isDebugEnabled())
						LOGGER.logDebug("updateWarranty -> Cliente: "+ wCustomer.get(CustomerSearch.CUSTOMERID));				
				...
				saveDataRenderSection(entities,arg1,"N");
			}
			
		} catch (Exception e) {
			ExceptionUtils.showError(ExceptionMessage.BussinessProcess.WARRANTIES_EXECUTE_UPDATE, e, arg1, LOGGER);
		}
		if (LOGGER.isDebugEnabled())
			LOGGER.logDebug("Salida updateWarranty -> ExecuteUpdateWarranty");
	}
			...
			if(valuesFieldByProduct.getDataList().size()>0){
				...
				if (valuesFieldByProduct.size() > 0) {
					for (DataEntity d : valuesFieldByProduct) {
						if (LOGGER.isDebugEnabled()) {
							LOGGER.logDebug("FieldByProductValues"+d.getData());
						}
				...
				if (LOGGER.isDebugEnabled()) {
					...
					LOGGER.logDebug("Ejecución del servicio insertListFieldByProductWarranties, entidad warrantiesGeneral"+warrantiesGeneral.getData());
				}			
				...
				LOGGER.logDebug("TEST_2 " + bankingProductManager);				
