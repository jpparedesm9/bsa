...
public class LoadCatalogProvince extends BaseEvent implements ILoadCatalog {
	private static final ILogger LOGGER = LogFactory.getLogger(LoadCatalogProvince.class);

	public LoadCatalogProvince(ICTSServiceIntegration serviceIntegration) {
	...
	public List<?> executeDataEvent(DynamicRequest entities, ILoadCatalogDataEventArgs args) {
		...
		try {
			...
			for (CatalogDto catalogDto : queryStoredProcedureManagement.getProvince(args, new BehaviorOption(true))) {
			...
			ExceptionUtils.showError(ExceptionMessage.BussinessProcess.WARRANTIES_CATALOG_PROVINCE, e, args, LOGGER);
		}
