...
public class ExecuteSaveWarranty extends BaseEvent implements IExecuteCommand {
	...
	public void executeCommand(DynamicRequest entities, IExecuteCommandEventArgs arg1) {
		...
		try {
			if (LOGGER.isDebugEnabled())
				LOGGER.logDebug("TRAMITE wGeneralData...");
			if (wGeneralData != null) {
				...
				if (wSituation.get(WarrantySituation.CONSTITUTION) != null && wSituation.get(WarrantySituation.CONSTITUTION).after(fechaproceso)) {
					arg1.getMessageManager().showErrorMsg("BUSIN.DLB_BUSIN_ESGEVIDAA_09146");
					arg1.setSuccess(false);
					return;
				} else if (wSituation.get(WarrantySituation.LASTVALUATION) != null && wSituation.get(WarrantySituation.LASTVALUATION).after(fechaproceso)) {
			...
			if (LOGGER.isDebugEnabled())
				LOGGER.logDebug("TRAMITE wCustomersList...");
			...
			if (LOGGER.isDebugEnabled())
				LOGGER.logDebug("TRAMITE NUMBER....");
			...
			int guarantorCode = 0;
			String guarantor = "";
			if (wGeneralData.get(WarrantyGeneral.TRAMITNUMBER) != null && wGeneralData.get(WarrantyGeneral.TRAMITNUMBER) != "") {
			...
			if (valuesFieldByProduct.getDataList().size() > 0) {
				...
				if (valuesFieldByProduct.size() > 0) {
					for (DataEntity d : valuesFieldByProduct) {
						...
						/*
						 * fpv.setField(d.get(com.cobiscorp.cobis.busin.model.
						 * FieldByProductValues.FIELDID).longValue());
						 * fpv.setProduct(d.get(com.cobiscorp.cobis.busin.model.
						 * FieldByProductValues.PRODUCTID));
						 * fpv.setRequest(d.get
						 * (com.cobiscorp.cobis.busin.model.FieldByProductValues
						 * .REQUESTID));
						 * fpv.setValue(d.get(com.cobiscorp.cobis.busin
						 * .model.FieldByProductValues.VALUE));
						 * fpv.setRegisterId
						 * (warrantiesGeneral.get(WarrantyGeneral
						 * .EXTERNALCODE)); valuesList.add(fpv);
						 * bankingProductId=fpv.getProduct(); request =
						 * fpv.getRequest();
						 */
					}
				}
