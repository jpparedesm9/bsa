	...
	public void changed(DynamicRequest entities, IChangedEventArgs args) {
		...
		try {
			...
			for (CatalogDto catalogDto : catalogDtoList) {
				LOGGER.logDebug("catalogDto.getCode()1: " + catalogDto.getCode().trim());
			...
			ExceptionUtils.showError(ExceptionMessage.BussinessProcess.CHANGE_PAYMENT_FREQUENCY, e, args, LOGGER);
		}
