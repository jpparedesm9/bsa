...
public class ItemStatusManagerInterceptor implements BundleContextAware {

	/** COBIS Logger */
	private final ILogger logger = LogFactory
			.getLogger(ItemStatusManagerInterceptor.class);
	...
	/**
	 * The interceptor method executed after the createItemStatusHistory method
	 * 
	 * @param joinPoint
	 *            Execution method context
	 */
	public void createItemStatusHistory(JoinPoint jointPoint) {
		try {
			logger.logDebug(MessageManager.getString(
					"ITEMSTATUSMANAGERINTERCEPTOR.001",
			...
			if (bundleContext != null) {
				...
				if (category.getModule().getModuleId() == Modules.PORTFOLIO
					...
					ServiceReference reference = bundleContext
							.getServiceReference(IPortfolioItemStatusManager.class
									.getName());

					if (reference != null) {

						// Get the implementation instance
						IPortfolioItemStatusManager serviceIntegration = (IPortfolioItemStatusManager) bundleContext
								.getService(reference);

						logger.logDebug(MessageManager.getString(
								"ITEMSTATUSMANAGERINTERCEPTOR.006",
								"createItemStatusHistory", serviceIntegration));

						// Call the method with the correct type
						serviceIntegration.createItemStatusHistory(bph);
					}
				}
			}
			...
			logger.logError(MessageManager.getString(
					"ITEMSTATUSMANAGERINTERCEPTOR.007",
					"createItemStatusHistory"), e);
			...
			logger.logDebug(MessageManager.getString(
					"ITEMSTATUSMANAGERINTERCEPTOR.002",
					"createItemStatusHistory"));
		}
