...
@Entity
@IdClass(OwnWarrantyTmpId.class)
@Table(name = "cr_gar_p_tmp", schema = "cob_credito")
@NamedQueries({ @NamedQuery(name = "OwnWarrantyTmp.getAllPromissoryNotes", query = "select new com.cobiscorp.ecobis.clientviewer.dto.OwnWarrantyTmpDTO(w.client, w.warrantyType, w.warrantyDescription, w.code, w.currency, w.initialValue, w.actualValue, w.actualMLValue, w.percet, w.MRC, sc.description, w.fixedTerm, w.accountType, w.guarantor, w.guarantorId, sc.state, w.clientName, w.activationDate, w.cancelationDate)"
		+ " from OwnWarrantyTmp w left join w.stateWarrantyCustodyOwn sc"
		+ " where w.code > ' ' and w.spidOwn = :spid ORDER BY w.activationDate DESC") })
@NamedNativeQueries({ @NamedNativeQuery(name = "OwnWarrantyTmp.delete", query = "Delete from cob_credito..cr_gar_p_tmp where spid = ?1") })
/**
 * Class used to access the database using JPA
 * related table: cr_gar_p_tmp bdd: cob_credito
 * @author bbuendia
 *
 */
public class OwnWarrantyTmp {
	...
	private StateWarrantyCustody stateWarrantyCustodyOwn;

	public OwnWarrantyTmp() {
	}

	public OwnWarrantyTmp(Integer client, String warrantyType,
			...
			String guarantorId, String stateCode, String clientName) {
		this.client = client;
	...
	public StateWarrantyCustody getStateWarrantyCustody() {
		return stateWarrantyCustodyOwn;
	}

	public void setStateWarrantyCustody(
			StateWarrantyCustody stateWarrantyCustody) {
		this.stateWarrantyCustodyOwn = stateWarrantyCustody;
	}

	public StateWarrantyCustody getStateWarrantyCustodyOwn() {
		return stateWarrantyCustodyOwn;
	}

	public void setStateWarrantyCustodyOwn(
			StateWarrantyCustody stateWarrantyCustodyOwn) {
		this.stateWarrantyCustodyOwn = stateWarrantyCustodyOwn;
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		OwnWarrantyTmp other = (OwnWarrantyTmp) obj;
		if (client == null) {
	...
	public String toString() {
		return "OwnWarrantyTmp [spid=" + spidOwn + ", client=" + client
