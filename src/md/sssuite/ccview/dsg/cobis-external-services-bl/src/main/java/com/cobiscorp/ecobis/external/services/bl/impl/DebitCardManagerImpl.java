...
public class DebitCardManagerImpl extends ServiceBase implements IExternalService {

	private static ILogger logger = LogFactory.getLogger(DebitCardManagerImpl.class);
	...
	public Object executeService(Object... values) {
		...
		try {
			...
			throw new BusinessException(7300040, "Ocurrió un error al consultar las Tarjetas de Débito");
		} finally {
	...
	private List<CardData> getDebitCardsByCustomer(Integer customer) {
		try {
			...
			cardInf.setOperacion("A"); // Operacion A = solo activas.
			...
			logger.logError("Ocurrió un error al consultar las Tarjetas de Débito", e);
