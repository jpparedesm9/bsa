...
public class LoadCatalogProvinces extends BaseEvent implements ILoadCatalog {
	...
	public List<?> executeDataEvent(DynamicRequest entities, ILoadCatalogDataEventArgs args) {
		...
		DataEntity physicalAddress = entities.getEntity(Business.ENTITY_NAME);
		String countryCode = physicalAddress.get(Business.COUNTRY).toString();
		String departmentCode = null;		
		/*if(null!=physicalAddress.get(Business.DEPARTMENT)){
			departmentCode = physicalAddress.get(Business.DEPARTMENT).toString();
		}*/
		try{
