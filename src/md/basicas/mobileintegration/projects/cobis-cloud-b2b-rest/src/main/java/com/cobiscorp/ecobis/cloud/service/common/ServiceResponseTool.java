...
import static com.cobiscorp.ecobis.cloud.service.common.DataTypeTool.convertType;
...
public class ServiceResponseTool {
		...
		if (!response.isResult()) {
			if (response.getMessages() != null) {
				for (Message message : response.getMessages()) {
					message.setMessage(message.getMessage().replaceAll("(\\[.*?\\])", "").replaceAll("&apos;", "'").trim());
				}
			}
		}
	}
	...
	public static <T> Response errorResponse(T data) {
		...
		ServiceResponse serviceResponse = new ServiceResponse();
		serviceResponse.setData(data);
		if (data instanceof ServiceResponse && ((ServiceResponse)data).getData() instanceof ServiceResponseTO) {
			ServiceResponseTO serviceResponseTO = (ServiceResponseTO) ((ServiceResponse)data).getData();
			return Response.serverError().entity(serviceResponseTO.getData()).build();
		}
	...
	public static Map<String, Object> extractOutputMap(ServiceResponse response) {
