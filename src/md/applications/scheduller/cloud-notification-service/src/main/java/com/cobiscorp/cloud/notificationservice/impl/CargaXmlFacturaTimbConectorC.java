...
public class CargaXmlFacturaTimbConectorC extends NotificationGeneric implements Job {
	private static final Logger LOGGER = Logger.getLogger(CargaXmlFacturaTimbConectorC.class);
	private static final String messageOk = "CARGA EXITOSA AL FTP INTERFACTURA FACTURA !!";
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		try {
			...
			LOGGER.error("Error al ejecutar Carga al Ftp Interfactura Factura: ", ex);
	...
	private void loadInsuranceReports() {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Inicia loadInsuranceReports para Carga de xml Factura");
		}

		FileExchangeResponse fileExchangeResponse;

		UploadFileJob uploadFileJob = new UploadFileJob(FileExchangeJob.Job.CAFACCC);
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			while (retry <= uploadFileJob.getRetryUpload()) {
				LOGGER.debug("Intento de carga de Interfactura Factura #" + retry);
				...
				try {
					...
					LOGGER.error("Intento de carga Interfactura Factura1 #" + retry + " fallido.");
