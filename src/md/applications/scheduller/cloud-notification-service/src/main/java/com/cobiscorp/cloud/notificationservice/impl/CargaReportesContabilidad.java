...
public class CargaReportesContabilidad extends NotificationGeneric implements Job {
	private static final Logger LOGGER = Logger.getLogger(CargaReportesContabilidad.class);
	private static final String messageOk = "CARGA EXITOSA ARCHIVOS REPORTES DE CONTABILIDAD FILESHARE!!";
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		try {
			...
			LOGGER.error("Error al ejecutar Carga Reportes Contabilidad FileShare", ex);
		}
	}

	private void cargarArchivos() {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Inicia CargaReportesContabilidad");
		}
		FileExchangeResponse fileExchangeResponse;
		UploadFileJob uploadFileJob = new UploadFileJob(FileExchangeJob.Job.CARCTFS);
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			while (retry <= uploadFileJob.getRetryUpload()) {
				LOGGER.debug("Intento de carga de Reportes contabilidad fileshare #" + retry);
				try {
					if (fileExchangeResponse.isSuccess()) {
					...
					if (retry == uploadFileJob.getRetryUpload()) {
						LOGGER.debug("CargaReportesContabilidad: Error al intentar uploadFiles: " + e.toString());
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			if (to != null && !"".equals(to)) {
				LOGGER.info("CargaReportesContabilidad: Envio mensaje reporte to: " + to);
