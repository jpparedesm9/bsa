...
public class CargaReportesInclusionFinanciera extends NotificationGeneric implements Job {
	private static final Logger LOGGER = Logger.getLogger(CargaReportesInclusionFinanciera.class);
	private static final String messageOk = "CARGA EXITOSA ARCHIVOS REPORTES DE INCLUSION FINANCIERA!!";
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		try {
			...
			LOGGER.error("Error al ejecutar Carga Reportes Inclusion Financiera A FileShare ", ex);
		}
	}

	private void cargarArchivos() {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Inicia CargaReportesInclusionFinanciera");
		}
		FileExchangeResponse fileExchangeResponse;
		UploadFileJob uploadFileJob = new UploadFileJob(FileExchangeJob.Job.CARINF);
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			while (retry <= uploadFileJob.getRetryUpload()) {
				LOGGER.debug("Intento de carga de Reportes Inclusion Financiera fileshare A # " + retry);
				try {
					if (fileExchangeResponse.isSuccess()) {
					...
					LOGGER.error("Intento de carga Inclusion Financiera A # " + retry + " fallido.");
					if (retry == uploadFileJob.getRetryUpload()) {
						LOGGER.debug("CargaReportesInclusionFinanciera: Error al intentar uploadFiles: " + e.toString());
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			if (to != null && !"".equals(to)) {
				LOGGER.info("Envio mensaje reporte CargaReportesInclusionFinanciera to: " + to);
