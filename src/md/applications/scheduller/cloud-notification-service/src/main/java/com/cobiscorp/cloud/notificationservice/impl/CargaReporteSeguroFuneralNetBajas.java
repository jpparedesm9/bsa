...
public class CargaReporteSeguroFuneralNetBajas extends NotificationGeneric implements Job {
	private static final Logger LOGGER = Logger.getLogger(CargaReporteSeguroFuneralNetBajas.class);
	private static final String messageOk = "CARGA EXITOSA DEL ARCHIVO DE REPORTE DE SEGUROS FUNERAL NET BAJAS!!";
	private static final String messageOkExtract = "EXTRACCION EXITOSA DE REPORTES DE SEGUROS FUNERAL NET ALTAS !!";
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		try {
			...
			LOGGER.error("Error al ejecutar Carga de Reporte de Seguros Funeral Net Bajas: ", ex);
	...
	private void downloadDirectDebits() {
		...
		UploadFileJob downloadDirectDebitsJob = new UploadFileJob(FileExchangeJob.Job.CRFNB);
		...
		while (retry <= downloadDirectDebitsJob.getRetryUpload()) {
			LOGGER.debug("Intento de carga de Reporte de Seguros Funeral Net Bajas #" + retry);
