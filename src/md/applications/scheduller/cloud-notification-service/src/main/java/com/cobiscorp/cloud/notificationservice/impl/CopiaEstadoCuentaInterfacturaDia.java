...
public class CopiaEstadoCuentaInterfacturaDia extends Thread implements Job {
	private static final Logger logger = Logger.getLogger(CopiaEstadoCuentaInterfacturaDia.class);
	private String tipoOperacion;
	private CopyFileJob copyFileJob;

	public CopiaEstadoCuentaInterfacturaDia(String tipoOperacion, CopyFileJob copyFileJob) {
		super();
		this.tipoOperacion = tipoOperacion;
		this.copyFileJob = copyFileJob;
	}

	public CopiaEstadoCuentaInterfacturaDia() {
		super();
	}
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		if (logger.isDebugEnabled()) {
			logger.debug("JobName para Copia de archivos de interfacura a la capa web: " + jobExecutionContext.getJobDetail().getName());
		}
		try {

			CopiaEstadoCuentaInterfacturaDia cpxmlRevolvente = new CopiaEstadoCuentaInterfacturaDia("REVOLVENTE", copyFileJob);
			cpxmlRevolvente.start();

			CopiaEstadoCuentaInterfacturaDia cpxmlGrupal = new CopiaEstadoCuentaInterfacturaDia("GRUPAL", copyFileJob);
			cpxmlGrupal.start();

		} catch (Exception ex) {
			logger.error("Exception CopiaEstadoCuentaInterfacturaDia: ", ex);
		}
	...
	public void run() {
		...
		try {
			...
			logger.debug("Tipo de Operacion Interfactura Dia timbrado" + tipoOperacion);
			...
			logger.debug("Estado para el tipo de operacion Interfactura Dia en Timbrado:--> " + timbrarXML + " " + tipoOperacion);

			if (timbrarXML.equalsIgnoreCase("SI")) {
				CopyFileJob copyFileJob = new CopyFileJob(FileJob.Job.CPYIFD);
				try {

					String pattern = obtenerPattern(copyFileJob, tipoOperacion);
					logger.error("pattern que ingresa a buscar Interfactura Dia segun tipo Operacion " + tipoOperacion + ": " + pattern);
					...
					if (createFolder(historicoFolderzip)) {
						...
						while (fileCollection.size() > 0) {
							...
							ResultSet resultParamSec = executeNotificationGeneralParamSec(cn, executeSP, "S", "sb_facturacion");
							...
							if (respcreateZipMasive) {
								executeNotificationGeneralIngFactura(cn, executeSP, "L", nameCompleto, "", "P"); // Para registrar el inicio
								...
								for (int i = 0; i < numberFilesZip; i++) {
									executeNotificationGeneralIngFactura(cn, executeSP, "I", nameCompleto, fileListCollection[i].getName(), "");
								}

								fileCollection = FileUtils.listFiles(pathSourceFolder, new WildcardFileFilter(pattern), null);

								executeNotificationGeneralIngFactura(cn, executeSP, "L", nameCompleto, "", "F"); // Para registrar que finalizo
							} else {
				...
				logger.error("- No se ejecuta ya que el parametro para timbrar xml de Interfactura Dia es : --> " + timbrarXML + "" + tipoOperacion);
			}
	...
	public static ResultSet executeNotificationGeneral(Connection cn, CallableStatement executeSP, String toperation) throws Exception {
		if (logger.isDebugEnabled())
			logger.debug("Ingresa executeNotification General copia XML para Interfactura Dia");
	...
	public static ResultSet executeNotificationGeneralIngFactura(Connection cn, CallableStatement executeSP, String operacion, String archivoZip, String factura, String estado) throws Exception {
		String query = "{ call cob_conta_super..sp_admin_conect_timb(?,?,?,?,?,?,?) }";
		...
		try {
			...
			logger.error("ERROR en executeNotificationGeneralIngFactura", ex);
			throw ex;
		}
	...
	public String obtenerPattern(CopyFileJob copyFileJob, String tipoOpercion) {
		...
		for (int i = 0; i < patterns.length; i++) {
			logger.info("- obtenerPattern copia Xml Interfactura Dia: " + i + " " + patterns[i]);
		}
