...
public class CargaReporteGeolocalizacion extends NotificationGeneric implements Job {
	private static final Logger LOGGER = Logger.getLogger(CargaReporteGeolocalizacion.class);
	private static final String messageOk = "CARGA EXITOSA ARCHIVOS REPORTE GEOLOCALIZACION FILESHARE!!";
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		try {
			...
			LOGGER.error("Error al ejecutar Carga Reporte Geolocalizacion FileShare", ex);
		}
	}

	private void cargarArchivos() {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Inicia CargaReporteGeolocalizacion");
		}
		FileExchangeResponse fileExchangeResponse;
		UploadFileJob uploadFileJob = new UploadFileJob(FileExchangeJob.Job.CARRGE);
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			while (retry <= uploadFileJob.getRetryUpload()) {
				LOGGER.debug("Intento de carga de Reporte Geolocalizacion fileshare #" + retry);
				try {
					if (fileExchangeResponse.isSuccess()) {
					...
					LOGGER.error("Intento de carga reporte Geolocalizacion #" + retry + " fallido.");
					if (retry == uploadFileJob.getRetryUpload()) {
						LOGGER.debug("Geolocalizacion: Error al intentar uploadFiles: " + e.toString());
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			if (to != null && !"".equals(to)) {
				LOGGER.info("Envio mensaje reporte Geolocalizacion to: " + to);
