...
public class CargaReporteSegurosNoCobrados extends NotificationGeneric implements Job {
    private static final Logger LOGGER = Logger.getLogger(CargaReporteSegurosNoCobrados.class);
    private static final String messageOk = "CARGA EXITOSA DEL ARCHIVO DE REPORTE DE SEGUROS NO COBRADOS!!";
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        try {
            ...
            LOGGER.error("Error al ejecutar Carga de Reporte de Seguros No Cobrados: ", ex);
    ...
    private void loadInsuranceReports() {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Inicia loadInsuranceReports");
        }

        FileExchangeResponse fileExchangeResponse;

        UploadFileJob uploadFileJob = new UploadFileJob(FileExchangeJob.Job.CRGRN);
        ...
        if (fileExchangeResponse.isSuccess()) {
            if (FileUtil.existFile(uploadFileJob.getUploadPath() + "\\" + uploadFileJob.getZipFileName())) {
                fileExchangeResponse = FileUtil.unzipFile(uploadFileJob.getUploadPath() + "\\" + uploadFileJob.getZipFileName());

                if (fileExchangeResponse.isSuccess()) {
                    fileExchangeResponse = FileUtil.removeFile(uploadFileJob.getUploadPath() + "\\" + uploadFileJob.getZipFileName());
                }
            }
        }
...
/*
            try {
                uploadFileJob.setUploadPath(FileExchangeJob.changePath(uploadFileJob.getUploadPath(), "b"));
                LOGGER.debug("La carpeta getUploadPath cambio a: " + uploadFileJob.getUploadPath());
            } catch (Exception e) {
                LOGGER.error("Error al intentar cambiar ruta.");
...
*/
            ...
            while (retry <= uploadFileJob.getRetryUpload()) {
                LOGGER.debug("Intento de carga de Reporte de Seguros No Cobrados #" + retry);
