...
public class PagoGarantiaLiquidaIndividual extends NotificationGeneric implements Job {

	private static final Logger LOGGER = Logger.getLogger(PagoGarantiaLiquidaIndividual.class);
	...
	// Se crea una copia de la clase PagoGarantiaLiquida
	// Para separa la generacion de fichas de Pago
	...
	public List<?> xmlToDTO(File inputData) {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Ingresa setParameterToSendMail PagoGarantiaLiquidaIndividual");
		}
		...
		try {
			...
			LOGGER.error("ERROR xmlToDTO PagoGarantiaLiquidaIndividual-->", e);
		}
	...
	public void executeByTransaction(JobExecutionContext arg0) throws Exception {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Ingresa executeByTransaction PagoGarantiaLiquidaIndividual");
		}
		Connection cn = null;
		...
		try {
			...
			ResultSet result = executeNotification(cn, executeSP, "L", null, null);
		...
		Exception e) {
			LOGGER.error("ERROR executeByTransaction -->PagoGarantiaLiquidaIndividual", e);
		} finally {
			ConnectionManager.saveConnection(cn);
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Finaliza executeByTransaction PagoGarantiaLiquidaIndividual");
			}
		}
	...
	public ResultSet executeNotification(Connection cn, CallableStatement executeSP, String operation, Integer transaction, String status) throws Exception {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Ingresa executeNotification PagoGarantiaLiquidaIndividual");
		}
