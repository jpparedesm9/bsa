...
public class CopiaConectorTimbrado extends Thread implements Job {

	private static final Logger logger = Logger.getLogger(CopiaConectorTimbrado.class);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		CopyFileJob copyFileJob = new CopyFileJob(FileJob.Job.CPYZFAC);
	...
	private static List<ConectorTimbrado> validatePathConnector(List<ConectorTimbrado> lisConectorTimbrados) {
		...
		for (int i = 0; i <= lisConectorTimbrados.size() - 1; i++) {
			File file = new File(lisConectorTimbrados.get(i).getRutaEntrada());
			logger.debug("Ruta a validar: " + file.getAbsolutePath());
			if (file.exists()) {
