...
public class CargaReportesODS extends NotificationGeneric implements Job {
	private static final Logger LOGGER = Logger.getLogger(CargaReportesODS.class);
	private static final String messageOk = "CARGA EXITOSA ARCHIVOS REPORTES ODS FILESHARE!!";
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		try {
			...
			LOGGER.error("Error al ejecutar Carga Reportes ODS FileShare", ex);
		}
	}

	private void cargarArchivos() {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Inicia CargaReportesODS");
		}
		FileExchangeResponse fileExchangeResponse;
		UploadFileJob uploadFileJob = new UploadFileJob(FileExchangeJob.Job.CARODS);
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			while (retry <= uploadFileJob.getRetryUpload()) {
				LOGGER.debug("Intento de carga de Reportes ODS fileshare #" + retry);
				try {
					if (fileExchangeResponse.isSuccess()) {
					...
					LOGGER.error("Intento de carga reportes ODS #" + retry + " fallido.");
					if (retry == uploadFileJob.getRetryUpload()) {
						LOGGER.debug("ODS: Error al intentar uploadFiles: " + e.toString());
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			if (to != null && !"".equals(to)) {
				LOGGER.info("Envio mensaje reporte ODS to: " + to);
