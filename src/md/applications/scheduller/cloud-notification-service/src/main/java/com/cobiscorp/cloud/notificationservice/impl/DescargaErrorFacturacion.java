...
public class DescargaErrorFacturacion extends NotificationGeneric implements Job {
	private static final Logger LOGGER = Logger.getLogger(DescargaErrorFacturacion.class);
	private static final String messageOk = "DESCARGA EXITOSA DEL ARCHIVO DE ERROR DE FACTURACION !!";
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		try {
			...
			LOGGER.error("Error al ejecutar Descarga de error de Facturación: ", ex);
	...
	private void downloadDirectDebits() {
		...
		DownloadFileJob downloadDirectDebitsJob = new DownloadFileJob(FileExchangeJob.Job.DAEFAC);
		...
		while (retry <= downloadDirectDebitsJob.getRetryDownload()) {
			LOGGER.debug("Descarga de error de facturación - intento #" + retry);
			try {
				...
				LOGGER.error("Descarga de error de facturación - intento #" + retry + " fallido.");
