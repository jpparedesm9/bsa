...
public class CargaReporteSeguroFuneralNetAltas extends NotificationGeneric implements Job {
	private static final Logger LOGGER = Logger.getLogger(CargaReporteSeguroFuneralNetAltas.class);
	private static final String messageOk = "CARGA EXITOSA DEL ARCHIVO DE REPORTE DE SEGUROS FUNERAL NET ALTAS!!";
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		try {
			...
			LOGGER.error("Error al ejecutar Carga de Reporte de Seguros Funeral Net Altas: ", ex);
	...
	private void downloadDirectDebits() {
		...
		UploadFileJob downloadDirectDebitsJob = new UploadFileJob(FileExchangeJob.Job.CRFNA);
		...
		while (retry <= downloadDirectDebitsJob.getRetryUpload()) {
			LOGGER.debug("Intento de carga de Reporte de Seguros Funeral Net Altas #" + retry);
