...
public class CargaReporteMovtosContab extends NotificationGeneric implements Job {
	private static final Logger LOGGER = Logger.getLogger(CargaReporteMovtosContab.class);
	private static final String messageOk = "CARGA EXITOSA DEL ARCHIVO MOVIMIENTO CONTABLE!!";
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		try {
			...
			LOGGER.error("Error al ejecutar Carga de Reportes Movimiento Contable", ex);
	...
	private void cargarArchivos() {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Inicia CargaReporteMovtosContab");
		}
		FileExchangeResponse fileExchangeResponse;
		UploadFileJob uploadFileJob = new UploadFileJob(FileExchangeJob.Job.CARMC);
		...
		if (fileExchangeResponse.isSuccess()) {
			...
			while (retry <= uploadFileJob.getRetryUpload()) {
				LOGGER.debug("Intento de carga de Reportes Movimiento Contable #" + retry);
				try {
					if (fileExchangeResponse.isSuccess()) {
					...
					LOGGER.error("Intento de carga Movimiento contable # " + retry + " fallido.");
