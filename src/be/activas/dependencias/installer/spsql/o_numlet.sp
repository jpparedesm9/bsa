/****************************************************************/
/* ARCHIVO:         o_numlet.sp                                 */
/* NOMBRE LOGICO:   sp_numeros_letras		                */
...
/****************************************************************/
/*                          PROPOSITO                           */
/* Realiza la transformacion de numeros a letras de acuerdo al  */
/* idioma del subtipo de instrumento. Se utiliza para la        */
/* impresion de cheques	                                        */
/****************************************************************/
/*                      MODIFICACIONES                          */
/* FECHA         AUTOR            RAZON                         */
/* 22-Ene-99 	 Ana M.L¢pez      Emision Inicial               */	
/* 31-Ago-09 	 Erika Alvarez    Inc.564 incluir "DE"          */	
/****************************************************************/
	...
	begin
		...
		if @w_cifras = 3 
		  ...
		  if @i_idioma = 'E'	
	            ...
	            begin
			if @w_aux <> 0			
			  select @w_cent = ' ' + nl_letras_esp from sb_numlet
			  where  nl_numero = @w_aux and
		          nl_tipo = 'C'
		   end
	          else
		    if @w_aux <> 0
                      select @w_cent = ' ' + nl_letras_otro from sb_numlet
		      where  nl_numero = @w_aux and
		      nl_tipo = 'C'
		...
		if @w_cifras = 2 
		 begin
		  ...
		  if @w_aux = 1 
		   begin
    			   if @i_idioma = 'E'	
				select @w_dec = ' ' + nl_letras_esp from sb_numlet
				where  nl_numero = convert(tinyint, @w_numero)
				and    nl_tipo = 'U'
			   if @i_idioma = 'O' or @i_idioma = 'I'
				select @w_dec = ' ' + nl_letras_otro from sb_numlet
				where  nl_numero = convert(tinyint, @w_numero)
				and    nl_tipo = 'U'
		...
		if @w_cifras = 1 
		 begin
		  ...
		  if @w_aux <> 0 
    	 	     if @i_idioma = 'E'	
			  select @w_unid = ' ' + nl_letras_esp from sb_numlet
		          where  nl_numero = @w_aux
			  and nl_tipo = 'U'
    	 	     if @i_idioma = 'O'	or @i_idioma = 'I'
			  select @w_unid = ' ' + nl_letras_otro from sb_numlet
		          where  nl_numero = @w_aux
			  and nl_tipo = 'U'
		 end 
	end	
...
begin
    ...
    select @w_texto = rtrim(ltrim(@w_txt)) + ' ' + rtrim(ltrim(@w_desc_moneda)) + ' CON ' + rtrim(ltrim(@w_centavos)) + '/100 M/CTE'
