/************************************************************************/
/*  Archivo:                sp_var_dias_ult_intento.sp                  */
/*  Stored procedure:       sp_var_dias_ult_intento                     */
...
if exists(select 1 from sysobjects where name ='sp_var_dias_ult_intento')
	drop proc sp_var_dias_ult_intento
go

create proc [dbo].[sp_var_dias_ult_intento](
	@t_debug       		char(1)     = 'N',
...
if @@rowcount = 0 and @w_tramite is null begin
   select @w_error = 2108069
   goto ERROR 
end

select @w_valor_nuevo = datediff(dd,cbs_fecha_ult_intento,getdate())
from cob_credito..cr_cuenta_buc_santander_job
where cbs_tramite = @w_tramite
