/************************************************************************/
/*  Archivo:                sp_var_porc_mujeres.sp     	                */
/*  Stored procedure:       sp_var_porc_mujeres                         */
...
/************************************************************************/
/*          PROPOSITO                                                   */
/*	Procedure tipo Variable, Retorna el valor de si los integrantes     */
...
use cob_credito
go
if object_id('sp_var_porc_mujeres') is not null
   drop procedure sp_var_porc_mujeres
go
create proc sp_var_porc_mujeres(
        @s_ssn             int            = null,
        @s_ofi             smallint       = null,
        @s_user            login          = null,
...
select @w_sp_name='sp_var_porc_mujeres'
...
select @w_integrantes = count(en_ente) from cobis..cl_ente
                        where en_ente in (select tg_cliente from cr_tramite_grupal where tg_tramite = @w_tramite and tg_monto > 0)

select @w_integrantes = isnull(@w_integrantes,0)

if @w_integrantes = 0 return 0

select @w_num_sexo_feme = count(p_sexo) from cobis..cl_ente
                        where en_ente in (select tg_cliente from cr_tramite_grupal where tg_tramite = @w_tramite and tg_monto > 0)
                        and p_sexo = 'F'    --género femenino
                        group by p_sexo

select @w_num_sexo_feme = isnull(@w_num_sexo_feme,0)
      
select @w_valor_nuevo = convert(varchar(20),(@w_num_sexo_feme * 100/@w_integrantes))
