...
/* Determina variables para matrices de cartera, Tasas Preferenciales  */
/***********************************************************************/
/*  FECHA          AUTOR              RAZON                            */
/*  26/01/2011     Y. Martinez        NYMY NR 223 Tasas Ýpreferenciales*/
/***********************************************************************/

use cob_credito
go

if exists (select 1 from sysobjects where name = 'sp_valida_matrices')
...
begin
   select 
   @w_error = 999999, 
   @w_msg = 'FECHA DE PROCESO NO PUEDE SER NULA'
   goto ERROR
end
...
begin
   select 
   @w_error = 190005, 
   @w_msg = 'LINEA DE CREDITO NO PUEDE SER NULO'
   goto ERROR
end
...
begin
   select 
   @w_error = 190005, 
   @w_msg = 'MONTO SOLICITADO NO PUEDE SER NULO'
   goto ERROR
end
...
begin
   select 
   @w_error = 190005, 
   @w_msg = 'TIPO DE CREDITO NO PUEDE SER NULO'
   goto ERROR
end
