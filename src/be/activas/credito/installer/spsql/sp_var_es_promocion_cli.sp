/****************************************************************/
/*   ARCHIVO:         	sp_var_es_promocion_cli.sp		        */
/*   NOMBRE LOGICO:   	sp_var_es_promocion_cli       			*/
/*   PRODUCTO:        		Credito                             */
/****************************************************************/
/*                     IMPORTANTE                           	*/
...
/****************************************************************/
/*                     PROPOSITO                            	*/
/*   Este procedimiento permite obtener la el la                */
/*   promocion de los clientes                                  */
/****************************************************************/
/*                     MODIFICACIONES                       	*/
/*   FECHA         AUTOR               RAZON                	*/
/*   28-May-2018   PXSG              Emision Inicial.     	    */
/****************************************************************/

use cob_credito
go

if exists (select 1 from sysobjects where name = 'sp_var_es_promocion_cli')
   drop proc sp_var_es_promocion_cli
go

create proc [dbo].[sp_var_es_promocion_cli](
	@t_debug       		char(1)     = 'N',
...
PRINT'@w_tr_promocion_grupo en Es Promocion CLI...'+ convert(VARCHAR(50),@w_tr_promocion_grupo)
   ...
   IF (@w_tr_promocion_grupo = 'S')
         BEGIN
          IF EXISTS(SELECT 1 FROM cob_credito..cr_grupo_promo_inicio WHERE gpi_grupo=@w_grupo AND gpi_ente=@w_cliente) -- Cambio por renovacion
           BEGIN
