/************************************************************************/
/*  Archivo:                SP_DISTRIBUC_REGIONAL.sp                    */
/*  Stored procedure:       SP_DISTRIBUIDOR_REGIONAL                    */
/*  Base de Datos:          cob_workflow                                */
/*  Producto:               Credito                                     */
/*  Fecha de Documentacion: 21/01/2020                                  */
/************************************************************************/
/*          IMPORTANTE                                                  */
...
/*          MODIFICACIONES                                              */
/*  FECHA       AUTOR           RAZON                                   */
/* 21/01/2020   srojas          Distribuidor Regional Oficinal          */
/* **********************************************************************/
...
if exists (select 1 from sysobjects where name = 'SP_DISTRIBUIDOR_REGIONAL')
   drop proc SP_DISTRIBUIDOR_REGIONAL
go

create proc SP_DISTRIBUIDOR_REGIONAL
(
   @s_ssn                     int         = null,
   @s_ofi                     smallint    = null,
   @s_user                    login       = null,
...
declare @w_oficinas_regional table
(
   oficina            int
)
...
-- Oficinas pertenecientes a la regional de la oficina del trámite
insert into @w_oficinas_regional
select of_oficina 
from cobis..cl_oficina
where of_regional     = (select of_regional
                         from  cobis..cl_oficina
...
and fu_oficina              in (select oficina from @w_oficinas_regional)
and ur_id_rol               = @i_id_rol_dest
and ((us_estado_usuario     = 'ACT') 
or  ((us_estado_usuario = 'INA') 
and (ur_id_usuario_sustituto in (select us_id_usuario
                                 from wf_usuario
                                 where us_estado_usuario = 'ACT'))))

select @w_num_act           = isnull(@w_num_act, 0) 


select @w_id_destinatario = us_id_usuario
from cob_workflow..wf_usuario, cobis..cl_funcionario
where fu_oficina in (select oficina from @w_oficinas_regional)
and us_login = fu_login
