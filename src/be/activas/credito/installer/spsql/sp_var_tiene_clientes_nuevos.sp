/****************************************************************/
/*   ARCHIVO:         	sp_var_tiene_clientes_nuevos.sp		    */
/*   NOMBRE LOGICO:   	sp_var_tiene_clientes_nuevos       		*/
/*   PRODUCTO:        		CARTERA                             */
/****************************************************************/
/*                     IMPORTANTE                           	*/
...
if exists (select 1 from sysobjects where name = 'sp_var_tiene_clientes_nuevos')
   drop proc sp_var_tiene_clientes_nuevos
go

create proc [dbo].[sp_var_tiene_clientes_nuevos](
	@t_debug       		char(1)     = 'N',
...
if @w_param_clientes_nuevos = 'S'  begin
   ...
   while (1=1) begin
      ...
      if (select isnull(en_nro_ciclo,0) from cobis..cl_ente where en_ente = @w_ente) = 0 begin
         select @w_nro_cliente_ciclo_0 = @w_nro_cliente_ciclo_0 + 1
      end
      
   end
end

if @w_nro_cliente_ciclo_0 = 0 begin
