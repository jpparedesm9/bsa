/************************************************************/
/*   ARCHIVO:         sp_sincroniza_solicitud_grp.sp        */
/*   NOMBRE LOGICO:   sp_sincroniza_solicitud_grupal        */
...
use cob_workflow
go
if exists (select 1 from sysobjects where name = 'sp_sincroniza_solicitud_grupal')
   drop proc sp_sincroniza_solicitud_grupal
go
create proc sp_sincroniza_solicitud_grupal 
        (@s_ssn        int         = null,
         @s_ofi        smallint    = null,
         @s_user       login       = null,
...
SELECT @w_sp_name = 'sp_sincroniza_solicitud_grupal'
...
select @w_tramite = isnull(io_campo_3,0)
  from cob_workflow..wf_inst_proceso
 where io_id_inst_proc = @i_id_inst_proc

update cob_credito..cr_tr_sincronizar
   set ti_sincroniza = 'N'
 where ti_tramite =  @w_tramite
   and ti_seccion = 'INGRESAR SOLICITUD'
