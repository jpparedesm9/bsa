...
/************************************************************************/
/*                           PROPOSITO                                  */
/*  Manejo de acuerdos (Ingreso / Modificacion / Consulta)              */
/************************************************************************/
/*                         MODIFICACIONES                               */
/*  FECHA         AUTOR             RAZON                               */
/************************************************************************/
...
if object_id('sp_acuerdo') is not null
...
begin
   if @i_cambio_plan_pago = 'S' begin
      ...
      delete cr_acuerdo where ac_acuerdo = @i_acuerdo and ac_fecha_proy not in (select av_fecha from cr_acuerdo_vencimiento_tmp where av_acuerdo = @i_acuerdo)
   end
...
if @i_modo = 'H' begin
   ...
   isnull(ac_saldo_cap,0) + isnull(ac_saldo_int,0) + isnull(ac_saldo_imo,0) + isnull(ac_saldo_honabo,0) + isnull(ac_saldo_ivahonabo,0) + isnull(ac_saldo_otros,0),
   ac_cuotas_vencidas,              
   (select case 
