...
/************************************************************************/
/*                              CAMBIOS                                 */
/* 09/dic/2019    ACH     Caso#150674 - Opcion 1 para conyuge           */
/* 06/ene/2021    ACH     Caso#151358 - Cam catalogo a tablas(direccion)*/
/* 18/mar/2021    ACH     Caso#147999 - Complementario                  */
/* 29/dic/2021    KVI     Caso#174184 - Importe Solicitud Individual    */
/* 26/ene/2022    KVI     Soporte#175052 - Filtro en conyuge            */
...
@w_ssn                int,
@w_sesn               int,
@w_banco_ant          varchar(24),--Caso #147999
@w_nro                int,
@w_nro_interno        int
...
begin
   ...
   --op_banco_anterior Caso #147999
   select @w_banco_ant = tg_prestamo 
   from cob_credito..cr_tramite_grupal 
   where tg_tramite = @i_tramite 
   and tg_cliente = @i_ente
   
   select @w_banco_ant = op_anterior 
   ...
   meses_vigentes             = datediff(m, a.en_fecha_mod, @w_fecha_proceso),
   ...
   bienes                     = a.p_bienes,
   otros_ingresos             = a.en_otros_ingresos,
   ...
   tiene_cred_grupal          = 'N',
   op_banco_ant               = @w_banco_ant -- Caso #147999
   into #datos_cliente
   ...
   update #datos_cliente set
      numero_ife = isnull(eb_ocr, eb_cic)
   from cobis..cl_ente_bio
   where cliente = eb_ente
  ...
  /*update #datos_cliente
   ...
   update #datos_cliente
   set   cod_conyuge = @i_ente--in_ente_d, se cambia por el cliente para que vuelva a consultar
   ...
   begin
    ...
    if @w_tipo_operacion = 'REVOLVENTE' begin	
       ...
       if @w_login_analista is not null
          ...
          select @w_login = oficial from #datos_cliente
       ...
       update #datos_cliente set    
       importe =  @w_monto_credito
    end
	
	
	--Req. 174184 Importe Crédito Individual
    if @w_tipo_operacion = 'INDIVIDUAL' 
	begin	
	  update #datos_cliente
	  set    importe = op_monto 
	  from   cob_cartera..ca_operacion
   ...
   begin
	  ...
	  HORA_LOCALIZACION             = isnull(hora_reunion,''),       -- 81 * HORA DE REUNION
	  INGRESOS_VENTAS               = otros_ingresos,     -- 82 * INGRESOS (VENTAS)
      origen_ingresos,                                    -- 83 
      OTROS_INGRESOS                = ventas,             -- 84 * OTROS INGRESOS  
	  TOTAL_INGRESOS                = total_ingresos,     -- 85 * TOTAL INGRESOS
	  GASTOS_NEGOCIO                = ct_operativos,      -- 86 * GASTOS DEL NEGOCIO
	  GASTOS_FAMILIARES             = ct_ventas,          -- 87 * GASTOS FAMILIARES
	  ...
	  IMPORTE_SOLICITADO            = importe,            -- 91 * IMPORTE SOLICITADO
	  ...
	  OP_BANCO_ANTERIOR             = convert(varchar(30), op_banco_ant)--Caso #147999
	  from #datos_cliente
   end
   else if @i_modo = 1   --conyuge
   begin
      select
      FECHA_NACIMIENTO           = convert(char(10), co_fecha_nacimiento, @i_formato),-- 1  * FECHA DE NACIMIENTO-----------convert(char(10), a.p_fecha_expira, @i_formato),
      TIPO_DOCUMENTO             = '',                 -- 2  * TIPO DE DOCUMENTO DE ID (CURP)
      NUMERO_DOCUMENTO           = '',                 -- 3  * NUMERO DE DOCUMENTO DE ID (CURP)
      fechaing                   = '',                 -- 4
      fechaexp                   = '',                 -- 5
      CODIGO_ENTIDAD_NACIMIENTO      = '',            -- 6  * CODIGO DE ENTIDAD DE NACIMIENTO
      DESCRIPCION_ENTIDAD_NACIMIENTO = '',--upper(isnull(desc_lugar_doc,'')),  -- 7  * DESCRIPCION DE ENTIDAD DE NACIMIENTO
      PRIMER_NOMBRE              = upper(isnull(co_nombre,'')),                 -- 8  * PRIMER NOMBRE
      APELLIDO_PATERNO           = upper(isnull(co_p_apellido,'')),             -- 9  * APELLIDO PATERNO
      APELLIDO_MATERNO           = upper(isnull(co_s_apellido,'')),             -- 10 * APELLIDO MATERNO
      ciudad_nac                 = '',                                          -- 11
      DESCRIPCION_GENERO         = '',--sexo,                   -- 12 * DESCRIPCION DEL GENERO
      CODIGO_GENERO              = '',--cod_sexo,               -- 13 * CODIGO DEL GENERO
      CODIGO_ESTADO_CIVIL        = '',--estado_civil,           -- 14 * CODIGO DE ESTADO CIVIL
      ...
      tipo_vivienda              = '',                     -- 29
      desc_tipo_vivienda         = '',                     -- 30
      NUMERO_DEPENDIENTES        = 0,--isnull(num_cargas,0),  -- 31 * NUM. DE DEPENDIENTES
      num_hijos                  = '',                        -- 32
      oficial                    = '',                        -- 33
      desc_oficial               = '',                        -- 34
      fechareg                   = '',                        -- 35
      ...
      desc_oficial_sup           = '',                        -- 50
      nivel_egr                  = '',                        -- 51
      SEGUNDO_NOMBRE             = upper(isnull(co_snombre,'')), -- 52 * SEGUNDO NOMBRE
      c_apellido                    = '',                 -- 53
      depar_doc                     = '',                 -- 54
      desc_depar_doc                = '',                 -- 55
      numord                        = '',                 -- 56
	  ...
	  NUMERO_SERIE_FIRMA            = '',--isnull(num_serie_firma,''),                 -- 94 * NUMERO DE SERIE DE FIRMA ELECTRONICA
	  TELEFONO_FIJO                 = '',--@w_telefono_fijo,   -- 95 * NUMERO DE TELEFONO FIJO
	  TELEFONO_CELULAR              = co_telefono,--@w_telefono_celular,-- 96 * NUMERO DE TELEFONO CELULAR
      TELEFONO_RECADO               = '',--telefono_recado,    -- 97 * NUMERO DE TELEFONO PARA RECADOS
	  NOMBRE_PERSONA_RECADO         = '',--upper(nombre_persona_recado),-- 98 * NOMBRE DE PERSONA PARA RECADOS
	  CTA_EN_BURO_CREDITO           = '',--ltrim(cta_en_buro_credito),  -- 99 * CUENTA CON ANTECEDENTES EN BURO
	  NOMBRE_ASESOR                 = '',--upper(isnull(desc_oficial,'')),--100 * NOMBRE DEL ASESOR
      ...
      TIENE_CRED_GRUPAL             = '',--tiene_cred_grupal,
	  OP_BANCO_ANTERIOR             = ''--Caso #147999
	  from cobis..cl_conyuge, cobis..cl_ente 
	  where co_ente = @i_ente
	  and   co_ente = en_ente
	  and   p_estado_civil not in ('DI', 'SO', 'VI')  --Soporte #175052
   end
...
begin
   ...
   begin
      ...
      ciudad             = (select ci_descripcion FROM cobis..cl_ciudad WHERE ci_ciudad = di_ciudad),						
      cod_parroquia      = di_parroquia,
      ...
      parroquia          = (select pq_descripcion from cobis..cl_parroquia where pq_parroquia = di_parroquia),							
      direccion          = di_descripcion,
      calle              = di_calle,                                              
      barrio             = di_barrio,
      principal          = di_principal,
   ...
   set rowcount 1
    ...
    Select @w_nro = nro_calle,
    @w_nro_interno = nro_calle_interno
    from #datos_direcciones
    
    if (@w_nro <0)
    begin
        update #datos_direcciones
        set nro_calle = 0
    end
   ...
   'DIRECCION'        = upper(isnull(direccion,''))+ ', ' + convert(VARCHAR(10),nro_calle) + ', ' + convert(varchar(10),nro_calle_interno),               --12 * CORREO ELECTRONICO
   'CALLE'            = upper(isnull(calle,''))+ ', ' + convert(VARCHAR(10),nro_calle) + ', ' + convert(varchar(10),nro_calle_interno),                   --13 * DIRECCIÓN (CALLE)
...
begin
   ...
   set   direccion      = di_calle,
         numero_externo = di_nro,
		 numero_interno = di_nro_interno,
		 /*colonia        = (select  b.valor
                                    ...
                                    where a.tabla  = 'cl_parroquia'
                                      and a.codigo = b.tabla
                                      and b.codigo = convert(varchar(10),c.di_parroquia)),*/
         colonia        = (select pq_descripcion from cobis..cl_parroquia where pq_parroquia = di_parroquia),							
		 ...
		 /*municipio      = (select  b.valor
                                    ...
                                    where a.tabla  = 'cl_ciudad'
                                      and a.codigo = b.tabla
                                      and convert(INT, b.codigo) = c.di_ciudad),*/
		 municipio      = (select ci_descripcion FROM cobis..cl_ciudad WHERE ci_ciudad = di_ciudad),									  
   ...
   'TIEMPO_ACTIVIDAD'     = (select valor from cobis..cl_catalogo where codigo = convert(varchar,tiempo_actividad) and tabla = (select TOP 1 codigo from cobis..cl_tabla where tabla = 'cl_referencia_tiempo')),
   'TIEMPO_ARRAIGO'       = (select valor from cobis..cl_catalogo where codigo = convert(varchar,tiempo_arraigo) and tabla = (select TOP 1 codigo from cobis..cl_tabla where tabla = 'cl_referencia_tiempo')),
...
begin
   ...
   'CIUDAD'                = upper(isnull(rp_ciudad,'')),
   'BARRIO'                = upper(isnull(rp_barrio,'')),
