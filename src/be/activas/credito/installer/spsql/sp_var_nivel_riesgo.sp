/************************************************************************/
/*  Archivo:                sp_var_nivel_riesgo.sp                      */
/*  Stored procedure:       sp_var_nivel_riesgo                         */
...
if exists(select 1 from sysobjects where name ='sp_var_nivel_riesgo')
	drop proc sp_var_nivel_riesgo
go

create proc [dbo].[sp_var_nivel_riesgo](
	@t_debug       		char(1)     = 'N',
...
if @@rowcount = 0 and @w_ente is null begin
   select @w_error = 2108057
   goto ERROR 
end

select @w_valor_nuevo = ea_nivel_riesgo_cg
