/************************************************************************/
/*  Archivo:                sp_var_calif_buro_cred_int.sp               */
...
/*                         MODIFICACIONES                               */
/*  FECHA              AUTOR          CAMBIO                            */
/*  05/06/2018         D. Cumbal      Cambios caso: 100389              */
/*  05-07-2019        srojas            Reestructuración Buro histórico */
/* **********************************************************************/
USE cob_credito
GO

IF OBJECT_ID ('dbo.sp_var_calif_buro_cred_int') IS NOT NULL
	DROP PROCEDURE dbo.sp_var_calif_buro_cred_int
GO
...
or bc_forma_pago_actual = '99') --and  bc_fecha_actualizacion = @w_fecha_ult_consul
...
begin
   ...
   if exists(select 1 from  #cr_buro_cuenta 
             where bc_tipo_contrato in ('RE','SM', 'HE')    --RE Bienes raices, SM  Segunda Hipoteca , HE  Préstamo tipo Home Equity
   ...
   begin        
            ...
            if exists(select 1 from  #cr_buro_cuenta 
                      where bc_tipo_contrato in  ('AU', 'RV')        --AU Compra de Auotomovil, RV Vehículo Recreativo  
            ...
            begin  
         if @w_nro_ciclo = 1 and exists (select 1 from  #cr_buro_cuenta 
                      ...
                      begin     
            if @w_nro_ciclo = 1 and exists (select 1 from  #cr_buro_cuenta 
                                            where bc_forma_pago_actual  not in ('01','02','03')) begin
