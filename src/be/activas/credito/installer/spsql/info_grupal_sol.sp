...
begin
	exec cobis..sp_cerror
		 ...
		 @t_from  = @w_sp_name,
		 @i_num   = 2108035,
		 @i_msg   = "NO EXISTE TRAMITE GRUPAL"
	return 2108035
end
...
begin
	...
	select	'OFICINA'		= 	tr_oficina,
			'DESC_OFICINA'	=	(select of_nombre from cobis..cl_oficina where of_oficina=tr_oficina),
			'FECHA_SOL'		=	convert(varchar(10),tr_fecha_crea ,@i_formato_fecha),
			'ID_GRUPO'		=	@w_grupo,
			'MONTO'			=	tr_monto,
			'TIPO_PLAZO'	=	(select td_descripcion from cob_cartera..ca_default_toperacion, cob_cartera..ca_tdividendo 
                                 where dt_tdividendo = td_tdividendo and dt_toperacion = tr_toperacion),
            'PLAZO'         =   isnull(op_plazo,0) + isnull(op_desplazamiento,0), --SRO 140073
	...
	begin 
		exec cobis..sp_cerror
		 ...
		 @t_from  = @w_sp_name,
		 @i_num   = @w_return,
		 @i_msg   = "ERROR AL TRANSFORMAR MONTO"
		return @w_return
	end
	...
	begin
		select 	@w_nom_ente_anf			=	(select UPPER(isnull(en_nombre,''))+' ' + UPPER(isnull(p_s_nombre,''))+' '+
	...
	/*Se obtiene la direccion del anfitrion*/
		...
		begin
			select top 1 @w_direccion_anf	=	isnull((select ci_descripcion from cobis..cl_ciudad where ci_ciudad=di_ciudad),'')
										+ ' ' 
										+isnull((select pq_descripcion from cobis..cl_parroquia where pq_parroquia=di_parroquia),'')
										+' '
										+isnull(di_calle,'')
										+' '
										+isnull(convert(varchar(20),di_nro),'')
										+ ' '
										+ isnull(convert(varchar(20),di_nro_interno),''),
...
begin
	...
	select 'CLIENTE'			=	tg_cliente, 
		   ...
		   'MONTO_LET'			= 	mc_monto,
		   'CUENTA'				=   (select ea_cta_banco from cobis..cl_ente_aux where ea_ente=tg_cliente),
		   'ROL'                =   cg_rol,
		   'AHORRO_VOLUNTARIO'  =   cg_ahorro_voluntario,
		   'CICLO_TRAMITE'      =   null/*(select dc_ciclo from cob_cartera..ca_ciclo, cob_cartera..ca_det_ciclo 
									 where ci_prestamo= dc_referencia_grupal and ci_tramite = @i_tramite
									 and dc_cliente = TG.tg_cliente)*/
