...
if exists (select 1 from sysobjects
            where name = 'sp_situacion_lineas')
begin
   ...
   -- order by tr_toperacion
    ...
    select distinct   
     ...
     'D',
	  --tr_plazo,
	 0,-- no existe tr_plazo
	 --isnull(0 --no existe tr_tplazo,'M')
	 'M'
    from cr_linea l, 
        cr_tramite, 
        cr_cotiz3_tmp b, 
...
BEGIN
    ...
    where sc_usuario    = @s_user
	 ...
	 and  cu_estado    not in ( select @w_estado_garantia) ---Se incluye condiciÃ³n de Activas y Canceladas
     ...
     and   tr_estado in ('A', 'N') 
     and   spid = @w_spid 
    order by cr_tramite.tr_toperacion
    if @@error <> 0 
    begin
        /* Error en insercion de registro */
 	exec cobis..sp_cerror
