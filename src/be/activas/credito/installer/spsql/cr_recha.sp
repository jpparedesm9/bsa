...
/*     Fecha de Documentacion:    18/Ago/95                            */
/***********************************************************************/
/*                         IMPORTANTE                                  */
...
/***********************************************************************/
/*                         PROPOSITO                                   */
/*     Este stored procedure permite rechazar las excepciones,         */
/*     instrucciones y tramites, cambiando su estatus a R o N          */
/*                                                                     */
/***********************************************************************/
/*                            MODIFICACIONES                           */
/*     FECHA          AUTOR            RAZON                           */
/*     18/Ago/95     Ivonne Ordonez    Emision Inicial                 */
/*     02/Jun/98     Augusto Montoya   Personalizacion Bco.Estado      */
/*     05/May/99     Gustavo Cobos     Actualizar fecha de rechazo     */
/*     03/Jun/99     Gustavo Cobos     Personalizacion Corfinsura      */
/*     03/Jun/99                       Controlar el riesgo del grupo   */
/*     03/Jun/99                       y el riesgo del sector          */
/*     24/abr/01     Sheela Burbano    Estados de cupo                 */
/*     19/Feb/03     Alfredo Zuluaga   Validaciones Bco Agrario Causal */
/*     09/AGO/04     Elcira Pelaez     Eliminar de CCA solo si no exist*/
/*                                     Transacciones en ca_transaccion */
/*     29/AGO/07    Sandra Lievano     Defecto 8682 - Operaciones      */
/*                                     anuladas en cartera             */
/*     27/Mar/2009   S.Ramirez          Fuentes de Recurso             */
/*     14/Sep/2010   J.Ardila          Req. 151. Contraofertas         */
/***********************************************************************/
...
begin
   ...
   if @w_op_estado in (1,2,4,9)
    begin
         select 
         @w_error = 2101198,
         @w_msg   = 'Error Estado de la Obligacion de cartera no valido para rechazo tramite'
         goto ERROR
    end   
   ...
   begin
      ...
      ---INC.39524
         select 
         @w_error = 2101198,
         @w_msg   = 'Error  hay transacciones sin Contabilizar'
         goto ERROR
   end
   else
   begin
	   ...
	   begin
	      ...
	      ---INC.39524
	         select 
	         @w_error = 2101198,
	         @w_msg   = 'Error  hay causacion sin Contabilizar'
	         goto ERROR
	   end
	   --else
	   --begin   
...
begin
   ...
   begin
      select @w_error = 2105001 ,
      @w_msg   = 'Error en actualizacion de monto reservado y saldo de fuente recursos'
      goto ERROR
   end
end
   ...
   if @w_tramite_org > 0 begin   
      ...
      if @@rowcount = 0 begin
         ...
         if @@error <> 0 begin
            select
            @w_error = 2105001,
            @w_msg   = 'Error en Actualizacion de Garantia'
            goto ERROR
         end
      end   
   end                
...
begin
   ...
   begin
      ...
      begin
         select 
         @w_error = 2101183, 
         @w_msg = 'Tramite no existe o no se encuentra en estado valido para esta tarea'
         goto ERROR
      end
