use cob_credito
go
if exists (select 1 from sysobjects where name = 'sp_crea_tr_redes' and type = 'P')
...
/* verifico que: el estado del tramite sea Aprobado para rutearlo a la siguiente estacion */

if @i_estado = 'A'
begin 
   /* RUTEO EL TRAMITE A LA SIGUIENTE ETAPA (CARTERA) */
   exec @w_return = sp_ruteo
        @s_ssn            = @s_ssn,
        @s_user           = @s_user,
        @s_sesn           = @s_sesn,
        @s_term           = @s_term,
