/* ********************************************************************* */
/*      Archivo:                sp_actividad_sincroniza.sp               */
...
USE cob_credito
go
IF OBJECT_ID ('dbo.sp_actividad_sincroniza') IS NOT NULL
	DROP PROCEDURE dbo.sp_actividad_sincroniza
GO
...
begin 
	-- Los nombres deben coincidir con los nombres de las actividades en la tabla cob_workflow..wf_actividad

    if(@w_toperacion = 'GRUPAL')
        SELECT @i_nombre_actividad = 'APLICAR CUESTIONARIO - GRUPAL'
    
    if(@w_toperacion = 'INDIVIDUAL')
        SELECT @i_nombre_actividad = 'APLICAR CUESTIONARIO - INDIVID'

    if(@w_toperacion = 'REVOLVENTE')--REVOLVENTE COLECTIVO
        SELECT @i_nombre_actividad = 'VERIFICAR DATOS'
end
