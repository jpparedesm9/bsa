...
if exists(select 1 from sysobjects where name ='sp_var_periodicidad_lcr')
	drop proc sp_var_periodicidad_lcr
go

create proc [dbo].[sp_var_periodicidad_lcr](
	@t_debug       		char(1)     = 'N',
...
if @@rowcount = 0 and @w_tramite is null begin
   select @w_error = 2108056
   goto ERROR 
end

select @w_valor_nuevo = tr_periodicidad_lcr
from cob_credito..cr_tramite
where tr_tramite = @w_tramite


IF @w_valor_nuevo IS NULL
SELECT @w_valor_nuevo = op_tplazo FROM cob_cartera..ca_operacion WHERE op_tramite = @w_tramite

PRINT 'Periodicidad LCR>>'+ @w_valor_nuevo
