...
if object_id ('sp_reporte_buro') is not null
begin
...
@w_secuencial      int,
@w_dhistotico      char(1),
@w_cadena_parametro varchar(30),
@w_posicion_signo  int,
@w_folio           varchar(64),
@w_cliente         int -- para validar envio

select @w_dhistotico = upper(pa_char) from cobis..cl_parametro where pa_nemonico = 'AHURO' and pa_producto = 'CRE'

if(@w_dhistotico = 'S')
begin

    select @w_cadena_parametro = pa_char from cobis..cl_parametro where pa_nemonico = 'CLGRRP' and pa_producto = 'CRE'
    select @w_posicion_signo = charindex(',', @w_cadena_parametro)

    select @w_cliente = convert(int,substring(@w_cadena_parametro, 0, @w_posicion_signo))
    select @w_folio = substring(@w_cadena_parametro, @w_posicion_signo + 1, len(@w_cadena_parametro))

    print '---->>>Cliente ingreso:' + convert(varchar(30),@i_cliente) + 
	      '---->>>Cliente consulta:' + convert(varchar(30),@w_cliente) + 
		  '---->>>Folio:' + convert(varchar(30),@w_folio)
	
	if(@i_cliente = @w_cliente)
	begin
        -- Por caso: ERR#151387, validar que el sp sp_reporte_buro_hist sea igual al presente solo con cambio a las tablas de historicos
        exec sp_reporte_buro_hist
	         @i_cliente       = @i_cliente,
	    ...
	    print 'consulta de buro historicos, no es el cliente, validar parametro'
...
where br_ib_secuencial = @w_secuencial
...
insert into #tmp_direcciones
 (
   ...
   di_reg_fecha    ) 
select 
db_direccion_uno + ' ' + db_direccion_dos,
db_colonia,
db_delegacion,
db_ciudad,
db_estado,
...
where db_ib_secuencial = @w_secuencial
