/************************************************************************/
/*  Archivo:                sp_var_indice_derecho_bio.sp                */
/*  Stored procedure:       sp_var_indice_derecho_bio                   */
...
/************************************************************************/
/*          PROPOSITO                                                   */
/* Procedure tipo Variable, retorna el valor del indice derecho guardado*/
/* en la tabla respuesta de biocheck						            */
/************************************************************************/
/*          MODIFICACIONES                                              */
/*  FECHA       AUTOR                   RAZON                           */
...
IF EXISTS (SELECT 1 FROM sysobjects o, sysusers u WHERE o.uid = u.uid AND o.name = 'sp_var_indice_derecho_bio' AND u.name = 'dbo' AND o.type = 'P')
    DROP PROCEDURE sp_var_indice_derecho_bio;
GO

CREATE PROCEDURE [dbo].[sp_var_indice_derecho_bio](
	@t_debug       		char(1)     = 'N',
...
declare	@w_sp_name 				varchar(64),
	...
	select @w_sp_name = 'sp_var_indice_derecho_bio'
	...
	if @w_ente is not null
		...
		select @w_resultado = rb_indice_derecho from cobis..cl_respuesta_bio where rb_ente = @w_ente
