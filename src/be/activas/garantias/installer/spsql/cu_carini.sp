/************************************************************************/
/*      Archivo:                cu_carini.sp                            */
/*      Stored procedure:       sp_cargaini_custodia                    */
...
/************************************************************************/
/*                              PROPOSITO                               */
/*      Este programa permite la carga inicial de las garantias a la    */
...
if exists (select 1 from sysobjects where name = 'sp_cargaini_custodia')
   drop proc sp_cargaini_custodia
go

create proc sp_cargaini_custodia
as
declare
   @w_fecha            datetime,
   @w_oficina          smallint,
...
select @w_sp_name = 'sp_cargaini_custodia'

select @w_fecha = fp_fecha 
from cobis..ba_fecha_proceso

if exists (select 1 from cu_cabecera_custodia where cc_tipo = 'I')
begin
...
begin
   ...
   values(@w_codigo_externo,  @w_tipo,             @w_cliente,       @w_banco,
          'B',                '',                  'GPE',            '',
          'garbatch',         @w_oficina,          getdate(),        @w_descripcion,
          '',                 getdate(),           @w_codigo_gar,    @w_regional)
   ...
   fetch cur_operaciones into  @w_banco, @w_codigo_externo, @w_descripcion, @w_tipo, @w_oficina
