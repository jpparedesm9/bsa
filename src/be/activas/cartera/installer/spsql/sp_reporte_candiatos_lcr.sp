...
/* 23/Jul/2021    Sonia Rojas       #161141 LCR V2.0 F2                  */
/*************************************************************************/
...
create table #clientes_1(
   ...
   gerente            varchar(64),
   asesor             varchar(64),
   colectivo          varchar(64),
   nivel_colectivo    varchar(64)
)
...
begin
   ...
   @w_colectivo       = colectivo,
   @w_nivel_colectivo = nivel_colectivo
   from #clientes_1
   where codigo  > @w_ente_eje
   order by codigo ASC
   if @@rowcount = 0 break
   
   if @w_colectivo is null begin
      if exists (select 1 from cob_cartera..ca_operacion 
                 where op_cliente = @w_ente_eje
                 and op_estado not in (0,3,99)) begin			  
         select @w_colectivo = 'CC'
      end
      else begin
         select @w_colectivo = 'ID'
      end
   end
    ...
    where 
	((operator_1 = '=' and variable_1 = @w_colectivo) 
    or (operator_1 = 'cualquier valor' AND variable_1 = ''))
	and ((operator_2 = '=' and variable_2 = @w_nivel_colectivo) 
    or (operator_2 = 'cualquier valor' AND variable_2 = ''))
	and ((operator_3 = '=' and variable_3 = @w_calif) 
    or (operator_3 = 'cualquier valor' AND variable_3 = ''))
    and variable_4 = @w_matriz
