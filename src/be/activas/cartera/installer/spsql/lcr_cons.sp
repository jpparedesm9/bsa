...
if @w_pago_minimo = 0 begin  --VALOR A PROYECTAR  EN CASO DE NO EXIGIBLES
   ...
   if @w_op_estado= @w_est_cancelado select @w_fecha_pago = @w_fecha_ven
...
create table #transaccion_desembolso(
    td_operacion       int,
    td_secuencial      int,
    td_consecutivo     int   null,
    td_linea           int   null,
	 td_fecha           datetime   null,
	 td_referencia      varchar(10) null,
	 td_fecha_str       varchar(8)  null,
	 td_estado          varchar(3)   null,
	 td_tran            varchar(10)  null,
	 td_monto           varchar(10)  null,
	 td_fecha_real      datetime 
)

insert into #transaccion_desembolso
select top 20 tr_operacion,tr_secuencial,null,null,null,null,null,tr_estado,tr_tran,dtr_monto,tr_fecha_real
...
update #transaccion_desembolso
set td_consecutivo = sod_consecutivo,
td_linea = sod_linea,
td_fecha = sod_fecha,
td_fecha_str = convert(varchar(10),sod_fecha,112)
from ca_santander_orden_deposito
where sod_operacion = td_operacion
and sod_secuencial= td_secuencial
and td_estado <> 'CON'


update #transaccion_desembolso
set td_referencia= convert(varchar(10),(td_consecutivo * 10000 + td_linea))
from ca_santander_orden_deposito
where sod_operacion = td_operacion
and sod_secuencial= td_secuencial
and td_estado <> 'CON'
...
update #transaccion_desembolso
set td_estado = 'P' --procesado
from ca_santander_resultado_desembolso
where rd_descripcion_referencia = td_referencia
and rd_fecha_transferencia= td_fecha_str
...
select 
fecha = td_fecha_real,
texto = case td_tran when 'DES' then 'UTILIZACION' else 'PAGO REALIZADO EXITOSAMENTE' end,
monto = td_monto,
estado = case td_estado when 'ING' then 'P' when 'CON' then 'F' when 'P' then 'F' end
from #transaccion_desembolso
