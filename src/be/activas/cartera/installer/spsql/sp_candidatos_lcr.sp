...
/*  13/Jul/2021    ACH         E#161681 comentario#6,visualizar estado P*/
/*  29/Jul/2021    KVI         Obtener tramite anterior                 */
/*  05/Ago/2021    S. Rojas    Req #161141                              */
/*  24/Nov/2021    ACH         Req#168893-Nota#16y19-ord alfabeticamente*/
/*  05/Ene/2022    DCU         Caso #175346                             */
/* **********************************************************************/
...
begin
	...
	select 'fechaIngreso'      = convert(varchar,cc_fecha_ing, 103),
			...
			'periodicidad'       = isnull(cc_periodicidad,'BW'),--PERIODICIDAD POR DEFECTO CATORCENAL SE BLOQUEA CAMPO EN PANTALLA
	...
	and   (cc_promocion is null or cc_promocion = 'P')--caso#161681
	--order by fu_nombre ASC, gr_nombre ASC  	--caso#168893
	order by ltrim(fu_nombre) ASC, ltrim(gr_nombre) ASC, ltrim(p_p_apellido) ASC, ltrim(p_s_apellido) ASC, ltrim(en_nombre) ASC, ltrim(p_s_nombre)ASC --caso#168893
end
...
begin
    ...
    --la primera vez que realiza un comentario / oficial
	if (@w_descripcion is null and @w_fecha_descrip is null)
	    select @w_fecha_descrip = getdate()
	...
	update cob_cartera..ca_lcr_candidatos set
		...
		cc_fecha_descrip = @w_fecha_descrip
	where cc_fecha_ing = @i_fecha_ing
	and   cc_grupo     = @i_grupo
	and   cc_cliente   = @i_cliente
...
if @i_operacion = 'A'
begin
	create table #cliente(codigo_cliente  int)
	...
	if (@w_date is null and @w_accion = 'P' and @w_fecha_descrip is null)
	    select @w_fecha_descrip = getdate()	
	...
	begin
		...
		--VALIDACION DE CANTIDAD DE INTEGRANTES REQ148219
		select @w_porcentaje_max = pa_smallint
		from cobis..cl_parametro
		where pa_nemonico ='MXILC'
		and pa_producto	= 'CCA'
		
		select @w_integrantes_grupo = count(*)
		from cobis..cl_cliente_grupo 
		where cg_grupo = @i_grupo
		AND cg_estado='V'
		
		print 'NUM INTEGRANTES DEL GRUPO '+convert(varchar(10),@w_integrantes_grupo)
							
		/*SELECT @w_integrantes_lcr=count(*) FROM cob_credito..cr_tramite WHERE tr_toperacion='REVOLVENTE'
		AND tr_cliente IN (SELECT cg_ente from cobis..cl_cliente_grupo 
									where cg_grupo = @i_grupo
									AND cg_estado='V')
		AND tr_estado='A'*/
		--Operaciones Activas
		...
		where  cg_grupo   = @i_grupo
		and    op_cliente = cg_ente
        and    op_toperacion = 'REVOLVENTE'
        and    op_fecha_fin  >= @w_fecha_proceso
        ...
        where io_estado ='EJE'
        and io_campo_1  = cg_ente
        and cg_grupo    = @i_grupo
        and io_campo_4  = 'REVOLVENTE'
		and not exists(select 1 from #cliente where codigo_cliente = io_campo_1)
		
		select @w_integrantes_lcr=count(1) from #cliente
		
		print 'NUM INTEGRANTES CON LCR '+convert(varchar(10),@w_integrantes_lcr)
			
		if(@w_integrantes_grupo=0 OR @w_integrantes_lcr*100/@w_integrantes_grupo >= @w_porcentaje_max)
		begin
			select @w_error = 724683 
			goto ERROR
		end
		--FIN VALIDACION DE CANTIDAD DE INTEGRANTES
	...
	begin
		update cob_cartera..ca_lcr_candidatos set
			   cc_descripcion = isnull(cc_descripcion, 'POSPUESTO')
		where cc_fecha_ing = @i_fecha_ing
		and   cc_grupo     = @i_grupo
		and   cc_cliente   = @i_cliente
	end
