...
/*                                   sp_ejecutar_reglas                 */
/* AGO          activacion a Cliente    COLECTIVOS                      */
/************************************************************************/
...
if @i_id_inst_proc is not null begin
   ...
   ----SI ES CANDIDATO SE EJECUTA REGLA DE MONTOS PARA CANDIDATOS
   --if exists(select * from cob_cartera..ca_lcr_candidatos where cc_cliente = @w_cliente)
   --begin
	--	print 'CLIENTE CANDIDATO'
	--	--EJECUCION DE LA REGLA DE MONTOS CANDIDATOS
	...
	--	print 'El monto actual ' + convert(varchar(10),@w_monto_actual)
	--   
	--   select @w_var_val = convert(varchar(10),@w_monto_actual)
	--   exec  @w_error       =  cob_pac..sp_rules_param_run
	--	@s_rol                   = 1,
	--	@i_rule_mnemonic         = 'CICLR',
	--	@i_modo                  = 'S',
	--	@i_tipo                  = 'S',
	--	@i_var_values            = @w_var_val, 
	--	@i_var_separator         = '|',
	--	@o_return_results        = @w_resultado 	OUT,
	--	@o_last_condition_parent = @w_parent  	out 
	--	
	--	select @w_resultado_monto = SUBSTRING(@w_resultado,0, CHARINDEX('|',@w_resultado))
   --
	--   if @w_error <> 0 or @w_resultado_monto is null begin 
	--	  select @w_msg = 'ERROR AL EJECUTAR REGLA DE MONTO INICIAL' 
	--	  goto ERROR_FIN
	--   end 
	--   print 'RESULTADO MONTO: '+@w_resultado_monto
	--   
...
--EN CASO DE SER PROSPECTO CAMBIARLO A CLIENTE 
if @w_estado  <>  'A' begin 
   ...
   @i_ente      = @w_cliente 
    ...
    if @w_error <> 0 or @@error <> 0 begin 
      select 
      @w_msg  = 'ERROR: AL ACTUALIZAR EL ESTADO DEL PROSPECTO A CLIENTE' ,
      @w_error= 70004
      goto ERROR_FIN   
   end
