...
if @@error <> 0
begin
   PRINT 'gentabla.sp Error UPD ca_rubro_op_tmp'
   return 710003
end 
...
begin
   ...
   begin
       ...
       exec @w_return       = sp_gen_rubtmp
            ...
            @s_ofi          = @w_oficina,
            @t_debug        = 'N',
            @i_crear_pasiva = 'N',  -- No deberia ir
...
--print 'credito mensual:' + @w_toperacion + ' ' + @w_div_mensuales
if exists (select 1 from cobis..cl_catalogo
...
begin
    ...
    while 1 = 1 begin
       ...
       if isnull(@w_plazo_o,0) < 1   begin
              if @i_crea_ext is null
                 print 'Periodicidad permitida multiplos de Mes'
              else
       ...
       if @w_plazo_o <= @w_plazo_e begin
          ...
          if @w_diferencia > 0 and @w_plazo > 1 begin
                 if @i_crea_ext is null
                    print 'Periodicidad Seleccionada solo permite un Dividendo como plazo (Pago Unico)'
                 else
...
begin
   ...
   select @w_tplazo = @w_tdividendo,
    ...
    --print 'tplazo:' + @w_tplazo + ' ' + 'tdividendo:' + @w_tdividendo
    --print 'dias_plazo:' + convert(varchar, @w_dias_plazo) + ' ' + 'dias_dividendo:' + convert(varchar, @w_dias_dividendo) + ' ' + 'periodo_cal:' + convert(varchar, @w_periodo_cal)
   ...
   --print 'plazo:' + convert(varchar, @w_plazo) + ' ' + 'dias_op:' + convert(varchar, @w_dias_op) + ' ' + 'dias_di:' + convert(varchar, @w_dias_di)
   --print 'tdividendo:' + @w_tdividendo
...
if @w_tipo_tabla = 'ROTATIVA' begin  
   ...
   if @@rowcount = 0 begin
      if @i_crea_ext is null
         print 'gentabla.sp tipo Dividendo'
      else
...
begin -- SI NO TIENE MIPYME, QUITAR EL ASOCIADO
   ...
   if @@error <> 0
   begin
      PRINT 'gentabla.sp Error upd ca_amortizacion_tmp rubro mipymes'
      return  710002
   end   	
end
...
begin
   ...
   if @@error <> 0
   begin
      PRINT 'gentabla.sp Error upd ca_amortizacion_tmp rubro Usaid'
      return  710002
   end   	
end
...
begin
   ...
   if @@error <> 0
   begin
      PRINT 'gentabla.sp Error Actualizando ca_operacion_tmp'
      return  710002
   end   
end 
