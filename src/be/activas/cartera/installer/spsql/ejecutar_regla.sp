...
/* srojas    13-Nov-2018    Ejecución de reglas sp asociado a variables */
/* ach       23-May-2023    Cambio de num error de 701103 por 705080    */
/************************************************************************/
use cob_cartera 
go
...
if @@rowcount = 0 begin
   select
   @w_error = 705080--,--La regla no existe o no está en Producción
...
END else if @i_tipo_ejecucion = 'EXT' begin
   ...
   if @w_inst_proceso is null begin
      ...
      if not exists (select  1 from  cob_workflow..wf_inst_actividad
      where ia_id_inst_proc = @w_inst_proceso) begin     
         return 0             
      end
