/************************************************************************/
/*   Stored procedure:     sp_conta_prv_fecha -conta_prv_fecha.sp       */
...
/************************************************************************/
/*                            PROPOSITO                                 */
/*  Contabilizara las PRV de una fecha dada por parametro               */
/************************************************************************/
/*                            CAMBIOS                                   */
/************************************************************************/
/*      FECHA           AUTOR             RAZON                         */
...
if exists (select 1 from sysobjects where name = 'sp_conta_prv_fecha')
   drop proc sp_conta_prv_fecha
go

--Inc. 51202 MAR.12.2012

CREATE proc [dbo].[sp_conta_prv_fecha]
...
@w_commit           = 'N',
@w_user             = 'sp_conta_prv_fecha',
@w_debug            = 'N',
@w_fecha_param1     = convert(datetime, rtrim(ltrim(@i_param1)))
...
if @w_debug = 'S'  print '--> sp_conta_prv_fecha.Inicio Contabilidad...'
...
if @w_debug = 'S'  print '--> sp_conta_prv_fecha. Encontro Corte...'
...
and  co_fecha_ini = @w_fecha_param1
...
if @w_debug = 'S' begin
   print '--> sp_conta_prv_fecha. Fecha Conta desde ' + cast(@w_fecha_mov as varchar)
   print '--> sp_conta_prv_fecha. Fecha Hasta ' + cast(@w_fecha_hasta as varchar)
end
...
while @w_fecha_mov <= @w_fecha_hasta begin
   ...
   print ''
   print 'antes de inicar estas son las transacciones de FECHA: --> '  + cast (@w_fecha_mov as varchar)
   select top 10 * from #transaccion_prv1
...
while 1 = 1 begin -- Proceso por Oficinas
   ...
   @w_user,           @w_descripcion,   convert(char(10),getdate(),101),     
   ...
   if @w_debug = 'S'  print '--> sp_conta_prv_fecha.FIN DEL PROCESO PARA LA OP...'
