...
begin 
   ...
   begin
      select @w_pos_pipe = charindex('|',@w_valor)
      
      if @w_pos_pipe > 0
      begin
         select @w_fecha = substring(@w_valor,1,@w_pos_pipe - 1)
         select @w_dias_char = substring(@w_valor,@w_pos_pipe+1,
                               datalength(@w_valor)-@w_pos_pipe)
         select @w_dias_calc = convert(int,@w_dias_char)
      end
      
      select @w_dividendo = @w_fila,
             @w_fecha_ven = convert(datetime,@w_fecha,@i_formato_fecha)
   ...
   begin --COLUMNAS DE RUBROS
      ...
      and    rot_tipo_rubro <> 'M' -- VERO 04/01/98
