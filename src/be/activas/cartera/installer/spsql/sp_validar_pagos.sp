...
/* 27/11/2017    MTA      Se aumenta la operacion a la tabla del log de */
/*                        pagos                                         */
...
--  GI Pagos por Garantías líquidas individuales
select @o_tipo   = case  
                     when @i_tipo in ('GL', 'PG', 'PI', 'CG','CI', 'GI') then @i_tipo --porCaso185234
                     else 'X'
                   end
...
begin
    select @w_error = 70179,
           @w_msg   = 'ERROR: EL PAGO TIENE MÁS DE 60 DÍAS DE ANTIGUEDAD. ' +
                      'FECHA DE PAGO: ' + convert(varchar(10), @o_fecha_pago, 101)
    goto ERROR_FIN
end
...
-------------------
--GARANTIA LIQUIDA INDIVIDUALES --porCaso185234
-------------------
if @o_tipo = 'GI'
begin

   if not exists (select 1 from cobis..cl_ente where en_ente = @o_codigo_int)
   begin
      select
      @w_error = 70183,
      @w_msg   = 'ERROR: EL GRUPO DE LA REFERENCIA NO EXISTE.'
   ...
   begin
	   if not exists (select 1 from cob_workflow..wf_inst_proceso
					  where io_campo_1  = @o_codigo_int  --Nro del cliente
					  and io_campo_7    = 'P'
...
begin
   ...
   begin
	   ...
	   begin
		  select @w_error = 70186,
				 @w_msg   = 'ERROR: NO EXISTE EL CRÉDITO RELACIONADO A LA REFERENCIA PARA LA CONCILIACION'
		  goto ERROR_FIN
	   end
   end
   select @o_codigo_int = @w_operacionca
   ...
   begin
	...
	begin
	   ...
	   begin
		  select
		  @w_error = 70186,
		  @w_msg   = 'ERROR: NO EXISTEN CREDITOS RELACIONADOS A LA REFERENCIA PARA CONCILIACION'
		  goto ERROR_FIN
	   end
	end
