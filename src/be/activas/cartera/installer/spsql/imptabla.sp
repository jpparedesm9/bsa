...
/*      Ago-04-2014   Fabián Quintero Req. 392 Pagos Flexibles          */
...
declare
   ...
   @w_ffin_gracia                  datetime,                            -- REQ 175: PEQUEÑA EMPRESA
   @w_deshacer_tran                char(1),                             -- REQ 175: PEQUEÑA EMPRESA
   ...
   @w_est_novigente                tinyint,                             -- REQ 175: PEQUEÑA EMPRESA
   @w_est_credito                  tinyint,                             -- REQ 175: PEQUEÑA EMPRESA
   ...
   @w_concepto_cap                 catalogo,                            -- REQ 175: PEQUEÑA EMPRESA
   @w_capitalizar                  money   ,                            -- REQ 175: PEQUEÑA EMPRESA
...
select @w_sp_name       = 'sp_imp_tabla_amort',
       @i_formato_fecha = 103,
       @w_deshacer_tran = 'N'                                                  -- REQ 175: PEQUEÑA EMPRESA
...
begin
   ...
   select @w_operacionca    = op_operacion,
          ...
          @w_fecha_ult_pro  = op_fecha_ult_proceso,       -- REQ 175: PEQUEÑA EMPRESA
          @w_dist_gracia    = op_dist_gracia,             -- REQ 175: PEQUEÑA EMPRESA
          @w_estado         = op_estado,                  -- REQ 175: PEQUEÑA EMPRESA
          @w_monto          = op_monto,                   -- REQ 175: PEQUEÑA EMPRESA
   ...
   and @w_tipo_amortizacion != @w_tflexible -- REQ.392 PARA LA TABLA DE AMORTIZACIÓN FLEXIBLE NO APLICA EL RECALCULO DE LA TABLA
   begin
   ...
   -- MAPEA CUOTA: Número de cuota, y Valor total de la cuota
  ...
  begin  --para que se genere con cuatro decimales.
   ...
   -- MAPEA INTERES: Número de cuota, Valor, Concepto, Tasa
   select cuota,
   ...
   -- MAPEA OTROS: Número de cuota, Valor
   select am_dividendo,
   ...
   -- MAPEA PAGOS: Número de cuota, Valor pagado
   select  am_dividendo,
   ...
   -- MAPEA MIPYME: Número de cuota, y valor
   select am_dividendo,
          round(convert(float, sum(am_cuota + am_gracia)), @w_num_dec)
   from   ca_amortizacion, ca_rubro_op
   where  ro_operacion = @w_operacionca
   ...
   -- MAPEA IVA MIPYME: Número de cuota, y valor
   select am_dividendo,
          round(convert(float, sum(am_cuota + am_gracia)), @w_num_dec)
   from   ca_amortizacion, ca_rubro_op
   where  ro_operacion   = @w_operacionca
   ...
   -- MAPEA FNG ANUAL: Número de cuota, y valor
   select am_dividendo,
          round(convert(float, sum(am_cuota + am_gracia)), @w_num_dec)
   from   ca_amortizacion,ca_rubro_op
   where  ro_operacion = @w_operacionca
   ...
   -- MAPEA IVA FNG ANUAL: Número de cuota, y valor
   select am_dividendo,
          round(convert(float, sum(am_cuota + am_gracia)), @w_num_dec)
   from   ca_amortizacion, ca_rubro_op
   where  ro_operacion = @w_operacionca
   ...
   -- MAPEA SEGURO DEUDORES VENCIDO: Número de cuota, y valor
   select am_dividendo,
          round(convert(float, sum(am_cuota + am_gracia)), @w_num_dec)
   from   ca_amortizacion, ca_rubro_op
   where  ro_operacion = @w_operacionca
   ...
   -- MAPEA MORA: Número de cuota, y valor
   select am_dividendo,
          round(convert(float, sum(am_cuota + am_gracia)), @w_num_dec)
   from   ca_amortizacion,ca_rubro_op
   where  ro_operacion = @w_operacionca
   ...
   -- MAPEA FINANCIACION DE MICRO SEGUROS: Número de cuota, Valor de CAPITAL, y Valor de Interés
   if @w_nro_tramites > 0
   begin
      set rowcount @w_tamanio  
   ...
   begin
      ...
      -- MAPEA CAPITALIZACIONES: Número de cuota, y monto
      select am_dividendo,
             case
   ...
   -- MAPEA DISPONIBLES DE TABLA FLEXIBLE: Número de cuota, y monto disponible
   if @w_tipo_amortizacion = @w_tflexible
   begin
