/************************************************************************/
/*Archivo             :   carevisa_Seg.sp                               */
/*Stored procedure    :   sp_revisa_boc_seg                             */
/*Base de datos       :   cob_cartera                                   */
/*Producto            :   Credito y Cartera                             */
/*Disenado por        :   Elcira Pelaez                                 */
/*Fecha de escritura  :   AGO.2013                                      */
/************************************************************************/
/*                       IMPORTANTE                                     */
...
if exists (select 1 from sysobjects where name = 'sp_revisa_boc_seg')
   drop proc sp_revisa_boc_seg
go
...
---oct.01.2013
create proc sp_revisa_boc_seg
as
declare 
   @w_sp_name       descripcion,
   @w_concepto_segdeuven  catalogo,
   @w_operacion     int,
   @w_cliente       int,
   @w_estado        smallint,
   @w_diferencia    money,
...
select 
@w_sp_name          = 'sp_revisa_boc_seg'


select @w_concepto_segdeuven = pa_char   
 from cobis..cl_parametro
where pa_nemonico = 'SEDEVE'
 and pa_producto = 'CCA'  
 set transaction isolation level read uncommitted
 
select @w_fecha_existe = max(rb_fecha)
from ca_revisa_boc
where rb_concepto = @w_concepto_segdeuven
...
begin
   print ' las diferencias ya se cargaron '
   return 0
end
...
--los clientes dobles tampoco los debe tomar

select cli = bo_cliente,'tot'=count(1)
into #eli_cliente
 from #oper
group by bo_cliente
having count(1) > 1
...
delete #oper
from #oper,#eli_cliente
where cli = bo_cliente
...
while 1 = 1 begin
      ...
      select @w_operacion  = op_operacion,
             @w_cliente    = bo_cliente,
             @w_estado     = op_estado,
             @w_diferencia = bo_diferencia,
             @w_cuenta     = bo_cuenta,
             @w_nombre     = cu_nombre
      from #oper
      ...
      if  exists (select 1 from ca_revisa_boc
           ...
           and   rb_concepto   = @w_concepto_segdeuven
           and   rb_fecha      = @w_fecha)
      begin    
         print 'existe ya en la tabla ca_revisa_boc'
         select @w_dif ='N'
      end
      ...
      if not exists (select 1 from ca_transaccion_prv
       ...
       begin
	        ...
	        insert into ca_revisa_boc (rb_fecha,           rb_operacion ,rb_cliente   ,rb_concepto  ,
	             ...
	             values               (@w_fecha,             @w_operacion,  @w_cliente,  @w_concepto_segdeuven,
	      ...
	      and am_concepto = @w_concepto_segdeuven
	      and am_acumulado > 0
	      ...
	      if @w_diferencia > 0
			...
			@w_concepto_segdeuven,        co_codigo * 1000 + @w_estado * 10 + 0,@w_diferencia,
			1,                   0,                   @w_oficina_op
			from ca_concepto
			where co_concepto = @w_concepto_segdeuven
