...
/*                             MODIFICACION                              */
/*    FECHA                 AUTOR                 RAZON                  */
/*    26/Jul/2019           DCU                   Emisión Inicial        */
/* ********************************************************************* */

use cob_cartera
go

if exists(select 1 from sysobjects where name = 'sp_lcr_gen_dividendos_tmp')
    drop proc sp_lcr_gen_dividendos_tmp
go
create proc sp_lcr_gen_dividendos_tmp (
...
--Puntajes
select operaciona_calif = operacion, min_sec= min(secuencial)
into #operacion_calif_t
...
set  puntaje    = 5
from #operacion_calif_t
where operacion  = operaciona_calif
and   secuencial = min_sec

select *
into #operacion_calif_d
from #operacion_calif_t

truncate table #operacion_calif_t


insert into #operacion_calif_t
select operaciona_calif = operacion, min_sec= min(secuencial)
from #prestamos_dividendos,
     #operacion_calif_d
where  operaciona_calif = operacion
and    secuencial       > min_sec
group by  operacion
...
truncate table #operacion_calif_d
insert into #operacion_calif_d
select * from #operacion_calif_t
truncate table #operacion_calif_t
