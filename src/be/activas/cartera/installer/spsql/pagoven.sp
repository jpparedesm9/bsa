...
@w_bandera          int,
@w_oficina          int,
@w_banco_fp         varchar(20),
@w_operacion_fp     int
 
return 0  --SE ELIMINA LLAMADA A LA LOGICA DEL PROGRAMA 
...
select op_operacion, op_banco, op_oficina 
into #operacion_fecha_valor
from cob_cartera..ca_operacion, cob_cartera..ca_estado
where op_estado  = es_codigo
and   es_procesa = 'S'
and   op_fecha_ult_proceso < @w_fecha_proceso

select @w_operacion_fp =0

while 1 = 1
begin
      select top 1  @w_banco_fp     = op_banco,
                    @w_oficina      = op_oficina,
                    @w_operacion_fp = op_operacion
      from #operacion_fecha_valor
      where op_operacion > @w_operacion_fp
