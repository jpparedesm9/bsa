...
if @w_tipo_garantia = @w_cod_gar_fag begin   
   ...
   if @w_td_tdividendo = 'UNICA' begin 
      ...
      if @i_tplazo is not null begin
        if @i_tplazo <> 'M'                                                                                                                                                                                                   
           select @w_plazo = @w_plazo * (select td_factor from cob_cartera..ca_tdividendo where td_tdividendo = @i_tplazo) / 30   
      end   
      else
      begin      
      ...
      where tr_tramite = @i_tramite 
      if @w_tplazo <> 'M'                                                                                                                                                                                                   
         select @w_plazo = @w_plazo * (select td_factor from cob_cartera..ca_tdividendo where td_tdividendo = @w_tplazo) / 30                   
      end
