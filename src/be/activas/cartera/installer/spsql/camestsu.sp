/************************************************************************/
/*   Archivo:              camestsu.sp                                  */
/*   Stored procedure:     sp_cambio_estado_suspenso                    */
...
if @i_en_linea = 'S'  and @i_front_end = 'S' --solo cuando es ejecutado desde front-end
   select 
   @w_trn         = 'SUM',
   @w_observacion = 'CAMBIO DE ESTADO MANUAL: '
else
   select 
   @w_trn         = 'SUA',
   @w_observacion = 'CAMBIO DE ESTADO AUTOMATICO: '
   
select @w_observacion = @w_observacion + ltrim(rtrim(es_descripcion))
from ca_estado
where es_codigo = @i_estado_ini

select @w_observacion = @w_observacion + ' A ' + ltrim(rtrim(es_descripcion))
from ca_estado
where es_codigo = @i_estado_fin
...
if @i_estado_fin <> @w_est_vigente begin
   ...
   while 1=1 begin  -- cursor de dividendos
      ...
      and @w_di_estado = @w_est_vigente begin
         ...
         dtr_monto_mn     =(am_acumulado - am_pagado) * -1,
         ...
         dtr_monto_mn     = (am_acumulado - am_pagado),
         dtr_moneda       = @w_op_moneda,
         dtr_cotizacion   = 1,
         dtr_tcotizacion  = 'N',
         dtr_afectacion   = 'D',
...
if @i_estado_fin <> @w_est_suspenso begin
   ...
   while 1=1 begin  -- cursor de dividendos
      ...
      dtr_monto_mn     = sum(am_acumulado - am_pagado) * -1,
      dtr_moneda       = @w_op_moneda,
      dtr_cotizacion   = 1,
      dtr_tcotizacion  = 'N',
      dtr_afectacion   = 'D',
      ...
      dtr_monto_mn     = sum(am_acumulado - am_pagado),
      dtr_moneda       = @w_op_moneda,
      dtr_cotizacion   = 1,
      dtr_tcotizacion  = 'N',
      dtr_afectacion   = 'D',
