/***********************************************************************/
/*	Archivo:			conta_prv_31_2010.sp                           */
/*	Stored procedure:		sp_conta_prv_31_2010                       */
/*	Base de Datos:			cob_cartera                                */
/*	Producto:			cob_cartera      	                           */
/*	Disenado por:			                                           */
/*	Fecha de Documentacion: 	20/Jul/98                              */
/***********************************************************************/
/*			IMPORTANTE		                                           */
...
if exists (select 1 from sysobjects where name = 'sp_conta_prv_31_2010')
	drop proc sp_conta_prv_31_2010
go

create proc sp_conta_prv_31_2010
   @s_user              login         = 'sp_conta_prv_31_2010',
   @i_debug             char(1)       = 'N'  
...
if @i_debug = 'S'  print '--> sp_conta_prv_31_2010.Inicio Contabilidad...'
...
select @w_fecha_proceso = '20101231'
...
if @i_debug = 'S'  print '--> sp_conta_prv_31_2010. Encontro Corte...'
...
if @i_debug = 'S' begin
   print '--> sp_conta_prv_31_2010. Fecha Conta ' + cast(@w_fecha_mov as varchar)
   print '--> sp_conta_prv_31_2010. Fecha Hasta ' + cast(@w_fecha_hasta as varchar)
end
...
while @w_fecha_mov <= @w_fecha_hasta begin
   ...
   from ca_transaccion_prv with (index = idx2), ca_operacion, ca_cast_31_2010
   where tp_estado    = 'ING'
...
while 1 = 1 begin -- Proceso por Oficinas
   ...
   -- Define clave
   update #detalles
   set de_clave = case when pa_stored = 'sp_ca01_pf' then de_calificacion +'.'+ de_gar_admisible +'.'+ de_clase +'.'+convert(varchar(1),de_moneda)
                       when pa_stored = 'sp_ca02_pf' then de_calificacion +'.'+ de_clase +'.'+convert(varchar(1),de_moneda) 
                       ...
                       when pa_stored = 'sp_ca07_pf' then de_calificacion +'.'+ de_gar_admisible +'.'+ de_clase +'.'+convert(varchar(1),de_moneda) +'.'+ de_org_fondos
                       when pa_stored = 'sp_ca08_pf' then de_clase +'.'+convert(varchar(1),de_moneda) +'.'+ de_org_fondos                       
                       else '' end
 ...
 --@w_re_ofconta,      de_tipo_area,       de_credito, -- FCP comento
   de_oficina,         de_tipo_area,       de_credito, -- FCP Inserto
