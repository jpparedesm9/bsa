...
go
/***************************************************************************/
/*   ARCHIVO:         reporte_pagos_tti.sp                                 */
/*   NOMBRE LOGICO:   sp_reporte_pagos_tti                                 */
...
/*                     MODIFICACIONES                                      */
/*   FECHA        AUTOR     RAZON                                          */
/* 05/03/2020     ACHP       Caso #136044 - Emision inicial                */
/* 25/10/2020     ACHP       Caso #147500 - RepCobranza no muestra reversos*/
/* 21/01/2020     DCU        Caso #152339 - Cabceras                       */
/* 24/03/2020     DCU        Caso #147999                                  */
/* 29/04/2021     WCA        Caso #147999 - Nota 133                       */
/* 05/08/2021     DCU        Caso #163544                                  */
/***************************************************************************/

if exists(select 1 from sysobjects where name = 'sp_reporte_pagos_tti')
    drop proc sp_reporte_pagos_tti
go
create proc sp_reporte_pagos_tti (
...
select @w_sp_name = 'sp_reporte_pagos_tti'
...
truncate table ca_reporte_pago_tmp_tti
...
from  ca_transaccion
where tr_tran = 'PAG'
--and   tr_estado != 'RV'--cs#147500
and   tr_fecha_ref = fecha_val
...
dh_iva_com     = sum(case when dtr_categoria = 'A' and (dtr_categoria_asoc IN ('O',  ' ')) then dtr_monto else 0 end),
dh_sobrante    = sum(case when dtr_concepto  = 'SOBRANTE'                       then dtr_monto else 0 end),
dh_div_min     = min(dtr_dividendo),
dh_dividendo   = dtr_dividendo--achp-#136044
into #pagos_det_h
from #pagos_det
group by dtr_operacion, dtr_secuencial, dtr_dividendo --achp-#136044
...
group by amh_operacion, amh_secuencial, amh_dividendo --achp-#136044
...
and   nro_cuota_pagada = amh_dividendo --achp-#136044
...
so_dividendo        = nro_cuota_pagada--convert(int,0)--achp-#136044
into #sobrante
...
------DETERMINAR EL SALDO CAPITAL EXIGIDO Y NO EXIGIDO DESPUES--achp-#136044*/
select di_operacion, di_dividendo, am_concepto, 'acumulado' = sum(am_acumulado - am_pagado), di_estado
into #saldo_act
from cob_cartera..ca_dividendo, cob_cartera..ca_amortizacion
where di_operacion = am_operacion
and   di_dividendo = am_dividendo
and   di_operacion in (select operacion from #pagos)
group by di_operacion, di_dividendo, am_concepto, di_estado

select
dh_operacion  = di_operacion,
dh_cap_ne      = sum(case when am_concepto = 'CAP' and di_estado <> 2         then acumulado else 0 end), 
dh_cap_ex      = sum(case when am_concepto = 'CAP' and di_estado = 2          then acumulado else 0 end), 
dh_dividendo   = di_dividendo
into #valor_act
from #saldo_act
where acumulado > 0
group by di_operacion, di_dividendo

update #reporte_pagos
set    saldo_cap_desp = (dh_cap_ne+dh_cap_ex),
       saldo_cap_ex_desp = dh_cap_ex
from   #valor_act
where  operacion = dh_operacion
and    nro_cuota_pagada = dh_dividendo
------
--nombre oficina
update #reporte_pagos set
oficina = a.of_nombre,
...
--cuotas pendientes --importe maxima y minimo dividendo --achp-#136044
select
mxc_operacion         = operacion,
mxc_max_cuota         = max(di_dividendo)
into #max_cuota
...
mc_min_cuota         = min(di_dividendo)
into #min_cuota
from #reporte_pagos, ca_dividendo
where operacion = di_operacion
and   di_fecha_can = @i_param1
group by operacion--, importe_tot

if @@error != 0 begin
   select 
   @w_error = 9999,
   @w_msg_error = 'Error al crear tablas de min cuota'
   goto ERROR_PROCESO
end
...
cuotas_pendientes = mxc_max_cuota - mc_min_cuota
from #min_cuota, #max_cuota
where mxc_operacion = operacion
and mc_operacion = operacion
...
------importe sobrante--achp-#136044
select
sa_operacion   = operacion,
sa_importe_sob = importe_sob
into #importe_sobrante
from #reporte_pagos
where importe_sob > 0
group by operacion, importe_sob

if @@error != 0 begin
   select 
   @w_error = 9999,
   @w_msg_error = 'Error al crear tablas de importe sobrante'
   goto ERROR_PROCESO
end

--actualzia el sobrante
update #reporte_pagos set 
importe_sob = sa_importe_sob
from #max_cuota, #importe_sobrante
where mxc_operacion = operacion
and   sa_operacion = operacion
and   mxc_max_cuota = nro_cuota_pagada
------
--cuotas en atraso
update #reporte_pagos set 
...
--Eventos del pago -- achp, el cliente indica que no es necesario el cambio
/*update #reporte_pagos set 
...
or_secuencial_rpa    = secuencial_rpa,  --LGU-MTA Caso 106284
...
or_archivo           = convert(varchar(64), ''), -- para web: W, dispositivo: D -- cs#147500
or_referencia		 = convert(varchar(64), '')
into #origen_pago
from #pagos
...
update #reporte_pagos set          
origen_pago = case 
              when or_descripcion = 'ELAVON' and (or_archivo is NULL OR ltrim(rtrim(or_archivo)) = '') then 'ELAVON WEB'
              when or_descripcion = 'PAGO POR RENOVACION' then 'BANCO SANTANDER' -- Cambio Ajuste Solicitud cliente Caseo 147999 - nota #131
              else or_descripcion end, -- ACA PONE ELAVON
tipo_pago   = or_tipo_pago,    --MTA
referencia= or_referencia
from #origen_pago
where or_operacion  = operacion
and   or_secuencial = secuencial
...
update #reporte_pagos set          
origen_pago = case
              when origen_pago = 'ELAVON' then 'TUIIO MOVIL'
              when origen_pago = 'ELAVON WEB' then 'GET NET'
			  else origen_pago end
...
--Carga cabecera
insert into ca_reporte_pago_tmp_tti(
...
rp_fecha_cuota_pagada,    rp_importe_tot, --achp-#136044
...
rp_reverso,               rp_origen_pago,           rp_usuario, rp_referencia
)
values( 
...
'FECHA DE LA CUOTA',                         'IMPORTE PAGO TOTAL', --achp-#136044
...
'REVERSO',                                   'ORIGEN DE PAGO',            'USUARIO', 'REFERENCIA APLICADA'
)
...
isnull(CONVERT(VARCHAR(10), fecha_cuota_pagada, 103),' '),     isnull(importe_tot,' '),--achp-#136044
...
isnull(reverso,' '),                                           isnull(origen_pago,' '),                                   isnull(usuario,' ') , isnull(referencia,' ')    
from #reporte_pagos
where nro_cuota_pagada > 0 ---achp-#136044--se envía solo eso debido a que se tiene sobrantes con # de cuota pagada menores a cero 
order by contrato, nro_cuota_pagada asc

if @@error != 0 begin
   select 
   @w_error = 9999,
   @w_msg_error = 'Error al insertar registro en tabla final ca_reporte_pago_tmp_tti'
   goto ERROR_PROCESO
end
--
...
select @w_file_rpt = 'COBRANZA_tti'
select @w_file_rpt_1     = @w_path + @w_file_rpt + '_' + @w_fecha_r + '.txt'
select @w_file_rpt_1_out = @w_path + @w_file_rpt + '_' + @w_fecha_r + '.err'



select @w_query = 'select rp_region,                rp_oficina,               rp_oficina_id,'        
select @w_query = @w_query + ' rp_gerente,            rp_coordinador,       rp_asesor,'            
select @w_query = @w_query + ' rp_contrato,           rp_grupo_id,          rp_grupo,'           
select @w_query = @w_query + ' rp_cliente_id,         rp_cliente,           rp_dia_pago,'
select @w_query = @w_query + ' rp_valor_cuota,        rp_cuotas_pendientes, rp_cuotas_en_atraso,'  
select @w_query = @w_query + ' rp_fecha_trn,          rp_fecha_valor,       rp_saldo_cap_antes,' 
select @w_query = @w_query + ' rp_saldo_cap_ex_antes, rp_fecha_ult_pago,    rp_nro_cuota_pagada,'  
select @w_query = @w_query + ' rp_fecha_cuota_pagada, rp_importe_tot,'
select @w_query = @w_query + ' rp_importe_cap,        rp_importe_int,       rp_importe_iva_int,'   
select @w_query = @w_query + ' rp_importe_imo,        rp_importe_iva_imo,   rp_importe_com,'  
select @w_query = @w_query + ' rp_importe_iva_com,    rp_importe_sob,       rp_saldo_cap_desp,'
select @w_query = @w_query + ' rp_saldo_cap_ex_desp,  rp_trn_corresp_id,    rp_tipo_pago,'   
select @w_query = @w_query + ' rp_reverso,            rp_origen_pago,       rp_usuario, rp_referencia'
select @w_query = @w_query + ' from cob_cartera..ca_reporte_pago_tmp_tti order by rp_region ASC '

  
select @w_bcp = 'bcp "' + @w_query + '" queryout "'	  
select @w_bcp = @w_bcp + @w_file_rpt_1 + '" -b5000 -c -C RAW -T -e ' + @w_s_app + 's_app.ini -T -e"' + @w_file_rpt_1_out + '"'
