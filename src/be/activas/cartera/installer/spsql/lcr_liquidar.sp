...
/************************************************************************/
/*  archivo:                lcr_liquida.sp                              */
/*  stored procedure:       sp_lcr_liquidar                              */
...
/*  08/23/2019        MTA             Control deshabilitar productos    */
...
@i_banco          cuenta,
@i_renovar        char(1)      = 'N',
@i_secuencial_des int          = 0,
@i_cuota_balon    int             ,
@i_soc_comercial  char(1)      = 'N' 
...
select @w_sp_name = 'sp_lcr_liquidar'
...
select @w_producto_cca = pd_producto
from   cobis..cl_producto
where  pd_abreviatura = 'CCA'
set transaction isolation level read uncommitted

select @w_estado_cca = pm_estado
from   cobis..cl_pro_moneda
where  pm_producto = @w_producto_cca
and    pm_moneda = 0
and    pm_tipo = 'R'
set transaction isolation level read uncommitted
...
select 
@w_secuencial = @i_secuencial_des,
@w_observacion = 'DESEMBOLSO FECHA VALOR'

if @i_secuencial_des = 0 
begin
   
   select @w_observacion  = 'DESEMBOLSO B2C'  

   select @w_secuencial_des  = isnull(max(dm_secuencial),0)  -- min
   from   ca_desembolso
   where  dm_operacion  = @w_operacionca
   and    dm_estado     = 'NA'

  if @w_secuencial_des <= 0 or @w_secuencial_des is null begin
     select 
     @w_error = 701121,
	...
	select @w_secuencial = @w_secuencial_des --lgu
...
-- OBTENER RESPALDO ANTES DE LA DES
if @i_secuencial_des <> 0 
begin  
...
@i_operacion       = @w_operacionca
   ...
   SELECT @w_secuencial_des = @i_secuencial_des
...
@s_term,              @w_fecha_ult_proceso,@w_secuencial_des, --isnull(@i_secuencial_des,@w_secuencial_des),
'ING',                @w_oficial,          isnull(@w_gar_admisible,''),
'N',                  isnull(@w_calificacion,''),
@w_observacion  ,     @s_date,             0)
...
dm_pagado
from   ca_desembolso
where  dm_secuencial = @w_secuencial_des
...
dm_estado          = 'A'
where  dm_secuencial = ( case when @i_secuencial_des= 0 then @w_secuencial_des else @i_secuencial_des end )
and    dm_operacion  = @w_operacionca
