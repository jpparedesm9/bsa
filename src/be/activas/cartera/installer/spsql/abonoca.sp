...
/*    10/Jun/2019           AGO              Req 116911 Ajustes LCR     */
/*    AGO-2019       PRO       			 REQ-115252 Se agrega calc. desc*/
/************************************************************************/
...
and    abd_tipo in ('PAG', 'DEV', 'SOB', 'CON')
...
if @w_monto_cancelacion <= @w_monto_mop begin
   select 
   @w_tipo_cobro = 'A',
   @w_evento_campana  = 'CAN'
...
if @w_tipo_tabla <> 'ROTATIVA' --Validar la cancelacion de op cuando no son LCR
BEGIN
...
if @w_tipo_tabla = 'MANUAL' and @w_tipo_reduccion = 'T' return 710418
...
if round(@w_sum_cuotas_ant,2) <> round(@w_op_monto,2) begin
   ...
   ELSE begin
      if @i_pago_ext = 'N'
         PRINT '1. abonoca.sp diferencia es @w_sum_cuotas_ant ' + CAST(@w_sum_cuotas_ant AS VARCHAR) + ' @w_op_monto '  + CAST(@w_op_monto AS VARCHAR)
      return 710540
   end
end
...
select @w_fpago_empleado = pa_char
from   cobis..cl_parametro with (nolock)
where  pa_producto = 'CCA'
and    pa_nemonico = 'CONEMP'
set transaction isolation level read uncommitted

if @w_fpago = @w_fpago_empleado 
begin
    ...
    if  ltrim(rtrim(@w_tipo_empleado)) = ltrim(rtrim(@w_tipo_persona))  
	    begin
	       select @w_tipo_reduccion   = 'C',
	              @w_tipo_aplicacion  = 'C'
...
if  @w_monto_mop1 >=  @w_saldo_oper1 begin
    if @w_precancelacion = 'S' begin
       select @w_monto_sobrante = @w_monto_mop,
              @w_cancelar       = 'S' 
            --@w_tipo_reduccion = 'N'
       --LA APLICACION SE ACTUALIZA EN LA CANCELACION DEL CREDITO, A SOLICITUD DE CIRO TORRES
...
--RESTRICCION PAGO A CREDITOS CON GARANTIA FAG Y EN COBRO JURIDICO REQ0212 PEQUEÐA EMPRESA
select @w_tramite = op_tramite from cob_cartera..ca_operacion
...
if @w_tipo_gar_act = @w_cod_gar_fag and @w_estado_cobranza = @w_param_prejuridico
if @w_cancelar = 'N'
if @s_rol <> @w_rol begin
    return 724132
end

--FIN RESTRICCION PAGO
...
if (@w_cancelar = 'S') select @w_calcular_new = 'S'
...
begin  
   if @w_tipo_reduccion not in ('T','C')
   begin
      if @i_pago_ext = 'N' 
	     if @i_en_linea = 'S'
            print 'Se realizo pago por monto superior al de la cuota. Favor validar con el cliente'
   end
   else
   begin      
...
/* PAGOS DEL FONDO NACIONAL DE GARANTIAS Y SUS DEVOLUCIONES */
exec @w_error =  sp_abono_fng
@i_operacionca   = @w_operacionca,
@i_secuencial    = @w_secuencial_pag,
...
if @w_error <> 0 begin
   if @i_pago_ext = 'N'   
      print @w_msg  -- no me gusta este print pero el abonoca de esta version no maneja los mensajes de error adicionales.
   return @w_error
end

-- EJECUTAR EL PROGRAMA DE DEVOLUCION DE SEGUROS 
-- FIN EJECUTAR EL PROGRAMA DE DEVOLUCION DE SEGUROS
...
if (@w_tipo_aplicacion not in ('D', 'P', 'S', 'O')) and (@w_monto_sobrante > 0)  and (@w_calcular_new = 'N')  and (@w_cancelar = 'N') begin
   ...
   begin
      ...
      select 
      @w_aceptar_anticipos = 'S',
      @w_tipo_aplicacion = 'C',
      @w_tipo_cobro = 'A'
	               
     ---if @i_pago_ext = 'N' PRINT 'abonoca.sp Salio de aplcnpto.sp ' + CAST (@w_monto_sobrante as varchar) + '  @w_tipo_reduccion ' + cast ( @w_tipo_reduccion as varchar)
     ---SI EN UNA APLICACION POR CONCEPTO HAY SOBRANTE ES POR QUE VA TODO A CAPITAL
     ---Y LO QUE SOBRA DE LAS CUOTAS VENCIDAS DEBE IR A REDUCCION DE TIEMPO o 
     --- DE CUOTA PERO NO  ADELANTO DE CUOTAS
     if  @w_tipo_reduccion = 'N' 
     begin
...
begin
   if @i_pago_ext = 'N'
      PRINT 'abonoca.sp @w_monto_sobrante ' + cast (@w_monto_sobrante as varchar) +  ' @w_aceptar_anticipos : ' + cast (@w_aceptar_anticipos as varchar) + '@w_tipo_aplicacion:  ' + cast (@w_tipo_aplicacion as varchar)
   return 710435
end   

-- SI ES UN PAGO POR RECONOCIMIENTO 
if @w_fpago in (select c.codigo
                from cobis..cl_tabla t, cobis..cl_catalogo c
                where t.tabla = 'ca_fpago_reconocimiento'
                and   t.codigo = c.tabla)
begin
   --SE APLICA CON REDUCCION DE CUOTA
   select @w_tipo_reduccion = 'C',
          @w_pago_especial  = 'S'

   -- REGISTRA PAGO POR RECONOCIMIENTO
   ...
   if @@error <> 0 return 708201
   if @w_error <> 0 return @w_error
   ---recobrar el estado de la cobranza en la operacion
end
...
if @w_aceptar_anticipos = 'S' and @w_monto_sobrante >= @w_vlr_despreciable begin
   select @w_abono_extraordinario = 'S'
   ...
   if @i_pago_ext = 'N' print 'abonoca.sp @w_nro_cuotas_pendientes ' + CAST (@w_nro_cuotas_pendientes as varchar)
  ...
  and   am_estado    <> @w_est_cancelado
   ...
   begin 
      ...
      if (@w_tipo_reduccion  = 'N') or (@w_cancelar = 'S') begin ---Pago Extraordinario Normal o precancelacion
         ...
         if @@error <> 0 return 708201
         if @w_error <> 0 return @w_error        
         --if @w_operacionca = 132792  return 710295
      end
   end
end
...
if exists (select 1
   ...
   if @w_monto_sobrante >= @w_vlr_despreciable   begin
      ...
      if @@error <> 0 begin
         if @i_pago_ext = 'N'
            PRINT 'abonoca.sp @w_fpago ' + CAST(@w_fpago AS VARCHAR)
         return 710295
      end
   end
end --EXISTE EL SOB
...
begin
   ...
   and    dtr_codvalor  <> 10097
   and    dtr_codvalor  <> 10047
   and    dtr_codvalor  <> 10092 --LCM NO TENER EN CUENTA LOS VALORES AMORTIZADOS
   and    dtr_codvalor  <> 10094 --LCM NO TENER EN CUENTA LOS VALORES AMORTIZADOS
   ...
   begin
      ...
      begin
         ...
         insert into ca_det_trn  with (rowlock)
	  	 ...
	  	 select @w_secuencial_pag,                 @w_operacionca,    -1,
	  	        ...
	  	        (co_codigo * 1000) + (@w_est_diferido * 10),     @w_dif_pago,      
	  	         round( (@w_dif_pago * @w_cotizacion) ,@w_num_dec_op),
	     ...
	     if @@error <> 0 begin
		    if @i_pago_ext = 'N'
	           PRINT 'reestint.sp Error Insertando en ca_det_trn pago al diferido'
   ...
   --- Solo se ejecuta este sp si es credito rotativo caso contrario no es necesario
	...
	select @w_tr_linea_credito = tr_linea_credito
	 from cob_credito..cr_tramite
	where tr_tramite = @w_tramite
	
	select @w_li_num_banco = li_num_banco
	from cob_credito..cr_linea
	where li_cliente =  @w_cliente
	and li_numero    = @w_tr_linea_credito
	and li_rotativa  = 'S'
   
   if @w_lin_credito is not null and @w_tramite is not null  and @w_lin_credito = @w_li_num_banco  and @w_monto_gar > 0
   begin
...
if @w_estado_act = @w_est_cancelado begin
   ...
   --INICIA CALCULO DE DESCUENTO 
   exec cob_cartera..sp_calc_descuento
   @i_banco = @w_banco,
   @i_opcion = 'I'
...
--REQ 389 ELIMAR EL CODIGO DE ca_capitaliza

--SUSPENSION DE CAUSACION
/*if (@w_op_estado = @w_est_suspenso) and (@w_estado_act <> @w_est_cancelado) begin
   select @w_monto_vencido = isnull(sum (am_acumulado - am_pagado), 0)
   from   ca_amortizacion, ca_dividendo,ca_rubro_op
   where  am_operacion = @w_operacionca
   and    am_operacion = di_operacion
   ...
   from   cobis..cl_ente 
   where  en_ente = @w_cliente
   select @w_situacion_cliente  = ltrim(rtrim(@w_situacion_cliente))
   set transaction isolation level read uncommitted
   
   select @w_suspension_manual = pa_char
   from   cobis..cl_parametro with (nolock)
   where  pa_producto = 'CCA'
   and    pa_nemonico = 'SMC'
   set transaction isolation level read uncommitted
   
   -- PARA CREDITOS EN UVR PASAR, LA CORRECION MONETARIA DE CAPITAL
   -- DE CUENTAS DE SUSPENSO A CUENTAS DE BALANCE
   if @w_monto_vencido < @w_vlr_despreciable begin
      -- CAMBIO DE LOS CAMPOS AM_CORRECCION_XXX A LA NUEVA TABLA CA_CORRECCION
      select @w_monto_aux = isnull(sum(isnull(co_correccion_sus_mn,0) - isnull(co_correc_pag_sus_mn,0)),0)
   ...
   ---SI LA OBLIGACION TIENE SMC SOLO SALE DE ESTE ESTADO  AUTOMATICAMENTE SI EL CLIENTE CAMBIA SI SITUACION
   ---DE SMC a NORMAL
   
   --EPB_09NOV2004 SUSPENSION POR CALIFICAICON
   select @w_calificacion_sus = valor
   ...
   select @w_calificacion_sus  = ltrim(rtrim(@w_calificacion_sus))
   
   if  @w_monto_vencido < @w_vlr_despreciable
   and @w_situacion_cliente  <> @w_suspension_manual
   and @w_calificacion < @w_calificacion_sus
   begin
      ...
      @i_estado_ini    = @w_est_suspenso,
      @i_estado_fin    = @w_est_vigente,
      @i_tipo_cambio   = 'S',
      @i_en_linea      = @i_en_linea
...
begin  ---8
   ...
   begin --- 5
      IF @w_debug = 'S' 
         PRINT ' entra.3  @w_gracia_extraordinario ' + convert(VARCHAR,@w_gracia_extraordinario) +
         '@w_dividendo_vigente  '  + convert(VARCHAR,@w_dividendo_vigente)
      ...
      begin ---3
         IF @w_debug = 'S' 
            PRINT ' entra.4  @w_dividendo_vigente  '  + convert(VARCHAR,@w_dividendo_vigente)
         ...
         where  am_operacion = @w_operacionca
         and    am_dividendo = @w_dividendo_vigente -- 1
         and    am_concepto  = @w_rubro_cap
...
begin
   if @i_pago_ext = 'N'
   begin
      PRINT 'despues de aplicar el pago hay descuadre de valores  CAP ANes' + cast(@w_sum_cuotas_ant as varchar) + ' CAP DESPUES : ' +  cast( @w_sum_cuotas_despues as varchar)
      return 710539
   end
end
...
if @w_monto_gar_mn > 0 begin --Pago de CAPITAL
 ...
 select @w_val_pagado = round(@w_sum_saldo_despues + @w_monto_gar_mn,2)
   ...
   begin
      if @i_pago_ext = 'N'
         PRINT 'abonoca.sp ' + '@w_sum_saldo_ant ' + CAST(@w_sum_saldo_ant AS VARCHAR) + '@w_val_pagado ' + CAST(@w_val_pagado AS VARCHAR) + '@w_sum_saldo_despues ' + cast(@w_sum_saldo_despues as varchar) + '@w_monto_gar_mn ' + cast(@w_monto_gar_mn as varchar)
      return  711022
   end
end
