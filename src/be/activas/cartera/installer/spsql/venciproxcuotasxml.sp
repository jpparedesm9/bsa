/************************************************************************/
/*   Archivo:              venciproxcuotasxml.sp.sp                     */
/*   Stored procedure:     sp_prox_vencimientos_cuotas_xml              */
...
if exists (select 1 from sysobjects where name = 'sp_prox_vencimientos_cuotas_xml')
   drop proc sp_prox_vencimientos_cuotas_xml
go

create proc sp_prox_vencimientos_cuotas_xml
(
    @s_user          login       = null,
    @s_ofi           smallint    = null,
...
select @w_sp_name = 'sp_prox_vencimientos_cuotas_xml'
...
select @w_ruta_xml = ba_path_destino,
       @w_nombre_xml = ba_arch_resultado
    from cobis..ba_batch 
    where ba_batch = 7077
...
SELECT @w_nombre_xml = isnull(@w_nombre_xml, 'referenciadepago')
...
IF NOT EXISTS(SELECT 1 FROM cob_cartera..ca_prox_vencimiento_cuotas prestamo)
BEGIN
	GOTO NO_DATOS
END 
...
select  @w_sql = 'select prestamo.vc_cliente as clienteId, ' +
      ...
      '(select isnull(vcd_referencia, ' + char(39) + char(32) + char(39) + ') as referencia, '+
       ...
       'from cob_cartera..ca_prox_vencimiento_cuotas_det '+
      ...
      'from  cobis..cl_oficina oficina, cob_cartera..ca_prox_vencimiento_cuotas prestamo ' +
      'where oficina.of_oficina = prestamo.vc_op_oficina  ' +
      'for XML path(' + char(39) + 'Prestamos' + char(39) + '), ' +
      'root(' + char(39) + 'VencimientosProxCuota' +  char(39) + '), elements ' 
