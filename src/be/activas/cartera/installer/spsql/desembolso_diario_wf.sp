...
/*   10-03-2020      ACHP        Caso#136317,validacion doc              */
/*   16-04-2021      ACHP        Caso#157223,validacion doc.Regresa Etapa*/
/*************************************************************************/
...
declare
   ...
   @w_id_paso           int,
   @w_faltan_docs       int, --numero de documentos (por solicirud) grupales faltantes
   @w_oficina           int,
   @w_num_docs_faltan   int --numero de documentos (por cliente) grupales faltantes
...
begin
    ...
    where td_codigo_tipo_doc = tr_codigo_tipo_doc
    ...
    NOT IN (select ri_codigo_tipo_doc from cob_workflow..wf_req_inst WHERE ri_id_inst_proc = @w_inst_proc) 
     ...
     /* # integrantes que participan contra documentos cargados*/
    select @w_num_docs_faltan = count(1) 
      from cob_credito..cr_tramite_grupal
     where tg_tramite = @w_tramite
       and tg_participa_ciclo = 'S'
       and tg_monto > 0
       and tg_cliente not in (select dd_cliente from cob_credito..cr_documento_digitalizado  
                              where dd_grupo = @w_grupo
                              and dd_inst_proceso = @w_inst_proc AND dd_cargado = 'S')
     
    PRINT 'faltan docs requerimiento: ' + convert(VARCHAR(10),@w_faltan_docs) + 
          ' -- faltan docs por cliente: ' + convert(VARCHAR(10),@w_num_docs_faltan) + ' - TRAMITE>>'+convert(varchar,@w_tramite) 
    
    if (@w_num_docs_faltan = 0 and @w_faltan_docs = 0)
    begin
        --VALIDA QUE TENGA TODOS LOS DOCUMENTOS DIGITALIZADOS CARGADOS
        if not exists(select  1 from cob_credito..cr_documento_digitalizado  
                      where dd_grupo = @w_grupo and dd_cargado= 'N' and dd_inst_proceso = @w_inst_proc )
        begin    
