...
while(@@fetch_status = 0)  begin
   ...
   if @w_referencial is not null and isnull(@w_porcentaje_red, 0) = 0 begin         -- REQ 175:PEQUEÑA EMPRESA - isnull(@w_porcentaje_red, 0) = 0
      ...
      @i_periodo_d      = 'D',
      @i_num_periodo_d  = @w_di_dias_cuota, -- A LA MODALIDAD DE LA CUOTA
      @i_modalidad_d    = @w_modalidad_op,
   ...
   select 
   @w_porcentaje     = round(@w_porcentaje,     @w_num_dec_tapl),
   @w_porcentaje_efa = round(@w_porcentaje_efa, @w_num_dec_tapl)
