...
/*    18/Ago/2017   LGU              Proceso por OP y generacion de XML */
/*    28/Ago/2017   Ma. Jose Taco    Aumentar io_campo5 para guardar    */
/*                                   tramite anterior                   */
/*    19/Abr/2021   ACH              ERR#156356,tome la oficina dl oficial*/
/*    22/1/2021     AGO              VALIDACION CON RENOVACIONES 2021   */
/*    19/Ago/2019   ACH              #144022                            */
/************************************************************************/
use cob_cartera
go
...
begin
	...
	select @w_user = fu_login,
	       @w_oficina_oficial = fu_oficina
	from   cobis..cc_oficial, cobis..cl_funcionario
	where  oc_funcionario = fu_funcionario
	and    oc_oficial     = @w_oficial

    --Caso #156356
	if @w_oficina_oficial is not null
	select @w_oficina = @w_oficina_oficial
	    
    print 'OP : '+ convert (varchar(15),@w_operacion) + ' BANCO : ' + @w_banco + ' USR: ' + @w_user + ' GR: ' + convert(VARCHAR, @w_cliente) + 
	      'OFICINA : '+ convert (varchar(15),@w_oficina)


    if exists (select 1 from cob_credito..cr_op_renovar where or_grupo= @w_cliente and or_finalizo_renovacion = 'N') begin 
	
		print' BATCH GRUPO TIENE TRAMITE DE RENOVACION INICIADA   GR: ' + convert(VARCHAR, @w_cliente)
         select @w_error = 150007
         select @w_msg = ' GRUPO TIENE RENOVACION INICIADA  GR: ' + convert(VARCHAR, @w_cliente)
         goto ERROR
    end 
