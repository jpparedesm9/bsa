...
/*  19/marzo/2019      AINCA             Bloquear ejecucion de sp       */
/*  05/ago/2019        MTACO       Caso 113432 optimizacion sp          */
/************************************************************************/
...
print '---->>>> antes de la creacion del xml de PAGOS' 
return 0
...
into #cab_cobranza
from   cob_credito..cr_tramite_grupal inner join cob_cartera..ca_operacion on (tg_operacion  = op_operacion)
inner join cob_cartera..ca_dividendo on(op_operacion  = di_operacion)
inner join cobis..cl_grupo on(tg_grupo = gr_grupo)
inner join cob_cartera..ca_ciclo on(tg_referencia_grupal = ci_prestamo)
left outer join cobis..cc_oficial on ( gr_oficial   = oc_oficial)
left outer join cobis..cl_funcionario on(oc_funcionario = fu_funcionario)
left outer join cobis..cl_sucursal on (gr_sucursal = sucursal)
WHERE  di_estado     = @w_est_vencida 
...
group by tg_grupo,tg_referencia_grupal, gr_nombre, ci_ciclo, gr_oficial,gr_sucursal, fu_nombre, fu_login,nombre
...
begin
    ...
    begin
        ...
        values (@w_codigo, @w_cod_entidad,@w_entidad,@w_usuario,'P',@w_fecha,1)
        ...
        select @w_cabdato_xml = (select monto_exigible  as amountChargeWholePayment,
            ...
            values (@w_codigo, @w_grupoId, 0,0,@w_dato_xml,'INGRESAR','Ingresar los pagos solidarios')
