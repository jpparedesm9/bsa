...
begin
   ...
   select 
        en_ente,   en_ced_ruc,   case when en_tipo_ced  = 'CC' then 2 when en_tipo_ced  = 'N' then 1 else ' ' end, (p_p_apellido + ' ' + p_s_apellido + ' ' + en_nombre),
        op_banco, 'I',  'N' 
   ...
   select 'comision' = case when ro_concepto_asociado  = @w_par_fag_des then 'A' 
                            when ro_concepto_asociado  = @w_par_fag_uni then 'U' else ' ' end,
          'banco'    = op_banco
   into #comision 
