/************************************************************************/
/*   Archivo:              mopsolwf.sp                                  */
/*   Stored procedure:     sp_modificar_oper_sol_wf                     */
...
/*  SEP-26-2017    Ma. Jose Taco    Borrar y Crear integrantes en       */
/*                                   actualizacion de grupos            */
/*   SEP-17-2020   Sonia Rojas      Req #140073                         */
/*  03/Mar/2021    Sonia Rojas      Req #147999                         */
/*  27/Ago/2021    ACH              MEJ#167698-SOP#147380               */
/************************************************************************/
...
--SE COMENTA PARA EVITAR BLOQUEOS CASO BLOQUEOS PRODUCCION
/*if @@trancount = 0
begin
   begin tran
   select @w_commit = 'S'
...
/**REQUERIMIENTO 98119 para que cuando recalcula la tabla al Guardar en la etapa de Eliminar Integrante, tome la fecha de dispersi?n que es parte del cambio **/
...
PRINT '@w_fecha_liq'+convert(VARCHAR(10),@w_fecha_liq,103)
PRINT '@w_fecha_ini'+convert(VARCHAR(10),@w_fecha_ini,103)
...
begin
	...
	--obtener el tramite anterior del grupo
	...
	and   tg_prestamo <> tg_referencia_grupal	--   MEJ#167698-SOP#147380
	...
	--si el tramite actual es promo, el anterior debio ser promo
...
if @w_dias_desplazados is not null and @w_dias_desplazados > 0 and @w_toperacion = 'GRUPAL' begin
   ...
   if (select datediff(dd, di_fecha_ini, di_fecha_ven) from ca_dividendo where di_operacion = @i_operacionca and di_dividendo = 1) < @w_dias_desplazados begin
      print 'Entrar a desplazar'
	  exec @w_error      = sp_desplazamiento
      @i_banco           = @w_banco,
      @i_cliente         = @w_cliente,
