...
select @w_long_max_conse = 6,  -- LONGITUD DEL NUMERO CONSECUTIVO
       @w_dig_conse      = datalength(convert(varchar,@w_consecutivo)) 
select @w_ceros   = @w_long_max_conse - @w_dig_conse
...
select @w_num_banco  = @w_num_banco + convert(varchar(4),@w_cta_ofi) + @w_consecutivo_c 
 
if @w_tipo = 'C'
   select @w_num_banco1 = @w_num_banco1 + convert(char(4),@w_cta_ofi) + @w_consecutivo_c 
