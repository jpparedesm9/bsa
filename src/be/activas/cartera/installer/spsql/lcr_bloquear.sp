...
/* ********************************************************************* */
/*                            MODIFICACIONES                             */
/* 19/01/2022    ACH    ERR #176798 Comentario #12                       */
/* ********************************************************************* */
...
@w_presenta			  char(1),
@w_num_pagos		  int,
@w_num_pagos_param    int,
@w_msn_num_pagos      varchar(256)

select @w_msn_num_pagos = mensaje from cobis..cl_errores WHERE numero = 724685
...
select @w_num_pagos_param = pa_int from cobis..cl_parametro where pa_nemonico = 'NPRLCR' and pa_producto = 'CCA'
select @w_msn_num_pagos = mensaje from cobis..cl_errores where numero = 724685
...
if @i_operacion = 'C' begin 
   ...
   select @w_rol_aut = pa_smallint
   from cobis..cl_parametro
   where pa_nemonico='RADBL'
   and pa_producto='CCA'
...
if @i_operacion = 'I' begin 
   ...
   if(@i_bloqueo = 'N')
   begin
		if not exists (select * from ca_lcr_autoriza_bloqueo where ab_operacion = @w_operacionca and ab_autorizado='S')
		begin
			select @w_error  = 724684
			goto ERROR
		end
   end
   ...
   if exists (select * from ca_lcr_autoriza_bloqueo where ab_operacion = @w_operacionca)
	begin
		update ca_lcr_autoriza_bloqueo
		set ab_autorizado 	= 'N',
		ab_fecha_aut		= getdate(),
		ab_usuario_aut		= @s_user
...
begin
	...
	SELECT @w_num_pagos = count(*) FROM ca_transaccion WHERE tr_secuencial> (
		SELECT max(tr_secuencial) 
		FROM ca_transaccion 
	...
	if(@w_num_pagos < @w_num_pagos_param)
	begin 
	    select @w_error = 724685
		select @w_msg = '[' + @w_sp_name +'] ' + replace(@w_msn_num_pagos, '#', @w_num_pagos_param)
		goto ERROR		
	end
	...
	begin
		insert into ca_lcr_autoriza_bloqueo (ab_operacion, ab_autorizado, ab_fecha_aut, ab_usuario_aut)
		values(@w_operacionca, 'S',	getdate(),@s_user)
	end
end
