/**************************************************************************/
/*   Archivo:              cuotasproxvencidas.sp.sp                       */
/*   Stored procedure:     sp_prox_vencimientos_cuotas                    */
...
/**************************************************************************/
/*                               PROPOSITO                                */
/*   Genera archivo xml con informacion de proximo vencimiento de cuotas  */
/*   para envio de correo de aviso de vencimiento a deudores              */
/**************************************************************************/
/*                             MODIFICACIONES                             */
/*    FECHA      AUTOR                        RAZON                       */
/*  14/07/2022    ACH    Basado en el batch 7076 Fichas de prox Pago Ind  */
/**************************************************************************/

use cob_cartera
go

if exists (select 1 from sysobjects where name = 'sp_prox_vencimientos_cuotas')
   drop proc sp_prox_vencimientos_cuotas
go

create proc sp_prox_vencimientos_cuotas
(
...
@w_dias_ant_ven_cuota   tinyint,
@w_est_etapa2           tinyint,
@w_fecha_venci_param    datetime,
@w_contador             tinyint

select @w_sp_name = 'sp_prox_vencimientos_cuotas', @w_valida_error = 'S'
...
select @w_dias_ant_ven_cuota = pa_int 
  from cobis..cl_parametro 
 where pa_producto = 'CCA' 
   and pa_nemonico = 'DAVCOU'
   
select @w_dias_ant_ven_cuota = isnull(@w_dias_ant_ven_cuota, 1)

--CALCULO DE FECHA HABIL DE VENCIMIENTO
select @w_contador = 0
select @w_fecha_venci_param = @w_fecha_proceso

print '>>fechaProceso: ' + convert(varchar(30), @w_fecha_proceso) + '>>#paramDias: ' + convert(varchar,@w_dias_ant_ven_cuota)
	
while (@w_contador < @w_dias_ant_ven_cuota) begin
    
	select @w_contador = @w_contador + 1
    select @w_fecha_venci_param = dateadd(dd, 1, @w_fecha_venci_param)
	
	print 'Contador: ' + convert(varchar(30), @w_contador) + '>> @w_dias_ant_ven_cuota: ' + convert(VARCHAR(30), @w_fecha_venci_param)        
    
	while exists( select 1 from cobis..cl_dias_feriados where df_ciudad = @w_ciudad_nacional  and df_fecha = @w_fecha_venci_param) begin
       PRINT 'Entro a ver si es feriado la fecha: ' + convert(varchar(30), @w_fecha_venci_param)
       select @w_fecha_venci_param = dateadd(dd, 1, @w_fecha_venci_param)       
    end
    
	print 'sale con fecha_venci_param: ' + convert(varchar(30), @w_fecha_venci_param)
end

/*ELIMINAR CONTENIDO DE LAS TABLAS*/
delete from ca_prox_vencimiento_cuotas_det
delete from ca_prox_vencimiento_cuotas 

/*UNIVERSO DE CONSULTA*/
print '>>paraUniverso>>fecha_proceso: ' + convert(varchar(30), @w_fecha_proceso) + '>>fecha_venci_param: ' + convert(varchar(30), @w_fecha_venci_param)
select 
...
and di_fecha_ven    = @w_fecha_venci_param
...
insert into ca_prox_vencimiento_cuotas 
...
and   di_estado        in (@w_est_vigente, @w_est_etapa2)
...
from ca_prox_vencimiento_cuotas
where vc_cliente >  @w_cliente
order by vc_cliente asc

while @@rowcount > 0 begin
   select @w_nombre_cliente = upper(vc_cliente_name) from cob_cartera..ca_prox_vencimiento_cuotas where vc_cliente = @w_cliente
   ...
   update  cob_cartera..ca_prox_vencimiento_cuotas
   ...
   while 1 = 1 begin
      ...
      @o_referencia     = @w_referencia out
       ...
       if @w_error <> 0 begin
   	   ...
   	   insert into ca_prox_vencimiento_cuotas_det 
...
exec @w_return = cob_cartera..sp_prox_vencimientos_cuotas_xml 
