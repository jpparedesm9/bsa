...
if (@w_max_fecha_can <> '01/01/1900')begin 
   ...
   --Si hoy tiene saldo cero, borra los demás registros de hoy
...
if(@i_canal = 'B2C') begin
   ...
   if @w_error <> 0 begin 
	  ...
	  select @w_msg = 'Error al registrar la transacción'
      goto ERROR_FIN
   end
end
...
select 
'Institucion'     = 'SANTANDER INCLUSIÓN FINANCIERA',
'Tipo'            = 'SOLICITUD DE DISPERSIÓN',
'Fecha_operacion' = getdate(),
'Hora_operacion'  = getdate(),
'Folio'           = @w_folio,
'Estado'          = 'EN PROCESO', --en proceso (catalogo)
'Mensaje'         = 'El dinero estará disponible en la cuenta aproximadamente en 60 minutos, si tu operación está dentro del horario establecido.',
