...
if not exists(select 1 from   cobis..cl_parametro where  pa_producto = 'CCA' and    pa_nemonico = 'RCTMAX')
begin
   insert into cl_parametro
         (pa_parametro,                           pa_nemonico, pa_tipo, pa_producto, pa_tinyint)
   values('MAXIMA GENERACION RUBROS DE CATALOGO', 'RCTMAX',    'T',     'CCA',       12)
   print 'CREADO PARAMETRO NECESARIO'
   print ''
end
go
