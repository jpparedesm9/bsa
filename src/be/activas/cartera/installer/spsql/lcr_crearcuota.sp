...
/************************************************************************/
/*                             MODIFICACION                             */
/* 13/06/2019         DCU                 Actualizacion Parametro       */
/* 10/Jun/2019        AGO                    Exigibilidad del Capital   */
/* 17/09/2019         LGU               Caso 125959 ajuste estado op lcr*/
/* 08/31/2021         AGO               Identificar Pago minimo de acuer*/
/*                                      do la regla LCRPAGMIN           */
/************************************************************************/  
...
if @w_error <> 0 begin
   select 
   @w_msg   = 'ERROR AL CREAR EL HISTORIAL'
   goto ERROR_FIN
end
...
EXEC @w_error = cob_cartera..sp_obtener_factor_pgmin
@i_operacion =@i_operacionca,
@o_factor = @w_factor_pag OUT

if @w_error <> 0 OR @w_factor_pag = 0 begin 
  print 'ERROR: ERROR AL EJECUTAR LA REGLA'
  select @w_factor_pag = 33.33
end 



select @w_porcentaje_pag_min = round((1.0/@w_factor_pag)*100,2)
...
if (@w_saldo_cap > @w_porcentaje_pag_min*@w_param_umbral )  select @w_cap_min = isnull(round(@w_saldo_cap/@w_porcentaje_pag_min,@w_numdec),0)


--CASO 2 SALDO CAPITAL ENTRE 3 VECES EL UMBRAL Y EL UMBRAL 
if (@w_saldo_cap between @w_param_umbral and (@w_porcentaje_pag_min*@w_param_umbral)) select @w_cap_min =@w_param_umbral


--CASO 3 EL SALDO DE CAPITAL ES MENOR AL UMBRAL 
if (@w_saldo_cap < @w_param_umbral) select @w_cap_min = @w_saldo_cap  

--CUMPLE ESTAS CUOTAS (3 VENCIDAS) SE DECLARA LA TOTALIDAD DEL CAPITAL COMO VENCIDO 
if (@w_div_vencidos >= @w_param_vencidos)  select @w_cap_min = @w_saldo_cap
...
if @w_crear_cuota = 'S' begin 
   ...
   select @w_observacion = 'GENERACION CUOTA MINIMA: '+convert(varchar, @w_factor_pag)+'% ' 
  ...
  if @@error <> 0 begin
   ...
   if @@error <> 0 begin
      select 
	  @w_error = 710001,
		@w_msg   = 'ERROR: AL ACTUALIZAR ESTADOS DEL CAP DEL ULTIMO DE CORTE DE LA TABLA DE AMORT.'
      goto ERROR_FIN
   end
...
if exists ( select 1 from ca_dividendo where di_operacion =  @i_operacionca and di_dividendo =@w_max_dividendo+1) and @w_saldo_cap <> @w_cap_min begin 
