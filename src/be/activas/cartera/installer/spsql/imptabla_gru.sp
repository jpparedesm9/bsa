...
/*                         MODIFICACIONES                               */
/*      FECHA           AUTOR      RAZON                                */
/*    01/Abr/2020       MTA       Agregar la gracia a los totales de la */
/*                                tabla                                 */
/*    17/Sep/2020       SRO       Req #140073                           */
/*    15/Mar/2022       ACH       Req #168293, para reporte             */
/*    12/Dic/2022       KVI       Req.197007-FlujoB2BGrupal RECA y pie  */
/************************************************************************/
...
@w_pieIf                        varchar(256),
@w_tipo_plazo                   varchar(256)


select 
@w_sp_name       = 'sp_imp_tabla_grupo',
@i_formato_fecha = 103
...
begin
    ...
    select 
	@w_nombre_grupo   = (select UPPER(isnull(en_nombre,''))+' ' + UPPER(isnull(p_s_nombre,''))+' '+
	...
	begin
		...
		-- Inicio Req.197007		
        select @w_pieIf = pa_char from cobis..cl_parametro where pa_nemonico IN ('PPREPA')
		
		if (@w_desplazamiento > 0)
		begin
			select @w_CONDUSEF = pa_char from cobis..cl_parametro where pa_nemonico IN ('RDRECA') and pa_producto = 'CRE' 
		end 
		else
		begin
			select @w_CONDUSEF = pa_char from cobis..cl_parametro where pa_nemonico IN ('RECASG') and pa_producto = 'CRE' 
		end
	    -- Fin Req.197007
	end
	else if(@w_toperacion_cod = 'INDIVIDUAL') begin
		select @w_CONDUSEF = pa_char from cobis..cl_parametro where pa_nemonico IN ('CRSIAO')
        select @w_pieIf = pa_char from cobis..cl_parametro where pa_nemonico IN ('PCTSIO')
        select @w_tipo_plazo = (select valor from cobis..cl_tabla t, cobis..cl_catalogo c 
                                where t.codigo = c.tabla and t.tabla = 'cr_tplazo_ind' 
								and c.codigo = (case when op_tplazo = 'W' and op_periodo_int = 2 and op_periodo_cap = 2 then 'BW' else op_tplazo end))								
      from cob_cartera..ca_operacion
      where op_banco = @i_banco
	end
	
   select
   'FECHA_IMP'      = @w_fecha_imp, --1
   'HORA'           = @w_hora,      --2
   'USUARIO'        = @s_user,      --3
   'OFICINA_ID'     = @w_oficina,   --4
   'OFICINA_NOMBRE' = @w_nombre_oficina, --5
   'GRUPO_ID'       = @w_grupo,          --6
   'GRUPO_NOMBRE'   = @w_nombre_grupo,   --7
   'DIR_REUNION'    = @w_direccion_reunion,--8
   'TIPO_OPERACION' = @w_toperacion_cod, --9
   'FECHA_DESEMBOLSO' = @w_fecha_des,    --10
   'FECHA_LIQUIDACION' = @w_fecha_liq,  --11
   'DESPLAZAMIENTO'    = @w_desplazamiento, --12
   'CONDUSEF' = @w_CONDUSEF, --13
   'PIEIF'    = @w_pieIf,   --14
   'TIPO_PLAZO' = @w_tipo_plazo --15
end -- C
   ...
   select 'Dividendo'         = A.di_dividendo,
          ...
          'Saldo Capital'     = (SELECT CASE WHEN A.di_dividendo = 1
                                             ...
                                             THEN (O.op_monto) - (select sum(am_cuota)
                                                                  ...
                                                                  and am_concepto  = 'CAP')
                                             END ),
          'Capital'           = (select sum(am_cuota + am_gracia)
                                 from ca_amortizacion
                                 where am_operacion = O.op_operacion
                                   and am_dividendo = A.di_dividendo
                                   and am_concepto  = 'CAP'),
          'Intereses'         = (select sum(am_cuota + am_gracia)
                                 from ca_amortizacion
                                 where am_operacion = O.op_operacion
                                  and am_dividendo  = A.di_dividendo
                                  and am_concepto   in ( 'INT', 'INT_ESPERA')),       
          'Mora'              = (select sum(am_cuota  + am_gracia )
                                 from ca_amortizacion
                                 where am_operacion = O.op_operacion
                                  and am_dividendo  = A.di_dividendo
                                  and am_concepto   = 'IMO'),       
          'Otros'             = (select sum(am_cuota  + am_gracia)
                                 from ca_amortizacion
                                 where am_operacion = O.op_operacion
                                   and am_dividendo = A.di_dividendo
                                   and am_concepto  not in ('CAP','INT','IMO', 'INT_ESPERA')),
          'Cuota'             = (select sum(am_cuota  + am_gracia) 
                                 from ca_amortizacion
                                 where am_operacion = O.op_operacion
                                   and am_dividendo = A.di_dividendo
								   and am_concepto in ('CAP','INT','IVA_INT',  'IVA_ESPERA', 'INT_ESPERA', 'SEGAD')),-- se suma segun lo que se piden en el reporte					   
          'IVA_Intereses'     = (select sum(am_cuota  + am_gracia)
                                 from  ca_amortizacion
                                 where am_operacion = O.op_operacion
                                 and   am_dividendo  = A.di_dividendo
                                 and   am_concepto   IN ('IVA_INT', 'IVA_ESPERA'))
