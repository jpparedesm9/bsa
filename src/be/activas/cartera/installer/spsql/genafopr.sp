...
/*   16/02/2022   D. Cumbal                   Santander no tiene Ctas Ah*/
/************************************************************************/
use cob_cartera
go
...
if @w_pcobis = 99 or @w_pcobis  = 0 return 0  -- NO SE EJECUTA NADA POR SER NO COBIS O NO ESTA RELACIONADO
...
begin
   ...
   begin --inicio  existe validacion con cobis-ahorros
       exec @w_error = cob_interfase..sp_verifica_cuenta_aho
                   @i_operacion =  'VAHO4',
                   @i_cuenta    =  @i_cuenta,
...
if @w_pcobis = 3
begin
   /*select @w_cc_estado = cc_estado
   from cob_cuentas..cc_ctacte
   where cc_cta_banco = @i_cuenta
   if @@rowcount = 0     */
   exec @w_error = cob_interfase..sp_verifica_cuenta_cte
                @i_operacion =  'VCTE4',
                @i_cuenta    =  @i_cuenta,
                @o_ah_estado    =  @w_ah_estado out
    if @w_ah_estado = null or @w_ah_estado = ''
...
begin
    ...
    and    pa_producto = 'CCA'
    ----PRINT 'genafopr.sp anes de ejecutar cob_interfase..sp_calcula_sin_impuesto @i_en_linea : '  + cast (@i_en_linea as varchar)
...
@w_ssn    = case when se_numero > 2147483000 then 100 else se_numero + 100 end,
