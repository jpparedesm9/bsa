...
/*      Producto:               Cartera                                 */
/*      Archivo:                apcuotan.sp                             */
/*      Procedimiento:          sp_aplicacion_cuota_normal              */
...
/*      17/10/2019      D.Cumbal              Incidencia 128541         */
/*      16/07/2020      ACH                   Incidencia 142582         */
/*      23/07/2020      D.Cumbal              Caso 142361 ajuste cambio */
/************************************************************************/
...
select @w_vlr_despreciable = 1.0 / power(10, @i_num_dec)
...
if @w_div_vigente = 0 and @w_toperacion <> 'REVOLVENTE'
begin
...
select @w_saldo_oper = round (@w_saldo_oper,@w_numdec_op)
...
begin
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            select @w_vlr_gracia = round(@w_gracia_acum / @w_num_div_tmp, @w_numdec_op)
   ...
   and    ro_concepto  = am_concepto
   and    ro_fpago    <> 'M'
   and    am_concepto not like '%ESPERA%'
end
...
while @@fetch_status = 0 begin
   ...
   while @@fetch_status = 0   begin
      ...
      while @@fetch_status = 0  begin
         ...
         if @w_monto_prioridad <= 0 select @w_salir_rubro = 'S'
   ...
   begin
      ...
      1,                  0,                @w_codvalor_nov,
      -@w_devolucion,
      -round(@w_devolucion*@i_cotizacion,   @w_numdec_op),
      @w_moneda,
   ...
   begin
      ...
      if @w_total_div < @w_vlr_despreciable select @w_cancelar_div = 'S'
   ...
   if @w_cancelar_div = 'S' and @i_cancelar <> 'S' begin
      ...
      begin
		 ...
		 and   am_concepto  not in (select ro_concepto 
                                       from   ca_rubro_op where ro_concepto_asociado in (select ro_concepto  from ca_rubro_op where ro_fpago  = 'M' and ro_operacion =@i_operacionca)          and    ro_operacion = @i_operacionca
                                       union
                                       select ro_concepto 
                                       from   ca_rubro_op
                                       where  ro_fpago    = 'M'
...
begin
   ...
   select @w_causacion_pte_mn = round(@w_causacion_pte * @i_cotizacion, @w_num_dec_n)
   select @w_causacion_pte    =  round(@w_causacion_pte,@w_numdec_op)
...
begin
   ...
   select @w_causacion_pte_mn = round(@w_causacion_pte_ajuste * @i_cotizacion, @w_num_dec_n)
   select @w_causacion_pte_ajuste    =  round(@w_causacion_pte_ajuste,@w_numdec_op)
