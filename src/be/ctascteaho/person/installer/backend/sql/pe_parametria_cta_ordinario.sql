...
delete cob_remesas..pe_categoria_profinal WHERE cp_categoria in ('R')
...
----------------------------------------------------------------------------------------------------------------------------
--- Parametro PCAASO
	if exists (select 1 from cobis..cl_parametro where pa_nemonico = 'PCAASO')
	begin
...
BEGIN
	...
	if exists(select 1 from tempdb..sysobjects where upper(name) like upper('%costos%'))
	begin
		drop table #costos
	...
	insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('MMAP',28,'39',1000,1000,1000)             -- 'MMAP' -- 'MONTO MINIMO APERTURA' -- 'pe_rubro'  'MONTO MINIMO'
	insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('SALDO MINIMO',50,'SMI',0,0,0)             -- 'MMAP' -- 'MONTO MINIMO APERTURA' -- 'pe_rubro'  'MONTO MINIMO'
	insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('SALDO MINIMO',50,'SMA',10000,10000,10000) -- 'SMC'  -- 'SALDO MINIMO' -- 'pe_rubro'  'SALDO MAXIMO DE LA CUENTA'
	insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CIERRE DE CUENTAS',14,'22',0,0,0)         -- 'CCTA' -- 'CIERRE DE CUENTAS' -- 'pe_rubro'  'MULTA POR GASTOS DE MANEJO'
	insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CIERRE DE CUENTAS',14,'3',0,0,0)          -- 'CCTA' -- 'CIERRE DE CUENTAS' -- 'pe_rubro'  'DEDUCCION DE INTERES CIERRE ANTICIPADO'
	insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CIERRE DE CUENTAS',14,'41',0,0,0)         -- 'CCTA' -- 'CIERRE DE CUENTAS' -- 'pe_rubro'  'CARGO CIERRE CUENTA ANTES DE TIEMPO'
	-- insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('PINT',1,'23',0,0,0)                    -- 'PINT' -- 'PAGO DE INTERES' -- 'pe_rubro'  'Interes por Cierre (JCA)'
	insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('PINT',1,'18',0,0,0)                       -- 'PINT' -- 'PAGO DE INTERES' -- 'pe_rubro'  'PAGO INT. SOBRE EL DISPONIBLE'
	-- insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('PINT',1,'2',0,0,0)                     -- RETENCION IMPUESTO POR INTERESSELECT
	insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('SECU',10,'15',1.00, 1.25, 1.50)           -- COSTO EXTRACTO CUENTA
	insert into #costos (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CDEV',7,'11',1.00, 1.25, 1.50)            -- COSTO DEVOLUCION CHEQUE

	select @w_contador_costos = count(1) from #costos
	SELECT @w_while2 = 0
