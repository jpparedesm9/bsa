...
delete cob_remesas..pe_categoria_profinal WHERE cp_categoria in ('A')
       
----------------------------------------------------------------------------------------------------------------------------
--- Tipo Rango sobre DISPONIBLE (para MMAP y SMS)
    if exists(select 1 from cob_remesas..pe_tipo_rango WHERE tr_descripcion = 'RUBRO SOBRE SALDO DISPONIBLE' and tr_estado = 'V')     
    begin
...
----------------------------------------------------------------------------------------------------------------------------
--- Parametro PCAASA
    if exists (select 1 from cobis..cl_parametro where pa_nemonico = 'PCAASA')
    begin
...
BEGIN
    ...
    --- Producto Final
        ...
        begin
            ...
            values (@w_pro_final,1,1,@w_mercado,4,0,'R',@w_descripcion_profinal,@w_codigo_dep,'S',2)
    ...
    ----------------------------------------------------------------------------------------------------------------------------
    --- WHILE DE CREACION DE costos_adic
    
    if exists(select 1 from tempdb..sysobjects where upper(name) like upper('%costos_adic%'))
    begin
        drop table #costos_adic
        
        if (@@error > 0)
        begin
            SELECT @w_msg_error = 'ERROR AL ELIMINAR LA TABLA #costos_adic'
        end 
    end
        
    create table #costos_adic (
        id                  INT         IDENTITY,
    ...
    insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('MMAP',28,'39',1000,1000,1000)             -- 'MMAP' -- 'MONTO MINIMO APERTURA' -- 'pe_rubro'  'MONTO MINIMO'
    insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('SALDO MINIMO',50,'SMI',0,0,0)             -- 'MMAP' -- 'MONTO MINIMO APERTURA' -- 'pe_rubro'  'MONTO MINIMO'
    insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('SALDO MINIMO',50,'SMA',10000,10000,10000) -- 'SMC'  -- 'SALDO MINIMO' -- 'pe_rubro'  'SALDO MAXIMO DE LA CUENTA'
    insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CIERRE DE CUENTAS',14,'22',0,0,0)         -- 'CCTA' -- 'CIERRE DE CUENTAS' -- 'pe_rubro'  'MULTA POR GASTOS DE MANEJO'
    insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CIERRE DE CUENTAS',14,'3',0,0,0)          -- 'CCTA' -- 'CIERRE DE CUENTAS' -- 'pe_rubro'  'DEDUCCION DE INTERES CIERRE ANTICIPADO'
    insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CIERRE DE CUENTAS',14,'41',0,0,0)         -- 'CCTA' -- 'CIERRE DE CUENTAS' -- 'pe_rubro'  'CARGO CIERRE CUENTA ANTES DE TIEMPO'
    -- insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('PINT',1,'23',0,0,0)                    -- 'PINT' -- 'PAGO DE INTERES' -- 'pe_rubro'  'Interes por Cierre (JCA)'
    insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('PINT',1,'18',10,10,10)                    -- 'PINT' -- 'PAGO DE INTERES' -- 'pe_rubro'  'PAGO INT. SOBRE EL DISPONIBLE'
    -- insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('PINT',1,'2',0.4,0.5,0.6)               -- RETENCION IMPUESTO POR INTERESSELECT
    insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('SECU',10,'15',1.00, 1.25, 1.50)           -- COSTO EXTRACTO CUENTA
    insert into #costos_adic (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CDEV',7,'11',1.00, 1.25, 1.50)            -- COSTO DEVOLUCION CHEQUE

    select @w_contador_costos_adic = count(1) from #costos_adic
    SELECT @w_while2 = 0
    

    WHILE (@w_while2 < @w_contador_costos_adic)
    BEGIN
        select @w_while2 = @w_while2 + 1
        select 
        @w_costo_actual  = costo_actual,
