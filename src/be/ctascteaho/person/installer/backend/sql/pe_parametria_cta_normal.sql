...
----------------------------------------------------------------------------------------------------------------------------
--- Parametro PCANOR
    if not exists (select 1 from cobis..cl_parametro where pa_nemonico = 'PCANOR')
    begin
        INSERT INTO cobis..cl_parametro (pa_parametro, pa_nemonico, pa_tipo, pa_char, pa_tinyint, pa_smallint, pa_int, pa_money, pa_datetime, pa_float, pa_producto)
        VALUES (@w_descripcion, 'PCANOR', 'I', NULL, NULL, NULL, @w_codigo, NULL, NULL, NULL, 'AHO')
        print 'Creación PCANOR : ' + cast(@w_codigo as varchar)
    end
    else
    begin
        update cobis..cl_parametro set pa_int = @w_codigo where pa_nemonico = 'PCANOR'
        print 'Actualización PCANOR : ' + cast(@w_codigo as varchar)
    end    
...
BEGIN
    ...
    begin
        ...
        select @w_descripcion_profinal = @w_descripcion + ' - COMPANIA.'
    ...
    --- Categoria ProFinal
        ...
        if exists( select 1 from cob_remesas..pe_categoria_profinal WHERE cp_profinal =  @w_pro_final and cp_categoria = @w_categoria)
            delete cob_remesas..pe_categoria_profinal WHERE cp_profinal =  @w_pro_final and cp_categoria = 'D'
    ...
    ----------------------------------------------------------------------------------------------------------------------------
    --- Ciclo ProFinal
        if exists( select 1 from cob_remesas..pe_ciclo_profinal WHERE cp_profinal = @w_pro_final AND cp_ciclo = @w_tipo_capitali)              
            delete cob_remesas..pe_ciclo_profinal WHERE cp_profinal = @w_pro_final AND cp_ciclo = @w_tipo_capitali
    ...
    if exists(select 1 from tempdb..sysobjects where upper(name) like upper('%costosn%'))
    begin
        drop table #costosn
        
        if (@@error > 0)
        begin
            SELECT @w_msg_error = 'ERROR AL ELIMINAR LA TABLA #costos'
           --    GOTO ERROR
        end 
    end
        
    create table #costosn (
        id                  INT         IDENTITY,
    ...
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('MMAP',28,'39',80, 150, 250)             -- 'MMAP' -- 'MONTO MINIMO APERTURA' -- 'pe_rubro'  'MONTO MINIMO'
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('SALDO MINIMO',50,'SMI',1000, 1400.00, 1500.00)             -- 'MMAP' -- 'MONTO MINIMO APERTURA' -- 'pe_rubro'  'MONTO MINIMO'
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('SALDO MINIMO',50,'SMA',10000,10000,10000) -- 'SMC'  -- 'SALDO MINIMO' -- 'pe_rubro'  'SALDO MAXIMO DE LA CUENTA'
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CIERRE DE CUENTAS',14,'22',5,10,20)         -- 'CCTA' -- 'CIERRE DE CUENTAS' -- 'pe_rubro'  'MULTA POR GASTOS DE MANEJO'
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CIERRE DE CUENTAS',14,'3',3.50,3.60,3.70)          -- 'CCTA' -- 'CIERRE DE CUENTAS' -- 'pe_rubro'  'DEDUCCION DE INTERES CIERRE ANTICIPADO'
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CIERRE DE CUENTAS',14,'41',5,10,20)         -- 'CCTA' -- 'CIERRE DE CUENTAS' -- 'pe_rubro'  'CARGO CIERRE CUENTA ANTES DE TIEMPO'
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('PINT',1,'18',4.25,4.50,5.00)                       -- 'PINT' -- 'PAGO DE INTERES' -- 'pe_rubro'  'PAGO INT. SOBRE EL DISPONIBLE'
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('PINT',1,'2',1.12,1.25,1.50)                        -- RETENCION IMPUESTO POR INTERESSELECT
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('PINT',1,'23',1.10,1.20,1.30) 
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('SECU',10,'15',1.00, 1.25, 1.50)           -- COSTO EXTRACTO CUENTA
    insert into #costosn (costo_actual, servicio_disponible, rubro, minimo, base, maximo) values ('CDEV',7,'11',1.00, 1.25, 1.50)            -- COSTO DEVOLUCION CHEQUE
           
    select @w_contador_costos = count(1) + 1 from #costosn
	SELECT @w_while2 = 0
    ...
    BEGIN
        ...
        begin
            ...
            VALUES (@w_pro_final, @w_servicio_disponible, @w_rubro, @w_ser_per, @w_tipo_rango_disp, 1)
