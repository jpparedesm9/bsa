...
/*     14/Jun/2016    C. Milan        Prints Indican parametria faltante    */
/*     01/Ago/2016    E. Williams     Correccion fecha de vigencia          */ 
/*                                    en pe_val_contratado                  */
/*     22/Sep/2016    T. Baidal       Commit la transaccion abierta cuando  */
/*                                    es batch y hay error para devolver    */
/*                                    mismo num de transacciones abiertas   */
/*     30/Sep/2016    T. Baidal       Correccion operador en consulta de    */
/*                                    valor contratado                      */
/****************************************************************************/
use cob_remesas
go
...
as
  declare
    ...
    @w_severidad     int,
    @w_desc_serv_dis descripcion,
    @w_msg	         varchar(255),
	@w_transaccion_local char(1)
  ...
  begin /*no existe mercado*/
    if @w_debug = 'S' and @i_tran_ext = 'N'
       print '@i_tipo_ente: ' + @i_tipo_ente + ' @i_prod_banc: ' + cast(@i_prod_banc as varchar)

	select @w_msg = mensaje from cobis..cl_errores where numero = 351511
	select @w_msg = '[genera costos] Mensaje: "'+@w_msg+'". Ser_Dis: "'+@i_servicio+'" - Rubro: "'+@i_rubro+'"'
        select @w_error = 351511
    goto ERROR
  end
  ...
  begin /*no existe producto final */
    ...
    select @w_msg = mensaje from cobis..cl_errores where numero = 351527
    select @w_msg = '[genera costos]: "'+@w_msg+'". Suc: '+convert(varchar(20),@w_sucursal)+' - Prod: '+convert(varchar(20),@i_producto) + ' - Mercado: '+convert(varchar(20),@w_mercado)+' - Moneda: '+convert(char(3),@i_moneda)
  ...
  select
    ...
    @w_desc_serv_dis = sd_descripcion
  ...
  begin /*no existe servicio disponible */
   ...
   select @w_msg = mensaje from cobis..cl_errores where numero = 351501
   select @w_msg = '['+convert(varchar(20),@w_desc_serv_dis)+'] : "'+@w_msg+'". Ser_Dis: '+convert(varchar(20),@w_servicio_dis)+' Nemo: "'+@i_servicio+'" - Rubro: "'+@i_rubro+'"'
  ...
  begin /*Error al buscar rubros */
   ...
   select @w_msg = mensaje from cobis..cl_errores where numero = 351531
   select @w_msg = '['+convert(varchar(20),@w_desc_serv_dis)+'] : "'+@w_msg+'". Ser_Dis:'+convert(varchar(20),@w_servicio_dis)+' Nemo: "'+@i_servicio+'" - Rubro: "'+@i_rubro+'"'
  ...
  begin /*No existe servicio personalizable*/
    ...
    select @w_msg = mensaje from cobis..cl_errores where numero = 351540
    select @w_msg = '['+convert(varchar(20),@w_desc_serv_dis)+'] : "'+@w_msg+'". Suc: '+convert(varchar(5),@w_sucursal)+' - Prod_Final: '+convert(varchar(20),@w_pro_final)+' - Ser_Dis: '+convert(varchar(20),@w_servicio_dis)+' Nemo: "'+@i_servicio+'" - Rubro: "'+@i_rubro+'"'
  ...
  begin /*Error al buscar tipo de rango */
    ...
    select @w_msg = mensaje from cobis..cl_errores where numero = 351532
    select @w_msg = '['+convert(varchar(20),@w_desc_serv_dis)+'] : "'+@w_msg+'". Ser_Dis:'+convert(varchar(20),@w_servicio_dis)+' Nemo: "'+@i_servicio+'" - Rubro: "'+@i_rubro+'"' + ' - Tipo_rango: '+convert(char(3),@w_tipo_rango)
  ...
  begin /*Error al buscar rango */
    ...
    select @w_msg = mensaje from cobis..cl_errores where numero = 351551
    select @w_msg = '['+convert(varchar(20),@w_desc_serv_dis)+'] : "'+@w_msg+ '". Ser_per: '+convert(varchar(15),@w_servicio_per)+' - T_atrib: "' + @w_tipo_atributo + '"  - T_rango: "'+convert(varchar(5),@w_tipo_rango)+'" - G_rango: "'+convert(varchar(5),@w_grupo_rango)+'" - Monto:'+ convert(varchar(25),@w_monto)
  ...
  begin /*Error al buscar costo */
    ...
    begin
     ...
     select @w_msg = mensaje from cobis..cl_errores where numero = 351552
     select @w_msg = '['+convert(varchar(20),@w_desc_serv_dis)+'] :"'+@w_msg+'". Suc: '+convert(varchar(5),@w_sucursal)+' - Ser_Per: '+convert(varchar(20),@w_servicio_per)+'. Ser_Dis: '+convert(varchar(20),@w_servicio_dis)+' Nemo: "'+@i_servicio+'" - Rubro: "'+@i_rubro+'"'+'- Categ: "'+@i_categoria+'"'
  ...
  begin    
	...
	--22/Sep/2016 Si es batch y si se ha abierto una transaccion realiza commit 
	else if @i_batch = 'S' and @w_transaccion_local = 'S' 
	begin
	   commit tran
	end
