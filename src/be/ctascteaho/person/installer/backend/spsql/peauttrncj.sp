...
/*     04/Jul/2016     Roxana Sánchez  Se modifico por el catalago          */
/*                                     cl_trn_central_local al catalogo     */
/*                                     pe_aut_trn_caja                      */
/*     15/Jul/2016     Eduardo Williams AHO-H77223 y AHO-H77224             */
/****************************************************************************/
use cob_remesas
go
if exists (select
...
begin 
   ...
   if not exists(select 1 from cobis..cl_catalogo
             where tabla = (select codigo from cobis..cl_tabla where tabla = 'pe_aut_trn_caja')
             and   codigo = @i_tran)
   begin 
...
begin 
   ...
   '1063' = at_categoria, 
   '1024' = at_autorizada, 
   '1356' = convert(varchar,at_fecha_crea,103), 
   '1362' = convert(varchar,at_fecha_upd,103),
   '1333' = at_estado
   from re_aut_tran_caja
...
begin
   ...
   where tabla = (select codigo from cobis..cl_tabla 
                  where tabla = 'pe_aut_trn_caja')
   and   codigo = convert(varchar,@i_tran)
