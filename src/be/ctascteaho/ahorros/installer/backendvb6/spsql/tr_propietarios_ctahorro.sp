...
as
  ...
  select
    @w_sp_name = 'sp_tr_propietarios_ctahorro'
  ...
  select
    @w_rpc = 'sp_propietarios_ctahorro'
  if @w_tipo = 'L'
    select
  ...
  begin
    ...
    begin
      insert into cob_ahorros..ah_tscliente_ctahorro
                  (secuencial,ssn_branch,tipo_transaccion,tsfecha,clase,
                   usuario,terminal,oficina,reentry,origen,
                   cta_banco,moneda,cliente,det_producto,oficina_cta,
                   turno)
      values      (@s_ssn,@s_ssn_branch,@t_trn,@s_date,'B',
                   @s_user,@s_term,@s_ofi,@t_rty,@s_org,
                   @i_cta,@i_mon,@i_cli,@i_dprod,@w_oficina_cta,
                   @i_turno)
      if @@error <> 0
      begin
        /* Error en creacion de transaccion de servicio */
