...
if exists (select
             1
           from   sysobjects
           where  name = 'sp_datos_mov_rec')
  drop proc sp_datos_mov_rec
go

/****** Object:  StoredProcedure [dbo].[sp_datos_mov_rec]    Script Date: 03/05/2016 9:52:01 ******/
set ANSI_NULLS off
GO
...
/* DETERMINAR LA CIUDAD DE LOS FERIADOS NACIONALES */-- Validacion Req 203
  ...
  begin
    select
      @w_error = 147000,
      @w_msg =
    'Error al eliminar registros cob_externos..ex_dato_transaccion_det'
    goto ERROR
  end
  ...
  begin
    ...
    select
      ...
      dt_secuencial = row_number()
                        over(
                          order by tm_cta_banco, tm_secuencial asc),
      ...
      dt_tipo_trans = isnull((select
                             ...
                             'XXXXX'),
      dt_moneda = isnull(tm_moneda,
                         0),
      dt_concepto = cc_concepto,
      dt_valor = case cc_campo1
                   when 'tm_valor' then isnull(tm_valor,
                                               0)
                   when 'tm_chq_propios' then isnull(tm_chq_propios,
                                                     0)
                   when 'tm_chq_locales' then isnull(tm_chq_locales,
                                                     0)
                   when 'tm_chq_ot_plazas' then isnull(tm_chq_ot_plazas,
                                                       0)
                   when 'tm_efectivo' then isnull(tm_efectivo,
                                                  0)
                   when 'tm_saldo_interes' then isnull(tm_saldo_interes,
                                                       0)
                   when 'tm_interes' then isnull(tm_interes,
                                                 0)
                   when 'tm_monto_imp' then isnull(tm_monto_imp,
                                                   0)
                   when 'tm_valor_comision' then isnull(tm_valor_comision,
                                                        0)
                   else 0
                 end,
      dt_reversa = isnull(tm_estado,
                          'N'),
      dt_naturaleza = tm_signo,
      dt_canal = isnull((select
                           eq_val_interfaz
                         from   cob_remesas..re_equivalencias
                         where  eq_modulo  = 4
                            and eq_mod_int = 36
                            and eq_tabla   = 'CANAL'
                            and t.tm_canal = eq_val_cfijo),
                        'OFI'),
      dt_oficina = tm_oficina,
      dt_secuencial_caja = isnull(tm_ssn_branch,
                                  tm_secuencial),
...
*****/
  ...
  begin
    insert into #ex_dato_transaccion_det
      ...
      select
        @w_fecha_proc,dt_secuencial,dt_banco,dt_toperacion,4,
        isnull((select
                  eq_val_interfaz
                from   cob_remesas..re_equivalencias
                where  eq_modulo    = 4
                   ...
                   and eq_val_cfijo = dt_concepto),
               dt_concepto),dt_moneda,valor,sum(dt_valor)
      from   #transaccion_cb,
    ...
    begin
      select
        @w_error = 143003,
        @w_msg =
      'Error en al Grabar en table cob_externos..ex_dato_transaccion_det'
      print @w_msg
      goto ERROR
    end
    ...
    begin
      select
        @w_error = 143003,
        @w_msg =
      'Error en al Grabar en table cob_externos..ex_dato_transaccion_det CAP'
      print @w_msg
      goto ERROR
    end
  ...
  insert into cob_externos..ex_dato_transaccion
              (dt_fecha,dt_secuencial,dt_banco,dt_toperacion,dt_aplicativo,
               ...
               dt_fecha_hora)
    select
      ts_tsfecha,ts_secuencial,ts_cta_banco,'0',4,
      ts_tsfecha,case ts_tipo_transaccion
        when 230 then 'CONS'
      end,isnull(ts_estado,
             'N'),'D','OFI',
      ts_oficina,ts_ssn_branch,substring(ts_usuario,
                1,
                14),substring(ts_terminal,
                1,
                20),ts_hora
    ...
    where  ts_tipo_transaccion in (230)
       ...
       and ts_ssn_branch not in (select
                                   dt_secuencial_caja
  ...
  begin
    select
      @w_error = 143003,
      @w_msg =
      'Error en al Grabar en table cob_externos..ex_dato_transaccion_det EFEMN '
    print @w_msg
    goto ERROR
  end
  ...
  insert into cob_externos..ex_dato_transaccion
              (dt_fecha,dt_secuencial,dt_banco,dt_toperacion,dt_aplicativo,
               ...
               dt_fecha_hora)
    select
      ts_tsfecha,ts_secuencial,ts_cta_banco,convert(varchar(10), ts_cheque),3,
      ts_tsfecha,case ts_tipo_transaccion
        when 64 then 'PAGCHQ'
      end,isnull(ts_estado,
             'N'),'D','OFI',
    ...
    where  ts_tipo_transaccion = 64
       ...
       and ts_secuencial not in (select
                                   dt_secuencial
                                 from   cob_externos..ex_dato_transaccion
                                 where  dt_fecha      = @w_fecha_proc
                                    and dt_aplicativo = 3)
  ...
  begin
    select
      @w_error = 201242,
      @w_msg =
    'Error al Grabar en tabla cob_externos..ex_dato_transaccion_det EFEMN '
    print @w_msg
    goto ERROR
  end
  ...
  insert into cob_externos..ex_dato_transaccion
              (dt_fecha,dt_secuencial,dt_banco,dt_toperacion,dt_aplicativo,
               ...
               dt_fecha_hora)
    select
      ts_fecha,ts_ssn,ts_banco_orig,'0',4,
      ts_fecha,'CONS',ts_correccion,'D',(select
         eq_val_interfaz
       from   cob_remesas..re_equivalencias
       where  eq_tabla     = 'CANAL'
          and eq_val_cfijo = ts_canal),
      isnull(ts_codcorr,
             ts_oficina),0,ts_login_ext,ts_terminal,ts_hora
  ...
  begin
    ...
    select
      @w_error = 143004,
      @w_msg =
    'Error en al Grabar en tabla cob_externos..ex_dato_transaccion (ws)'
    print @w_msg
    goto ERROR
  end
  ...
  begin
    select
      @w_error = 143004,
      @w_msg =
      'Error en al Grabar en tabla cob_externos..ex_dato_transaccion_det (ws)'
    print @w_msg
    goto ERROR
  end
  ...
  begin
    ...
    begin
      select
        @w_error = 147000,
        @w_msg =
      'Error al eliminar registros cob_externos..ex_carga_archivo_ctas'
      goto ERROR
    end
  end
  ...
  begin
    ...
    begin
      select
        @w_error = 147000,
        @w_msg =
      'Error al eliminar registros cob_ahorros..ah_carga_archivo_cuentas'
      goto ERROR
    end
  end
