...
/************************************************************************/
/*  Archivo:            tr_cons_cierre_ah.sp                            */
/*  Stored procedure:   sp_tr_cons_cierre_ah                            */
...
if exists (select
             1
           from   sysobjects
           where  name = 'sp_tr_cons_cierre_ah')
  drop proc sp_tr_cons_cierre_ah
go

create proc sp_tr_cons_cierre_ah
(
  @s_ssn          int,
  ...
  @i_val_inac     char(1) = 'S',--PARA VALIDAR O NO LA INACTIVIDAD  ROL 12122006
...
as
  ...
  select
    @w_sp_name = 'sp_tr_cons_cierre_ah'
  ...
  if @w_tipo = 'L'
    exec @w_return = @w_rpc
      ...
      @i_cta          = @i_cta,
      @i_mon          = @i_mon,
      @i_cob_multa    = @i_cob_multa,
      @i_val_inac     = @i_val_inac,-- ROL 12/12/2006  valida inactividad o  no
