...
/*    contables de MONEDA, CLASE CLIENTE, PRODUCTO, ESTADO y CONCEPTO;  */
/*    en el campo estado debe incluir el estado T, Traslado al Tesoro   */
/*    Nacional y En el concepto Capital e Interes                       */
/************************************************************************/
/*                             MODIFICACION                             */
/*    FECHA                 AUTOR                 RAZON                 */
...
if exists (select
             1
           from   sysobjects
           where  name = 'sp_ah04_pf')
  drop proc sp_ah04_pf
go
...
as
  ...
  begin
    insert into #ca_catalogo
      select
        3,pb_pro_bancario,pb_descripcion
  ...
  begin
    insert into #ca_catalogo
      select
        5,cat.codigo,cat.valor
      from   cobis..cl_tabla tab,
             cobis..cl_catalogo cat
      where  tab.tabla  = 're_concepto_contable'
         and tab.codigo = cat.tabla
