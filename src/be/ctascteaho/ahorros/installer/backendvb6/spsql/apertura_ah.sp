...
/* 1era. llamada al SP debe ser con 'T' por det_producto con el */
  ...
  begin
    ...
    if (@w_tipo_documento <> @w_tipo_cedula
        and @w_tipo_ente = 'P')
    begin
      select
        @w_estado_sol = ''
    ...
    select
      @w_sucursal_t = isnull(of_sucursal,
                             of_regional)
    from   cobis..cl_oficina
  ...
  /* Obtencion de la descripcion de la direccion del estado de cuenta */
    ...
    begin
      select
        @w_descdir_ec = rtrim(ltrim(di_descripcion)),
    ...
    if isnull(@w_descdir_ec,
              ' ') = ' '
       and @i_estado_cuenta = 'S' --HA20060831
    begin
      select
        @w_error = 252072
      goto ERROR
  ...
  /* zona = provincia = departamento                                  */
      ...
      begin
        select
          @w_prx_mes = convert(varchar(10), dateadd(month,
                                                    ...
                                                    @w_fecha_hoy), 101)
        select
          @w_fecha_prx_corte = substring(@w_prx_mes, 1, 2) + '/' + @w_dia_ciclo
                               +
                               '/'
                               +
    ...
    /****** En este caso envian a validar la fecha actual y si es festivo se regresan un dia *****/
      ...
      select
        @w_fecha_prx_ec = dateadd(dd,
                                  1,
                                  convert(datetime, @w_fecha_prx_corte, 101))
    ...
    /*** La determinacion del anterior dia laboral  se          ****/
      ...
      select
        @w_fecha_prx_corte = convert(varchar(10), @w_fecha_prx_ec, 101)
    end

    if @i_capit = 'D'
    begin
      select
        @w_fecha_prx_capita = @w_fecha_hoy
    end
    else
    begin
      if @i_capit = 'M'
    ...
    /****** En este caso envian a validar la fecha actual y si es festivo se regresan un dia *****/
      /****** Por ello a la fecha le sumamos un dia e invocamos el sp restando un dia          *****/
      select
        @w_ult_dia_mes = convert(datetime, dateadd(dd,
                                                   1,
                                                   @w_ult_dia_mes), 101)
  ...
  /**EN @i_tipo_cta SE DEBE ENVIAR EL PRODUCTO FINAL; YA QUE LOS PREFIJOS DE NUMEROS DE CUENTA**/
    ...
    values      (@w_cuenta,@o_cta_banco,@w_filial,--1
                 ...
                 $0,$0,$0,$0,$0,--37
                 $0,$0,$0,$0,$0,--38
                 @i_titularidad,0.0,'G',@i_permite_sldcero,
    ...
    values      ( @s_ssn,isnull(@s_ssn_branch,
                  ...
                  ,
                  @i_tipocta_super,@i_turno,@i_prodbanc,
                  @i_fideicomiso)
