...
as
  ...
  if @w_debug = 'S'
     and @i_tran_ext = 'N'
  begin
    print '@i_cta: ' + @i_cta + ' @i_val: ' + cast (@i_val as varchar) +
          ' @i_cau: ' +
          @i_cau + ' @i_alt '
          + cast(@i_alt as varchar)
  end
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        print '@t_ssn_corr ' + cast(@t_ssn_corr as varchar) + ' @i_alt_corr '
              + cast(@i_alt_corr as varchar)
        print '@i_cta ' + @i_cta + ' @i_cau ' + @i_cau + ' @i_mon ' + cast(
              @i_mon
              as
              varchar)
  ...
  begin
    ...
    begin
      ...
      select
        @w_monto_dev = isnull(tm_monto_imp,
                              0),
        @w_iva_dev = isnull(tm_valor_comision,
                            0),
        @w_val_mon = isnull(tm_valor,
                            0)
      from   cob_ahorros..ah_tran_monet
      where  isnull(tm_ssn_branch,
      ...
      begin
        ...
        where  isnull(ts_ssn_branch,
                      ts_secuencial) = @t_ssn_corr
      ...
      begin
        if @w_debug = 'S'
           and @i_tran_ext = 'N'
        begin
          print '0. @t_ssn_corr ' + cast(@t_ssn_corr as varchar) + ' @i_alt ' +
                cast
                (
                @i_alt as
                varchar)
          ...
          print
          '@i_mon ' + cast(@i_mon as varchar) + ' @w_val_mon ' + cast(@w_val_mon
          as
          varchar
          )
          print '@w_val ' + cast(@w_val as varchar) + ' @i_val ' + cast(@i_val
                as
                varchar)
        end
        select
      ...
      begin
        ...
        select
          @w_monto_imp_atm = isnull(tm_monto_imp,
                                    0),
          @w_valor1 = isnull(tm_valor,
                             0)
        from   cob_ahorros..ah_tran_monet
        where  isnull(tm_ssn_branch,
                      tm_secuencial) = @t_ssn_corr
        ...
        begin
          if @w_debug = 'S'
             and @i_tran_ext = 'N'
          begin
            print '2.@t_ssn_corr ' + cast(@t_ssn_corr as varchar) +
                  ' @i_alt_corr '
                  + cast(@i_alt_corr as varchar)
            print '@s_user ' + @s_user + ' @i_cta ' + @i_cta + ' @w_causa_atm '
                  +
                  @w_causa_atm
            print '@i_mon ' + cast(@i_mon as varchar) + ' @w_valor1 ' + cast(
                  @w_valor1
                  as
                  varchar
                                       )
            print '@w_valor2 ' + cast(@w_valor2 as varchar) + ' @i_val ' + cast(
                  @i_val
                  as
                  varchar
                                                 )
      ...
      begin
        ...
        begin
          if @w_debug = 'S'
             and @i_tran_ext = 'N'
          begin
            print '3.@t_ssn_corr ' + cast(@t_ssn_corr as varchar) +
                  ' @i_alt_corr '
                  + cast(@i_alt_corr as varchar)
            print '@s_user ' + @s_user + ' @i_cta ' + @i_cta + ' @i_cau ' +
                  @i_cau
            print '@i_mon ' + cast(@i_mon as varchar) + ' @i_val ' + cast(@i_val
                  as
                  varchar)
...
/* Determinar si el cobro del valor en suspenso es por segunda vez */
  ...
  select
    ...
    @w_disponible = round(ah_disponible,
                          @w_numdeci),
    @w_12h = ah_12h,
    @w_24h = ah_24h,
    @w_48h = ah_48h,
    ...
    @w_debitos = round(ah_debitos,
                       @w_numdeci),
    @w_creditos = round(ah_creditos,
                        @w_numdeci),
    @w_capitalizacion = ah_capitalizacion,
    @w_lineas = ah_linea,
    @w_debhoy = round(ah_debitos_hoy,
                      @w_numdeci),
    @w_credhoy = round(ah_creditos_hoy,
                       @w_numdeci),
...
/* FIN -EXTRAER TOPES PARA EL TIPO DE PRODUCTO */
  ...
  if @w_debug = 'S'
     and @i_tran_ext = 'N'
    print '@w_monblq: ' + @w_monblq + ' @i_cau ' + @i_cau + ' @w_cauemb ' + cast
          (
                                   @w_cauemb as varchar)
  ...
  select
    @o_saldo_para_girar = @w_saldo_para_girar
  ...
  begin
    if @w_tasa_imp > 0
        or @i_devolucion_imp > 0
    begin
      if @i_devolucion_imp > 0
        select
          @o_monto_imp = @i_devolucion_imp
      else
      begin
        select
        ...
        if @w_comision > 0
          select
            @w_monto_imp_atm = round((@w_comision * @w_tasa_imp),
                                     @w_numdeci_imp)
        else
          select
            @w_monto_imp_atm = 0
  ...
  begin
    ...
    begin--1
      ...
      begin--2
        ...
        begin--3
          ...
          select
            @w_impuesto = round((@i_val * @w_piva / 100),
                                @w_numdeci_imp)

          if @t_trn = 264
             and (@i_val + @w_impuesto) > @w_saldo_para_girar
          begin
            select
              @w_valor = @w_saldo_para_girar
            select
              @w_piva = @w_piva / 100
            select
              @w_valor = @w_valor / (1 + @w_piva)
            select
              @w_impuesto = round((@w_valor * @w_piva),
                                  @w_numdeci_imp)
          end
          else
            select
      ...
      select
        @w_total_val_imp = @w_val_mon + @w_iva_dev,
        @o_valiva = @w_iva_dev
    ...
    if (@i_titular_prods = @w_cliente)
       and (@i_titularidad_prods = 'I')
       and (@w_ah_ctitularidad = @i_titularidad_prods)
      select
        @w_imp_gmf = 'S' -- es exento de gravamen
    ...
    begin
      ...
      begin
        if @t_trn = 264
           and ((@i_val + @w_impuesto + @w_val_2x1000) > @w_saldo_para_girar)
        begin
          if @w_impuesto = 0
            select
              @w_valor = @w_saldo_para_girar,
          ...
          select
            @w_valor = @w_valor / (1 + @w_tasa_gmf)
          select
            @w_val_2x1000 = round(((@w_valor + @w_impuesto) * @w_tasa_gmf),
                                  @w_numdeci_imp)
          select
            @w_impuesto = @w_valor * @w_piva
          select
            @w_timpuesto = @w_piva,
            @o_valiva = @w_impuesto
        end
        select
          @o_monto_imp = isnull(@o_monto_imp, 0) + isnull(@w_val_2x1000, 0) +
                         isnull
                         (
                                      @w_impuesto, 0),
          @w_nemo2 = 'GMF'
  ...
  if @t_trn = 264
     and (@o_monto_imp) > @w_saldo_para_girar
    select
      @w_val_efe = @w_valor
  else
    select
      @w_val_efe = @i_val,
      @w_val_efe_atm = isnull(@w_comision,
                              0)
  ...
  begin
    if @t_trn = 264
      select
        @w_ndebmes = @w_ndebmes + (1 * @w_tfactor)
    else
      select
        @w_ncredmes = @w_ncredmes + (1 * @w_tfactor)
  end
  ...
  begin
    ...
    begin
      ...
      if (@i_val + @w_impuesto + @w_val_2x1000) > @w_saldo_para_girar
         and @w_accion = 'V'
      begin
        select
          @w_suspensos = @w_suspensos + 1
        ...
        begin
          ...
          if @w_codigo_pais = 'CO'
            ...
            select
              @w_val2 = round((@w_saldo_para_girar / (1 + @w_tasa_imp)),
                              @w_numdeci)

          --Req 371 CAV 28/02/2014 Se modifica @w_val para que calcule en base al numero de decimales parametrizados
          select
            @w_val = round((@i_val - @w_val2),
                           @w_numdeci)
          select
            @o_monto_imp = @w_saldo_para_girar - @w_val2
          select
            @w_val_efe = @i_val - @w_val,
            @w_sus_flag = 1
        end
        ...
        values      (@w_cuenta,@w_sec,@i_cau,'N','N',
        ...
        values      (@s_ssn,@s_ssn_branch,@i_alt,@w_trn_sus,@s_ofi,
        ...
        begin
          ...
          insert into cob_ahorros..ah_tran_servicio
                      (ts_secuencial,ts_ssn_branch,ts_cod_alterno,
                       ...
                       ts_clase_clte,
                       ts_cliente,ts_hora)
          values      (@s_ssn,@s_ssn,1,752,@w_oficina_p,
                       @s_user,@s_term,'N',null,'N',
                       'L',@s_srv,@i_fecha,@i_cta,0,
                       @w_val,null,1,'50',@w_prod_banc,
                       @w_categoria,@w_oficina_p,(
                       'DISMINUCION CUPO CB POSICIONADO '
                       +
                       cast(@w_oficina_p as varchar)),
                       @w_tipocta,@w_clase_clte,
                       @w_cliente,getdate())
      ...
      else if (@i_val + @o_monto_imp) > @w_saldo_para_girar
         and @w_accion = 'E'
      ...
      begin
        ...
        begin
          ...
          values      (@w_cuenta,@w_lp_sec,@w_nemo,@w_val_efe,@i_fecha,
                       @w_control,@w_signo,'N')
          ...
          begin
            ...
            insert into cob_ahorros..ah_linea_pendiente
                        (lp_cuenta,lp_linea,lp_nemonico,lp_valor,lp_fecha,
                         lp_control,lp_signo,lp_enviada)
            values      (@w_cuenta,@w_lp_sec + 1,@w_nemo2,@o_monto_imp,@i_fecha,
                         @w_control + 1,'D','N')
        ...
        select
          @w_debitos = round((@w_debitos + (@w_val_efe + @o_monto_imp) *
                                           @w_tfactor)
                       ,
                             @w_numdeci)
        select
          @w_debhoy = round((@w_debhoy + (@w_val_efe + @o_monto_imp) *
                                         @w_tfactor)
                      ,
                            @w_numdeci)

        select
          @w_alic = round((@w_val_efe + @o_monto_imp) * @w_alicuota,
                          @w_numdeci)

        select
          @w_promedio1 = @w_promedio1 - @w_alic * @w_tfactor,
          @w_prom_disp = @w_prom_disp - @w_alic * @w_tfactor,
          @w_disponible = round((@w_disponible - (@w_val_efe + @o_monto_imp) *
                                                 @w_tfactor)
                          ,
                                @w_numdeci),
          @w_saldo_contable = round((@w_saldo_contable - (
                                     @w_val_efe + @o_monto_imp)
                                                         *
                                                         @w_tfactor),
                                    @w_numdeci),
          @w_saldo_para_girar = round((@w_saldo_para_girar - (
                                       @w_val_efe + @o_monto_imp)
                                                             *
                                                             @w_tfactor),
                                      @w_numdeci)
        select
          @w_monto_imp = @w_monto_imp + (@w_val_2x1000 * @w_tfactor)

      end

      select
        @w_disp2 = round(@w_disponible,
                         @w_numdeci),
        @w_cont2 = round(@w_saldo_contable,
                         @w_numdeci)
      ...
      begin
        ...
        if (@w_comision + @w_monto_imp_atm) > @w_saldo_para_girar
           and @w_accion = 'V'
        begin
          select
            @w_suspensos = @w_suspensos + 1
          ...
          begin
            select
              @w_val2_atm = round((@w_saldo_para_girar / (1 + @w_tasa_imp)),
                                  @w_numdeci)
            select
              @w_val_atm = @w_comision - @w_val2_atm
            select
              @w_monto_imp_atm = @w_saldo_para_girar - @w_val2_atm
            select
              @w_val_efe_atm = @w_comision - @w_val_atm
          end
          ...
          insert into cob_ahorros..ah_val_suspenso
                      (vs_cuenta,vs_secuencial,vs_servicio,vs_estado,
                       ...
                       vs_valor,vs_fecha,vs_oficina,vs_hora,vs_ssn,
                       vs_clave,vs_impuesto)
          values      (@w_cuenta,@w_sec,@w_causa_atm,'N','N',
                       @w_val_atm,@i_fecha,@s_ofi,getdate(),@s_ssn_branch,
                       0,@i_imp)
          if @@error <> 0
          begin
            select
          ...
          insert into cob_ahorros..ah_tsval_suspenso
                      (secuencial,ssn_branch,alterno,tipo_tran,oficina,
                       ...
                       turno,serial,tipo_cliente,cliente)
          --Req 371 28/02/2014 Se agregan campos tipo_cliente, cliente
          values      (@s_ssn,@s_ssn_branch,@i_alt + 1,@w_trn_sus,@s_ofi,
                       @s_user,@s_term,'N',null,'N',
                       'L',@s_srv,@i_fecha,@i_cta,@i_mon,
                       @w_val_atm,null,1,@w_causa_atm,@s_ssn_branch,
                       @w_prod_banc,@w_categoria,@w_oficina_p,'C',
                       @o_tipocta_super
                       ,
                       @i_turno,@i_serial,@w_clase_clte,
                       @w_cliente)
        ...
        else if (@w_comision + @w_monto_imp_atm) > @w_saldo_para_girar
           and @w_accion = 'E'
        begin /* Cuenta sin fondos */
          select
            @w_error = 251033
        ...
        begin
          ...
          begin
            ...
            insert into cob_ahorros..ah_linea_pendiente
                        (lp_cuenta,lp_linea,lp_nemonico,lp_valor,lp_fecha,
                         lp_control,lp_signo,lp_enviada)
            values      (@w_cuenta,@w_lp_sec + 2,@w_nemo,@w_val_efe_atm,@i_fecha
                         ,
            ...
            begin
              ...
              insert into cob_ahorros..ah_linea_pendiente
                          (lp_cuenta,lp_linea,lp_nemonico,lp_valor,lp_fecha,
                           lp_control,lp_signo,lp_enviada)
              values      (@w_cuenta,@w_lp_sec + 3,@w_nemo2,@w_monto_imp_atm,
                           @i_fecha,
                           @w_control + 3,'D','N')
          ...
          select
            @w_disponible = round((@w_disponible - (@w_val_efe_atm +
                                                    @w_monto_imp_atm)
                                                   *
                                                          @w_tfactor),
                                  @w_numdeci),
            @w_saldo_contable = @w_saldo_contable - (
                                @w_val_efe_atm + @w_monto_imp_atm
                                                    )
                                                    *
                                                    @w_tfactor

          select
            @w_debitos = round((@w_debitos + (@w_val_efe_atm + @w_monto_imp_atm)
                                             *
            @w_tfactor
            ),
            @w_numdeci)
          select
            @w_debhoy = round((@w_debhoy + (@w_val_efe_atm + @w_monto_imp_atm) *
                                           @w_tfactor)
                                               ,
                              @w_numdeci)

          select
            @w_alic = round((@w_val_efe_atm + @w_monto_imp_atm) * @w_alicuota,
                            @w_numdeci)
          ...
          select
            ...
            @w_saldo_para_girar = round((@w_saldo_para_girar -
                                        ...
                                        @w_numdeci)
          select
            @w_monto_imp = @w_monto_imp + (@w_monto_imp_atm * @w_tfactor)
          ...
          values      (@s_ssn,@i_alt + 1,@t_trn,@s_ofi,@s_user,
                       @s_term,'N',null,'N','L',
                       @s_srv,@i_fecha,@i_cta,@w_signo,1,
                       @s_ssn_branch,@i_mon,@w_causa_atm,null,null,
                       @w_val_efe_atm,null,0,@w_saldo_contable,round(
                       @w_disponible
                       ,
                             @w_numdeci),
                       null,null,@w_oficina_p,@w_prod_banc,@w_categoria,
                       @w_val_2x1000,@w_tipo_exonerado_imp,@i_serial,
                       @s_ssn_branch
                       ,
                       @o_tipocta_super,
    ...
    begin
      ...
      if isnull(@w_monto_imp_his,
                0) > 0
         and isnull(@w_val_2x1000,
                    0) = 0
        select
          @w_val_2x1000 = @w_monto_imp_his,
          @o_monto_imp = @w_monto_imp_his
      ...
      if @w_debug = 'S'
        ...
        print '@o_monto_imp: ' + cast(@o_monto_imp as varchar) +
              ' @w_val_2x1000: '
              + cast(@w_val_2x1000 as varchar)

      select
        @w_alic = round((@w_val_efe + @o_monto_imp) * @w_alicuota,
                        @w_numdeci)
      select
        @w_creditos = round((@w_creditos + (@w_val_efe + @o_monto_imp) *
                                           @w_tfactor)
                      ,
                            @w_numdeci),
        @w_credhoy = round((@w_credhoy + (@w_val_efe + @o_monto_imp) *
                                         @w_tfactor)
                     ,
                           @w_numdeci),
        @w_promedio1 = @w_promedio1 + (@w_alic * @w_tfactor),
        ...
        @w_disponible = round((@w_disponible + (@w_val_efe + @o_monto_imp) *
                        ...
                        ,
                              @w_numdeci),
        @w_saldo_contable = @w_saldo_contable + (@w_val_efe + @o_monto_imp) *
                                                @w_tfactor,
        @w_saldo_para_girar = @w_saldo_para_girar + (@w_val_efe + @o_monto_imp)
                                                    *
                                                    @w_tfactor

      select
        @w_monto_imp = @w_monto_imp - (@o_monto_imp * @w_tfactor)

      /*** Validamos el saldo maximo de la cuenta JLOYO *****/
      if @w_saldo_max > 0
         and @w_disponible > @w_saldo_max
      begin
    ...
    else if @t_trn not in(264, 253)
       and (@i_val + @o_monto_imp) > @w_saldo_para_girar
      return 1
    ...
    if isnull(@w_promedio1,
              -1) < 0
      select
        @w_promedio1 = 0

    if isnull(@w_prom_disp,
              -1) < 0
      select
        @w_prom_disp = 0
    ...
    begin
      ...
      values      (@s_ssn,@i_alt,@t_trn,@s_ofi,@s_user,
                   ...
                   null,null,@w_oficina_p,@w_prod_banc,@w_categoria,
                   @w_val_2x1000,@w_tipo_exonerado_imp,@i_serial,@w_concepto,
                   @s_ssn_branch,
      ...
      begin
        ...
        insert into cob_ahorros..ah_tran_servicio
                    (ts_secuencial,ts_ssn_branch,ts_cod_alterno,
                     ...
                     ts_clase_clte,
                     ts_cliente,ts_hora)
        values      (@s_ssn,@s_ssn,1,751,@w_oficina_p,
                     @s_user,@s_term,'N',null,'N',
                     'L',@s_srv,@i_fecha,@i_cta,0,
                     @i_val,null,1,'52',@w_prod_banc,
                     @w_categoria,@w_oficina_p,('AUMENTO CUPO CB POSICIONADO ' +
                                                cast(
                                                @w_oficina_p as varchar)),
                     @w_tipocta
                     ,
                     @w_clase_clte,
                     @w_cliente,getdate())
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        insert into cob_ahorros..ah_tran_servicio
                    (ts_secuencial,ts_ssn_branch,ts_cod_alterno,
                     ...
                     ts_prod_banc,ts_categoria,ts_oficina_cta,ts_accion,
                     ts_tipocta_super
                     ,
                     ts_turno,ts_autoriz_aut,ts_clase_clte,ts_cliente,
                     ts_estado)
        --Req 371 28/02/2014 Se agregan campos tipo_cliente, cliente
        values      (@s_ssn,@s_ssn_branch,@i_alt,@w_trn_sus,@s_ofi,
        ...
        begin
          ...
          values      (@s_ssn,@s_ssn,1,752,@s_ofi,
                       @s_user,@s_term,'S',@t_ssn_corr,'N',
                       'L',@s_srv,@i_fecha,@i_cta,0,
                       @i_val,null,1,'50',@w_prod_banc,
                       @w_categoria,@w_oficina_p,('CORR: ' + cast(@t_ssn_corr as
                                                  varchar
                                                  )
                                                  +
                       ' DISMINUCION CUPO CB POSICIONADO '
                        + cast(@s_ofi as varchar)),@w_tipocta,@w_clase_clte,
                       @w_cliente,getdate(),'R')
        end
        else
        begin
          ...
          begin
            ...
            begin
              ...
              print '@s_ssn ' + cast(@s_ssn as varchar) + ' @i_alt ' + cast(
                    @i_alt
                    as
                    varchar)
                    + ' @s_ssn_branch ' + cast(@s_ssn_branch as varchar)
            end
            select
              @w_error = 251067
            goto ERROR
      ...
      select
        @w_debitos = round((@w_debitos - (@w_val_mon + @o_monto_imp + @w_iva_dev
                                         ))
                     ,
                           @w_numdeci)
      select
        @w_debhoy = round((@w_debhoy - (@w_val_mon + @o_monto_imp + @w_iva_dev))
                    ,
                          @w_numdeci)
      select
        @w_alic = round((@w_val_mon + @o_monto_imp + @w_iva_dev) * @w_alicuota,
                        @w_numdeci)
      select
        @w_disponible = round((@w_disponible + (@w_val_mon + @o_monto_imp +
                                                @w_iva_dev
                                               ))
                                                               ,
                              @w_numdeci),
        @w_saldo_contable = round((@w_saldo_contable + (
                                   @w_val_mon + @o_monto_imp
                                   +
                                   @w_iva_dev)),
                                  @w_numdeci),
        @w_saldo_para_girar = round((@w_saldo_para_girar + (
      ...
      begin
        ...
        select
          @w_debitos = round((@w_debitos - (@w_valor1 + @w_monto_imp_atm)),
                             @w_numdeci)
        select
          @w_debhoy = round((@w_debhoy - (@w_valor1 + @w_monto_imp_atm)),
                            @w_numdeci)
        select
          @w_alic = round((@w_valor1 + @w_monto_imp_atm) * @w_alicuota,
                          @w_numdeci)

        select
          @w_disponible = round((@w_disponible + (@w_valor1 + @w_monto_imp_atm))
                          ,
                                @w_numdeci),
          @w_saldo_contable = round((@w_saldo_contable + (
                                     @w_valor1 + @w_monto_imp_atm
                                                         ))
                              ,
                                    @w_numdeci),
          @w_saldo_para_girar = round((@w_saldo_para_girar + (
        ...
        begin
          ...
          begin
            ...
            insert into cob_ahorros..ah_linea_pendiente
                        (lp_cuenta,lp_linea,lp_nemonico,lp_valor,lp_fecha,
                         lp_control,lp_signo,lp_enviada)
            values      (@w_cuenta,@w_lp_sec + 2,'CORR',@w_valor1,@i_fecha,
                         @w_control + 2,@w_signo,'N')
            if @@error <> 0
            begin
              select
            ...
            begin
              ...
              insert into cob_ahorros..ah_linea_pendiente
                          (lp_cuenta,lp_linea,lp_nemonico,lp_valor,lp_fecha,
                           lp_control,lp_signo,lp_enviada)
              values      (@w_cuenta,@w_lp_sec + 3,'CORR',@w_monto_imp_atm,
                           @i_fecha,
          ...
          begin
            if @w_debug = 'S'
               and @i_tran_ext = 'N'
            begin
              print '4.@t_ssn_corr ' + cast(@t_ssn_corr as varchar) +
                    ' @i_alt_corr '
                    + cast(@i_alt_corr as varchar)
          ...
          values      (@s_ssn,@i_alt + 1,@t_trn,@s_ofi,@s_user,
                       ...
                       @s_srv,@i_fecha,@i_cta,@w_signo,1,
                       @s_ssn_branch,@i_mon,@w_causa_atm,null,null,
                       @w_valor1,null,0,@w_saldo_contable,round(@w_disponible,
                             @w_numdeci),
                       null,null,@w_oficina_p,@w_prod_banc,@w_categoria,
                       @w_est,@w_monto_imp_atm,@w_tipo_exonerado_imp,@i_serial,
                       @w_concepto
                       ,
                       @s_ssn_branch,@o_tipocta_super,@i_turno,getdate(),
                       @i_stand_in,
    ...
    begin
      ...
      select
        @w_creditos = round((@w_creditos + (@w_val_mon + isnull(@o_monto_imp, 0)
                                            +
                                                                      isnull(
                                            @w_iva_dev
                                                                      ,
      ...
      select
        @w_credhoy = round((
                           ...
                           @w_numdeci)
      select
        @w_alic = round((@w_val_mon + isnull(@o_monto_imp, 0) + isnull(
                         @w_iva_dev
                         ,
                         0
                         ))
      ...
      select
        @w_disponible = round((@w_disponible - (@w_val_mon + isnull(@o_monto_imp
                                                ,
                                                0)
                                               ))
                        ,
                              @w_numdeci),
        @w_saldo_contable = @w_saldo_contable - (
                            @w_val_mon + isnull(@o_monto_imp,
                            0
                            )
                                                ),
        @w_saldo_para_girar = @w_saldo_para_girar - (
                              @w_val_mon + isnull(@o_monto_imp,
                              0
                              ) + isnull(@w_iva_dev
                              , 0)),
    ...
    begin
      ...
      insert into cob_ahorros..ah_linea_pendiente
                  (lp_cuenta,lp_linea,lp_nemonico,lp_valor,lp_fecha,
                   lp_control,lp_signo,lp_enviada)
      values      (@w_cuenta,@w_lp_sec,'CORR',@w_val_mon,@i_fecha,
                   @w_control,@w_signo,'N')
      if @@error <> 0
      begin
        select
      ...
      begin
        ...
        insert into cob_ahorros..ah_linea_pendiente
                    (lp_cuenta,lp_linea,lp_nemonico,lp_valor,lp_fecha,
                     lp_control,lp_signo,lp_enviada)
        values      (@w_cuenta,@w_lp_sec + 1,'CORR',@o_monto_imp,@i_fecha,
    ...
    set    ah_suspensos = @w_suspensos,
           ...
           ah_promedio1 = @w_promedio1,
           ah_prom_disponible = @w_prom_disp,
           ah_disponible = round(@w_disponible,
                                 @w_numdeci),
    ...
    begin
      ...
      select
        @w_val_mon = 0 --Para evitar que busque TX Monetaria
    end
    else
    begin
      select
      ...
      begin
        ...
        begin
          print '4 @w_return: ' + cast(@w_return as varchar)
          print '@s_ssn ' + cast(@s_ssn as varchar) + ' @i_alt ' + cast(@i_alt
                as
                varchar)
                + ' @s_ssn_branch ' + cast(@s_ssn_branch as varchar)
        end
    ...
    begin
      ...
      begin
        if @w_debug = 'S'
           and @i_tran_ext = 'N'
        begin
          print '5.@t_ssn_corr ' + cast(@t_ssn_corr as varchar) +
                ' @i_alt_corr '
                + cast(@i_alt_corr as varchar)
          print '@i_cta ' + @i_cta + ' @i_cau ' + @i_cau
          print '@i_mon ' + cast(@i_mon as varchar) + ' @w_val_mon ' + cast(
                @w_val_mon
                as
                                                                 varchar)
      ...
      values      ( @s_ssn,@i_alt,@t_trn,@s_ofi,@s_user,
                    ...
                    null,null,@w_oficina_p,@w_prod_banc,@w_categoria,
                    @w_est,@o_monto_imp,@w_tipo_exonerado_imp,@i_serial,
                    @s_ssn_branch,
                    @o_tipocta_super,@i_turno,getdate(),@i_stand_in,@i_canal,
  ...
  /* recalcular por valores en suspenso */
    ...
    if @w_tfactor <> 1
      select
        @w_gmf_reintegro = isnull(round(isnull(@o_monto_imp,
    ...
    begin
      ...
      select
        @w_saldo_para_girar = @w_saldo_para_girar + @w_gmf_reintegro
    end
  end
...
begin              
  ...
  begin
    print '@w_rollback ' + cast(@w_rollback as varchar) + ' @w_sev ' + cast(
          @w_sev as
          varchar)
    if @w_sev = 1
      if @w_rollback = 'S'
