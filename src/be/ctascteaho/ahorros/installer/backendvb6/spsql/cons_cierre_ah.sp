...
/* cobra una multa por cierre anticipado de cuenta */
  ...
  begin
    ...
    begin
      ...
      if (@w_contractual = 'S'
         ...
         and @w_prod_banc = @w_par_prod_cont
        select
          @w_multa_aux = round(@w_interes_con_tot * @w_multa_aux,
                               @w_numdeci)
    end
    ...
    begin
      ...
      if @w_tasa_imp > 0
        select
          @w_monto_imp_multa = round((@w_multa * @w_tasa_imp),
                                     @w_numdeci_imp)
    end
    else
      select
    ...
    begin
      ...
      begin
        select
          @w_multa = round((@w_saldo_cierre / (@w_tasa_imp + 1)),
                           @w_numdeci)
        select
    ...
    --    - @w_monto_imp_multa
    if @w_debug = 'S'
      print 'w_monto_imp_multa ' + cast(@w_monto_imp_multa as varchar) +
                              ' w_impuesto '
            + cast(@w_impuesto as varchar)
    select
  ...
  begin
    ...
    begin
      select
        @w_retiro = round((@w_retiro / (@w_tasa_imp + 1)),
                          @w_numdeci)
      select
  ...
  if @w_return <> 0
    return @w_return
  if @w_debug = 'S'
    print '@w_monto_imp_fin  ' + cast(@w_monto_imp_fin as varchar) +
                                                 ' @w_monto_imp '
          + cast(@w_monto_imp as varchar) + ' @w_sldcred ' + cast(@w_sldcred as
                                                 varchar)
  select
    @w_monto_imp = isnull(@w_monto_imp, 0) + @w_monto_imp_fin
  select
    @w_retiro = @w_retiro - isnull(@w_monto_imp_fin,
                                   0) + @w_gmf_reintegro
  select
    @o_retiro = @w_retiro
