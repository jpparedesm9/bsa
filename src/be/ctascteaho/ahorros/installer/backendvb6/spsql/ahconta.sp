  ...
  begin
    ...
    where  tc_fecha     = @w_fecha_mov
       ...
       and clave        = case tclave
                            when 'sp_ah01_pf' then convert(varchar, tc_moneda) +
                                                   '.' +
                                                   tc_clase_clte + '.'
                                                   + convert(varchar,
                                                   tc_prod_banc) + '.'
                                                   + tc_estcta
                            when 'sp_ah02_pf' then convert(varchar, tc_moneda) +
                                                   '.' +
                                                   tc_concepto
                            when 'sp_ah03_pf' then convert(varchar, tc_moneda) +
                                                   '.' +
                                                   tc_estcta
                            when 'sp_ah04_pf' then
                            convert(varchar, tc_moneda) +
                            '.' +
                            tc_clase_clte + '.'
                            + convert(varchar,
                            tc_prod_banc) + '.'
                            + tc_estcta + '.' + tc_concepto
                            when 'sp_ah05_pf' then convert(varchar, tc_moneda) +
                                                   '.' +
                                                   tc_clase_clte + '.'
                                                   + tc_estcta
                            else 'n/a'
