...
/*  21/Jul/2016  J. Tagle        Control Cuentas Aporte Social          */
/*  01/Ago/2016  E. Williams     AHO-H77223 correccion i_personaliza    */
/*                               e i_codigo en sp_genera_costos         */
/*  25/Ago/2016  J. Tagle        Activación Aporte Social con MMAP      */
...
as
  ...
  begin
    select
	  @w_error   = 251007,
	  @w_sev     = 0,
	  @w_mensaje = 'TRANSACCION NO PERMITIDA PARA CUENTA CB'
    goto ERROR
...
/***********FIN REQ 330****************************************************/
  ...
  begin    
    ...
    if (isnull(@w_val, 0) + isnull(@i_loc, 0) + isnull(@i_prop, 0) + isnull(@i_plaz, 0)) < @w_deposito_min
    begin
	    select
	        @w_sev   = 0,
			@w_error = 251098-- 251098 Monto depositado inferior al monto minimo establecido
        goto ERROR
  ...
  ------------------------------------------------------------------------------------------  
  -- print 'DEpositando: ' + convert(varchar(10),isnull(@w_val, 0) + isnull(@i_loc, 0) + isnull(@i_prop, 0) + isnull(@i_plaz, 0) ) 
  ...
  begin   
	  ...
	  select @w_deposito_max = ca_saldo_max from cob_ahorros..ah_cuenta_aux where ca_cuenta = @i_cuenta
	  
      if @t_trn = 252 select @w_val = @i_efe
      else select @w_val = @i_credit
  
      if (isnull(@w_val, 0) + isnull(@i_loc, 0) + isnull(@i_prop, 0) + isnull(@i_plaz, 0)) + @w_disponible > @w_deposito_max
      begin
	      select
	        @w_sev   = 0,
			@w_error = 357569
...
-------------------------------------------------------------------------------------------
-- Al ACTIVARSE Prod Aporte Social debe activarse BLOQUEO de RETIROS y DEBITOS
-------------------------------------------------------------------------------------------

if (@w_prod_banc = @w_cta_aso) OR (@w_prod_banc = @w_cta_asa) 
begin
    select @w_estado = ah_estado from cob_ahorros..ah_cuenta where ah_cta_banco = @w_cta_banco
    if @w_estado = 'A'
    begin   
        -- Verificar que no exista bloqueo tipo 2 para cuenta @w_cta_banco  
        if not exists(select 1 from   cob_ahorros..ah_ctabloqueada where  cb_cuenta = @i_cuenta and cb_tipo_bloqueo = '2' and cb_estado = 'V')
        begin
            ...
            exec @w_return = cob_ahorros..sp_tr_bloqueo_mov_ah
                ...
                @t_trn      = 211,
                @i_mon      = 0,
                @i_cta      = @w_cta_banco, -- CUENTA
                @i_tbloq    = '2',          -- TIPO BLOQUEO:  Contra Retiro y Debito
                @i_aut      = @s_user,      -- AUTORIZANTE
                @i_causa    = '6',          -- CAUSA:       Decision del banco
                @i_solicit  = 'SISTEMA',    -- SOLICITANTE:   Sistema
                @i_observacion = 'BLOQUEO AUTOMAT. DE RETIRO Y DEBITO A CTA. DE APORTE SOCIAL',
				@i_batch    = @i_batch
                    
            if @w_return <> 0 
            begin
                -- return @w_return
                -- NO SE PUDO REALIZAR BLOQUEO EN CUENTA DE APORTE SOCIAL
				select
	                @w_error = 357572
                goto ERROR
...
/* FIN - REQ 306 DEPOSITO INICIAL APERTURA AHO */
  ...
  begin
    ...
    --19/Sep/2016 Se descomenta y se comenta uso de ofi_centro
