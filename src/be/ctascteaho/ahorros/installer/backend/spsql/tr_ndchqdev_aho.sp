...
/************************************************************************/
/*  Archivo:            tr_ndchqdev_aho.sp                              */
/*  Stored procedure:   sp_tr_ndchqdev_aho                              */
...
/************************************************************************/
/*                               PROPOSITO                              */
/*  Nota de Debito de cheque devueltos en ahorros                       */
/************************************************************************/
/*                          MODIFICACIONES                              */
/*   FECHA           AUTOR           RAZON                              */
...
if exists (select
             1
           from   sysobjects
           where  name = 'sp_tr_ndchqdev_aho')
  drop proc sp_tr_ndchqdev_aho
go

create proc sp_tr_ndchqdev_aho
(
  @s_ssn           int,
  @s_srv           varchar(30),
  @s_lsrv          varchar(30),
  ...
  begin
    ...
    insert into cob_ahorros..ah_notcredeb
                (secuencial,ssn_branch,tipo_tran,oficina,usuario,
                 ...
                 saldodisp,departamento,banco,indicador,oficina_cta,
                 interes,prod_banc,categoria,tipocta_super,turno,
                 hora,canal,cliente)
    values      (@p_lssn,@s_ssn_branch,@t_trn,@s_ofi,@s_user,
                 @s_term,@t_corr,@t_ssn_corr,@t_rty,'L',/* @i_nchq,*/
                 @w_server_rem,@s_date,@i_ctadb,@w_signo,null,
                 @i_val,@p_val_deb,@p_rssn,@i_mon,@p_sldcont,
                 @p_slddisp,@i_dpto,@w_bco,@w_ind,@w_oficina_cta,
                 @w_comision,@w_prod_banc,@w_categoria,@w_tipocta_super,@i_turno
                 ,
