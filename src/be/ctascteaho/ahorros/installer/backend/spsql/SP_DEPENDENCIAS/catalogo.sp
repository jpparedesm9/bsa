...
/*      Ingreso y Actualizacion de Catalogos.                           */
/************************************************************************/
/*                              MODIFICACIONES                          */
/*      FECHA           AUTOR           RAZON                           */
/*      24/06/2016      Ignacio Yupa    Migración a CEN - Reentry       */
/************************************************************************/
USE cobis
GO
...
(
    ...
    @s_org                  char(1) = NULL,
    @s_culture              varchar(10) = null,   --FLO ECA: 12/12/2011
    @t_debug                char(1) = 'N',
...
begin
     ...
     begin
         ...
         begin
            /* enviar cabecera */
            select 'CODIGO', upper(rtrim(@w_titulo))'        ', 'ESTADO'
         end
...
--  select 'CODIGO', upper(rtrim(@w_titulo))'           ', 'ESTADO'       ARO 2 de Junio del 2000
    select codigo, 
           valor,
           estado
    from   cl_catalogo
    ...
    /* ARO: 2 de Junio del 2000: Correcci¾n Siguientes */        
        if @@rowcount=0 and @s_culture is null
        Begin      
        exec sp_cerror
            @t_debug    = @t_debug,
            @t_file     = @t_file,
     ...
     begin
        /* enviar cabecera */
        select 'CËDIGO', upper(rtrim(@w_titulo))'            '
