...
as
  ...
  /*  Transaccion monetaria  */
    ...
    select @w_val3 = isnull(@i_val,0) - isnull(@o_multa,0) - isnull(@o_valir,0)
    ...
    begin	
      ...
      values      (@s_ssn,@t_trn,@s_ofi,@s_user,@s_term,
                   ...
                   isnull(@s_ssn_branch,@s_ssn),
                   @o_tipocta_super,@i_turno,isnull(@i_fcancel,'E'),@w_secuencial,@i_ctatrn,
  ...
  /*  Transaccion monetaria  */
    ...
    begin
      ...
      exec cobis..sp_cerror
        ...
        @t_file  = @t_file,
        @t_from  = @w_sp_name,
		@i_msg = 'Error en creacion de registro en ah_cierre',
        @i_num   = 251079
      return 251079
    end
    ...
    select @w_val3 = isnull(@i_val,0) - isnull(@o_valir,0)
    -- - isnull(@o_int_perdido,0) --PCOELLO YA NO ES NECESARIO RESTAR LA MULTA

    if @i_cobra = 0
    begin
	  -- JTA Ctas de Aporte Social deben cerrar solo con Saldo sin intereses acumulados, esos intereses se pasan por ND a la cta relacionada en ahtcierr
	  if (@o_prod_banc in (select pa_int from cobis..cl_parametro where  pa_producto = 'AHO' and pa_nemonico = 'PCAASA'))
	  begin
	  	  select @w_val3 = @w_val3 - @o_sldint + @o_impuesto
	  end
      ...
      values      (@s_ssn,@t_trn,@s_ofi,@s_user,@s_term,
				   ...
				   @o_prod_banc,@o_categoria,@o_monto_imp_ret,
                   @o_tipo_exoneracion_imp,isnull(@s_ssn_branch,@s_ssn),
                   @o_tipocta_super,@i_turno,isnull(@i_fcancel,'E'),@w_secuencial,@i_ctatrn,
                   @i_minuta,getdate(),@i_canal,@o_cliente,@w_indicador,
                   @w_gmf_valcie,@o_clase_clte,@i_cau)
      if @@error <> 0
