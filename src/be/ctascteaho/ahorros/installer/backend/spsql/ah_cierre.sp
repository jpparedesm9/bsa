...
/*  21/Jul/2016 J. Tagle       Prod Aporte Social Control creditos vigentes*/
...
as
  ...
  select @i_monto_maximo = 'N' --lgra
  /* Validacion de Retiros */
  if @i_monto_maximo = 'S' and @t_corr = 'N'
  begin
    select @w_monto_maximo_deb = pa_money
	...
	-----------------------------------------------------------------------------------
	-- Impedir cerrar cuenta si posee creditos vigentes (sp_cascara CCA)
	-----------------------------------------------------------------------------------	
	...
	begin
		...
		exec @w_return = cob_interfase..sp_consulta_cartera
			   @i_cliente      = 34,   
		...
		begin
			  exec cobis..sp_cerror
				...
				@t_file  = @t_file,
				@t_from  = @w_sp_name,
				@i_num   = 357571  -- CLIENTE POSEE CREDITOS VIGENTES CON BLOQUEOS
			  return 357571
		end	  
	...
	-----------------------------------------------------------------------------------
	-- JTA Impedir cerrar cuenta si esta relacionada a una de Aporte Social
	-----------------------------------------------------------------------------------	
	if exists(select 1 From cob_ahorros..ah_cuenta_aux where ca_cta_banco_rel = @i_cta) -- cta es relacionada?
	and exists(select 1 from cob_ahorros..ah_cuenta where ah_estado = 'A' AND ah_cta_banco in (select ca_cta_banco   -- la de aporte social esta activa?
																							   From cob_ahorros..ah_cuenta_aux 
																							   where ca_cta_banco_rel = @i_cta))
	begin
		/* Cuenta vinculada con Cuenta de Aporte Social Adicional */
		exec cobis..sp_cerror
			@t_debug = @t_debug,
			@t_file  = @t_file,
			@t_from  = @w_sp_name,
			@i_msg   = 'Cuenta esta relacionada con una Cuenta de Aporte Social.',
			@i_num   = 258005
		return 1
	end
	-----------------------------------------------------------------------------------	
