...
select @w_sp_name = 'sp_totaliza_tran_mon_ah'
...
if exists (select 1
			...
			and tc_tipo   	= 'M')
begin
	print ' Existen registros Contabilizados en la re_trn_contable para esa fecha'
    exec sp_errorlog
		@i_fecha       = @i_param1,
		@i_error       = 0,
		@i_usuario     = 'batch',
...
if exists(select 1
			...
			and tc_tipo  = 'M') --
begin
    print 'Se borran los registros del dia ' + cast(@i_param1 as varchar) + ' de transacciones Monetarias'
...
begin
    ...
    print 'Transaccion :' + cast (@w_tipo_tran as varchar) + ' Causa :' 
			+ cast (isnull(@w_causa, '') as varchar) + '- Concepto : ' + isnull(@w_concepto, '')
			+ 'concepto_contable :' + isnull(@w_concepto_imp, '') + ' Tipo_imp:' + isnull(@w_tipo_imp, '')
    ...
    begin
		...
		begin
			...
			insert into #re_trn_contable
					(tc_producto, tc_moneda, tc_tipo_tran, tc_causa, tc_ofic_orig, tc_ofic_dest, 
					tc_perfil, tc_concepto, tc_valor, tc_valor_me,
				...
				select 4, isnull(tm_moneda, 0), tm_tipo_tran, tm_causa, tm_oficina, isnull(tm_oficina_cta, tm_oficina), 
						isnull((select tp_perfil
							from cob_remesas..re_trn_perfil
							where tp_producto  	= 4
							and tp_tipo_tran 	= tm_tipo_tran), 'NO DEFIDO'), @w_concepto, 
						case tm_moneda /**** Valor ****/
							when 0 then
								case @w_campo1
									when 'tm_valor' then sum(isnull(tm_valor, 0))
									when 'tm_chq_propios' then sum(isnull(tm_chq_propios, 0))
									when 'tm_chq_locales' then sum(isnull(tm_chq_locales, 0))
									when 'tm_chq_ot_plazas' then sum(isnull(tm_chq_ot_plazas, 0))
									when 'tm_efectivo' then sum(isnull(tm_efectivo, 0))
									when 'tm_saldo_interes' then sum(isnull(tm_saldo_interes, 0))
									when 'tm_interes' then sum(isnull(tm_interes, 0))
									when 'tm_monto_imp' then sum(isnull(tm_monto_imp, 0))
									when 'tm_valor_comision' then sum(isnull(tm_valor_comision, 0))
									when 'tm_base_gmf' then sum(isnull(tm_base_gmf, 0))
									else 0
								end
							else 0
						end, 
						case tm_moneda /**** Valor_ME ****/
							when 0 
								then 0
							else
								case @w_campo1
									when 'tm_valor' then sum((isnull(tm_valor, 0) * valor))
									when 'tm_chq_propios' then sum((isnull(tm_chq_propios, 0) * valor))
									when 'tm_chq_locales' then sum((isnull(tm_chq_locales, 0) * valor))
									when 'tm_chq_ot_plazas' then sum((isnull(tm_chq_ot_plazas, 0) * valor))
									when 'tm_efectivo' then sum((isnull(tm_efectivo, 0) * valor))
									when 'tm_saldo_interes' then sum((isnull(tm_saldo_interes, 0) * valor))
									when 'tm_interes' then sum((isnull(tm_interes, 0) * valor))
									when 'tm_monto_imp' then sum((isnull(tm_monto_imp, 0) * valor))
									when 'tm_valor_comision' then sum((isnull(tm_valor_comision, 0) * valor))
									when 'tm_base_gmf' then sum((isnull(tm_base_gmf, 0) * valor))
								else 0
							end
						end,
						count(1), 'M', tm_prod_banc, isnull(tm_clase_clte, 'X'), tm_tipocta_super,
						isnull(tm_cliente, 0), 
						case @w_campo2 /**** Base ****/
							when 'tm_valor' then sum((isnull(tm_valor, 0) * valor))
							when 'tm_chq_propios' then sum((isnull(tm_chq_propios, 0) * valor))
						...
						case when tm_tipo_tran = 304 
									and exists(select 1
											...
											and ah_estado             	= 'I'
											and datepart(yy, tn_fecha) 	< datepart(yy, tm_fecha))
								then 'T'
							when tm_tipo_tran = 304
									and exists(select 1
												...
												and ah_estado             	= 'I'
												and datepart(yy, tn_fecha) 	= datepart(yy, tm_fecha)
												and datepart(mm, tn_fecha) 	< datepart(mm, tm_fecha))
								then 'T'
							else 
								(select ah_estado from cob_ahorros..ah_cuenta where ah_cta_banco = tm_cta_banco)
						end, @w_concepto_imp
					from cob_ahorros..ah_tran_monet_tmp t, #cotizacion
					where tm_tipo_tran           = @w_tipo_tran
					--and    isnull(tm_causa,'')    = @w_causa
					and isnull(tm_indicador, 0) = @w_indicador
					and tm_estado is null
					and isnull(tm_moneda, 0)    = moneda
				
					--and tm_prod_banc in (@w_cta_aho, @w_cta_pro, @w_cta_med, @w_cta_aso, @w_cta_asa) --PRMR TEMPORAL QUITAR
				
					group by tm_tipo_tran, tm_causa, tm_moneda, tm_oficina, tm_prod_banc, tm_clase_clte,
							 tm_oficina_cta, tm_fecha, tm_tipocta_super, tm_cta_banco, tm_cliente
		end
		else
		begin
			print '........2'
			...
			insert into #re_trn_contable
					...
					tc_cliente,tc_base,tc_tipo_impuesto,tc_referencia,tc_fecha,
					tc_estado,tc_estcta,tc_concepto_imp)
				select 	4,isnull(tm_moneda, 0),tm_tipo_tran,tm_causa,tm_oficina,
						isnull(tm_oficina_cta, tm_oficina), 
						isnull((select tp_perfil
							from cob_remesas..re_trn_perfil
							where tp_producto  	= 4
							and tp_tipo_tran 	= tm_tipo_tran), 'NO DEFIDO'),
						@w_concepto, 
						case tm_moneda /**** Valor ****/
							when 0 then
								case @w_campo1
						...
						count(1), 'M', tm_prod_banc, isnull(tm_clase_clte, 'X'), tm_tipocta_super, 0, 	
						case @w_campo2 /**** Base ****/
								when 'tm_valor' then sum((isnull(tm_valor, 0) * valor))
								when 'tm_chq_propios' then sum((isnull(tm_chq_propios, 0) * valor))
								when 'tm_chq_locales' then sum((isnull(tm_chq_locales, 0) * valor))
					...
					group by tm_tipo_tran, tm_causa, tm_moneda, tm_oficina, tm_prod_banc,
							tm_clase_clte, tm_oficina_cta, tm_fecha, tm_cta_banco, tm_tipocta_super
		end
	end
	else
	begin
		...
		begin
			...
			insert into #re_trn_contable
					...
					tc_estado,tc_estcta,tc_concepto_imp)
				select
						4,isnull(tm_moneda, 0),tm_tipo_tran,tm_causa,tm_oficina,
						isnull(tm_oficina_cta, tm_oficina),
						isnull((select tp_perfil
									from cob_remesas..re_trn_perfil
									where tp_producto  	= 4
    ...
    insert into cob_remesas..re_trn_contable
		...
		select
			...
			where  tc_valor > 0 or tc_valor_me > 0
