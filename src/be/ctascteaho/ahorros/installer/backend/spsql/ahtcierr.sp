...
/*   26/Ago/2016 C.Milán        Levantar Bloqueo a Cuentas de Aporte Soc*/
/*   03/Sep/2016 J. Tagle       Cierre Cuentas de aporte Social         */
/*   23/Sep/2016 J. Tagle       Ajustes Cierre Ctas Aporte Social       */
/************************************************************************/
...
declare
		...
		@w_monto_aporte			money,
		@w_estado_bloqueo       char(1),
		@w_server               varchar(30),
		@w_cta_banco_rel        cuenta

select @w_sp_name = 'sp_cierre_ctah'
...
--CMI Encuentra parametros de producto bancario de Cuentas de Aporte Social
...
select @w_valcie = round(@i_val, @w_numdeci)
...
if exists(select 1 from cobis..cl_producto where pd_producto = 14)
begin
   -- Validacion para que la cuenta de ahorros no tenga operaciones de DPF pendientes
   if exists(select dp_cuenta
...
if @w_resultado = 1 or @w_return <> 0 /* Si existe un producto asociado con Cartera */
begin
	exec cobis..sp_cerror
		@t_debug = @t_debug,
		@t_file  = @t_file,
...
if @w_resultado = 1 or @w_return <> 0 /* Si existe un producto asociado con Garantias */
begin
	exec cobis..sp_cerror
		@t_debug = @t_debug,
		@t_file  = @t_file,
...
if exists(select 1 from cobis..cl_producto where pd_producto = 21)
begin
   if exists (select 1
...
if @w_gmfmarca = 'S'
	print 'Cuenta marcada con exoneracion de GMF. Por favor diligenciar Formato de Desmarcacion de Cuenta'
...
--CMI LEVANTA EL BLOQUEO DE MOVIMIENTOS DE LAS CUENTAS DE APORTE SOCIAL
if @o_prod_banc in (@w_proban_aporteadic, @w_proban_aporteord)
begin
    select @w_estado_bloqueo = cb_estado
	...
	if @@rowcount = 1 
        begin
            ...
            exec @w_return = cob_ahorros..sp_tr_bloqueo_mov_ah
                ...
                @i_mon      = 0,
                @i_cta      = @i_cta,       -- CUENTA
                @i_tbloq    = '2',          -- TIPO BLOQUEO:  Contra Retiro y Debito
                @i_aut      = @s_user,      -- AUTORIZANTE
                @i_causa    = '7',          -- CAUSA:       Cierre de Cuenta
                @i_solicit  = 'SISTEMA',    -- SOLICITANTE:   Sistema
                @i_observacion = 'LEVANTAMIENTO BLOQUEO AUT. DE RETIRO Y DEBITO A CTA. DE APORTE SOCIAL'
               
            if @w_return <> 0 
            begin
            -- NO SE PUDO REALIZAR LEVANTAMIENTO DE BLOQUEO EN CUENTA DE APORTE SOCIAL
                exec cobis..sp_cerror
                    @t_debug = @t_debug,
                    @t_file  = @t_file,
                    @t_from  = @w_sp_name,
                    @i_num   = @w_return,
					@i_msg   = 'NO SE PUDO REALIZAR LEVANTAMIENTO DE BLOQUEO EN CUENTA DE APORTE SOCIAL'
                return @w_return           
            end
		end
end
--FIN LEVANTA BLOQUEO DE CUENTAS DE APORTE SOCIAL
...
begin
    select @w_msg = '[' + @w_sp_name + ']' + ' LA CUENTA TIENE VALORES EN EMBARGO'
	exec cobis..sp_cerror
		@t_debug = @t_debug,
		@t_file  = @t_file,
		@t_from  = @t_from,
...
if @w_monto_bloq > 0 or @w_monto_emb > 0
begin
...
if @i_nctrl <> @o_control and @i_tadmin = 0
begin
...
if @i_sldlib <> @w_saldo_libreta and @i_tadmin = 0
begin
    /* Numero de Control Errado */
    exec cobis..sp_cerror
		@t_debug = @t_debug,
...
if @t_trn = 213
begin
    select @w_date = convert(varchar(10), @s_date, 101)
	...
	-- JTA Cuentas de Aporte Adicional no deben considerar Interes acumulado, dicho interes se va por NC a cta Relacionada
	if @o_prod_banc in (@w_proban_aporteadic)
	begin
		select @w_valcie = @w_valcie - @w_sldint
    end	
	
    select @o_cliente = @w_ente
    select @o_sldint = isnull(@w_sldint, 0)
	select @o_multa = isnull(@o_multa, 0)
    ...
    if exists (select 1
		...
		select @w_es_contractual = 'N'

    /*CALCULA SALDO MINIMO */
    if @w_saldo_min_mes = -1
    begin
		if @o_slddisp >= 0
			select @w_saldo_min_mes = @o_slddisp
		else
			select @w_saldo_min_mes = 0
    end
    ...
    select @w_val_cierre = round((@o_slddisp + @o_sldint + @w_sldmantval + @i_gmf_reintegro), @w_numdeci)

    --print 'Cobra Comision por cierre ' + @i_cobra_multa_ant
    select @w_pais = isnull(pa_char, 'PA')
		from cobis..cl_parametro
		where pa_producto = 'ADM'
    ...
    begin
		select @w_dias = pa_smallint --pa_tinyint  lgr
			from cobis..cl_parametro
			where pa_producto = 'AHO'
			and pa_nemonico = 'MCI'

		/* cobra una multa por cerrar una cuenta antes de tiempo */
		if (datediff(dd, @w_fec_aper, @s_date) < @w_dias) and @t_ejec <> 'R'
		begin
			/* REQ 217 Cobro por Cierre Anticipado */
			if @w_es_contractual = 'S' and @w_prod_banc = @w_par_prod_cont and @w_categoria <> @w_camcat
			begin
				select @w_cant_plazos = cc_plazo,
						@w_interes_con = cc_intereses
					from cob_remesas..re_cuenta_contractual
					where cc_cta_banco = @i_cta
					and cc_estado    = 'A'

				select 	@w_interes_con_tot	= round(isnull(@w_interes_con, 0) + isnull(@w_sldint, 0), @w_numdeci),
						@w_porc_tiempo 		= round(convert(float, (datediff(mm, @w_fec_aper, @s_date))) / convert(float, @w_cant_plazos), @w_numdeci),
						@w_rubro = '3'
				...
				select 	@w_multa = round(@w_interes_con_tot * @w_multa, @w_numdeci)
			end
			else
			begin
				if @w_pais = 'CO' and @w_banco = 'BM'
					select @w_rubro = '41'
				else
					select @w_rubro = '22'
		...
		select @w_multa = isnull(@w_multa, 0)
		select @o_multa = isnull(@w_multa, 0)
		
		-- JTA - CTAS de APORTE SOCIAL ORDINARIO Y ADICIONAL NO COBRAN RUBRO DE MULTA a pesar de q este parametrizado en person
		if @o_prod_banc in (@w_proban_aporteadic, @w_proban_aporteord)
		begin 
			select @w_multa = 0.00
			select @o_multa = 0.00
    ...
    if @w_val_cierre >= @o_multa
		select @w_val_cierre = @w_val_cierre - @o_multa
    else
    begin
		select 	@o_multa = @w_val_cierre,
    ...
    --PCOELLO SI APLICA PARA GLOBAL BANK  FIN COMENTARIO */
    select @w_val_cierre = isnull(@w_val_cierre, 0)
    select @w_retiro = isnull(@w_val_cierre, 0)

    if @w_codigo_pais = 'CO' -- Colombia
		select @w_valcie = @w_valcie
    else
		select @w_valcie = @w_valcie - @w_multa --lgr                         

    select @w_valor_impuesto = isnull(@w_valor_impuesto, 0)
    select @o_valir = isnull(@w_valor_impuesto, 0)

    select @w_val_cierre = isnull(@w_val_cierre, 0)
    select @w_retiro = isnull(@w_val_cierre, 0)

	select @w_valcie = @w_valcie - @w_valor_impuesto --lgr                   
    ...
    select @w_fecha = @s_date--dateadd(dd,1,@s_date)                          
    ...
    begin
		...
		select @w_dias_capi = abs(datediff(dd, @w_fecha_ult_capi, @w_fecha))
		...
		if @w_debug = 'S'
			print '@w_base_rtfte ' + cast(@w_base_rtfte as varchar) + ' @w_cpto_rte  ' + @w_cpto_rte

		---------------------------------------------------------
		-- CALCULO RETE FUENTE
		---------------------------------------------------------
		if @w_cpto_rte is not null and @w_sldint >= @w_base_rtfte
		begin
			...
			if @w_exento = 'N'
				select @w_tasa_impuesto = @w_porcentaje
			else
		...
		begin
			select @w_tasa_impuesto = 0
			if @w_tasa_imp > 0
				select @w_valor_impuesto = round((@w_sldint * @w_tasa_imp), @w_numdeci_imp)
			else
				select @w_valor_impuesto = 0
		...
		begin
			...
			if @w_tasa_impuesto > 0		
				select @w_valor_impuesto = round((@w_sldint * round((@w_tasa_impuesto / 100),2)), @w_numdeci_imp)
			else
				select @w_valor_impuesto = 0
			
			select @w_tasa_imp_usado = round((@w_tasa_impuesto / 100),2)
		end
  
		if @w_debug = 'S'
			print '@w_tasa_impuesto ' + cast(@w_tasa_impuesto as varchar) + ' @w_valor_impuesto ' + cast(@w_valor_impuesto as varchar)
		...
		begin
			...
			if @w_actualiza = 'S' and isnull(@w_acumu_deb, 0) <> 0
			begin
		...
		select @w_imp_retencion = @w_total_gmf, @w_base_gmf_retencion = @w_base_gmf
	...
	begin
        ...
        if @w_exento = 'N'
			select @w_tasa_reteica = @w_porcentaje
        else
			select @w_tasa_reteica = 0

        if @w_tasa_reteica > 0
			select @w_valor_reteica = round((@w_sldint * @w_tasa_reteica), @w_numdeci_imp)
        else
	...
	if @w_debug = 'S'
        print '@w_tasa_reteica ' + cast(@w_tasa_reteica as varchar) + ' @w_valor_reteica ' + cast(@w_valor_reteica as varchar)
	...
	begin                         
        ...
        select 	@w_imp2 = @w_imp2 + @w_total_gmf
        select 	@w_imp_reteica = @w_total_gmf,
				@w_base_gmf_reteica = @w_base_gmf
	end

	select 	@w_fecha_ult_mov_int = @w_fecha,
			@w_fecha_ult_capi = @w_fecha

	select 	@w_total_interes = round(@w_sldint - isnull(@w_valor_impuesto, $0) - @w_valor_reteica - @w_imp2, @w_numdeci) --JAR 14/01/2010 
			
	---------------------------------------------------------
	-- NCREDITO al cerrar CUENTAS DE APORTE SOCIAL ADICIONAL    
	---------------------------------------------------------	
	-- JTA luego de obtener el saldo actual a las cuentas de Aporte Social Adicional
	-- debe capitalizarse ese interes en la ah_cuenta_aux..ca_cta_banco_rel
	if @o_prod_banc in (@w_proban_aporteadic)
	begin
		  select @w_cta_banco_rel = ca_cta_banco_rel
		  from   cob_ahorros..ah_cuenta_aux
		  where  ca_cta_banco = @w_cta_banco  	
 
		  exec  cob_ahorros..sp_ahndc_automatica
			  @s_srv        = @s_srv,
			  @s_ofi        = @s_ofi,
			  @s_ssn        = @s_ssn,
			  @s_ssn_branch = @s_ssn_branch,
			  ...
			  @i_val        = @w_total_interes,  -- @w_sldint,
		  ...
		  begin
			  ...
			  exec cobis..sp_cerror
				...
				@t_from  = @w_sp_name,
				@i_num   = 255004,
				@i_msg   = 'ERROR AL EJECUTAR NOTA CREDITO AUTOMATICA A CUENTA RELACIONADA'
			return 255004
		  end	
		  
		  select @w_valcie = @w_valcie + @w_valor_impuesto
	end
	else
	begin	
	---------------------------------------------------------
	-- CIERRE CUENTAS NORMALES
	---------------------------------------------------------		
		...
		select 	@w_sldint = round(@w_sldint, @w_numdeci)
		select 	@o_slddisp = @o_slddisp + @w_sldint
		select 	@w_saldo_contable = @w_saldo_contable + @w_sldint
		...
		select @w_saldo_interes_ir = @w_sldint
		...
		insert into ah_notcredeb
			...
			values
				( @s_ssn,@s_ssn,1,@w_trn_code,@w_oficina,
	...
	if @w_debug = 'S'
        print '@w_retencion  ' + @w_retencion + ' @w_valor_impuesto ' + cast (@w_valor_impuesto as varchar)

	---------------------------------------------------------
	-- RETENCION - w_valor_impuesto
	---------------------------------------------------------			
	if (@w_retencion = 'S' and @w_valor_impuesto > 0)
	begin
        select @o_slddisp = @o_slddisp - @w_valor_impuesto - @w_imp_retencion
        select @w_saldo_contable = @w_saldo_contable - @w_valor_impuesto - @w_imp_retencion

        select @w_prom_disponible = @w_prom_disponible - convert(money, round((@w_valor_impuesto * @w_alicuota), @w_numdeci))
        select @w_promedio1 = @w_promedio1 - convert(money, round((@w_valor_impuesto * @w_alicuota), @w_numdeci))
        ...
        insert into ah_notcredeb
			...
			values
				( @s_ssn,@s_ssn,2,@w_trn_code,@w_oficina,
				@s_user,@s_term,'N',null,'N',
				'L',@i_srv,@w_fecha,@w_cta_banco,'D',
				1,null,@w_moneda,null,@w_sldint,
				@w_valor_impuesto,@w_tasa_imp_usado,@w_saldo_contable,@o_slddisp, @w_oficina,
				@w_filial,@w_prod_banc,@w_categoria,@w_imp2,@w_tipo_exonerado_imp,
	...
	---------------------------------------------------------
	-- RETEICA - w_valor_reteica
	---------------------------------------------------------
	if @w_valor_reteica > 0
	begin
        select @o_slddisp = @o_slddisp - @w_valor_reteica - @w_imp_reteica
        select @w_saldo_contable = @w_saldo_contable - @w_valor_reteica - @w_imp_reteica

        select @w_prom_disponible = @w_prom_disponible - convert(money, round((@w_valor_reteica * @w_alicuota), @w_numdeci))
        select @w_promedio1 = @w_promedio1 - convert(money, round((@w_valor_reteica * @w_alicuota), @w_numdeci))
        ...
        insert into ah_notcredeb
			...
			values
				(@s_ssn,@s_ssn,3,@w_trn_code,@w_oficina,
				@s_user,@s_term,'N',null,'N',
				'L',@i_srv,@w_fecha,@w_cta_banco,'D',
				1,null,@w_moneda,null,@w_sldint,
	...
	select	@w_creditos = @w_creditos + @w_sldint,
	...
	begin
        ...
        select @w_gmfbco = round((@w_sldint * @w_tasagmf), @w_numdeci)
...
if @w_sldint > 0 and @w_codigo_pais <> 'CO' -- lgr                       
begin
	--select  @w_valcie      = @w_valcie - @o_sldint,                       
	--        @o_int_perdido = @w_sldint                                    
	select @o_int_perdido = @w_sldint
...
begin
	...
	if @@rowcount = 0 or @w_com_iva = 'N'
	begin
        select 	@w_impuesto_iva = 0,
				@w_accion = 'E',
				@w_imp_gmf = 'N',
				@w_com_iva = 'N'
	end
	...
	begin--3                
        ...
        select @w_impuesto_iva = round((@w_multa * @w_piva / 100), @w_numdeci_imp)
	end --3            
	else
        select @w_impuesto_iva = 0

	select @w_totval = @w_multa + @w_impuesto_iva
	...
	begin
		...
		select @w_monto_imp_multa = 0, @w_base_gmf = 0

	if (@w_multa + @w_impuesto_iva + @w_monto_imp_multa) > @o_slddisp
	begin
        select @w_msg = '[' + @w_sp_name + '] ' + 'VALORES POR COBRAR -- MAYOR A SALDO DE CIERRE'
        exec cobis..sp_cerror
			@t_debug = @t_debug,
			@t_file  = @t_file,
			@t_from  = @w_sp_name,
	...
	------------                
	select 	@o_slddisp = @o_slddisp - @o_multa - @w_monto_imp_multa - @w_impuesto_iva
	select 	@w_prom_disponible = @w_prom_disponible - (@o_multa + @w_monto_imp_multa + @w_impuesto_iva) *
				@w_alicuota, @w_promedio1 = @w_promedio1 - (@o_multa + @w_monto_imp_multa + @w_impuesto_iva) * @w_alicuota
	...
	insert into cob_ahorros..ah_notcredeb
			...
			base_gmf,clase_clte)
		values
			(@s_ssn,4,@w_trn,@i_oficina,@s_user,
			@s_term,'N',null,'N','L',
...
begin
	...
	if @w_codigo_pais = 'CO' -- Colombia                                    
	begin
        select @w_valor_cal = @w_valcie - @i_gmf_reintegro + @w_valor_impuesto
        ...
        begin
			select @o_slddisp = @o_slddisp + isnull(@w_gmf_reitengro_cie, 0)
	...
	select @w_gmf_valcie = @w_base_gmf
	...
	select @w_cauegina = pa_char --Causal egreso dev. fondos inactivas
		from cobis..cl_parametro
		where pa_producto = 'CTE'
		and pa_nemonico = 'CEDLA'

	if @w_cauegina is null
        select @w_cauciina = '035'

	if @i_fcancel = 'E' or (@i_causa = '6' or @i_causa = @w_cauciina)
	begin
        if @i_causa = '6'
			select @w_causa = '012'
			--DEVOLUCION DE FONDOS CIERRE UNILATERAL BANCO
		else if @i_causa = @w_cauciina
			select @w_causa = @w_cauegina
        --ENTREGA SALDO CTA AHORROS CANCELACION INACTIVA 
        else
			select @w_causa = '014' --DEVOLUCION DE FONDOS DE CUENTA CERRADA-AHORROS
	...
	begin
        ...
        begin
			...
			select 	@w_ente_b = null
			select 	@w_ente_b = en_ente,
			...
			select @w_campo3 = @w_tipo_ced_b + '-' + @w_ced_ruc_b

			/*Datos de cliente si es Cheque de GErencia cmu*/

			select @w_ente_c = null
			...
			select @w_campo1 = @w_tipo_ced_c + '-' + @w_ced_ruc_c
        end
        else
        begin
        ...
        begin
			...
			exec cobis..sp_cerror
				@i_num = 205031,
				@i_msg = 'ERROR EN PARAMETRO DE DEVOLUCION DE FONDOS DE CUENTAS CERRADAS'
			return 205031
        end
...
/* PEDRO COELLO AGOSTO 1 DEL 2006 Creacion de transaccion de servicio */
	...
	values
		(@s_ssn,@s_ssn_branch,213,@s_date,@s_user,
...
--CMI EN LA APERTURA DE CUENTA SE CONTABILIZO LA APERTURA DE CUENTAS DE APORTE SOCIAL CON EL MONTO MAXIMO DE APERTURA. POR ESTA RAZON
--SE DEBE CONTABILIZAR TAMBIEN EL CIERRE CON ESTE MONTO EN LAS CUENTAS DE APORTE SOCIAL. SE USARA TRN 200
if @o_prod_banc in (@w_proban_aporteadic, @w_proban_aporteord)
begin
	--SI CON CUENTAS DE APORTE SOCIAL SE DEBE BUSCAR EL MONTO MAXIMO DE APERTURA
	select @w_monto_aporte = 0
		
	select @w_monto_aporte = ca_saldo_max  
		from cob_ahorros..ah_cuenta_aux
	...
	begin
		...
		exec cobis..sp_cerror
			...
			@t_file  = @t_file,
			@t_from  = @w_sp_name,
			@i_msg   = 'NO SE ENCONTRO EL MONTO MAXIMO PARA CUENTA DE APORTE SOCIAL',
			@i_num   = 251001
		return 251001
	end
	insert into ah_tran_servicio
			(ts_secuencial,ts_ssn_branch,ts_tipo_transaccion,ts_tsfecha,
			ts_usuario, ts_cod_alterno,
			ts_terminal,ts_ctacte,ts_filial,ts_valor,ts_oficina,
			ts_oficina_cta,ts_prod_banc,ts_categoria,ts_moneda,
			ts_tipocta_super ,
			ts_turno,ts_saldo,ts_cta_banco,ts_cliente,
			ts_clase_clte)
		values
			(@s_ssn,@s_ssn,200,@s_date,@s_user, 3,
			@s_term,@w_cuenta,@w_filial,@w_monto_aporte,@i_oficina,
...
--FIN Transaccion de Servicio por Cuentas de Aporte Social
insert into cob_ahorros..ah_his_cierre
		...
		hc_sec_ord_pago)
	values
		(@w_numrg,@w_cuenta,@i_orden,@i_causa,@w_valcie,
...
if @i_fcancel = 'C' or @i_causa = '6'
	select @o_indicador = 0
if @i_fcancel = 'E'
	select @o_indicador = 1
if @i_fcancel = 'G'
	select @o_indicador = 3
if @i_causa = @w_cauciina
	select @o_indicador = 2
if @i_cobra = 1
begin
	select 	@w_prom_disponible = @w_prom_disponible - (@w_multa) * @w_alicuota,
			@w_promedio1 = @w_promedio1 - (@w_multa) * @w_alicuota

	update cob_ahorros..ah_cuenta
		set ah_disponible = round(@o_slddisp, @w_numdeci),
			ah_promedio1 = round(isnull(@w_promedio1, 0), @w_numdeci),
			ah_prom_disponible = round (isnull(@w_prom_disponible, 0), @w_numdeci),
...
begin
	...
	update cob_ahorros..ah_cuenta
		set ah_estado = 'C',
			...
			ah_promedio1 = round(isnull(@w_promedio1, 0), @w_numdeci),
			ah_prom_disponible = round (isnull(@w_prom_disponible, 0), @w_numdeci),
			ah_debitos = ah_debitos + @w_valcie + @o_multa + @o_valir + isnull(@w_imp2, 0) + isnull(@w_monto_imp_retiro, 0) + isnull(@w_monto_imp_multa, 0),
			ah_debitos_hoy = ah_debitos_hoy + @w_valcie + @o_multa + @o_valir + isnull( @w_imp2, 0) + isnull(@w_monto_imp_retiro, 0) + isnull(@w_monto_imp_multa, 0),
...
select 	@o_int_cap = round (@o_int_cap, @w_numdeci),
		@o_sldint = round (@o_sldint, @w_numdeci),
		@o_impuesto = round (@o_impuesto, @w_numdeci_imp),
		@o_multa = round (@o_multa, @w_numdeci),
		@o_monto_imp = round (@w_monto_imp, @w_numdeci),
		@o_monto_imp_ret = round (@w_monto_imp_retiro, @w_numdeci_imp),
		@o_gmf_valcie = round (@w_gmf_valcie, @w_numdeci_imp),
		@o_sldmantval = @w_sldmantval,
		@o_idlote = @w_idlote,
		@o_secuencial = @w_num_orden,
		@o_valir = round(@o_valir, @w_numdeci),
		@o_gmf_reitengro_cie = isnull(@w_gmf_reitengro_cie, 0)
