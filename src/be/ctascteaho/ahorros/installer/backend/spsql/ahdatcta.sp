/************************************************************************/
/*      Archivo:                ahdatcta.sp                             */
/*      Stored procedure:       sp_dat_diario_ctas                      */
/*      Base de datos:          cob_ahorros                             */
/*      Producto:               Ahorros                                 */
/*      Disenado por:           Doris Lozano                            */
/*      Fecha de escritura:     30-Abr-2012                             */
/************************************************************************/
/*                             IMPORTANTE                               */
...
/************************************************************************/
/*      FECHA           AUTOR             RAZON                         */
/*      30/04/2012      D. Lozano         Emision inicial               */
...
if exists (select
             1
           from   sysobjects
           where  name = 'sp_dat_diario_ctas')
  drop proc sp_dat_diario_ctas
go

/****** Object:  StoredProcedure [dbo].[sp_dat_diario_ctas]    Script Date: 03/05/2016 9:52:01 ******/
set ANSI_NULLS off
GO
set QUOTED_IDENTIFIER off
GO

create proc sp_dat_diario_ctas
(
  @t_show_version bit = 0,
  @i_param1       tinyint,-- Empresa
  @i_param2       datetime,-- Fecha Ini Mes a Reportar
  @i_param3       datetime -- Fecha Fin Mes a Reportar   
)
as
  declare
    @w_empresa        tinyint,
  ...
  if exists(select
              1
            from   sysobjects
            where  name = 'tmp_datos_aho')
    drop table tmp_datos_aho

  create table tmp_datos_aho
  (
    fecha          datetime null,
  ...
  where  co_fecha between @w_fecha_ini and @w_fecha_fin

  if @@error <> 0
  begin
    select
      @w_error = 2805004,
      @w_msg = 'Error en Insercion en cob_ahorros..tmp_datos_aho'
    goto ERROR
  end

  insert into cob_ahorros..tmp_datos_aho
    select
  ...
  select
    @w_comando =
    @w_s_app + 's_app bcp -auto -login cob_ahorros..tmp_datos_aho out '

  select
    @w_destino = @w_path_destino + 'tmp_datos_aho.txt',
    @w_errores = @w_path_destino + 'tmp_datos_aho.err'
  ...
  if @w_error <> 0
  begin
    print 'Error Generando Archivo tmp_datos_aho '
    return @w_error
  end

  ----------------------------------------
  --Union de archivo @w_nombre_plano con archivo tmp_datos_aho.txt
  ----------------------------------------
