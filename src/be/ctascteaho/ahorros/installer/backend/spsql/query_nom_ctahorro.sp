...
/*  22/Jun/2016     Juan Tagle      Recursos para Grids                 */
/*  25/Ago/2016     Juan Tagle      Cuenta Relacionada para apertura    */
/*  08/Sep/2016     D. Fu           Consulta por num. de cuenta migrada */
/************************************************************************/
use cob_ahorros
go
...
@w_cta_contractual int,
@w_cta_aso         int,
@w_cta_asa         int,
@w_cuenta_banc     cuenta,
@w_parmig          char(1)
...
select @w_cuenta_banc = null
if isnull(@i_cta_mig,'') <> ''
begin
    select @w_cuenta_banc = ca_cta_banco 
    from   cob_ahorros..ah_cuenta_aux 
    where  ca_cta_banco_mig = @i_cta_mig
end

if isnull(@i_cta,'') = ''
    select @i_cta = @w_cuenta_banc
else begin
    if isnull(@i_cta_mig,'') <> '' and @w_cuenta_banc <> @i_cta
    begin
...
begin
   ...
   begin
      ...
      begin
         ...
         @i_num       = 251001,
         @i_sev       = 0,
         @i_msg       = 'No existe cuenta de ahorros o cuenta no pertenece al cliente titular'
         return 251001
      end
   end
   else
   ...
   -- Pantalla de Apertura usa campo Cta. Relacionada para provisionar intereses
   -- Deben listarse cuentas que no sean Aporte social ni Contractual
   select @w_cta_contractual = -1,
          @w_cta_aso = -1,
          @w_cta_asa = -1
   ...
   begin
       ...
       select @w_cta_contractual = pa_int
       ...
       and    pa_nemonico = 'PCAASA'  
        ...
        if exists (select 1
              ...
              where ah_cta_banco = @i_cta
              and   ah_moneda    = @i_mon
              and ah_prod_banc in (@w_cta_contractual,@w_cta_aso,@w_cta_asa))
        begin
             /* No existe cuenta_banco */
             exec cobis..sp_cerror
             @t_debug     = @t_debug,
             ...
             @i_num       = 2609858,
             @i_sev       = 0,
             @i_msg       = 'CUENTA DE AHORROS NO PUEDE SER CONTRACTUAL, APORTE SOCIAL O EN ESTADO GENERADO'
             return 2609858
        end
   end
...
begin
  ...
  select @w_tipo_per = ah_tipocta from cob_ahorros..ah_cuenta where (ah_cta_banco = @i_cta or ah_cliente = @i_codcliente)
   ...
   if @@rowcount = 0
   begin
      select @w_error = 258003 -- 208158 --NO EXISTEN REGISTROS
   ...
   begin
    ...
    -- Si la cuenta tiene cuenta migrada
       select @w_parmig = pa_char
       from   cobis..cl_parametro
       where  pa_producto = 'AHO'
       and    pa_nemonico = 'CTAMIG'
   ...
   begin
        ...
        begin 
         select 
            ...
            EstadoCta    as '508958',
            CuentaInt    as '508959',
            Oficinaori   as '508960',
            Tipo         as '508961',
            Categoria    as '508962',
            Tipo_Ente    as '508963',
            (select top 1 ca_cta_banco_mig from cob_ahorros..ah_cuenta_aux where ca_cta_banco = NoCuenta)  as '509560'
            from   #tmp_cuentas
            where  ((Oficina = @s_ofi AND @w_trn_ofior = 'S') or @w_trn_ofior = 'N')
            and Producto not in (@w_cta_contractual,@w_cta_aso,@w_cta_asa)
            and EstadoCta = 'A'
        end
        else
        begin 
