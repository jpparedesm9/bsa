...
as
  ...
  select
    @w_fecha_ini = dateadd(dd,1 - (datepart(dd,@i_fecha)),@i_fecha),
    @w_fecha_fin = dateadd(dd,-1,@i_fecha),
    @w_sp_name = 'sp_ahmensual_contract',
    @w_debug = 'N'
  ...
  begin
    ...
    if @i_tipo_atributo = 'B'	   /* Saldo promedio */
      ...
      select @w_monto = @w_saldo_contable
    else if @i_tipo_atributo = 'A' /* Saldo Disponible */
      select @w_monto = @w_saldo_disponible
      ...
      select @w_monto = @i_min_dispmes
    else
      select @w_monto = $1

    select @w_tasa = (@i_tasa_pactada + (@i_ptos_premio / 100))
    /* RECALCULO CON PUNTOS PREMIO */
    select @w_factor1 = 1 + @w_tasa
    select @w_factor2 = (convert(float, 1)) / @w_dias_anio
    /*calculo del factor para 1 dia */
    select @w_factor = (power (@w_factor1,@w_factor2)) - 1
    select @w_ptos_nominal = @w_factor * @w_dias_anio
    select @w_tasa_recalculo = (@w_ptos_nominal * 100)
    select @w_valor_acreditar = round((@w_monto * @w_factor),@i_numdeci)
    select @w_valor_acreditar = @w_valor_acreditar * @w_dias_cap

    /* ACTUALIZAR SALDOS DIARIOS */
    if @w_debug = 'S'
      print 'CUENTA: ' + cast(@i_cta_banco as varchar) + ' FECHA: ' + cast(@w_fecha as varchar)
    ...
    begin
      ...
      set    sd_tasa_disponible = round(isnull(@w_tasa_recalculo,0),2),
             sd_int_hoy = isnull(@w_valor_acreditar,0)
      ...
      begin
        select
          @w_error = 203022,
          @w_msg = 'ERROR ACTUALIZANDO SALDOS DIARIOS. CUENTA: ' + convert(varchar,@i_cta_banco)
        goto ERROR
      end
    end

    /* ACUMULADOR DE VALORES ACREDITADOS */
    select @w_valor_acreditar = (@w_valor_acreditar - @w_int_hoy)
    select @w_valor_total = isnull(@w_valor_total, 0) + isnull(@w_valor_acreditar, 0)

    delete from #saldo_diario where  sd_fecha = @w_fecha
  ...
  select
    @o_ajuste = isnull(@w_valor_total,0)
