...
if @@error <> 0
begin
	select @w_mensaje = ' ERROR AL ACTUALIZAR LOS PERFILES CON EXEPCION'
	goto ERRORFIN
end
...
if @@rowcount = 0
begin
	select	@w_mensaje = 'NO ESTA DEFINIDA EL AREA CONTABLE PARA AHORROS',
			@w_error = 251091
	goto ERRORFIN
end
...
select distinct
	...
	order by tc_fecha, tc_ofic_dest, tc_ofic_orig,
			tc_perfil, tc_producto, tc_tipo_tran

if @@error <> 0
begin
	select @w_mensaje = ' ERROR AL DETERMINAR EL UNIVERSO DE COMPROBANTES A GENERAR '
	goto ERRORFIN
end
...
if @@error <> 0
begin
	select @w_mensaje = ' ERROR AL CREAR TABLA TEMPORAL #perfil_contable'
	goto ERRORFIN
end
...
if @@error <> 0
begin
	select @w_mensaje = ' ERROR AL DESARROLLAR LOS PERFILES CONTABLES DEL PRODUCTO '
	goto ERRORFIN
end
...
begin
    ...
    select	@w_mensaje = '',
			@w_error = 0,
			@w_descripcion = 'Pe:' + @w_perfil + ' ' + 'OO:' + convert(varchar, @w_oficina_orig) + 
							' ' + 'OD:' + convert(varchar, @w_oficina_dest)

    -- Oficina Contable
    select @w_re_oficina_orig = re_ofconta
	...
	if @@rowcount = 0
	begin
		select @w_mensaje = 'NO EXISTE CODIGO CONTABLE PARA LA OFICINA (ORIGEN):' + convert(varchar, @w_oficina_orig)
		goto ERROR1
	end

	-- Oficina Contable
	select @w_re_oficina_dest = re_ofconta
		from cob_conta..cb_relofi with (nolock)
		where re_filial	= 1
		and re_empresa	= 1
    ...
    if @@rowcount = 0
    begin
		select @w_mensaje = 'NO EXISTE CODIGO CONTABLE PARA LA OFICINA (DESTINO):' + convert(varchar, @w_oficina_dest)
		goto ERROR1
    end
	...
	select
			...
			de_cuenta		= cuenta,
			de_debcred		= case
								when tc_valor < 0 and debcred = '1' then '2'
								when tc_valor < 0 and debcred = '2' then '1'
								else debcred
							  end,
			de_area			= area,
			de_oficina		= case
								...
								else (select isnull(ta_ofi_central, 0)
			...
			de_debito		= case
								when tc_moneda = 0 then 
									(case when tc_valor < 0 and debcred = '1' then 0
										when tc_valor < 0 and debcred = '2' then abs(tc_valor)
                                        when tc_valor > 0 and debcred = '2' then 0
                                        when tc_valor > 0 and debcred = '1' then abs(tc_valor)
                                    end)
								else 0
							  end,
			de_debito_me	= case when tc_moneda <> 0 then 
									(case
                                        when tc_valor < 0 and debcred = '1' then 0
										when tc_valor < 0 and debcred = '2' then abs(tc_valor)
										when tc_valor > 0 and debcred = '2' then 0
										when tc_valor > 0 and debcred = '1' then abs(tc_valor)
                                        end)
			...
			de_credito		= case when tc_moneda = 0 then 
									(case
										when tc_valor < 0 and debcred = '1' then abs(tc_valor)
                                        when tc_valor < 0 and debcred = '2' then 0
                                        when tc_valor > 0 and debcred = '2' then abs(tc_valor)
			...
			de_credito_me	= case when tc_moneda <> 0 then 
									(case when tc_valor < 0 and debcred = '1' then abs(tc_valor)
										when tc_valor < 0 and debcred = '2' then 0
                                        when tc_valor > 0 and debcred = '2' then abs(tc_valor)
                                        when tc_valor > 0 and debcred = '1' then 0
                                    end)
			...
			de_con_rete		= case tc_tipo_impuesto when 'R' then tc_concepto_imp else null end,
			de_valret		= case tc_tipo_impuesto when 'R' then tc_valor else 0 end,
			de_con_ivareten = case tc_tipo_impuesto when 'V' then tc_concepto_imp else null end,
			de_iva_retenido = case tc_tipo_impuesto when 'V' then tc_valor else 0 end,
			de_con_ica		= case tc_tipo_impuesto when 'C' then tc_concepto_imp else null end,
			de_valor_ica	= case tc_tipo_impuesto when 'C' then tc_valor else 0 end,
			de_con_dptales	= case tc_tipo_impuesto when 'E' then tc_concepto_imp else null end,
			de_valor_dptales= case tc_tipo_impuesto when 'E' then tc_valor else 0 end,
			de_con_iva		= case tc_tipo_impuesto when 'I' then tc_concepto_imp else null end,
			de_valor_iva	= case tc_tipo_impuesto when 'I' then tc_valor else 0 end,
			de_con_timbre	= case tc_tipo_impuesto when 'T' then tc_concepto_imp else null end,
			de_valor_timbre = case tc_tipo_impuesto when 'T' then tc_valor else 0 end,
			de_con_ivapagado= case tc_tipo_impuesto when 'P' then tc_concepto_imp else null end,
			de_valor_ivapagado = case tc_tipo_impuesto when 'P' then tc_valor else 0 end,
		...
		and clave       = case tclave
							when 'sp_ah01_pf' then convert(varchar, tc_moneda) + '.' + tc_clase_clte + '.' + 
									convert(varchar, tc_prod_banc) + '.' + tc_estcta
                            when 'sp_ah02_pf' then convert(varchar, tc_moneda) + '.' + tc_concepto
                            when 'sp_ah03_pf' then convert(varchar, tc_moneda) + '.' + tc_estcta
                            when 'sp_ah04_pf' then convert(varchar, tc_moneda) + '.' + tc_clase_clte + '.' + 
									convert(varchar, tc_prod_banc) + '.' + tc_estcta + '.' + tc_concepto
	...
	if @w_err <> 0
	begin
		select @w_mensaje = ' ERROR AL GENERAR LOS ASIENTOS CONTABLES EN LA TABLA asiento_aho '
		goto ERROR1
	end

	if @w_rows = 0
	begin
		select @w_mensaje = ' ERROR NO EXISTE PARAMETRIZACION CONTABLE '
		goto ERROR1
	end

    if @i_debug = 'S'
	begin
		print 'Transaccion = ' + convert(varchar, @w_trn)
		select * from cob_ahorros..asiento_aho
	end

    /* Valores Debito y Credito Moneda Nacional */
	select	@w_debito = isnull(sum(de_debito), 0),
    ...
    begin
		select @w_mensaje = ' ERROR, NO CUADRAN DEBITOS CON CREDITOS. Deb: ' + convert(varchar, @w_debito) + 
    ...
    begin
		select @w_mensaje = ' ERROR, NO CUADRAN DEBITOS CON CREDITOS EN MONEDA EXTRANJERA . Deb: ' + 
							convert(varchar, @w_debito_me) + ' Cre: ' + convert(varchar, @w_credito_me)
		goto ERROR1
    end
	
    if @i_debug = 'S'
	begin
		print 'Se generara el comprobante para la Transaccion = ' + convert(varchar, @w_trn)
		print	'@w_debito = ' + convert(varchar, @w_debito) + 
				' - @w_credito = ' + convert(varchar, @w_credito) + 
				' - @w_debito_me = ' + convert(varchar, @w_debito_me) + 
				' - @w_credito_me = ' + convert(varchar, @w_credito_me) + 
				' - @w_asiento = ' + convert(varchar, @w_asiento) 
	end
	...
	if @w_error <> 0
    begin
		select @w_mensaje = ' ERROR AL GENERAR NUMERO COMPROBANTE (sp_cseqcomp)'
		goto ERROR1
    end
    ...
    begin
		...
		insert into cob_conta_tercero..ct_scomprobante_tmp with (rowlock)
				(sc_producto, sc_comprobante, sc_empresa, sc_fecha_tran, sc_oficina_orig,
				sc_area_orig, sc_digitador, sc_fecha_gra, sc_descripcion, sc_perfil,
				sc_detalles, sc_tot_debito, sc_tot_credito, sc_tot_debito_me, sc_tot_credito_me,
				sc_automatico, sc_reversado, sc_estado, sc_mayorizado, sc_observaciones,
				sc_comp_definit, sc_usuario_modulo, sc_tran_modulo, sc_error)
			values
				(@w_cod_producto, @w_comprobante, 1, @w_fecha_mov, @w_re_oficina_orig,
				@w_ar_origen, @w_sp_name, convert(char(10), getdate(), 101), @w_descripcion, @w_perfil,
		...
		if @@error <> 0
		begin
			select @w_mensaje = 'ERROR AL INSERTAR REGISTROS EN LA TABLA ct_scomprobante_tmp '
			goto ERROR1
		end
		...
		insert into cob_conta_tercero..ct_sasiento_tmp with (rowlock)
				...
				sa_credito_me,sa_concepto,sa_debito_me,sa_cotizacion,sa_tipo_doc,
				sa_tipo_tran,sa_moneda,sa_opcion,sa_ente,sa_con_rete,
				sa_base,sa_valret,sa_con_iva,sa_valor_iva,sa_iva_retenido,
				sa_con_ica,sa_valor_ica,sa_con_timbre,sa_valor_timbre,sa_con_iva_reten,
				sa_con_ivapagado,sa_valor_ivapagado,sa_documento,sa_mayorizado,sa_con_dptales,
				sa_valor_dptales,sa_posicion,sa_debcred,sa_oper_banco,
				sa_cheque,sa_doc_banco,sa_fecha_est,sa_detalle,sa_error)
			select
				...
				de_con_iva,de_valor_ivapagado,convert(varchar, de_cliente),'N', de_con_dptales,
		...
		if @@error <> 0
		begin
			select @w_mensaje = 'ERROR AL INSERTAR REGISTROS EN LA TABLA ct_sasiento_tmp '
			goto ERROR1
		end		
	
		if @i_debug = 'S'
		begin
			print 'Los comprobantes para la Transaccion = ' + convert(varchar, @w_trn)

			select	sa_asiento, sa_cuenta, sa_debito, sa_credito, sa_debcred, sa_concepto, sa_tipo_tran, 
					sc_tran_modulo, sc_producto, sc_comprobante, sc_empresa, sc_fecha_tran, sc_oficina_orig, sc_area_orig, sc_perfil, 
					sc_detalles, sc_tot_debito, sc_tot_credito, sc_estado, sc_mayorizado, sc_error--, *
			...
			--and sc_tran_modulo in (@w_trn)
			order by sc_fecha_tran desc, sc_tran_modulo, sa_comprobante, sa_asiento
		end
	end --@w_asiento > 0
	...
	if @@error <> 0
	begin
		select @w_mensaje = ' ERR AL MARCAR REGISTROS EN LA TABLA re_trn_contable COMO CONTABILIZADO '
		goto ERROR1
	end
	...
	if @@error <> 0
	begin
		select @w_mensaje = ' ERR AL REGISTRAR EN LA TABLA re_trn_contable LA CUENTA USADA AL DEBITO '
		goto ERROR1
	end
	...
	update cob_remesas..re_trn_contable with (rowlock)
		set tc_comprobante = null,
			tc_mensaje	= 'ERROR, NO EXISTEN CUENTAS CONTABLES RELACIONADAS AL COMPROBANTE',
			tc_estado	= 'ERR'
		where tc_fecha	 = @w_fecha_mov
		and tc_ofic_dest = @w_oficina_dest
		and tc_ofic_orig = @w_oficina_orig
	...
	if @@error <> 0
	begin
		select @w_mensaje = ' ERR AL MARCAR REGISTROS EN LA TABLA re_trn_contable COMO ERRONEOS '
		goto ERROR1
	end
