...
/************************************************************************/
/*  Archivo:            tr_lev_ah_consumo.sp                            */
/*  Stored procedure:   sp_tr_lev_ah_consumo                            */
...
/************************************************************************/
/*                              PROPOSITO                               */
/*  Este programa realiza la transaccion del Levantamiento manual       */
/*      del bloqueo por consumos para las cuentas de ahorros            */
/*  306 = Levantamiento Manual de Consumo en Ahorros                    */
/************************************************************************/
/*                              MODIFICACIONES                          */
/*      FECHA           AUTOR           RAZON                           */
/*  01/Jul/1999 V.Molina E.     Emision inicial                         */
/*  18/Oct/1999 Juan F. Cadena  Inclusion de comision en bloqueo        */
...
if exists (select
             1
           from   sysobjects
           where  name = 'sp_tr_lev_ah_consumo')
  drop proc sp_tr_lev_ah_consumo
go

create proc sp_tr_lev_ah_consumo
(
  @s_ssn          int,
  @s_ssn_branch   int,
  @s_srv          varchar(30),
...
as
  ...
  select
    @w_sp_name = 'sp_tr_lev_ah_consumo'
  ...
  exec @w_return = @w_rpc
    ...
    @i_fecha         = @i_fecha,
    @i_turno         = @i_turno,
    @o_tarjeta       = @w_tarjeta out,
    @o_valor_imp     = @w_valor_imp out,
  ...
  begin
    ...
    begin
      ...
      values      (@s_ssn,@s_ssn_branch,@t_trn,@s_date,@s_user,
                   ...
                   @i_fecha,@i_valor,@i_mon,getdate(),@w_tarjeta,
                   null,'L',@i_clave,@w_oficina_cta,@w_valor_imp,
