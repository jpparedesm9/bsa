  ...
  begin
    ...
    select
      ...
      dp_fecha_radicacion = case ah_estado
                              ...
                              else ah_fecha_ult_mov --ah_fecha_aper
                            end,
      ...
      dp_origen = ah_origen,
      dp_provisiona = (select isnull(pf_provisiona, 'S')
                        ...
                        where pb_pro_bancario = me_pro_bancario
                          ...
                          and pf_sucursal in (select case of_subtipo when 'O' then of_regional
                                                                     else of_oficina
                                                     end
                                                from   cobis..cl_oficina
                                                where  of_oficina = ah_oficina)),
      dp_blqpignoracion = case when (select count(1)
                                       from cob_ahorros..ah_his_bloqueo
                                      where hb_cuenta = ah_cuenta
                                        and hb_causa  = 18
                                        and hb_accion = 'B') > 0
                               then 'S'
                               else 'N'
    ...
    /*** Actualizamos campos null en el campo dp_categoria_producto ***/
    update #cuenta_cb
       set dp_categoria_producto = ah_prod_banc
      from ah_cuenta_tmp
     where ah_cta_banco = dp_banco
       and dp_categoria_producto is null

    if @@error <> 0
    begin
      select
        @w_error = 499999,
        @w_msg = 'Error al actualizar las cuentas con dp_categoria_producto nulo'
      goto ERROR
    end 
    ...
    insert into cob_externos..ex_dato_pasivas
                (dp_fecha,dp_banco,dp_toperacion,dp_aplicativo,
                 ...
                 dp_ley_exen,dp_origen,dp_provisiona,dp_blqpignoracion)
      select
