...
IF OBJECT_ID ('dbo.sp_tr_crea_acciond') IS NOT NULL
	DROP PROCEDURE dbo.sp_tr_crea_acciond
GO
  ...
  begin
    ...
    begin tran
     ...
     insert into cob_remesas..re_accion_nd
                  (an_producto, an_causa, an_accion, an_comision, an_impuesto,  an_saldomin)
         values   (@i_producto, @i_causal, @i_accion, @i_comision, @i_impuesto, @i_saldomin)
     if @@error <> 0
        begin
          exec cobis..sp_cerror
               @t_debug	 = @t_debug,
