...
--Transaccion 408
select @w_producto = 3
select @w_transaccion = 408

--ad_procedure
if exists (select 1 from ad_procedure where pd_stored_procedure = 'sp_rem_chequexcta' and pd_base_datos = 'cob_remesas')
begin
    delete from ad_procedure where pd_stored_procedure = 'sp_rem_chequexcta' and pd_base_datos = 'cob_remesas'
end
...
VALUES (@w_cod_procedure, 'sp_rem_chequexcta', 'cob_remesas', 'V', getdate(), 'rechqcta.sp')
...
VALUES (@w_transaccion, 'CONSULTA DE CHEQUES POR CUENTA', 'CHCU', 'CONSULTA DE CHEQUES POR CUENTA')
