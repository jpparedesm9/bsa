...
begin  
	...
	values ('CALCULO PROVISION DIARIA', 'PRVDIA', 'C', 'N', NULL, NULL, NULL, NULL, NULL, NULL, 'AHO')
end
else
begin
	UPDATE cobis..cl_parametro 
	SET pa_char = 'N'
	WHERE pa_nemonico = 'PRVDIA'
end
...
-- pe_parametria.sql
if exists (select 1 from cob_remesas..pe_tipo_rango where tr_descripcion = 'RUBRO SOBRE SALDO DISPONIBLE')
begin
	UPDATE cob_remesas..pe_tipo_rango
	SET tr_tipo_atributo = 'E'
...
SELECT @w_codigo = pa_int from cobis..cl_parametro where pa_nemonico = 'PCAASA' and pa_producto = 'AHO'
		...
		begin
			...
			if @w_return <> 0 print 'Error en la obtencion de secuencial pe_pro_final'
			INSERT INTO cob_remesas..pe_tipo_rango (tr_tipo_rango, tr_descripcion, tr_tipo_atributo, tr_moneda, tr_estado)
			VALUES (@w_tipo_rango_prom, 'RUBRO SOBRE SALDO DISPONIBLE', 'E', 0, 'V')    
			if @@error != 0 print 'Error en la inserccion de registro en pe_tipo_rango'
			else print 'Creación pe_tipo_rango PROM OK!'
		end
		else select @w_tipo_rango_prom = tr_tipo_rango from cob_remesas..pe_tipo_rango WHERE tr_descripcion = 'RUBRO SOBRE SALDO DISPONIBLE'  and tr_estado = 'V' 
		-- Rango
...
if exists (select codigo from cobis..cl_catalogo where tabla in (select codigo From cobis..cl_tabla where tabla like 'pe_rubro')  and valor = 'INTERES SOBRE EL DISPONIBLE')
begin
	select @w_rubro = vs_rubro from cob_remesas..pe_var_servicio 
	WHERE vs_servicio_dis = @w_servicio_disponible 
	and vs_rubro in (select codigo from cobis..cl_catalogo
	...
	if @w_rubro is null print 'No existe VARServicio INTERES SOBRE EL DISPONIBLE'
	else print 'Rubro: ' + convert(varchar(10),@w_rubro)
end
else
begin
	print 'No existe Rubro 18 - INTERES SOBRE EL DISPONIBLE'
end
