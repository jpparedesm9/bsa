...
rem ********************************************************
rem *    ARCHIVO:         fullspw_master.cmd			
rem *    NOMBRE LOGICO:   fullspw_master.cmd			
rem *    PRODUCTO:        COBIS	
rem ********************************************************
rem *                     IMPORTANTE			
...
rem ********************************************************
rem *                     PROPOSITO			
rem *   Maestro de Compilacion de stored procedures
rem ********************************************************
rem *                     MODIFICACIONES			
...
for %%j in (1 2 3) do (
	for %%k in (%6\*.error) do (
		  ...
		  echo Ejecutando el dump   
                  SET /a ii=0
                  sqlcmd -U%1 -P%2 -S%3 -n -i dump.sql -o dump.out
                  sqlcmd -U%1 -P%2 -S%3 -n -i dump2.sql -o dump2.out
                )		    
                SET /a ii+=1
		echo Ejecutando %%~nk.sp
		del %6\%%~nk.error
		sqlcmd -U%1 -P%2 -S%3 -n -i %5\%%~nk.sp -o %6\%%~nk.error  -m 1
...
:ayuda
echo fullspw_master.bat [parametro 1] [parametro 2] [parametro 3] [parametro 4] [parametro 5] [parametro 6] [parametro 7]
