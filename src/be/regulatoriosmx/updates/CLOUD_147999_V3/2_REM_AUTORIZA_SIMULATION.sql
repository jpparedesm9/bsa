...
begin
    ...
    if not exists (select 1 from cts_serv_catalog where csc_service_id = 'LoanGroup.CreditRenovation.SimulationCreditRenovation')	
    ...
    values('LoanGroup.CreditRenovation.SimulationCreditRenovation',  'cobiscorp.ecobis.loangroup.service.ICreditRenovation', 'simulationCreditRenovation', '', 0, null, null, 'N')

    if not exists (select 1 from ad_servicio_autorizado where 
    ts_servicio = 'LoanGroup.CreditRenovation.SimulationCreditRenovation' and ts_rol = @w_rol and ts_producto = @w_producto and ts_moneda = @w_moneda)    
    ...
    values('LoanGroup.CreditRenovation.SimulationCreditRenovation', @w_producto, 'R', @w_moneda, getdate(), @w_rol, 'V', getdate())
end
else
begin 
    print 'cambio rol'
