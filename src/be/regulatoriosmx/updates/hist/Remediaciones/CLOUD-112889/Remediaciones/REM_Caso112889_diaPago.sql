...
select @w_grupo = 97 --GRUPO

select @w_ciclo = isnull(max(dc_ciclo_grupo),-1) from ca_det_ciclo where dc_grupo = @w_grupo

select 
operacion = op_operacion
into #operaciones_res
from ca_operacion 
where op_operacion in (select dc_operacion from ca_det_ciclo where dc_ciclo_grupo = @w_ciclo and dc_grupo = @w_grupo) 

select * from #operaciones_res

select * from ca_operacion_his 
where oph_operacion in (select operacion from #operaciones_res)

select * from cob_cartera_his..ca_operacion_his 
where oph_operacion in (select operacion from #operaciones_res)

select  * from ca_dividendo
where di_operacion in (select operacion from #operaciones_res)

select  * from ca_dividendo_his
where dih_operacion in (select operacion from #operaciones_res)

select  * from cob_cartera_his..ca_dividendo_his 
where dih_operacion in (select operacion from #operaciones_res)
go

drop table #operaciones_res
go
...
select 
@w_fecha_obj = '02/05/2019', --COLOCAR LA FECHA EN LA QUE SE DEBIO DESEMBOLSAR EL PRES
@w_grupo = 97 --GRUPO
