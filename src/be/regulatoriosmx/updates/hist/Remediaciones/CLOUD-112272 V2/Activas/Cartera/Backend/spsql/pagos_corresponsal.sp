...
/*                                         am_cuota                     */
/* 24/07/2018         SRO                  Anulación de Pagos           */
/************************************************************************/  
use cob_cartera
go
...
@i_en_linea      char(1) = 'N',
@i_externo           char(1)   = 'N', -- S cuando la invocación viene desde un agente externo N(cuando la invocación es desde Cobis)
...
if @i_operacion = 'I' BEGIN    
   ...
   if not exists (select 1 from ca_producto where cp_producto = @i_corresponsal )  begin
	...
	if @@rowcount = 0 begin
       --select 
       --@w_error = 70209,
	   --@w_msg   = 'ERROR: EL CORRESPONSAL NO ESTÁ REGISTRADO'
	   --GOTO ERROR_FIN
	   --SRO. Caso 111264 Error en reverso nota de débito
       select 
    ...
    if @i_accion = 'I' begin
       ...
       if @@rowcount = 0 or @w_tanquear = 'N' begin -- si no existe registro o SI no usa TANQUEO, entonces INSERTA 
         ...
         -- si hay tanqueo, debe retornar la combinación del secuencial de corresponsal_trn con el secuencial de tanqueo
      ...
      -- Existe una tabla de errores por corresponsal que se consulta en ERROR_FIN, @w_error = 0 es para indicar ejecución exitosa
      select @w_error = 0
   ...
   if @i_accion = 'R' begin
      ...
      if @w_rowcount = 0 begin
         ...
         if @@rowcount = 0 begin
            ...
            @w_est = 'E',
            @w_error = 70180,
            @w_msg = 'ERROR: Transacción con id de corresponsal: '+ @i_trn_id_corresp +' a reversar no existe.'
            GOTO ERROR_FIN
         end
         ...
         @w_est = 'E',
         @w_error = 70211,
         @w_msg = 'ERROR: Existe más de un registro con id de corresponsal: '+ @i_trn_id_corresp +' y monto: ' + convert(varchar,(convert(money,isnull(@i_monto_pago,0))/100))
         GOTO ERROR_FIN
	  end
...
while @@fetch_status = 0  begin
 ...
 ---Saldo para precancelar 
   ...
   BEGIN
      ...
      select @w_valor_vigente    = isnull(sum(am_cuota),0) * (1 + @w_sensibilidad)
	  from  #prestamos_grupo, ca_dividendo, ca_amortizacion
      where am_operacion = di_operacion
      and   am_dividendo = di_dividendo
      and   am_operacion = operacion 
      ...
      if @w_monto > @w_valor_vigente and @w_monto < @w_saldo_precancelar select @w_formula = 2
      if @w_monto >= @w_saldo_precancelar select @w_formula = 3
      
 	  if @w_formula = 1  begin 
	              
        insert into #pagos_grupales
        select banco, operacion, oficina, cliente, sum(am_cuota), 0
        from  #prestamos_grupo, ca_dividendo, ca_amortizacion
        where am_operacion = di_operacion
        and   am_dividendo = di_dividendo
      	and   am_operacion = operacion 
	...
	if @w_formula = 2  begin 
		...
		print 'registros..' + convert(varchar, @@rowcount)		  
	end   
	...
	if @w_formula = 3  begin 
	  ...
	  --print 'porc: ' + convert(varchar, @w_porcentaje) + ' monto: ' + convert(varchar, @w_monto) + ' exigible: ' + convert(varchar, @w_total_exigible)
	  --print 'total_pago: ' + convert(varchar, @w_total_pago) + ' diferencia: ' + convert(varchar, @w_diferencia ) 
   ...
   if @w_tipo = 'RP' begin
      ...
      while @@fetch_status = 0  begin
         ...
         if @w_error != 0 begin
            select 
            @w_est = 'E',
            @w_msg = 'ERROR AL EJECUTAR REVERSO DE LA OPERACION : ' + convert(varchar,@w_banco)          
