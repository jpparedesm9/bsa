...
IF NOT  EXISTS (SELECT name FROM sys.indexes  
            WHERE name = N'indx_cr_lista_negra_ln_rfc_ln_tipo_lista')  
            BEGIN
                CREATE NONCLUSTERED INDEX [indx_cr_lista_negra_ln_rfc_ln_tipo_lista]
                ON [dbo].[cr_lista_negra] ([ln_rfc],[ln_tipo_lista])
            END

/*************************************************************/
/*   ARCHIVO:         	sp_regla_matriz_riesgoTemp.sp        */
/*   NOMBRE LOGICO:   	sp_regla_matriz_riesgoTemp           */
/*   PRODUCTO:        	credito                              */
/*************************************************************/
/*                     IMPORTANTE                            */
...
IF OBJECT_ID ('dbo.sp_regla_matriz_riesgoTemp') IS NOT NULL
	DROP PROCEDURE dbo.sp_regla_matriz_riesgoTemp
GO

CREATE PROCEDURE sp_regla_matriz_riesgoTemp (	
	@t_debug                   char(1) 	     = 'N',
...
select @w_sp_name = 'sp_regla_matriz_riesgoTemp'
...
begin
	...
	PRINT 'B'
	   insert into @w_ente 
	   select en_ente from cobis..cl_ente,cobis..cl_ente_aux
	   where en_ente=ea_ente
	   and   ea_estado='A'
	   and   ea_nivel_riesgo is NULL
	   and   ea_puntaje_riesgo is NULL
	   --AND en_ente between 0 and 200
	   ...
	   while (1 = 1) begin
	     ...
	     */			   
         ...
         begin
		    ...
		    IF(@w_contador=10000)
		    BEGIN
		    PRINT'@w_contador----> '+convert(VARCHAR(50),@w_contador)
           ...
           if @@error <> 0
            ...
            IF  EXISTS ((SELECT 1 FROM cobis..cl_alertas_riesgo WHERE ar_ente= @i_ente AND ar_tipo_lista='C' ))
               BEGIN
                 ...
                 if(replace(@w_msm_ea_nivel_riesgo,' ','')<>replace(@w_cli_a3ccc,' ','') and replace(@w_msm_ea_nivel_riesgo,' ','')<>replace(@w_cli_a3bloq,' ',''))
                       begin
