--ASESOR ASIGNADO: MARÍA GUADALUPE HERNÁNDEZ BARRIOS USUARIO: mghernandezba
--ASESOR A REASIGNAR: ELENA BERENICE RENDÓN USUARIO: ebrendon
--
--GRUPO:
--
--738 - LLUVIA TUIIO
...
--
--ASESOR ASIGNADO: MARÍA GUADALUPE HERNÁNDEZ BARRIOS USUARIO: mghernandezba
--ASESOR A REASIGNAR: JOSÉ MANUEL HERNÁNDEZ USUARIO: jmhernandezsa
--
--GRUPO:
--
--355 - AMARANTA
...
--
--ASESOR ASIGNADO: MARÍA GUADALUPE HERNÁNDEZ BARRIOS USUARIO: mghernandezba
--ASESOR A REASIGNAR: NEREIDA ALCÁNTARA DÍAZ   USUARIO: nalcantara
--
--GRUPO:
--
--640 - MARAVILLOSAS DE HOGARES
--
--ASESOR ASIGNADO: LUIS BERNARDO RAFAEL JIMÉNEZ USUARIO: ljimenezp
--ASESOR A REASIGNAR: NEREIDA ALCÁNTARA DÍAZ   USUARIO: nalcantara
--
--GRUPO:
--
--319 - ALEGRES TUIIO
--1099 - CARDENALES TUIIO
--210 - FRESITAS
--747 - GOLONDRINAS TUIIO
--1264 - LUNITAS TUIIO
--1362 - MUÑECAS TUIIO
--258 - PROSPERIDAD TUIIO
--303 - REAL DE SAN MATEO
--425 - SÚPER PAGADORAS TUIIO
--301 - TURQUESA TUIIO
--378 - VENCEDORAS TUIIO
...
select @w_oficial_dest = fu_funcionario  
from  cobis..cl_funcionario
where  fu_login = 'ebrendon'

if @@rowcount > 0 begin
   insert into #grupos values (738, @w_oficial_dest)
   insert into #grupos values (1248, @w_oficial_dest)
end

select @w_oficial_dest = fu_funcionario  
from  cobis..cl_funcionario
where  fu_login = 'jmhernandezsa'

if @@rowcount > 0 begin
   insert into #grupos values (355, @w_oficial_dest)
   insert into #grupos values (415, @w_oficial_dest)
end


select @w_oficial_dest = fu_funcionario  
from  cobis..cl_funcionario
where  fu_login = 'nalcantara'

if @@rowcount > 0 begin
   insert into #grupos values (640, @w_oficial_dest)
   insert into #grupos values (319, @w_oficial_dest)
   insert into #grupos values (1099, @w_oficial_dest)
   insert into #grupos values (210, @w_oficial_dest)
   insert into #grupos values (747, @w_oficial_dest)
   insert into #grupos values (1264, @w_oficial_dest)
   insert into #grupos values (1362, @w_oficial_dest)
   insert into #grupos values (258, @w_oficial_dest)
   insert into #grupos values (303, @w_oficial_dest)
   insert into #grupos values (425, @w_oficial_dest)
   insert into #grupos values (301, @w_oficial_dest)
   insert into #grupos values (378, @w_oficial_dest)
end
