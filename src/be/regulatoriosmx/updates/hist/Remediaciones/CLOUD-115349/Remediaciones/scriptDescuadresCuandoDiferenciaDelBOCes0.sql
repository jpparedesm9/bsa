...
and bo_cliente in (45845,46114,47474,48274,84293,85017,86415,87236) --- AQUI COLOCAR LA LISTA DE CLIENTES A ALINEAR PREVIO A ANALISIS DE CADA CASO
group by bo_cliente
...
from   cob_cartera..ca_operacion 
where  op_cliente    = 8121
and    op_toperacion = 'GRUPAL'
and    op_estado     = 1

select 'operacionACambiarParaCliente8121' = @w_operacion
select * from ca_det_trn, ca_transaccion
where  tr_operacion  = @w_operacion
and    tr_estado     = 'ING'
and    tr_operacion  = dtr_operacion
and    tr_secuencial = dtr_secuencial
and    dtr_concepto  = 'VAC0'
and    tr_toperacion  = 'GRUPAL'
and    dtr_secuencial = 113


update ca_det_trn
set    dtr_monto     = 585.59,
       dtr_monto_mn  = 585.59
...
select @w_operacion  = op_operacion
from   cob_cartera..ca_operacion 
where  op_cliente    = 8380
and    op_toperacion = 'GRUPAL'
and    op_estado     = 1

select 'operacionACambiarParaCliente8380' = @w_operacion
select * from ca_det_trn, ca_transaccion
where  tr_operacion  = @w_operacion
and    tr_estado     = 'ING'
and    tr_operacion  = dtr_operacion
