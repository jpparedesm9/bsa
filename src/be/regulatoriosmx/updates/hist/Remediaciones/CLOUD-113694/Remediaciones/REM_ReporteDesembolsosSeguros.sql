...
select @w_sp_name             = 'sp_borrar_seguros_tmp'

select @w_fecha_proc          = fp_fecha from cobis..ba_fecha_proceso 
select @w_ffecha              = 103
select @w_nro_poliza          = pa_char  from cobis..cl_parametro where pa_nemonico='NROPOL' and pa_producto='CCA' if @@rowcount = 0 begin select 'sale por error, no hay parametro' end
select @w_anio_poliza         = pa_int  from cobis..cl_parametro where pa_nemonico='ANIPOL' and pa_producto='CCA' if @@rowcount = 0 begin select 'sale por error, no hay parametro' end
select @w_producto            = pa_char  from cobis..cl_parametro where pa_nemonico='SEGPRO' and pa_producto='CCA' if @@rowcount = 0 begin select 'sale por error, no hay parametro' end
select @w_seguro_vida         = pa_money from cobis..cl_parametro where pa_nemonico='SVIDA'  and pa_producto='CCA' if @@rowcount = 0 begin select 'sale por error, no hay parametro' end
select @w_seguro_infarto_cer  = pa_money from cobis..cl_parametro where pa_nemonico='SINFCE' and pa_producto='CCA' if @@rowcount = 0 begin select 'sale por error, no hay parametro' end
select @w_seguro_infarto_mioc = pa_money from cobis..cl_parametro where pa_nemonico='SINFMI' and pa_producto='CCA' if @@rowcount = 0 begin select 'sale por error, no hay parametro' end
select @w_seguro_cancer       = pa_money from cobis..cl_parametro where pa_nemonico='SCAN'   and pa_producto='CCA' if @@rowcount = 0 begin select 'sale por error, no hay parametro' end
select @w_porc_comision       = pa_float from cobis..cl_parametro where pa_nemonico='COMSEG' and pa_producto='CCA' if @@rowcount = 0 begin select 'sale por error, no hay parametro' end
...
select 
@w_destino  = @w_path + 'AUX_SEGUROS_TUIIO_' +  replace(CONVERT(varchar(10), @w_fecha_proc, @w_ffecha),'/', '')+ '.txt',
@w_errores  = @w_path + 'AUX_SEGUROS_TUIIO_' +  replace(CONVERT(varchar(10), @w_fecha_proc, @w_ffecha),'/', '')+ '.err'
...
if @w_error <> 0 begin
   ...
   SELECT ' no ejecuto bcp, salio errro'
end
               
SELECT ' FIN, termino OK'
go

drop table #seguros_nuevos
drop table #dir_actuales
drop table #direcciones
drop table #tel_actuales
drop table #telefonos
drop table #mail_actuales
drop table #mails
go