...
begin
    ...
    begin
		...
		select        d.pr_codigo_proceso,
		              d.pr_nombre_proceso,
		              count(case
		                    when d.pr_tiempo_creacion - d.pr_tiempo_feriados <=  d.pr_tiempo_estandar then 1
		                   end) as aTiempo,
		               count(case
		                     when d.pr_tiempo_creacion - d.pr_tiempo_feriados  >  d.pr_tiempo_estandar then 1
		                    end) as atrasados,
		        ...
		        select pr_codigo_proceso,
		               pr_nombre_proceso,
		               datediff(minute, io_fecha_crea, getdate()) as pr_tiempo_creacion,
		               (
		                select count(1)*1440
		                from cobis..cl_dias_feriados df
		                where df_fecha >= ip.io_fecha_crea
		                and df_fecha  <= getdate()
		                and df.df_ciudad = o.of_ciudad )  as pr_tiempo_feriados,
		               pr_tiempo_estandar,
		               r.codigo codigo_regional,
               		   r.valor  nombre_regional,               
		               o.of_oficina as codigo_oficina,
		               o.of_nombre as nombre_oficina
		...
		group by d.codigo_oficina, d.nombre_oficina, d.pr_codigo_proceso, d.pr_nombre_proceso, d.codigo_regional, d.nombre_regional
...
begin
    ...
    begin
		...
		group by d.codigo_oficina, d.nombre_oficina, d.pr_codigo_proceso, d.pr_nombre_proceso
