...
IF OBJECT_ID ('dbo.ca_incremento_cupo') IS NOT NULL begin
   drop table ca_incremento_cupo
end

create table ca_incremento_cupo(
   ic_operacion          int,
   ic_fecha_proceso      datetime,
   ic_monto_aprobado_ini money,
   ic_incremento         money,
   ic_monto_aprobado_fin money
)

create index idx1 on ca_incremento_cupo (ic_operacion)
go
...
select  @w_cliente =  10753  , @w_incremento = 300
...
select  @w_cliente = 164 , @w_incremento = 300
