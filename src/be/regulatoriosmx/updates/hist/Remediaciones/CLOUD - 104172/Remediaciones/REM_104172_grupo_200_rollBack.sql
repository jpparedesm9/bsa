...
select @w_grupo = 200
select @w_oficial_login = 'facoria' -- antes facoria
select @w_oficial_cod = 112 -- 112 antes
select @w_oficina = 2403 -- 2403 antes
--para rollabck 5419, 5480, 5594
--Clientes
UPDATE cobis..cl_ente 
SET    en_oficial    = @w_oficial_cod,
       c_funcionario = @w_oficial_login,
       en_oficina    = @w_oficina
WHERE  en_ente in (5419, 5480, 5594)

UPDATE cob_cartera..ca_operacion 
SET    op_oficial = @w_oficial_cod 
WHERE  op_cliente in (5419, 5480, 5594)
...
WHERE  tr_cliente in (5419, 5480, 5594)

UPDATE cob_cartera..ca_operacion_his 
SET    oph_oficial = @w_oficial_cod 
WHERE  oph_cliente in (5419, 5480, 5594)
