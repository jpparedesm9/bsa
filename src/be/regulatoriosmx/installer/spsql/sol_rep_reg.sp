...
/*                               MODIFICACIONES                         */
/*  FECHA              AUTOR          CAMBIO                            */
/* 12/01/2018          D. Cumbal      Aumento validacion                */
/************************************************************************/
...
begin
	...
	if (abs(datediff(MM, @w_fecha_proc, convert(datetime, convert(varchar, @i_anio) + '/' + convert(varchar, @i_mes) + '/01'))) > @w_param_PMSREP)
	begin	
	...
	from cob_conta..cb_solicitud_reportes_reg 
	where sr_status = 'I' --sr_fecha = @w_fecha_proc
...
begin
	...
	values (@w_fecha_proc, @i_reporte, @i_mes, @i_anio)
