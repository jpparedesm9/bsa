/************************************************************************/
/*  Archivo               : ods_saldos_cont_nv.sp                       */
/*  Stored procedure      : sp_ods_saldos_cont_nv                       */
/*  Base de datos         : cob_conta_super                             */
/*  Producto              : Contabilidad                                */
/*  Disenado por          : Dario Cumbal                                */
/*  Fecha de documentacion: 01 de Marzo de 2021                         */
/************************************************************************/
/*                          IMPORTANTE                                  */
...
if exists (select 1 from sysobjects where  name = 'sp_ods_saldos_cont_nv')
   drop proc sp_ods_saldos_cont_nv
go

create proc sp_ods_saldos_cont_nv
(
...
if  @w_fecha  is not null select @w_fecha_proceso = @w_fecha
---NUMERO DE DECIMALES
...
select @w_batch = case @w_treporte when 'D' then 287935 else 287936 end
...
if  @w_treporte = 'D' begin --diario 
   ...
   print '@w_cont: ' + convert(varchar,@w_cont)
   if @w_cont < 5 and @i_valida_dia = 'S'
   BEGIN
