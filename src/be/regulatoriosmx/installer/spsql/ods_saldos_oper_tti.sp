...
/* ********************************************************************* */
/*      Archivo:                sp_ods_saldos_oper_tti.sp                */
/*      Stored procedure:       sp_ods_saldos_oper_tti                   */
...
/*    09/Mar/2020     MTA      Caso 136241 cambiar la mora               */
/*                             de 90 a 120 dias                          */
/*   25/09/2020       ACH      Caso: 146729                              */
/*   15/04/2021       DCU      Caso: 156978                              */
/*   29/11/2021       ACH      Caso #173628 Catalogos HRC simple reportes*/
/*	 21/12/2021		  JEOM	   Caso #172727		       					 */
/*   27/01/2022       DCU      Caso: #177407                             */
/* ********************************************************************* */

use cob_conta_super
go
if exists (select 1 from sysobjects where name = 'sp_ods_saldos_oper_tti')
    drop proc sp_ods_saldos_oper_tti
go
create proc sp_ods_saldos_oper_tti
	@i_param1   DATETIME
as
...
TRUNCATE TABLE sb_ods01_tti

select *, tipo_amort = convert(varchar(20),null) into #sb_ods01_tti from sb_ods01_tti where 1 = 2 



INSERT INTO #sb_ods01_tti
SELECT  
--	do_banco,
	...
	--JEOM 21/12/2021 INI
	[campo 41] = do_estado_cartera,			
	--JEOM 21/12/2021 FIN
	tipo_amort = do_tipo_amortizacion
FROM cob_conta_super..sb_dato_operacion  --with (index (idx3) )
WHERE do_estado_cartera IN (@w_est_vigente,@w_est_etapa2, @w_est_vencido,@w_est_cancelado) --(1,2,3,4)  creditos con atraso de pago igual o menor a 90 días
AND do_fecha = @i_param1
AND do_aplicativo = 7
--AND do_dias_mora_365 < 120
...
update #sb_ods01_tti set 
campo22 =  replace(convert(VARCHAR, isnull(fecha,''), 111), '/','')   
from #pagos
where campo07= banco


alter table #sb_ods01_tti   drop column tipo_amort

insert into    sb_ods01_tti
select * from  #sb_ods01_tti 


select @w_query   = 'select * from cob_conta_super..sb_ods01_tti order by campo07'
