...
select r0453_fecha            = do_fecha,
      r0453_periodo 				= convert(varchar, datepart(yy, do_fecha)) + right('0'+ convert(varchar,datepart(mm,do_fecha)),2),
		r0453_clave_entidad			= convert(numeric(6),@w_clave),
		r0453_subreporte			= convert(numeric(4),@w_subreporte),
		r0453_identificador_acred	= convert(varchar(12),dc_cliente),
		r0453_persona_jurid			= (select convert(numeric(3),eq_valor_cat)            -- 5
                                         from cob_conta_super..sb_equivalencias
                                        where eq_catalogo  = 'TIPO_ENTE'
                                          and eq_valor_arch = dc_subtipo),
		r0453_nom_raz_social		= upper(ltrim(rtrim(isnull(dc_nombre,'0')))),
		r0453_p_apellido			= case dc_subtipo when 'P' then upper(ltrim(rtrim(isnull(dc_p_apellido,'0')))) else '0' end,
		r0453_s_apellido			= case dc_subtipo when 'P' then upper(ltrim(rtrim(isnull(dc_s_apellido,'0')))) else '0' end,
		...
		r0453_genero				= case when dc_sexo in('F','M')
		...
		r0453_product_credit		= convert(varchar(200),do_tipo_operacion) + '',                -- 15
		...
		r0453_fecha_ult_pago_cap    = isnull(convert(varchar,do_fecha_ult_pago,112),'19000101'),   -- 20
		...
		r0453_dias_mora				= DATEDIFF(day,do_fec_pri_amort_cubierta,@i_param1),           -- 25
		...
		r0453_saldo_capital			= convert(numeric(16),do_saldo_cap),
		r0453_saldo_interes			= convert(numeric(16),do_saldo_int),
		r0453_mora					= convert(numeric(16),do_valor_mora),
		r0453_inte_refinan			= convert(numeric(16),do_inte_refina),                        -- 30
		r0453_monto_castigo			= convert(numeric(16),do_monto_castigo),
		r0453_monto_condo			= convert(numeric(16),do_monto_condo),
		r0453_monto_boni			= null,    --consultar
		r0453_fecha_castigo			= isnull(convert(varchar,do_fecha_castigo,112),'19000101'),
		r0453_tipo_acred_rel		= null,    --consultar                                        -- 35
		r0453_estima_prevent_total	= null,    --consultar
        r0453_clave_preven          = isnull(@w_clave_sic,'1234567890'),
        r0453_fecha_sic             = isnull(convert(varchar,@w_fecha_sic,112),'19000101'),
		r0453_tipo_cobranza			= null,    --'pendiente',
        r0453_garantia_liquida      = (select isnull(sum(dc_valor_actual - dc_valor_uti_opera),0)
                                        ...
                                        group by dg_banco),
        r0453_garantia_hipotecaria  = (select isnull(sum(dc_valor_actual - dc_valor_uti_opera),0)
                                         from cob_conta_super..sb_dato_custodia, cob_conta_super..sb_dato_garantia
                                        where dc_garantia  = dg_garantia
                                          and dc_fecha     = dg_fecha
                                          and dc_categoria = 'H'
...
(
    ...
    VALOR_ULT_PAGO_CAP,          FECHA_ULT_PAGO_INT,        VALOR_ULT_PAGO_INT,         FEC_PRI_AMORT_NO_CUBI,      DIAS_MORA,          
...
from #reporte_r0453 a
where r0453_fecha = (select max(r0453_fecha) from #reporte_r0453 b where b.r0453_ope_banco = a.r0453_ope_banco)
