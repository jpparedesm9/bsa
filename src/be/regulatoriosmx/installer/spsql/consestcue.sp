/********************************************************************/
/*  Archivo:          consestcue.sp                                 */
/*  Stored procedure: sp_cons_estado_cuenta                         */
...
/********************************************************************/
/*                           PROPOSITO                              */
/* Realiza consulta de estado de cuenta de una operacion            */
/********************************************************************/
/*                MODIFICACIONES                                    */
/*  FECHA         AUTOR           RAZON                             */
/* 02/Feb/2017    D. Cumbal   Emision Inicial                       */
/* 29/May/2019    PXSG        Modificacion soporte 117889           */
/* 15/07/2019     PXSG        Requerimiento #115931                 */
/* 09/09/2019     RMA         Soporte #126040                       */
/* 10/10/2020     ACH         Error #148652-tb-comentario35         */
/* 24/02/2020     ACH         Error #152856                         */
/* 15/03/2021     ACH         ERR #154890 - dia de pago             */
/* 26/02/2021     SRO         Req #147999                           */
/* 05/08/2021     ACH         ERR #166476-Fecha de corte inconsiste */
/*                            en Estados de cuenta de julio         */
/* 05/08/2021     ACH         Err#168771 valorInsoluto en 0 para REN*/
/* 20/10/2021     ACH         ERR#170283 error en saldos            */
/* 05-Jul-2022    A.GO        REQ #174670 EDO DE CTA SIMPLE         */
...
if exists (select 1 from sysobjects where name = 'sp_cons_estado_cuenta')
   drop proc sp_cons_estado_cuenta
go

create proc sp_cons_estado_cuenta 
(
...
declare                         
	...
	@w_tir                   				  float,
	@w_tea                   				  float,
	@w_desc_toperacion_ind                    varchar(300),
	@w_periodo_cap                            int
...
--REQ 174670
select @w_desc_toperacion_ind = "CRÉDITO SIMPLE TU CRÉDITO + NEGOCIO"
...
BEGIN
   ...
   if(@w_toperacion='GRUPAL' or @w_toperacion='INDIVIDUAL')
   begin
...
BEGIN
   ...
   FROM  cob_conta_super..sb_ns_estado_cuenta 
   WHERE in_fecha_fin_mes = @i_fecha 
   ...
   SELECT 'secuencial'     = @i_secuencial,
          ...
          --Producto        
          'Producto'       =  case when @w_toperacion ='GRUPAL' then isnull(@w_desc_toperacion,'') else @w_desc_toperacion_ind end,
...
if @i_operacion = 'I' OR @i_operacion = 'A' --Informacion CrÃ©dito / TODO
begin
   if datepart(dd, @i_fecha) > 1 -- procesar con mes anterior
   begin                  
      select @w_mes_aux  = datepart(mm,@i_fecha),
   ...
   select @w_cod_tplazo = case when op_periodo_cap = 2 then 'B' + op_tplazo else op_tplazo end,
          @w_operacion  = op_operacion      
   from   cob_cartera..ca_operacion
   where  op_banco = @i_banco
   print '@w_cod_tplazo: '  + @w_cod_tplazo
   ...
   if @@error <> 0  begin
    ...
    select  
            ...
            'Dia_Pago'           = do_dia_pago, --porcaso194284
            'TasaAnual'		  = do_tasa,
...
begin
   ...
   begin
	  ...
	  --SRO. 147999 PAGO ADEUDO en OperaciÃ³n Renovada 
...
if @i_operacion = 'I1' --Informacion CrÃ©dito
begin
...
if @i_operacion = 'I2' --Informacion CrÃ©dito Revolvente
begin
   select 
