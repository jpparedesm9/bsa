/*************************************************************************/
/*   Archivo:              sp_cobis_centro.sp                            */
/*   Stored procedure:     sp_cobis_centro                               */
...
/*************************************************************************/
/*                                   PROPOSITO                           */
/*   Genera reporte COBIS_CENTRO.txt del requerimiento 172199            */
/*                                                                       */
...
IF OBJECT_ID ('dbo.sp_cobis_centro') IS NOT NULL
	DROP PROCEDURE dbo.sp_cobis_centro
GO

CREATE proc [dbo].[sp_cobis_centro](
   @i_param1  DATETIME
)
AS
...
IF OBJECT_ID('tempdb..##tmp_cobis_centro') IS NOT NULL DROP TABLE ##tmp_cobis_centro

SELECT
datos = cobis.dbo.fn_llena_vacio_derecha((cobis.dbo.fn_llena_0(a.of_oficina,4)+REPLICATE(' ',6)+a.of_nombre),50)+cobis.dbo.fn_llena_vacio_derecha(cobis.dbo.fn_llena_0(of_oficina_padre,4),10)
INTO ##tmp_cobis_centro
FROM cobis..cl_oficina a, cob_conta..cb_oficina b
WHERE a.of_oficina = b.of_oficina


select @w_reporte = 'COBIS_CENTRO'
select @w_sql = 'select * from ##tmp_cobis_centro'
