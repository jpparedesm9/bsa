...
/*                     MODIFICACIONES                       */
/*   FECHA        AUTOR            RAZON                    */
/* 28/JUL/2017    PRO     Emision inicial                   */
/* 18/OCT/2018    MTA     Agregar campo 46                  */
/* 11/MAR/2019    ACH     Se quita espacio en colonia       */
/* 05/ABR/2019    AOR     Se ajusta campo 46                */
/* 09/MAY/2019    DCU     Validacion Nombres                */
/* 13/MAY/2019    AEOC    Caso   # 110097                   */
/* 17/MAY/2019    DCU     Req. 110097                       */
/* 24/JUN/2019    DCU     Ajuste LCR                        */
/* 02/JUL/2019    DCU     Ajuste Caso 121057 LCR            */
/* 23/MAR/2020    MTA     Eliminar SN en direccion Cso136547*/
/* 15/ABR/2020    ACH     quitar espacio en PA -PE          */
/* 03/Jul/2020    DCU     Caso: 142143                      */
/* 09/Sep/2020    JSA     Optimizacion Batch                */ 
/* 27/Abr/2021    AGO     Req: 154017                       */ 
/* 02/Jul/2021    AGO     Req: 154017 Campos TELEFONO       */
/*                        validacion 999 días               */ 
/* 02/Sep/2021    DCU     Caso: 168128                      */
/* 26/Oct/2022    KVI     Error.191254                      */
/* 22/Dic/2022    ACH     ERR#199164 Fecha Ultimo Pago...   */
/************************************************************/
if exists(select 1 from sysobjects where name = 'sp_genera_buro')
   drop proc sp_genera_buro
go
create proc sp_genera_buro (
...
select @w_sp_name= 'sp_genera_buro'
...
select
    ...
    '11' = case when do_tipo_operacion not in ('REVOLVENTE') then  dbo.fn_formatea_texto(1,
           (case when do_periodicidad  <> 1 then 
                 (select eq_valor_arch from @w_FREC_PAGOS where eq_valor_cat = convert(varchar(2),do_periodicidad ) + do_tplazo )
		    else 
		         case (select eq_valor_arch from @w_FREC_PAGOS where eq_valor_cat = do_tplazo) when null then ''
                 else (select upper(eq_valor_arch) from @w_FREC_PAGOS where eq_valor_cat = do_tplazo)
                 end
            end
            ),null,'S','S','S') 
			else dbo.fn_formatea_texto(1,'Z',null,'S','S','S') end,
    -- monto a pagar
    '12' = CASE
              when do_tplazo= 'W' and do_tipo_operacion not in ('REVOLVENTE') 
		         THEN convert(bigint,round(cuota * 4,@w_redondeo)*@w_multiplicador)
              when do_tplazo = 'Q' and do_tipo_operacion not in ('REVOLVENTE')
                 THEN convert(bigint,round(cuota * 2,@w_redondeo)*@w_multiplicador)            
              when do_tplazo = 'M' and do_tipo_operacion not in ('REVOLVENTE')
    ...
    '16'  = case when do_estado_cartera = 3 and do_tipo_operacion <> 'REVOLVENTE' then 
	             ...
	             when do_estado_cartera = 3 and do_tipo_operacion = 'REVOLVENTE' and do_fecha_vencimiento < @w_fecha_rep and
				 ...
				 when do_estado_cartera = 3 and do_tipo_operacion = 'REVOLVENTE' and do_fecha_vencimiento < @w_fecha_rep and
                      do_fecha_vencimiento < do_fecha_proceso then
	        	      dbo.fn_formatea_texto(8,isnull(replace(convert(varchar(10),do_fecha_proceso,103),'/',''),'01011900'),null,'S','S','S')
	        	 else '' 
			end,
...
set -- numero de pagos    
    ...
    [16]  = case when do_estado_cartera = 3 and do_tipo_operacion <> 'REVOLVENTE' then 
	        	 ...
	        	 when do_estado_cartera = 3 and do_tipo_operacion = 'REVOLVENTE' and do_fecha_vencimiento < @w_fecha_rep and
                      do_fecha_vencimiento < do_fecha_proceso then
	        	      '16' + dbo.fn_formatea_texto(8,isnull(replace(convert(varchar(10),do_fecha_proceso,103),'/',''),'01011900'),null,'S','S','S')
	        	 else '' end,			
...
ON #sb_buro_empleo2 (be_ente)
  ...
  -- ------------------------------------------------------------------------------------
	...
	from #sb_reporte_buro_cuentas2 
	inner join #sb_buro_empleo2 on rb_ente = be_ente
