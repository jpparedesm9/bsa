/************************************************************************/
/*   Archivo:                 sp_xml_comp_pag.sp                        */
/*   Stored procedure:        sp_xml_comp_pag                           */
...
/*   20/03/2020 AGO    Modificacion folio                               */
/*   12/05/2020 AIN    Modificación creación xml                        */
/*   06/07/2021 DCU    Modifimacion validacion fecha                    */
/************************************************************************/

use cob_conta_super
go

if not object_id('sp_xml_comp_pag') is null
   drop proc sp_xml_comp_pag
go

create proc sp_xml_comp_pag
...
if  @w_ejecutar_comp <> 'S' return 0

--PARA REPROCESAR ENTRAR TRUNCADO Y PASAR A TABLA DE HISTORICOS

-- SE OBTIENE LA FECHA DE FIN DE MES ANTERIOR


SELECT @w_reporte = 'COMCTA'
if exists (select 1 from cob_conta..cb_solicitud_reportes_reg where sr_reporte = @w_reporte)
begin
   update cob_conta..cb_solicitud_reportes_reg set 
   sr_mes    = month(@i_param1),
   sr_anio   = year(@i_param1),
   sr_status = 'I'
...
begin
   ...
   values (getdate(), 'COMCTA', month(@i_param1), year(@i_param1), 'I')
end
...
begin
   ...
   select @w_fecha_fin_mes_anterior_his = max(rxh_fecha_fin_mes) 
   from cob_credito_his..cr_resultado_xml_his
   where rxh_fecha_fin_mes < @w_fecha_fin_mes_anterior
   ...
   rxh_fecha_fin_meshis_max = rxh_fecha_fin_mes,
   rxh_fecha_fin_mes_max    = @w_fecha_fin_mes_anterior,
   rxh_pago_compl_hist      = rxh_pago_compl,
   max_sec_id               = isnull(max(rxh_sec_id),1)
   into #maximo_secuenciales
   from cob_credito_his..cr_resultado_xml_his
   where rxh_fecha_fin_mes = @w_fecha_fin_mes_anterior_his
   group by num_operation,rxh_fecha_fin_mes,rxh_pago_compl
...
-- 
    ...
    from #operaciones_reportar,cob_credito_his..cr_resultado_xml_his
    where tr_banco         = num_operation
    and   rxh_total_saldo_act > 0
    and   (rxh_cuota_desde = dtr_dividendo or    rxh_cuota_hasta = dtr_dividendo) 
    and rxh_fecha_fin_mes < 	@w_fin_mes_ant_hab  
  ...
  /*Insert de los cr_resultado xml*/
   ...
   rxh_deuda_pagar,
   max(rxh_folio_ref),
   rxh_total_saldo = rxh_total_saldo_act,
   isnull(max(rxh_sec_id),1)
   from  cob_credito_his..cr_resultado_xml_his,#operaciones_hijas_finales
   where rxh_deuda_pagar>0
   and   rxh_fecha_fin_mes<@w_fecha_fin_mes_anterior
   and   rxh_met_fact='PPD'
   and   rxh_genera_cmpl='S'
   and   tr_banco=num_operation
   and   rxh_sec_id>0
   and   rxh_total_saldo_act > 0
   and   rxh_fecha_fin_mes < @w_fin_mes_ant_hab
   group by    num_operation,rxh_deuda_pagar,rxh_fecha_fin_mes,
               rxh_cuota_desde,rxh_cuota_hasta,
               rxh_total_saldo_act 
  ...
  and   a.folio_ref  = b.folio_ref
   ...
   update #operacione_complemento_final set
   compl_hijo = case 
                when isnull(rxh_total_saldo_act,0)-isnull(compl_hijo,0)<=0.03 then
                  compl_hijo + (isnull(rxh_total_saldo_act,0)-isnull(compl_hijo,0))
                else
                  compl_hijo
                end
   from cob_credito_his..cr_resultado_xml_his
   where   banco           = num_operation
   and     rxh_cuota_desde = cuota_ini_c
   and     rxh_cuota_hasta = cuota_hasta_c
   and rxh_fecha_fin_mes < 	@w_fin_mes_ant_hab
   ...
   select   
   banco              = banco,
   sec_id             = max(rxh_sec_id),
   rxh_cuota_desde    = rxh_cuota_desde,
   rxh_cuota_hasta    = rxh_cuota_hasta,
   rfc_ente           = rfc_ente,
   id_ente            = id_ente,
   tipo_operacion_his = rxh_tipo_operacion,
   rxh_folio_ref      = max(rxh_folio_ref),--se aumento
   rxh_fin_mes        = rxh_fecha_fin_mes,
   rxh_id_documento   = convert(varchar(60),null),
   rxh_estd_timbrado  = convert(char(1),null)
   into #operaciones_max_his
   from #operacione_complemento_final,cob_credito_his..cr_resultado_xml_his
   where banco         = num_operation
   and   cuota_ini_c   = rxh_cuota_desde
   and   cuota_hasta_c = rxh_cuota_hasta
   group by  banco,rxh_cuota_desde,rxh_cuota_hasta,rfc_ente,id_ente,rxh_tipo_operacion,rxh_fecha_fin_mes
   
   /*Encuentro el folio Fiscal de las historicas*/
   update #operaciones_max_his
   set rxh_id_documento  = in_folio_fiscal_hist,
       rxh_estd_timbrado = in_estd_timb_hist
   from #operaciones_max_his,cob_conta_super..sb_ns_estado_cuenta_hist
   where banco             = nec_banco_hist
   and   rxh_cuota_desde   = in_cuota_desde_hist
   and   rxh_cuota_hasta   = in_cuota_hasta_hist
   and   rxh_fin_mes       = in_fecha_fin_mes_hist 
   and in_folio_ref_hist   = rxh_folio_ref 
...
update  cob_credito_his..cr_resultado_xml_his set
   rxh_total_saldo_act = rxh_total_saldo_act - compl_hijo,
   rxh_pago_compl = rxh_pago_compl + compl_hijo 
   ...
   and   cuota_hasta_c = rxh_cuota_hasta
	...
	update cob_credito_his..cr_complemento_pago_xml_his
	set rxh_saldo_inso=case when isnull(deuda_pagar-compl_hijo,0)<=0 then 0
	                        else isnull(deuda_pagar-compl_hijo,0) end
	from #operacione_complemento_final,cob_credito_his..cr_complemento_pago_xml_his
	where banco          = num_operation_his
	and   folio_ref      = rxh_folio_ref
   ...
   select 
   num_operation_his_act  = num_operation_his,
   insert_date_his_act    = insert_date_his,
   rxh_sec_id_max_act     = isnull(max(rxh_sec_id),0)
   into #max_sec_actuales
   from cob_credito_his..cr_complemento_pago_xml_his
   where insert_date_his  < @i_param1
   group by num_operation_his,insert_date_his
   ...
   --Actualización de nombres para los xml de complementos
   update cob_conta_super..sb_est_complemento_xml set 
   ...
   --Insercción tabla de xml para complementos
   ...
   select 
    ...
    'RegimenFiscal="' + isnull(se_RegimenFiscal,'') +'"/>' +
	 ...
	 'Nombre="' + isnull((select TOP 1 en_nomlar from cobis..cl_ente where en_ente = se_id_ente),'') +'" ' +
    ...
    'UsoCFDI="' + isnull(se_UsoCFDI,'') +'"/>'+ 
     ...
     '<cfdi:Complemento>'+
          ...
          'Version="'+ isnull(se_Version,'') + '">' +
             '<pago20:Totales '+
             'MontoTotalPagos="'+ isnull(se_Monto,'') +'" ' +
             'TotalTrasladosBaseIVA16="'+ isnull(convert(varchar(50), CAST(convert(money,se_ImpPagado)/1.16 AS decimal(15,2)) ),'0')+'" ' +
             'TotalTrasladosImpuestoIVA16="' + isnull(convert(varchar(50), CAST((convert(money,se_ImpPagado)/1.16)*0.16 AS decimal(15,2)) ),'0')  +'"/>'+
             --Pago
             '<pago20:Pago '+
             ...
             'RfcEmisorCtaOrd="' + isnull(se_RfcEmisorCtaOrd,'') + '">' +
                  ...
                  '<pago20:ImpuestosDR>'+
                     '<pago20:TrasladosDR>'+
                        '<pago20:TrasladoDR '+
                           'BaseDR="' + isnull(convert(varchar(50), CAST(convert(money,se_ImpPagado)/1.16 AS decimal(15,2)) ),'0')  +'" ' +
                           'ImpuestoDR="' + isnull('002','')  +'" ' +
                           'TipoFactorDR="Tasa" ' +
                           'TasaOCuotaDR="' + isnull('0.160000','')  +'" ' +
                           'ImporteDR="' + isnull(convert(varchar(50), CAST((convert(money,se_ImpPagado)/1.16)*0.16 AS decimal(15,2)) ),'0')  +'"/>'+
                     '</pago20:TrasladosDR>'+
                  '</pago20:ImpuestosDR>'+
                  '</pago20:DoctoRelacionado>'+
                  '<pago20:ImpuestosP>'+
                     '<pago20:TrasladosP>'+
                        '<pago20:TrasladoP '+
                           'BaseP="' + isnull(convert(varchar(50), CAST(convert(money,se_ImpPagado)/1.16 AS decimal(15,2)) ),'0')  +'" ' +
                           'ImporteP="' + isnull(convert(varchar(50), CAST((convert(money,se_ImpPagado)/1.16)*0.16 AS decimal(15,2)) ),'0')  +'" '+
                           'ImpuestoP="' + isnull('002','')  +'" ' +
                           'TasaOCuotaP="' + isnull('0.160000','')  +'" ' +
                           'TipoFactorP="Tasa" ' +'/>'+
                     '</pago20:TrasladosP>'+
                  '</pago20:ImpuestosP>'+
             '</pago20:Pago>'+
