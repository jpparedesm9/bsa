...
/************************************************************************/
/*   Archivo:              riesgo_hrc.sp                                */
/*   Stored procedure:     sp_riesgo_hrc                                */
...
if exists (select 1 from sysobjects where name = 'sp_riesgo_hrc')
   drop proc sp_riesgo_hrc
go
create proc sp_riesgo_hrc
...
select @w_sp_name = 'sp_riesgo_hrc'
...
select @w_ffecha_data = 120, @w_ffecha_reporte = 111, @w_batch = 36427
...
@w_nombre_arch  = lower(isnull(ba_arch_resultado, 'cobis_hrc'))
from  cobis..ba_batch
...
cod_moneda              = 'MXN',
imp_concedido           = convert(varchar(20), op_monto),
...
SELECT do_numero_ciclos, do_numero_integrantes, do_fecha, do_banco, do_codigo_cliente, do_tramite, do_grupo
...
WHERE do_banco = tg_prestamo
AND   do_grupo = tg_grupo
AND   tg_participa_ciclo = 'S'
GROUP BY tg_grupo

UPDATE #operaciones_hrc 
SET   num_hijas_grupo = nro_integrantes
FROM  #integrantes
WHERE num_linea_madre = tg_grupo
