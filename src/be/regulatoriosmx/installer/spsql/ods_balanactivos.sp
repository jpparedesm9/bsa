/*************************************************************************/
/*   Archivo:              ods_balanactivos.sp                           */
/*   Stored procedure:     sp_ods_balance_activos                        */
...
if exists(select 1 from sysobjects where name = 'sp_ods_balance_activos')
    drop proc sp_ods_balance_activos
go

create proc sp_ods_balance_activos
(
...
select @w_sp_name = 'sp_ods_balance_activos'

select @w_formato_fecha = 112, @w_batch = 7522
...
and   do_aplicativo = 7 
and   do_estado_cartera in (@w_est_vigente, @w_est_vencido)
...
truncate table sb_ods_balanactivos

insert into sb_ods_balanactivos 
(
 ob_num_cuenta,
 ob_cod_cta_cont,
 ob_cod_divisa,
...
select @w_query   = 'select ob_registro_archivo from cob_conta_super..sb_ods_balanactivos order by ob_num_cuenta, ob_cod_cta_cont'

select @w_destino = @w_path + @w_nom_arch, -- COB_ODS_02<aaaa_mm_dd>.txt
       @w_errores = @w_path + @w_nom_log   -- COB_ODS_02<aaaa_mm_dd>.err
