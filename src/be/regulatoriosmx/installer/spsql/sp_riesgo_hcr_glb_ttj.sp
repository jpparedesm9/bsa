...
/* 17/08/2020      ACHP          Caso #143664 - cambio de nombre        */
/* 12/11/2021      DCU           Caso #172460                           */
/* 29/11/2021      ACH           Caso #173628 Catalogos HRC simple repor*/
/* 10/01/2022      DCU           Caso #170130                           */
/* 12/01/2022      KVI           #173512-Archivo tome ultimo dia natural*/
...
if exists (select 1 from sysobjects where name = 'sp_riesgo_hrc_glb_ttj')
   drop proc sp_riesgo_hrc_glb_ttj
go
create proc sp_riesgo_hrc_glb_ttj
...
declare 
		...
		@w_sub_prod_rep         int,
		@w_est_etapa2           int,
        @w_fecha_fin_mes        datetime, --Caso #173512
...
select @w_es_fin_mes = 'N', @w_formato_fecha = 112
...
select @w_ffecha_data = 120, @w_ffecha_reporte = 111, @w_batch = 36435
...
@w_nombre_arch  = isnull(ba_arch_resultado, 'ARC_NODS_HRC')
from  cobis..ba_batch
...
flg_mora_sistema         = convert(char(1), '1'),
fec_prox_corte           = convert(varchar(10), '0001-01-01'), 
cod_pais_origen          = convert(varchar(10), '484'),
cod_pais_residencia      = convert(varchar(10), '484'),
cod_tipo_persona         = convert(char(1), NULL),
cod_sector_economico     = convert(varchar(10), NULL), 
cod_unidad_negocio       = convert(varchar(1), NULL),
cod_unidad_negocio_ope_ori = convert(varchar(1), NULL),
cod_sector_contable        = convert(varchar(1), NULL),
...
imp_seguro_vida          = convert(varchar(20),'0'),
...
imp_pago_domiciliado     = convert(varchar(20), '0'),
...
tperiodo_cap = convert(char(2), null), 
periodo_int  = (periodo_int * td_factor), 
tperiodo_int = convert(char(2), null),
...
flg_mora_sistema         = case WHEN dias_vencido <= 30 then '1'
                                WHEN dias_vencido >= 31 AND dias_vencido <= 89 then '2'
		                        WHEN dias_vencido >= 90 then '3'
							end
from  #div_vencidos
...
truncate table cob_conta_super..sb_riesgo_hrc_ttj
insert into cob_conta_super..sb_riesgo_hrc_ttj
(
rh_fec_info                     ,
rh_num_cred                     ,
...
/******************************************************************/
update sb_riesgo_hrc_ttj set 
rh_desc_nombre_cliente   = replace(replace(replace(replace(replace(replace(replace(rh_desc_nombre_cliente,'Ó','O'),'Í','I'),'É','E'),'Á','A'),'Ñ','N'),'Ü','U'),'Ú','U'),
rh_desc_entidad          = replace(replace(replace(replace(replace(replace(replace(rh_desc_entidad,'Ó','O'),'Í','I'),'É','E'),'Á','A'),'Ñ','N'),'Ü','U'),'Ú','U'),
rh_desc_producto         = replace(replace(replace(replace(replace(replace(replace(rh_desc_producto,'Ó','O'),'Í','I'),'É','E'),'Á','A'),'Ñ','N'),'Ü','U'),'Ú','U'),
rh_desc_subproducto      = replace(replace(replace(replace(replace(replace(replace(rh_desc_subproducto,'Ó','O'),'Í','I'),'É','E'),'Á','A'),'Ñ','N'),'Ü','U'),'Ú','U'),            
rh_desc_sector_economico = replace(replace(replace(replace(replace(replace(replace(rh_desc_sector_economico,'Ó','O'),'Í','I'),'É','E'),'Á','A'),'Ñ','N'),'Ü','U'),'Ú','U'),            
rh_desc_actividad_economica_esp = replace(replace(replace(replace(replace(replace(replace(rh_desc_actividad_economica_esp,'Ó','O'),'Í','I'),'É','E'),'Á','A'),'Ñ','N'),'Ü','U'),'Ú','U') 
...
select @w_sql = @w_sql + ' from cob_conta_super..sb_riesgo_hrc_ttj '

	   
-------------------------------------------Caso #143664 Ini
print 'Generando Cabeceras'
print 'borrar-w_fecha_ini: ' + convert(varchar(30),@w_fecha_corte)
...
begin
    ...
    select @w_report_nombre = @w_nombre_arch + '_M_' + replace(convert(varchar(10), @w_fecha_fin_mes, @w_ffecha_reporte), '/', '')
    select @w_destino = @w_ruta_arch + @w_report_nombre + '.txt',
           @w_errores = @w_ruta_arch + @w_report_nombre + '.err'
...
select @w_report_nombre = @w_nombre_arch + '_D_' + replace(convert(varchar(10), @w_fecha_corte, @w_ffecha_reporte), '/', '')
select @w_destino = @w_ruta_arch + @w_report_nombre + '.txt',
       @w_errores = @w_ruta_arch + @w_report_nombre + '.err'
-------------------------------------------Caso #143664 Fin
select @w_comando = 'bcp "' + @w_sql + '" queryout "'
