...
@w_clvp + case  (select DISTINCT dc_pais from sb_dato_cliente where dc_fecha = @i_fecha)  when null then @w_clvp else convert(varchar(1), len(dc_pais)) + dc_pais end,
@w_creq,
@w_creq,
@w_creq,
@w_creq	
...
select @w_comando = 'echo. ' +  '2> '  +  @w_destino
exec @w_error = xp_cmdshell @w_comando
if @w_error <> 0 begin
...
select
   @w_comando = 'del ' +
   @w_path + @w_nombre + '.txt'     + ' ' + @w_path + @w_nombre_det + '.txt'
exec @w_error = xp_cmdshell @w_comando
if @w_error <> 0 begin
...
select
   @w_comando = 'del ' +
   @w_path + @w_nombre + '.err'     + ' ' + @w_path + @w_nombre_det + '.err'
exec @w_error = xp_cmdshell @w_comando
if @w_error <> 0 begin
   select @w_msg = 'EJECUCION comando bcp FALLIDA (Eliminacion de Archivos intermedios .txt). REVISAR ARCHIVOS DE LOG GENERADOS. '
   goto ERRORFIN
