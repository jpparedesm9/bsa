/************************************************************************/
/*   Archivo:                 sp_xml_estado_cuenta_rep.sp               */
/*   Stored procedure:        sp_xml_estado_cuenta_rep                  */
...
/*   03/03/2021    DCU          Modificación creación xml               */
...
if not object_id('sp_xml_estado_cuenta_rep') is null
   drop proc sp_xml_estado_cuenta_rep
go

create proc sp_xml_estado_cuenta_rep(
...
begin
   ...
   /*insert into cob_credito_his..cr_resultado_xml_his (
      ...
      rxh_pago_compl_fin , rxh_genera_cmpl)                      
    select                                                                                    
      linea              , file_name          ,     id_ente           ,      status         , 
      rfc_ente           , num_operation      ,     insert_date       ,      processing_date, 
   ...
   /* Tabla para Obtener los operaciones Revolventes a reportar en el mes */
    ...
    select @w_ultima_ejecion = max(insert_date)
    from cob_credito..cr_resultado_xml
    where rx_fecha_fin_mes = @w_fin_mes_ant
    
    
    select 
    num_operation = num_operation, 
    rxh_int_ant   = isnull(rx_int_ant,0),
    max_sec_id    = max(rx_sec_id),
    rx_iva_int_ant= isnull(rx_iva_int_ant,0)  
    into #interes_enterior
    from cob_credito..cr_resultado_xml
    where rx_fecha_fin_mes = @w_fin_mes_ant
    group by num_operation,rx_int_ant, rx_iva_int_ant
   ...
   */
	...
	if exists(select 1 from sb_operacion_tmp)
	  ...
	  begin
		...
		WHERE dr_fecha = @w_fin_mes_hab
		AND dr_aplicativo = 7
		and dr_toperacion  in ('GRUPAL','REVOLVENTE')
		and dr_concepto  <> 'CAP'
		GROUP BY dr_banco, dr_concepto

		/*Inserto las operaciones Individuales*/
		/*INSERT INTO #amortizacion_aux
			SELECT 'D' ,dr_banco, dr_concepto, 0, 0, 0
		FROM cob_conta_super..sb_dato_operacion_rubro
		WHERE dr_fecha = @w_fin_mes_hab
		AND dr_aplicativo = 7
		and dr_concepto  <> 'CAP'
		and dr_banco in (select op_banco_rev from #operaciones_rev)
		GROUP BY dr_banco, dr_concepto*/
	  end
    ...
    AND tr_tran='PAG'
	  ...
	  and   do_estado_cartera not in (2, 4)
  ...
  dd_concepto, 
   ...
   select @w_mes_proceso = case 
                           when month(@i_param1) = 1 then 'ENERO'
                           when month(@i_param1) = 2 then 'FEBRERO'
                           when month(@i_param1) = 3 then 'MARZO'
                           when month(@i_param1) = 4 then 'ABRIL'
                           when month(@i_param1) = 5 then 'MAYO'
                           when month(@i_param1) = 6 then 'JUNIO'
                           when month(@i_param1) = 7 then 'JULIO'
                           when month(@i_param1) = 8 then 'AGOSTO'
                           when month(@i_param1) = 9 then 'SEPTIEMBRE'
                           when month(@i_param1) = 10 then 'OCTUBRE'
                           when month(@i_param1) = 11 then 'NOVIEMBRE'
                           else 'DICIEMBRE'
                           end  
                           
   
   update sb_est_rubro_ope set
   ero_concepto = isnull(ero_concepto,'') + ' '  + convert(varchar(4),year(@i_param1)) + ' - ' + @w_mes_proceso
