/*************************************************************************/
/*   Archivo:              ods_movresultados.sp                          */
/*   Stored procedure:     sp_ods_mov_resultados                         */
...
if exists(select 1 from sysobjects where name = 'sp_ods_mov_resultados')
    drop proc sp_ods_mov_resultados
go

create proc sp_ods_mov_resultados
(
...
select @w_sp_name = 'sp_ods_mov_resultados'

select @w_formato_fecha = 112, @w_batch = 7523
...
--select @w_anio = 'D'+right(convert(varchar(4), datepart(yyyy, @w_fecha_corte)),2)
...
into #resultados 
from cob_conta_super..sb_ods_movresultados
where 1=2
...
truncate table sb_ods_movresultados

insert into sb_ods_movresultados
(
 om_num_cuenta,
 om_cod_cta_cont,
 om_cod_divisa,
...
select @w_query   = 'select om_registro_archivo from cob_conta_super..sb_ods_movresultados order by om_num_cuenta, om_cod_cta_cont'

select @w_destino = @w_path + @w_nom_arch, -- COB_ODS_03<aaaa_mm_dd>.txt
       @w_errores = @w_path + @w_nom_log   -- COB_ODS_03<aaaa_mm_dd>.err
