...
if exists (select 1 from sysobjects where name = 'sp_boc_opt')
   drop proc sp_boc_opt
go
---iNC.114979 MAR.06.2014
create procedure sp_boc_opt
(
...
while @w_cont < 3 begin  -- cambiar a contador < 3 cuando este listo pasivas y administradas
   ...
   exec @w_error  = cob_conta..sp_boc_ini_opt
   @i_empresa     = 1,
   @i_fecha       = @i_fecha,
   @i_perfil      = @w_perfil,
...
/* ENTRAR BORRANDO */
delete cob_conta..cb_boc_opt with (rowlock)
...
delete cob_conta..cb_boc_det_opt with (rowlock)
...
insert into cob_conta..cb_boc_opt
select  
bo_fecha       = @i_fecha,
bo_cuenta      = bo_cuenta,
...
insert into cob_conta..cb_boc_det_opt
select
...
delete cob_conta..cb_boc_opt where bo_producto = 19
...
select @w_destino  = @w_path + 'boc_opt' + replace(convert(varchar, @i_fecha, 102), '.', '') + '_' + replace(convert(varchar, getdate(), 108), ':', '') + '.txt',
      @w_destino2 = @w_path + 'lineas_boc_opt'      + replace(convert(varchar, @i_fecha, 102), '.', '') + '_' + replace(convert(varchar, getdate(), 108), ':', '') + '.txt',
       @w_errores  = @w_path + 'boc_opt' + replace(convert(varchar, @i_fecha, 102), '.', '') + '_' + replace(convert(varchar, getdate(), 108), ':', '') + '.err'

select @w_comando = @w_cmd + @w_path + 'boc_opt -b5000 -c -T -e ' + @w_errores + ' -t"\t" ' + '-config ' + @w_s_app + 's_app.ini'
...
select @w_comando = 'copy ' + @w_destino2 + ' + ' + @w_path + 'boc_opt ' + @w_destino
