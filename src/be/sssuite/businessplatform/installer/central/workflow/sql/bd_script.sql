...
while 1 = 1 begin
    ...
    if not exists(select 1 from sysdatabases where name = @w_nombre) begin
        print 'creando  Dispositivos para %1!' + cast(@w_nombre as varchar)
   
        /*Creando Dispositivos*/

        select @w_vdevno = @w_numlibres + max(isnull(convert(tinyint, substring(convert(binary(4), d.low), v.low, 1)),0))
...
--        cntrltype = 0
        ...
        print 'creando Base de Datos %1!' + cast(@w_nombre as varchar)
        ...
        print 'Base de Datos %1! ya Existe...' + cast(@w_nombre as varchar)
