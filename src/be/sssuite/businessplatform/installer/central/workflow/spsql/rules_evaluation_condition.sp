...
CREATE PROCEDURE [dbo].[sp_rules_evaluation_condition]
(
...
begin
	...
	begin
     ...
     if (@i_operator = 'es nulo/vacio' and (@i_real_value is null or @i_real_value = ''))
            begin
                 select @o_return_value = 1
                return 0
            end

            if (@i_operator = 'cualquier valor')
            begin
                 select @o_return_value = 1
                 return 0
            end
    ...
    begin
		...
		begin
			...
			select @w_valmin = convert(numeric(20,8),@i_min_value)
			select @w_valmax = convert(numeric(20,8),@i_max_value)
			select @w_valreal = convert(numeric(20,8),@i_real_value)
