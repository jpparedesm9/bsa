use cob_workflow
go
if exists (select 1 from sysobjects where name = 'sp_bp_products')
begin
  drop procedure sp_bp_products
end
go

CREATE PROCEDURE sp_bp_products
/*******************************************************************/
...
exec @w_return       = cob_credito..sp_riesgos_asb
	...
	set etapa_act = (select ia_nombre_act 
							...
							where a.ia_id_inst_proc = DEU.id_inst_act --
