...
if exists (SELECT 1
           ...
           WHERE CONSTRAINT_NAME = 'page_view_fk2')
   alter table bpl_an_page_view
      drop constraint page_view_fk2
GO

if exists (SELECT 1
           FROM  INFORMATION_SCHEMA.TABLE_CONSTRAINTS
           WHERE CONSTRAINT_NAME = 'bpl_view_fk1')
   alter table bpl_view
      drop constraint bpl_view_fk1
GO
...
print '@pa_id %1!' + convert(VARCHAR,@pa_id)
print '@la_id %1!' + convert(VARCHAR,@la_id) 

DELETE FROM bpl_view
DELETE FROM bpl_an_page_view
