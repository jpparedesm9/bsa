/**************************************************************************/
/*   ARCHIVO:         sp_notif_aprobado.sp                                */
/*   NOMBRE LOGICO:   sp_notif_aprobado                                   */
...
/*                            NOTIFICACION AUTOMATICA - SOLICITUD DE CREDITO*/
/**************************************************************************/
use cob_workflow
go
if exists (select 1 from sysobjects where name = 'sp_notif_aprobado')
   drop proc sp_notif_aprobado
go
create proc sp_notif_aprobado
        (@s_ssn        int         = null,
         @s_ofi        smallint,
         @s_user       login,
         @s_date       datetime,
...
begin
    select @w_email_body = '' + @w_nombre + '.<br><br>' +
                           ...
                           'correspondiente a la Operaci&#243;n No. ' + @w_banco + ' fue APROBADA.<br><br><br>'
