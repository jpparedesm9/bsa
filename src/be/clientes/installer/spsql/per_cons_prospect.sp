/************************************************************************/
/*  Archivo:            pers_con_prospect.sp                            */
/*  Stored procedure:   sp_persona_cons_prospect                        */
...
/*  23/Abr/07   Clotilde Vargas      Agregar categoria del cliente      */
/*  12/09/2012  AVI                  Vista consolidada                  */
/*  29/Nov/2012 F.LOPEZ              Se agrega Excepcion PAD            */
/*  14/Mar/2013 F.LOPEZ              Se agrega Op Cons. Otros Productos */
/*  22/05/2013  D.VERA               Se corrige query categoria cliente */
/*  19/05/2015  Ma. Jose Taco        Se agrega nueva operacion para     */
/*                                   modulo de cuentas                  */
/*  16/Jul/201 B.Sanson              Ajustes impresion FIE              */
/*  16/Jun/2015 Viviana Arias L.     En opcion Y se retorna lo contrario*/
/*                                   de en_retencion para impuestos.    */
/************************************************************************/
use cob_pac
go

if exists (select * from sysobjects where name = 'sp_persona_cons_prospect')
   drop proc sp_persona_cons_prospect
go
CREATE PROCEDURE sp_persona_cons_prospect (
   @s_ssn                  int             = null,
   @s_user                 login           = null,
...
-------------------------------------------
select @w_sp_name = 'sp_persona_cons_prospect'

if @t_show_version = 1
begin
    print 'Stored procedure sp_persona_cons, Version 4.0.0.19 ' + 'sp_persona_cons_prospect'
    return 0
end
   ...
   begin
      if @t_trn = 132
         begin
            ...
            select  @w_p_apellido                 = a.p_p_apellido,
               ...
               @w_es_cliente                 = (select case when (en_ente in (select cl_cliente from cobis..cl_cliente)) then 'S' else 'N' end from cobis..cl_ente where en_ente = @i_persona and en_subtipo = 'P'),
            ...
            from  cobis..cl_ente as a left join cobis..cl_ente_aux as b
            on a.en_ente  = b.ea_ente
            ...
            if @w_cod_sex is not NULL
               begin
                  select @w_sexo = a.valor
                  from cobis..cl_catalogo a,
                       cobis..cl_tabla b
                  where a.codigo = @w_cod_sex
            ...
            if @w_categoria is not null
               begin
                  select @w_desc_categoria = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo     = @w_categoria
                  and a.tabla        = b.codigo
            ...
            if @w_cod_sector is not NULL
               begin
                  ...
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo     = @w_cod_sector
                  and a.tabla        = b.codigo
                  and b.tabla           = 'cl_sectoreco'
            ...
            if @w_situacion_cliente is not NULL
               begin
                  select  @w_des_situacion_cliente = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo     = @w_situacion_cliente
                  and a.tabla        = b.codigo
                  and b.tabla           = 'cl_situacion_cliente'
            ...
            if (@w_estado_civil is not NULL and @w_estado_civil <> '')
               begin
                  select @w_des_estado_civil = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo     = @w_estado_civil
                  and a.tabla        = b.codigo
            ...
            if @w_nivel_estudio is not NULL
               begin
                  select @w_des_niv_est = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo       = @w_nivel_estudio
                  and a.tabla        = b.codigo
            ...
            if @w_tipo_vinculacion is not NULL
               begin
                  select @w_des_tipo_vinculacion = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo     = @w_tipo_vinculacion
                  and a.tabla        = b.codigo
                  and b.tabla           = 'cl_relacion_banco'
            ...
            if @w_tipo is not NULL
               begin
                  select @w_des_tipo = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo = @w_tipo
                  and a.tabla    = b.codigo
                  and b.tabla       = 'cl_ptipo'
            ...
            if @w_profesion is not NULL
               begin
                  select @w_des_profesion = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo     = @w_profesion
                  and a.tabla        = b.codigo
                  and b.tabla           = 'cl_profesion'
            ...
            if (@w_tipo_vivienda is not NULL and @w_tipo_vivienda <> '')
               begin
                  select @w_des_tipo_vivienda = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo     = @w_tipo_vivienda
                  and a.tabla        = b.codigo
                  and b.tabla           = 'cl_tipo_vivienda'
            ...
            if @w_promotor is not NULL
               begin
                  select @w_des_promotor = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo     = @w_promotor
                  and a.tabla        = b.codigo
                  and b.tabla           = 'cl_promotor'
            ...
            if @w_ingre is not NULL
               begin
                  select @w_des_ingresos = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo     = @w_ingre
                  and a.tabla        = b.codigo
                  and b.tabla           = 'cl_ingresos'
            ...
            if @w_depart_doc is not NULL
               begin
                  select @w_des_dep_doc  = a.valor
                  from cobis..cl_catalogo a, cobis..cl_tabla b
                  where a.codigo       = convert(char(4),@w_depart_doc)
                  and a.tabla        = b.codigo
                  and b.tabla           = 'cl_provincia'
            ...
            if @w_tipo_discapacidad is not NULL
               begin
                  select @w_desc_discapacidad = a.valor
                  from    cobis..cl_catalogo a, cobis..cl_tabla b
                  where   a.codigo   = @w_tipo_discapacidad
                  and     a.tabla    = b.codigo
                  and     b.tabla       = 'cl_discapacidad'
            ...
            --DESCRIPCION DE NIVEL DE EGRESOS
             if @w_egresos is not NULL
               begin
                  select @w_desc_egresos = a.valor
                  from    cobis..cl_catalogo a, 
                          cobis..cl_tabla b
                  where   a.codigo   = @w_egresos
                  and     a.tabla    = b.codigo
                  and     b.tabla       = 'cl_nivel_egresos'
            ...
            if @w_calif_cli is not NULL
               begin
                  select @w_des_calif_cliente = a.valor
                  from    cobis..cl_catalogo a, cobis..cl_tabla b
                  where   a.codigo   = @w_calif_cli
                  and     a.tabla    = b.codigo
                  and     b.tabla       = 'ca_calif_cliente'
            ...
            if @o_ea_cat_aml is not NULL
               begin
                  select @w_ea_desc_aml = a.valor
                  from    cobis..cl_catalogo a, cobis..cl_tabla b
                  where   a.codigo   = @o_ea_cat_aml
                  and     a.tabla    = b.codigo
                  and     b.tabla       = 'cl_categoria_AML'
               ...
               if @o_ea_estado is not NULL
                begin
                   select @w_ea_estado_desc = a.valor
                   from    cobis..cl_catalogo a,cobis..cl_tabla b
                   where   a.codigo   = @o_ea_estado
                   and     a.tabla    = b.codigo
                   and     b.tabla       = 'cl_estados_ente'
                ...
                select @w_conyuge = en_nombre+' '+p_s_nombre+' '+p_p_apellido+' '+p_s_apellido
                  from cobis..cl_ente 
               ...
               begin
                  if  @i_modo = 0
                     ...
                     begin
                        ...
                        select @w_mayoria_edad = isnull(pa_tinyint,18) from cobis..cl_parametro where pa_nemonico='VGAGE' and pa_producto = 'CLI'
            ...
            from cobis..cl_direccion where di_ente = @i_persona
