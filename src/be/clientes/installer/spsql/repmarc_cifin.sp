/************************************************************************/
/*   Archivo:             repmarc_cifin.sp                              */
/*   Stored procedure:    sp_marcacion_cifin                            */
/*   Base de datos:       cobis                                         */
/*   Producto:            cartera                                       */
...
/*                          MODIFICACIONES                              */
/*  FECHA     AUTOR             RAZON                                   */
/* 15-11-11    L.Moreno    Emisión Inicial - Req: 315                   */
/* 21-09-12    L.Moreno    Inc 81538: Mostrar ultimo dia del mes y      */
/*                         las oficinas para los reg. no procesados     */
...
if exists (select
             *
           from   sysobjects
           where  name = 'sp_marcacion_cifin')
           drop proc sp_marcacion_cifin
go

create procedure sp_marcacion_cifin
(
  @i_param1 varchar(255),-- Fecha Inicial
  @i_param2 varchar(255),-- Fecha Final
...
as
  ...
  /* ACTUALIZA INFORMACION DE CAUSAL Y NOMBRE DEL CLIENTE EN CASO QUE NO TENGA NOMBRE */
  update rep_mov_gmf_tmp
  ...
  /* ACTUALIZA OFICINA DEL USUARIO */
  update rep_mov_gmf_tmp
  set    rm_zona = z.of_nombre,
         rm_cod_ofi = fu_oficina,
         rm_nom_ofi = o.of_nombre
  from   cobis..cl_funcionario with (nolock),
         cobis..cl_oficina o with (nolock),
         cobis..cl_oficina z with (nolock)
  where  (fu_login     = rm_usu_marc
           or fu_login     = rm_usu_desmarc)
     and o.of_oficina = fu_oficina
  ...
  begin
    select
      @w_error = 808022,
      @w_msg = 'ERROR AL ACTUALIZAR OFICINA DE LOS USUARIOS.'
    goto ERROR_INF
  end
