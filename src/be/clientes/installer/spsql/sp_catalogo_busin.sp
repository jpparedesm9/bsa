...
if exists (select 1 from sysobjects where name = 'sp_catalogo_busin')
   drop proc sp_catalogo_busin
go

CREATE PROCEDURE dbo.sp_catalogo_busin (
	@s_ssn                  int = NULL,
	@s_user                 login = NULL,
	@s_sesn                 int = NULL,
...
select  @w_sp_name = 'sp_catalogo_busin'
...
begin
 ...
 descripcion */
  ...
  begin tran
   ...
   insert into cobis..ts_catalogo (secuencia, tipo_transaccion, clase, fecha,
...
/********************************** Para Unix *******************************/
  ...
  exec @w_cmdtransrv
	...
	select nl_nombre,'N'
       ...
       from cobis..ad_nodo_oficina, cobis..ad_server_logico
...
begin
		...
		if @i_tabla = 'cl_moneda'
			begin
				...
				from    cobis..cl_catalogo c,
						cobis..cl_moneda m
				where   tabla = @w_cod_tabla
				and		c.codigo = convert(CHAR(1), m.mo_moneda)
				-- and   estado = 'V' --CLL INC#97
				order   by c.codigo
			end		
		else
		begin
	...
	select count(*) from cobis..cl_catalogo where tabla = @w_cod_tabla
     end
