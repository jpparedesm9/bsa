use cobis
go
print 'Inicio script 02_REM_cs212473_ING_NUEVOS_MUNICIPIOS:'+convert(varchar(30),getdate(),103)+'--'+convert(varchar(30),getdate(),108)
--------------------
...
select distinct ltrim(rtrim(cp_estado)), ltrim(rtrim(cp_municipio)), ltrim(rtrim(cp_nombre_municipio))
...
order by ltrim(rtrim(cp_estado)), ltrim(rtrim(cp_municipio))

select @w_secuencial = 0

select '#list_estado_municipio_para_validar' = count(*) from #codigos_modificados
--select 'municipios_nuevos'='municipios_nuevos',* from #codigos_modificados -- codigos postales nuevos
select 'list_estado_municipio_para_validar' = 'list_estado_municipio_para_validar', * from #codigos_modificados
...
begin
    ...
    select @w_estado = ltrim(rtrim(@w_estado))		
    select @w_municipio = ltrim(rtrim(@w_municipio))
    select @w_nombre_municipio = ltrim(rtrim(@w_nombre_municipio))
...
print 'Fin script 02_REM_cs212473_ING_NUEVOS_MUNICIPIOS:'+convert(varchar(30),getdate(),103)+'--'+convert(varchar(30),getdate(),108)
-- 4s
go
