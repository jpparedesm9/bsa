               ...
               begin
                  if  @i_modo = 0
                     ...
                     begin
                        ...
                        SELECT @w_fecha_mod_cli=en_fecha_mod FROM cobis..cl_ente WHERE en_ente=@i_persona
					   ...
					   SELECT @w_cliente=c.en_ente, 
						...
						FROM cl_ente c, cc_oficial co
						WHERE  c.en_oficial=co.oc_oficial AND en_ente=@i_persona
						...
						SELECT @w_mail_oficial=isnull(co.oc_mail, '1') FROM cl_ente c, cc_oficial co
						WHERE  c.en_oficial=co.oc_oficial AND en_ente=@i_persona
						...
						BEGIN
						    ...
						    select @w_email_body = 'Para :'+' '+@w_nombre_cli +' '+ @w_apellido + char(13) + char(13) +
						                           'Se debe actualizar la información de Datos del Cliente.' + char(13) + char(13) + char(13) +
						                           'Saludos Cordiales'
						    ...
						    exec cobis..sp_despacho_ins @s_ssn = @s_ssn, @s_user = @s_user, @s_sesn = @s_sesn, @s_term = @s_term, @s_date = @s_date, @s_srv = @s_srv, @s_ofi = @s_ofi,
						                                ...
						                                @i_cliente = @w_cliente, @i_to = @w_mail_oficial, @i_body = @w_email_body, @i_subject = 'NOTIFICACION AUTOMATICA - INFORMACION DEL CLIENTE DESACTUALIZADA'
						     
						   PRINT ('mail')+CONVERT(VARCHAR(200),@w_mail_oficial)                             
