    ...
    begin
     	set rowcount 20
				...
				select  'Etiqueta'= ef_etiqueta,
                        ...
                        'Orden' = fn_orden
                   from aw_funcionalidad                                                --inicio mig syb-sql 18042016
                        inner join aw_func_rol on fr_func = fn_func
                        inner join aw_etiqueta_funcionalidad on ef_func = fn_func
                        inner join cl_tabla T ON T.tabla   = 'aw_idioma'
                        inner join cl_catalogo C on C.tabla = T.codigo
                        			 	  AND ef_idioma = C.codigo
                        left outer join aw_ayuda_funcionalidad on fn_func = af_func
                                                              and C.codigo = af_idioma                                            
                  where fr_rol    = @i_rol
					and ef_idioma = @i_idioma
					and (fn_padre = @i_padre or ((rtrim(fn_padre) is null or rtrim(fn_padre) = '') and (@i_padre is NULL)))
				  order by ef_etiqueta                                                      --fin mig syb-sql 18042016
				...
				where ef_func=fn_func
                    and fn_func*=af_func  -- outer join 
                    and fr_rol=@i_rol
