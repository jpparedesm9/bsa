...
/*      08/Abr/2013     D. Vera         Eliminacion logica REENTRY      */
/*      18-04-2016      BBO             Migracion Sybase-Sqlserver FAL  */
/*      22-04-2016      ELO             Migracion Sybase-Sqlserver FAL  */
/*      28-06-2016      J.Hdez          Ajuste Vesion Falabella Funci   */
/************************************************************************/
...
declare
    ...
    --CODIGO DE INTERCEPTOR 09052016
...
if @t_show_version = 1 
begin
      print 'Stored procedure sp_usuario_rol, version 4.0.0.2'
      return 0
END
...
/* chequeo de que la fecha de caducidad sea posterior a la fecha actual*/
if (@i_fec_cad_rol<>NULL)
begin
   if ((
	   datepart(yy,@i_fec_cad_rol) < datepart(yy,getdate())
       ) or

       (
	   datepart(yy,@i_fec_cad_rol) = datepart(yy,getdate()) 
	   and 
	   datepart(mm,@i_fec_cad_rol) < datepart(mm,getdate())
       ) or
      ...
      (
	   ...
	   datepart(mm,@i_fec_cad_rol) = datepart(mm,getdate())
	   and 
	   datepart(dd,@i_fec_cad_rol) < datepart(dd,getdate())
       )
       ) 
begin
...
/* FIN chequeo de que la fecha de caducidad sea posterior a la fecha actual*/
  ...
  --Codigo de interceptor 09052016
	...
	if (@i_fecha_ini_rol is not NULL)
	begin
	   if ((
   ...
   /* Actualizamos el registro ingresado por el sp original */
    ...
    commit tran
    --fin Codigo de interceptor 09052016
...
/* FIN chequeo de que la fecha de caducidad sea posterior a la fecha actual*/
  ...
  /*   fin de verificacion     ****************************/ 
     ...
     if exists( select 1 from ts_usuario_rol where  secuencia= @s_ssn and tipo_transaccion = 571 and clase = 'P' and fecha = @s_date
     ...
     if exists( select 1 from ts_usuario_rol where  tipo_transaccion = 570 and clase = 'N' and fecha = @s_date
          and oficina_s = @s_ofi and usuario = @s_user and terminal_s = @s_term and srv = @s_srv and lsrv = @s_lsrv
           and login = @i_login and  rol = @i_rol and  fecha_aut = @v_fecha_aut)	
