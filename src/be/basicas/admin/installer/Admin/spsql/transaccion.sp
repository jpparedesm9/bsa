...
/*					tabla cl_trn_atrib              */
/*	25/Jul/2012	Pedro Montenegro Para la actualizaci[on de una  */
/*					transacci[on verificar la 	*/
/*					existencia en tabla cl_trn_atrib*/
...
select @w_sp_name = 'sp_transaccion'
	...
	/***Fin Variables para valores por defecto ***/
...
--ADU: 2001-06-12
   select @w_descripcion = tn_descripcion,
    ...
    from cl_trn_atrib
         left outer join cl_ttransaccion on tn_trn_code = ta_transaccion
   where tn_trn_code = @i_transaccion         
    --fin migra syb-sql  
    /*******MIGRACION SYB-SQL
...
--FIN ADU
  ...
  begin tran
   ...
   begin
	...
	--PRMR Inc-838
	select @w_contador = isnull(count(*), 0) 
		from cl_trn_atrib
	...
	begin
		insert into cl_trn_atrib (ta_transaccion, ta_tipo_ejec, ta_graba_log, ta_off_line, ta_tipo)
				  values (@i_transaccion, @i_tipo_ejec, @i_graba_log, @i_off_line, @i_ta_tipo)
...
begin
 /* chequeo de claves foraneas */
  ...
  select  @w_descripcion = tn_descripcion,
    ...
    --inicio migra syb-sql  
    from cl_ttransaccion
         left outer join cl_trn_atrib on ta_transaccion = tn_trn_code
   where tn_trn_code = @i_transaccion         
    --fin migra syb-sql  
    /*******MIGRACION SYB-SQL
    from cl_ttransaccion, cl_trn_atrib
