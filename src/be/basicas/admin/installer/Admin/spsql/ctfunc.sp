...
if exists (select 1 from sysobjects where name = 'sp_contexto_funcionalidad')
...
begin
	...
	begin
		/***** CONTROL NULOS ********/
		if ( @i_func is null or @i_contexto is null )
		Begin
		...
		if not exists ( select 1 from aw_funcionalidad where fn_func = @i_func)
		Begin
			exec cobis..sp_cerror
	   		@t_debug = @t_debug,
	   		@t_file	 = @t_file,
		...
		if not exists (select 1 from aw_contexto where co_contexto = @i_contexto)
		Begin
			exec cobis..sp_cerror
	   		@t_debug = @t_debug,
	   		@t_file	 = @t_file,
		...
		insert into aw_contexto_func(	cf_func, cf_contexto)
...
begin
	...
	begin
		...
		if not exists (select 1	from aw_contexto_func
		...
		begin
			...
			begin
				...
				from 
  				  aw_contexto,
				  aw_nombre_contexto,
				  aw_contexto_func LEFT OUTER JOIN aw_ayuda_contexto ON ac_contexto=cf_contexto and ac_idioma=@i_idioma
				where 
				  cf_func=@i_func
			...
			select 'Codigo'=pf_prereq,
				    'Descripcion'=substring(C.valor,1,30)
			from aw_prereq_func, cl_catalogo C,	cl_tabla T
