...
if exist Resultado.out del Resultado.out

echo ******  COMPILACION DE STORED PROCEDURES    ******
for /L %%i in (1,1,3) do (
echo Compilacion parte %%i
echo Compilacion parte %%i >> Resultado.out
if exist Salida.out del Salida.out
for %%f in (.\*.sp) do (
     echo Compilando Store Procedure %%f >> Resultado.out
     echo ----------------------------------------------------- %%f >> Salida.out           
     sqlcmd -S%1 -U%2 -P%3 -m 1 -i %%f >> Salida.out 
     )
)
