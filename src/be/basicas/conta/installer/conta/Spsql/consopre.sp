...
/*	Mayoriza los saldos presupuestados a nivel de oficinas y areas  */
/*	de movimiento                                                   */
/*				MODIFICACIONES				*/
/*	FECHA		AUTOR		RAZON				*/
...
if exists (select * from sysobjects where name = 'sp_consopre')
	drop proc sp_consopre  
go
create proc sp_consopre (
	@s_ssn		int = null,
...
begin
	...
	begin
		   if NOT EXISTS (select 1 from cob_conta..cb_saldo_presupuesto
		   ...
		   begin 
			...
			( sap_cuenta,sap_oficina,sap_area,sap_fecha,
			  sap_empresa,sap_presupuesto)
                     values (@i_cuenta,@i_oficina,@i_area,@i_fecha,
                          @i_empresa,@i_presupuesto)
			...
			return 1
                   ...
                   /* SI EXISTE REGISTRO DE LA CUENTA, LO ACTUALIZA */
                     ...
                     set sap_presupuesto = isnull(sap_presupuesto,0) + @i_presupuesto 
		     where sap_empresa = @i_empresa and
		        sap_oficina = @i_oficina and 
			sap_area    = @i_area and
			sap_cuenta  = @i_cuenta and
