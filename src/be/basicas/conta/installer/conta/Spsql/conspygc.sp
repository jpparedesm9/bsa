/************************************************************************/
/*	Archivo: 		conspygt.sp                             */
/*	Stored procedure: 	sp_consulta_pyg_terceros_cta            */
/*	Base de datos:  	cob_conta                               */
/*	Producto:               Contabilidad                            */
/*	Disenado por:           Mauricio Rincon                         */
/*	Fecha de escritura:     Abr-18-2006                             */
/************************************************************************/
/*				IMPORTANTE                              */
...
/************************************************************************/
/*				PROPOSITO                               */
/*	Consulta Movimientos PYG desde Front-end modulo de Contabilidad */
/************************************************************************/
/*				MODIFICACIONES                          */
/*	FECHA		AUTOR			RAZON                   */
/*	18/Abr/06	Mauricio Rincon		Consulta Movimiento PYG */
/*	                                        por Cuenta y Ente	*/
/************************************************************************/
use cob_conta
go

if exists (select * from sysobjects where name = 'sp_consulta_pyg_terceros_cta')
   drop proc sp_consulta_pyg_terceros_cta
go
...
create proc sp_consulta_pyg_terceros_cta (
@s_ssn            int         = null,
...
declare  @w_corte   int,
   ...
   select @w_sp_name = 'sp_consulta_pyg_terceros_cta'
   select @w_spid = @@spid
   select @w_flag = '00'
...
---/****************************************/
   if (@t_trn <> 6090 and @i_operacion = 'S') /* Consulta Saldos de PYG */
   begin              
...
---/**************************/
   ...
   -- selecciona corte fin de mes
    ...
    -- print ' verifica la existencia de la tabla sit_saldo_tercero %1!' +@w_nombre_tabla_saldos
     ...
     -- inserta datos en la tabla de trabajo
      ...
      select @w_cadena5 = ' and st_cuenta = "' + @i_cuenta_ini + '"'
   ...
   */
      ...
      begin
         if @i_modo = 0
         begin
            print 'No existen Movto PYG para esta CUENTA,ENTE,FECHA,OFICINA y AREA'
            return 1
         end  
      end      
   ...
   */	
         ...
         select "PERIODO" = st_periodo,
                ...
                "LLAVE" = right("0000000000"+convert(varchar,st_ente),10) +
          ...
          where st_spid = @w_spid
            ...
            and st_area > 0
            and st_cuenta  = @i_cuenta_ini
            and st_empresa = @i_empresa
            and st_empresa = je_empresa
            and (of_oficina = @i_oficina or @i_oficina is null)
            and st_ente    = en_ente
      	 order by st_periodo,st_corte,st_cuenta,st_ente,st_producto,st_oficina,st_area
      ...
      begin
   	 ...
   	 set rowcount 20
         ...
         select "PERIODO" = st_periodo,
           ...
           from cb_saldo_tercero_rep a,
            ...
            and (of_oficina = @i_oficina or @i_oficina is null)
            and (right("0000000000"+convert(varchar,st_ente),10) +
                 right("00000"+convert(varchar,st_producto),5)   +
                 right("00000"+convert(varchar,st_oficina),5)    +
   	 ...
   	 if @@rowcount = 0
         begin
            if @@error = 0
            begin
   	       print 'No existen mas registros de Movto PYG para esta CUENTA'
   	       select @w_flag = '10'
               select @w_flag
            end
         end
