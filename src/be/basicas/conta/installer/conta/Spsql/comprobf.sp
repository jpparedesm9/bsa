...
/*    24/08/2020   D. Cumbal      Caso 144440                      */
/*******************************************************************/
use cob_conta
go
...
begin
    ...
    select @w_autorizado = co_autorizado,
           @w_mayorizado = co_mayorizado
    ...
    if @@rowcount <= 0
    begin
         select @w_error = 601060
         goto ERROR
    end


    if @w_mayorizado = 'S' and @i_autorizado != 'S'
    begin
         select @w_error = 607056
         goto ERROR
    end 
    ...
    if @i_autorizado = 'S' and @w_mayorizado = 'N' --mayoriza en linea el comprobante al momento de autorizarlo--
    begin 
         ...
         begin
              ...
              select @w_empresa     = as_empresa,
                     @w_oficina_dest= as_oficina_dest,
                     @w_area_dest   = as_area_dest,
                     @w_cuenta      = as_cuenta,
                     @w_credito     = as_credito,
                     @w_debito      = as_debito,
                     @w_credito_me  = as_credito_me,
                     @w_debito_me   = as_debito_me,
                     @w_cotizacion  = as_cotizacion,
                     @w_mayorizado  = as_mayorizado,
                     @w_tipo_doc    = as_tipo_doc,
                     @w_tipo_tran   = as_tipo_tran,
                     @w_moneda      = as_moneda
              from cb_asiento
              where as_empresa      = @i_empresa 
                and as_fecha_tran   = @i_fecha_tran 
                and as_comprobante  = @i_comprobante 
                and as_asiento      = @w_asiento 
                and as_oficina_orig = @i_oficina_orig
              ...
              begin
                   ...
                   insert #_cuentas_tmp values (@w_i, @w_cuenta,'U', 0)
                    ...
                    --select @w_msg = 'Asiento --> ' + convert(varchar,@w_asiento) + ' - ' + @w_cuenta
                    --print @w_msg
              end --while 1=1
                   ...
                   exec @w_return = cob_conta..sp_mayoriza         
                        null,null,null,null,null,null,null,null,
                        6301,'N',null,null,
                        @i_empresa,@i_fecha_tran,@w_cuenta,
                        @w_oficina_dest,@w_area_dest,@w_credito,
                        @w_debito,@w_credito_me,@w_debito_me, 1
