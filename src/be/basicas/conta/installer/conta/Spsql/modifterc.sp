/************************************************************************/
/*	Archivo: 		    modifterc.sp 			*/
/*	Stored procedure: 	sp_modif_terc			*/
...
/*      Pertenecientes a las modificaciones de cuentas asociadas        */
/*      a procesos de terceros.                                         */
/************************************************************************/
/*				MODIFICACIONES				*/
/*	FECHA		AUTOR		   RAZON			*/
/*      01/04/2008                         Emision Inicial              */
...
if exists (select 1 from sysobjects where name = 'sp_modif_terc')
	drop proc sp_modif_terc
go

create proc sp_modif_terc
(  
     @s_ssn         int         = null,  
...
if (@t_trn <> 6212 and @i_operacion = 'Q')   
begin  
...
Begin  
     ...
     Begin  
          select "SECUENCIAL"   = secuencial,  
                 ...
                 "OFICINA"      = oficina,  
                 "EMPRESA"      = empresa  
          from cob_conta..ts_cuenta_proceso  
          where proceso in (6003,6004, 6095)  
          and   fecha   >= @i_fecha_ini  
