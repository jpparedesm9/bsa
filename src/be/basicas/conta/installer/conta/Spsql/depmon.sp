...
/*	17/Mar/1997	F. Salgado	Personalizacion CajaCoop	*/
/*					FS001: Adicion opcion 'Q' para  */
/*					consultar saldos historicos	*/
...
if exists (select * from sysobjects where name = 'sp_depmon')
	drop proc sp_depmon    

go
create proc sp_depmon    (
	@s_ssn		int = null,
...
select @w_sp_name = 'sp_depmon'


/************************************************/
/*  Tipo de Transaccion =     			*/

if (@t_trn <> 6702 and @i_operacion = 'S') or
   (@t_trn <> 6710 and @i_operacion = 'Q')
begin
...
begin  
	...
	begin
 		select cu_nombre,
		       sum(hi_saldo)
                  from cb_cuenta, cb_jerarquia, cb_jerararea, cob_conta_his..cb_hist_saldo
                 where cu_empresa = @i_empresa
		   and cu_cuenta = @i_cuenta
                   and je_empresa = @i_empresa
                   and je_oficina_padre = @i_oficina
