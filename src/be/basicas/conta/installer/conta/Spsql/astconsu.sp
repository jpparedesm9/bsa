/************************************************************************/
/*	Archivo: 		atconsu.sp			        */
/*	Stored procedure: 	sp_asientot_consu				*/
...
if exists (select * from sysobjects where name = 'sp_asientot_consu')
	drop proc sp_asientot_consu
go

create proc sp_asientot_consu   (
	@s_ssn		int = null,
...
select @w_sp_name = 'sp_asientot_consu'
...
/*  JCQ 04/06/2004 ESTAS RUTINAS YA NO APLICAN
	...
	begin
		...
		begin
			...
			print 'cuenta %1! oficina %2! area %3!',@i_cuenta,@i_oficina_dest,@i_area_dest
			--exec cobis..sp_cerror
			--@t_debug = @t_debug,
			--@t_file	 = @t_file,
			--@t_from	 = @w_sp_name,
