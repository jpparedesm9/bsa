/************************************************************************/
/*	Archivo: 		comprobs.sp                             */
/*	Stored procedure: 	sp_scomprobanteh                         */
...
/* 28-Jul-2006  Mauricio Rincon         NREQ-611 Consulta sc_automatico */
/************************************************************************/

use cob_conta
go

if exists (select * from sysobjects where name = 'sp_scomprobanteh')
	drop proc sp_scomprobanteh  
go

create proc sp_scomprobanteh (
	@t_rty			char(1)		= null,
	@t_trn			smallint	= null,
	@t_debug		char(1)		= 'N',
	...
	@i_usuario_modulo	descripcion	= 'conbatch',
...
select @w_sp_name = 'sp_scomprobanteh'


/*             TIPO DE TRANSACCION                */
if (@t_trn = 6344 and @i_operacion <> 'U') or
   (@t_trn = 6345 and @i_operacion <> 'I') or
   (@t_trn = 6305 and @i_operacion <> 'A') or
   (@t_trn = 6306 and @i_operacion <> 'Q')
	  ...
	  begin
	   ...
	   select 	'PRODUCTO'	= sc_producto,
			...
			'TRAN. MODULO'	= sc_tran_modulo,
		        'CODIGO FUENTE' = sc_automatico
		from cob_conta_historico..ct_scomprobante
		where sc_empresa = @i_empresa
		and sc_producto =  @i_producto
		...
		and sc_fecha_tran between @i_fecha_tran and @i_fecha_tran1
		and sc_comprobante between @i_comprobante and @i_comprobante1
		and sc_oficina_orig = @i_oficina_orig 
		and sc_area_orig between @i_area_orig and @i_area_orig1
		and sc_estado like @i_estado
		order by sc_empresa, sc_producto, sc_fecha_tran, sc_comprobante
	 ...
	 if @i_oficina_orig is null
	  begin
	   select @i_oficina_orig = 0, @i_oficina_orig1 = 32767
		...
		and sc_oficina_orig between @i_oficina_orig and @i_oficina_orig1
		and sc_area_orig between @i_area_orig and @i_area_orig1
		and ((sc_comprobante > @i_comprobante and  sc_fecha_tran = @i_fecha_tran)
		or (sc_fecha_tran >  @i_fecha_tran  and sc_fecha_tran <= @i_fecha_tran1))
		order by sc_empresa, sc_producto, sc_fecha_tran, sc_comprobante
