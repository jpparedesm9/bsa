...
if exists (select * from sysobjects where name = 'sp_posdia2') 
	drop proc sp_posdia2  
go 

create proc sp_posdia2 (
	@s_ssn			int = null,
...
begin
  if @i_modo = 0
	...
	select  cp_condicion,		cp_cuenta,	
      		sum(bl_saldo_mna),   	sum(bl_saldo_dol),  
      		sum(bl_saldo_uvc), 	sum(bl_saldo_mna_s), 
      		sum(bl_saldo_dol_s), 	sum(bl_saldo_uvc_s), 
		bl_oficina	
	...
	group by cp_condicion,	cp_cuenta,	
		 bl_oficina
