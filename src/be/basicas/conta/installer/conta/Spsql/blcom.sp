...
/************************************************************************/
/*                              PROPOSITO                               */
/*      Emite listados de Balance General para un nùmero de meses,      */
...
if exists (select * from sysobjects where name = 'cb_blcom')
   drop table cb_blcom
go

create table cb_blcom
(
...
if exists (select * from sysobjects where name = 'sp_balance_com')
   drop proc sp_balance_com
go

create proc sp_balance_com(
   @s_ssn		  int = null,
...
truncate table cb_blcom

insert into cb_blcom 
select
...
update cb_blcom
set bl_nombre = cu_nombre
from cob_conta..cb_cuenta
where cu_empresa = convert(tinyint,@i_empresa)
and   bl_empresa = cu_empresa
and   bl_cuenta  = cu_cuenta
and   bl_cuenta > ' '
and   bl_oficina >= 0

declare cursor_saldo1 cursor for
select round(sum(hi_saldo),2), hi_cuenta, je_oficina_padre
from cob_conta_his..cb_hist_saldo,cob_conta..cb_oficina,cob_conta..cb_jerarquia,cob_conta..cb_blcom
where hi_corte   = convert(int,@w_corte_i)
and   hi_periodo = convert(int,@w_periodo_i)
and   hi_empresa = convert(tinyint,@i_empresa)
and   bl_cuenta = hi_cuenta
and   of_empresa = convert(tinyint,@i_empresa)
and   of_organizacion = convert(tinyint,@i_nivel_of)
and   bl_oficina = je_oficina_padre
and   je_empresa = of_empresa
and   je_oficina_padre = of_oficina
and   hi_oficina = je_oficina
group by je_oficina_padre, hi_cuenta
...
begin   
   ...
   update cob_conta..cb_blcom
   set bl_saldo_ant = @w_concepto
   where bl_cuenta = @w_cuenta
   and   bl_oficina =  @w_oficina
   fetch cursor_saldo1  into @w_concepto,@w_cuenta,@w_oficina
end
close cursor_saldo1
...
update cob_conta..cb_blcom
set bl_saldo_ant =bl_saldo_ant * (-1), 
bl_saldo_act = bl_saldo_act * (-1)
from cob_conta..cb_cuenta
where bl_cuenta = cu_cuenta
and   bl_saldo_act <> 0
and   bl_saldo_ant <> 0
and   cu_categoria = 'C'

update cob_conta..cb_blcom
set bl_diferencia = round((bl_saldo_act - bl_saldo_ant),2)

update cob_conta..cb_blcom
set bl_porcentaje = (bl_diferencia / bl_saldo_ant) * 100
where bl_saldo_ant <> 0

update cob_conta..cb_blcom
set bl_porcentaje = 100
where bl_saldo_ant = 0
and   bl_saldo_act <> 0
