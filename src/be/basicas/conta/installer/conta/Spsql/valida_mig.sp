...
if exists (select 1 from sysobjects where name = 'sp_valida_mig')
   drop proc sp_valida_mig
go
 
create proc sp_valida_mig
...
select @w_numreg = 60000

select @w_numreg = pa_int
from cobis..cl_parametro
where pa_producto = 'CON'
and   pa_nemonico = 'NMRC'
...
select 
@w_dia = case when datepart(dd, @w_fecha) < 10 then '0' + convert(varchar, datepart(dd, @w_fecha)) else convert(varchar, datepart(dd, @w_fecha)) end,
@w_mes = case when datepart(mm, @w_fecha) < 10 then '0' + convert(varchar, datepart(mm, @w_fecha)) else convert(varchar, datepart(mm, @w_fecha))end,
...
if exists (select 1 from cobis..cl_tabla T (nolock), cobis..cl_catalogo C (nolock)
   ...
   where cc_archivo    = @w_usuario_arch + @w_fechav
