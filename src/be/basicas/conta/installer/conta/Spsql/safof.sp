...
if exists (select * from sysobjects where name = 'sp_safof') 
	drop proc sp_safof  
go 

create proc sp_safof (
	@s_ssn			int = null,
...
declare
	...
	@w_existe	int,		/* codigo existe = 1 
				               no existe = 0 */
        @w_oficina      char(3)
select @w_oficina = convert(char(3),@i_oficina) 
select @w_today = getdate()
select @w_sp_name = 'sp_safof'
...
/************************************************/
/*  Tipo de Transaccion =     			*/
/*   (@t_trn <> 6752 and @i_operacion = 'Q') or */

if (@t_trn <> 6752 and @i_operacion = 'S') or
   (@t_trn <> 6753 and @i_operacion = 'A') or
   (@t_trn <> 6753 and @i_operacion = 'K')
begin
 ...
 if @i_modo = 0
    begin   
       ...
       where     of_empresa   		= @i_empresa
             and bl_empresa             = @i_empresa
             and cp_empresa   		= @i_empresa
             and of_oficina             = bl_oficina
             and of_organizacion	= @i_organizacion
             and bl_periodo   		= @w_periodo
             and bl_corte     		= @w_corte
             and cp_proceso   		= @i_proceso
             and bl_cuenta    		= cp_cuenta
             order by of_oficina,bl_cuenta
    end
  else
     begin
	set rowcount 20 
       select of_oficina,	of_descripcion,
              bl_cuenta,	bl_saldo_mna
       from 	cb_oficina,	cb_balsuper,	
	        cb_cuenta_proceso
       where of_empresa   	 = @i_empresa
             and of_empresa      = @i_organizacion
             and bl_empresa   	 = @i_empresa
             and cp_empresa      = @i_empresa
             and ((of_oficina    = bl_oficina) or
                  (of_oficina    > bl_oficina)) 
             and bl_periodo   	 = @w_periodo
             and bl_corte     	 = @w_corte 
             and ((bl_cuenta  	 = cp_cuenta) or
                  (bl_cuenta     > cp_cuenta))
             order by of_oficina,bl_cuenta
     end
end
