...
@i_monblq           char(1) = 'S', --No tiene en cuenta montos bloqueados en caso de embargo
@i_is_batch         char(1) = 'N',
@i_ofi              smallint = null,
@i_tran_ext         char(1) = 'N', --CAV Valida que la transaccion sea por WS / N -> No
@i_corresponsal     char(1) = 'N',  --Req. 381 CB Red Posicionada
@i_monto_trn        money   = 0,    -- Req 381 CB Red Posicionada - Validar cupo de la cuenta vs valor del movimiento a realizar     
@i_trn              int     = null, -- ADI: ORS 949 Codigo de transaccion para no validar cupo cuando es Nota Credito
...
select  @w_sp_name = 'sp_ahcalcula_saldo'
...
if @@rowcount = 0
begin
   select @w_error = 2609822, @w_sev = 0
   goto ERROR
end
...
select  @w_saldo_minimo = pd_saldo_minimo
...
if @w_cred_24 = 'S'
begin
    select  @w_linea_cr24h = lc_monto_aut
...
if @w_cred_rem = 'S'
begin
    select  @w_linea_crrem = lc_monto_aut
      from  cob_ahorros..ah_lincredito
     where  lc_cuenta = @i_cuenta
       and  lc_tipo = 'R'
       and  lc_fecha_aut <= @i_fecha
       and  lc_fecha_ven >= @i_fecha
    if @@rowcount = 0
        select @w_linea_crrem = 0
end
else
    select  @w_linea_crrem = 0
    
if @i_monblq = 'N'
   select @w_monto_bloq = 0

select @o_saldo_para_girar = @w_disponible - @w_monto_bloq - @w_monto_consumos - @w_monto_emb
select @o_saldo_contable   = @w_12h + @w_24h + @w_48h

if @w_linea_cr24h < @o_saldo_contable
    select  @o_saldo_para_girar = @o_saldo_para_girar +
                      @w_linea_cr24h    
else
    select  @o_saldo_para_girar = @o_saldo_para_girar + 
                      @o_saldo_contable
...
if @w_linea_crrem < @w_remesas
    ...
    select  @o_saldo_para_girar = @o_saldo_para_girar + 
                      @w_remesas
if @w_prod_banc = @w_prod_bancario and @i_is_batch = 'N' and @i_trn = 264
begin
   select @w_cupo_utilizado = isnull(round(sum(vs_valor),2),0)
   ...
   select top 1 @w_cupo = isnull(cc_valor_cupo,0)
   from cob_remesas..re_mantenimiento_cupo_cb 
   where cc_cta_banco = @o_cta_banco
   and   cc_tipo_mov  <> 'V' -- SE EXCLUYE CAMBIO DE VIGENCIA DADO QUE QUEDA MONTO EN CERO
   order by cc_fecha_ingreso desc, cc_hora_ingreso desc
       
   if @i_monto_trn > ((@w_cupo - @w_cupo_utilizado) + @o_saldo_para_girar)
   begin
    /*  Fondos Insuficientes  */
    select @w_error = 251033, @w_sev = 0
    goto ERROR    
   end
   ...
   begin
    ...
    select @w_error = 2101026, @w_sev = 0
    goto ERROR    
   end
end
...
select @w_pais = pa_char
  from cobis..cl_parametro
 where pa_nemonico =  'ABPAIS' /* Abraviatura del Pais donde se esta trabajando **/
   and pa_producto = 'ADM'

if @@rowcount =0
begin
    /*No existe producto*/
    select @w_error = 351518, @w_sev = 0
    goto ERROR    
end
...
begin
   if @i_valida_saldo = 'S'  /*** En caso de transacciones que no necesiten validar el saldo Minimo ***/
   begin
      ...
      select  @w_saldo_minimo = @w_com_trna
      ...
      begin
         /*No existe producto*/
         select @w_error = 101085, @w_sev = 0
         goto ERROR    
      end

      if @w_valor_SML  > 0           /*** EL Saldo Maximo de la cuenta esta dado en Salarios Minimos ***/
         select @o_saldo_max = @o_saldo_max * @w_valor_SML 
   end     
end
