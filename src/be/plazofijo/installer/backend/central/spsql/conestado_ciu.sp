/************************************************************************/
/*      Archivo:                cones_ci.sp                             */
/*      Stored procedure:       sp_conestado_ciu                        */
/*      Base de datos:          cobis                                   */
/*      Producto:               Plazo Fijo                              */
...
if object_id('sp_conestado_ciu') is not null
   drop proc sp_conestado_ciu
go

create proc sp_conestado_ciu (
@s_ssn                  int             = NULL,
@s_user                 login           = NULL,
...
select @w_sp_name = 'sp_conestado_ciu'
...
select operacion           = op_operacion, 
       ...
       moneda              = op_moneda,
       tasa                = op_tasa, 
       tipo_plazo          = op_tipo_plazo,
       tipo_monto          = op_tipo_monto, 
       accion_sgte         = op_accion_sgte,
       pignorado           = op_pignorado, 
       retenido            = op_retenido,
       total_int_ganados   = op_total_int_ganados,
       total_int_pagados   = op_total_int_pagados,
       fecha_ven           = op_fecha_ven,
       fecha_ingreso       = op_fecha_ingreso, 
       telefono            = op_telefono ,
       num_dias            = op_num_dias, 
       monto               = op_monto, 
--      direccion          = di_descripcion,
       custodia            = isnull(op_custodia, 'N'),
       ejecutivo           = op_oficial,
       bloqueo_legal       = op_bloqueo_legal --GAR GB-DP00027
into   #operacion
...
begin
   ...
   begin
      delete #operacion
      from   #operacion
      where  bloqueo_legal not like @i_bloqueo_legal
   end
   ...
   select 'Cuenta'              = num_banco,
          ...
          'Estado'              = estado,
          'Fecha Venc.'         = convert(varchar(10),fecha_ven, @i_formato_fecha),
          'Fecha Ing.'          = convert(varchar(10),fecha_ingreso, @i_formato_fecha), /* GESCY2K B211 */
 --       'Direccion '          = direccion,
          'Telefono '           = telefono,
          'Ejecutivo'           = ejecutivo
   from   #operacion
