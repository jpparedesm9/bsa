/************************************************************************/
/*	Archivo: 		bt_conta_r.sp			        */
/*	Stored procedure: 	sp_batch_conta_r			*/
...
/*	02-Ene-2007	H.Ayarza	Contabilizacion Pendiente.      */
/*      05/Dic/2016     A.Zuluaga       Desacople                       */
/************************************************************************/
use cob_pfijo
go
if exists (select * from sysobjects where name = 'sp_batch_conta_r')
	drop proc sp_batch_conta_r
go
create proc sp_batch_conta_r   (
		@s_ssn		 int         = null,
...
select * from cob_pfijo..pf_scomprobante
 where sc_estado in ('A','R','X') --CVA MAY-30-06
  and  sc_fecha_tran  = @i_fecha_proceso  --HA20070102
...
begin
   ...
   -----------------------------------------------------------
   -- Verificar que la oficina Origen no caiga en dia Feriado
   -----------------------------------------------------------
   /***************************************************
   select 1 
     from cobis..cl_dias_feriados,
          cobis..cl_oficina
    where of_oficina = @w_oficina_orig
      and of_ciudad = df_ciudad
      and df_fecha = @w_sc_fecha_tran
...
--print 'bt_conta.sp DISPARA cob_conta..sp_sasiento'
     ...
     --HA20070102 end /** Verificacion de Feriado en OFICINA */
     GOTO SIGUIENTE
