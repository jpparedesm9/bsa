/************************************************************************/
/*      Archivo:                consop_i.sp                            */
/*      Stored procedure:       sp_cons_operacion_iv                    */
/*      Base de datos:          cob_pfijo                               */
/*      Producto:               Plazo Fijo-IVR                          */
...
/*                                    para banca virtual.               */
/*      03/15/2002 Gabriela Estupiñan Cambios para IVR, incluir @i_ente */ 
/************************************************************************/   
use cob_pfijo
go
...
if object_id('sp_cons_operacion_iv') is not null
   drop proc sp_cons_operacion_iv
go

create proc sp_cons_operacion_iv (
      @s_ssn                  int             = NULL,
      @s_user                 login           = NULL,
      ...
      @i_ente                 int,  -- GES 03/14/2002 Cambio solicitado por IVR
...
select @w_sp_name = 'sp_cons_operacion_iv',
...
if ( @i_operacion = 'Q' ) and ( @t_trn <> 14455 )
begin
...
--     @o_anio_comercial     = isnull(op_anio_comercial,'N') --04/04/2000 Fecha Comercial    GES 06/01/2000 Comentado version BCA
  ...
  and op_ente = @i_ente     -- GES 03/14/2002 Cambio solicitado por IVR
...
/* GES 10/04/01 CUZ-031-019 COMENTADO
   ...
   FIN COMENTADO */
    ...
    'Fecha de Renovacion'  = convert(char(10),@o_fecha_valor, @i_formato_fecha),
    'Fecha de Vencimiento'   = convert(char(10),@o_fecha_ven, @i_formato_fecha),
    'Tasa (%)' 	     	     = @o_tasa,
    'Proximo pago de int.'   = convert(char(10),@o_fecha_pg_int, 
     @i_formato_fecha),
