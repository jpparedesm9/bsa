...
Partial Public Class FTran212Class
        ...
        Select Case Index
            Case 0
                ...
                If txtCampo(1).Text.Trim() = "" Then
                    COBISMessageBox.Show(FMLoadResString(2552), FMLoadResString(500018), COBISMessageBox.COBISButtons.OK, COBISMessageBox.COBISIcon.Error)
                    txtCampo(1).Focus()
                    Exit Sub
                End If
                ...
                If FMTransmitirRPC(sqlconn, ServerName, "cob_ahorros", "sp_tr_bloqueo_mov_ah", True, FMLoadResString(2553)) Then
                    PMChequea(sqlconn)
                    cmdBoton(0).Enabled = False
                    cmdBoton(4).Enabled = True
                    COBISMessageBox.Show(FMLoadResString(501826), FMLoadResString(501547), COBISMessageBox.COBISButtons.OK, COBISMessageBox.COBISIcon.Information)
                Else
    ...
    Private Sub FTran212_Load(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Load
    ...
    Public Sub PLInicializar()
        ...
        Me.Text = FMLoadResString(20016)
    End Sub

    Private Sub FTran212_Closed(ByVal eventSender As Object, ByVal eventArgs As EventArgs) Handles MyBase.Closed
    ...
    Private Sub PLImprimir()
        If COBISMessageBox.Show(FMLoadResString(501827), FMLoadResString(500092), COBISMessageBox.COBISButtons.OKCancel, COBISMessageBox.COBISIcon.Question) = System.Windows.Forms.DialogResult.OK Then
            Me.Cursor = System.Windows.Forms.Cursors.WaitCursor
            FMCabeceraReporte(VGBanco, DateTimeHelper.ToString(DateTime.Today), "LEVANTAMIENTO DE MOVIMIENTO EN CUENTAS DE AHORROS", DateTimeHelper.ToString(DateTimeHelper.Time), Me.Text, VGFecha, CStr(1))
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.FontBold = True
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print(FileSystem.TAB(4), "Número de Cuenta        :   ", mskCuenta.Text)
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print("")
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print(FileSystem.TAB(4), "Nombre de la Cuenta     :   ", lblDescripcion(0).Text)
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print("")
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print(FileSystem.TAB(4), "Causa de Levantamiento  :   ", lblDescripcion(2).Text)
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print("")
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print(FileSystem.TAB(4), "Tipo de Bloqueo         :   ", lblDescripcion(1).Text)
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print("")
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print(FileSystem.TAB(4), "Solicitado por          :   ", txtCampo(2).Text)
            COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print("")
            ...
            If VGCodPais <> "CO" Then
                COBISCorp.Framework.UI.Components.COBISPrinterNet.DefInstance.Print("---  U L T I M A   L I N E A  ---")
            End If
