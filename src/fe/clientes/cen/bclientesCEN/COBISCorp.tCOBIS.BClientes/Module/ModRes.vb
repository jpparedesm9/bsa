...
Public Module ModRes
    ...
    Dim VTResString As String = String.Empty
    ...
    Public Function FMLoadResString(ByRef parCodMsg As String) As String
        If VGCulture Is Nothing Then
            'If Not COBISCorp.eCOBIS.Commons.Resources.ResourceHandlerManager.GetString(VGResourceManager, "str" + parCodMsg) = Nothing Then FMLoadResString = "" : Exit Function
                ...
                If VTsCtlType = "ToolStrip" Then
                    ...
                    For i = 0 To VLNum - 1
                        If parFrm.COBISResourceProvider.GetImageID(a.Items.Item(i)).ToString <> "" Then
                            Try
                                a.Items.Item(i).Image = COBISCorp.eCOBIS.Commons.Resources.ResourceHandlerManager.GetIcon(VGResourceManager, "ico" + CStr(parFrm.COBISResourceProvider.GetImageID(a.Items.Item(i)))).ToBitmap
    ...
    '    While (VTForm.GetType().BaseType.Name <> "Form")
    '        VTForm = VTForm.Parent
    '    End While
    '    Try
    '        Dim VTsCtlType As String = ""
    '        Dim VTnVal As Integer = 0
    '        If VTForm.GetType().BaseType.Name = "Form" And Convert.ToString(parFrm.Tag) <> "" Then
    '            If IsNumeric(parFrm.Tag) And parFrm.Tag <> "" Then
    '                parFrm.Text = COBISCorp.eCOBIS.Commons.Resources.ResourceHandlerManager.GetString(VGResourceManager, "str" + CStr(CInt(Convert.ToString(parFrm.Tag))))
    '            End If
    '        End If
    '        Dim VTFnt As Font = parFrm.Font
    '        For Each VTCtl As Object In ContainerHelper.Controls(parFrm)
    '            Try
    '                VTsCtlType = VTCtl.GetType().Name
    '                If VTsCtlType = "Menu" Then
    '                    VTCtl.Text = COBISCorp.eCOBIS.Commons.Resources.ResourceHandlerManager.GetString(VGResourceManager, "str" + CStr(VTForm.COBISResourceProvider.GetResourceID(VTCtl)))
    '                ElseIf VTsCtlType = "TabStrip" Then
    '                    For Each VTObj As Object In VTCtl.Tabs
    '                        VTObj.Caption = COBISCorp.eCOBIS.Commons.Resources.ResourceHandlerManager.GetString(VGResourceManager, "str" + CStr(VTObj.Tag))
    '                        VTObj.ToolTipText = COBISCorp.eCOBIS.Commons.Resources.ResourceHandlerManager.GetString(VGResourceManager, "str" + CStr(VTObj.ToolTipTextID))
    '                    Next VTObj
    ...
    '                        VTObj.ToolTipText = COBISCorp.eCOBIS.Commons.Resources.ResourceHandlerManager.GetString(VGResourceManager, "str" + CStr(VTObj.ToolTipText))
    '                    Next VTObj
    ...
    '                    VTCtl.HelpLine = COBISCorp.eCOBIS.Commons.Resources.ResourceHandlerManager.GetString(VGResourceManager, "str" + CStr(VTForm.COBISResourceProvider.GetImageID(VTCtl)))
   
    Public Sub PMLoadResStrings(ByRef parFrm As Object)
        Dim VTForm As Object = parFrm
        Dim controls As Object = Nothing
            ...
            ElseIf VTsCtlType = "vaSpread" Or VTsCtlType = "COBISSpread" Then
                ...
                For VTi As Integer = 0 To VTCtl.MaxCols - 1
                    ...
                    Else
                        ...
                        End If
                        wLen = Len(VTCtl.ActiveSheet.ColumnHeader.Columns(VTi).Label)
                    End If
                Next
    ...
    Public Function FMMsgBox(ByRef parCodMsg As Integer, ByRef parButtons As COBISMsgBox.COBISButtons, ByRef parCodTitulo As Integer, ByRef parMsg As String, ByRef parTitulo As String) As Integer
        ...
        Else
        End If
        Dim VTRetorno As Integer = COBISMessageBox.Show(VTMsg, parButtons, VTTitulo)
        Return VTRetorno
    End Function

    Public Function FMResolveResString(ByVal parResID As Integer, ByVal ParamArray parReemplazos() As String) As String
        Dim ErrParesErrados As Boolean = False
        Dim result As String = String.Empty
        Dim VTMacro As String = String.Empty
        Dim VTValor As String = String.Empty
        Dim VTPos As Integer = 0
        result = ""
        Try
            For VTNumMacros As Integer = parReemplazos.GetLowerBound(0) To parReemplazos.GetUpperBound(0) Step 2
                VTMacro = parReemplazos(VTNumMacros)
                ErrParesErrados = True
                VTValor = parReemplazos(VTNumMacros + 1)
                ErrParesErrados = False
                Do
                    VTPos = (VTResString.IndexOf(VTMacro) + 1)
                    If VTPos > 0 Then
                        VTResString = Strings.Left(VTResString, VTPos - 1) & VTValor & Strings.Right(VTResString, VTResString.Length - VTMacro.Length - VTPos + 1)
                    End If
                Loop Until VTPos = 0
            Next VTNumMacros
            Return VTResString
        Catch excep As System.Exception
            If Not ErrParesErrados Then
                Throw excep
            End If
            If ErrParesErrados Then
