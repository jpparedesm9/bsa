...
designerEvents.api.loanheaderinfoform = designerEvents.api.loanheaderinfoform || designer.dsgEvents();

function VC_LOANHEADFO_861316(cobisMainModule) {
    cobisMainModule.controllerProvider.register("VC_LOANHEADFO_861316_MAIN_CTRL", ["$scope", "breadcrumbs",

    function($scope, breadcrumbs) {
        $scope.breadcrumbs = breadcrumbs;
    }]);
    cobisMainModule.controllerProvider.register("VC_LOANHEADFO_861316_CTRL", ["cobisMessage", cobis.modules.CONTAINER + ".preferencesService", "dsgnrUtils", "designer", "$scope", "$location", "$document", "$parse", "$filter", "$modal", "$q", "$compile", "$timeout", "$translate",
    ...
    function(cobisMessage, preferencesService, dsgnrUtils, designer, $scope, $location, $document, $parse, $filter, $modal, $q, $compile, $timeout, $translate) {
        ...
        $scope.vc = designer.initViewContainer({
            ...
            taskVersion: "1.0.0",
            viewContainerId: "VC_LOANHEADFO_861316",
            hasCloseModalEvent: false,
            serverEvents: true
        },
            "${contextPath}/resources/ASSTS/CMMNS/T_LOANHEADERNFI_316",
        designerEvents.api.loanheaderinfoform,
        $scope.rowData);
        ...
        if (!$scope.vc.loaded) {
            ...
            $scope.vc.temporarySave = function() {
                ...
                $scope.vc.execute("temporarySave", VC_LOANHEADFO_861316, data, function() {});
            };
            $scope.vc.temporaryRead = function() {
                if (page.hasTemporaryDataSupport) {
                    ...
                    return $scope.vc.executeService("readTemporaryData", VC_LOANHEADFO_861316, data).then(

                    function(response) {
                        var result = $scope.vc.processTemporaryResponse(response);
                        if (result) {
                            $scope.vc.execute("deleteTemporaryData", VC_LOANHEADFO_861316, data, function() {});
            ...
            $scope.vc.viewState.VC_LOANHEADFO_861316 = {
                style: []
            }
            $scope.vc.model.Loan = {
        ...
        $scope._initPage_ProcessRender = function(page) {
            if (page.hasSearchRenderEvent) {
                $scope.vc.render('VC_LOANHEADFO_861316');
            }
            return page;
        };
...
if (typeof cobisMainModule === "undefined") {
    var cobisMainModule = cobis.createModule("VC_LOANHEADFO_861316", ["ngResource", "ngRoute", "kendo.directives", "ui.bootstrap", "oc.lazyLoad", "pascalprecht.translate", cobis.modules.CONTAINER, "designerModule"], ["DSGNR", "ASSTS"]);
    ...
    function($controllerProvider, $compileProvider, $filterProvider, $locationProvider, $routeProvider, $provide, $translateProvider, $translatePartialLoaderProvider, $ocLazyLoadProvider) {
        ...
        $routeProvider.when("/VC_LOANHEADFO_861316", {
            templateUrl: "VC_LOANHEADFO_861316_FORM.html",
            controller: "VC_LOANHEADFO_861316_CTRL",
            labelId: "ASSTS.LBL_ASSTS_DATOSGEAL_65676",
            resolve: {
            ...
            redirectTo: function(routeParams, path, search) {
                return "/VC_LOANHEADFO_861316?" + $.param(search);
            }
        });
        VC_LOANHEADFO_861316(cobisMainModule);
    }]);
} else {
    VC_LOANHEADFO_861316(cobisMainModule);
}