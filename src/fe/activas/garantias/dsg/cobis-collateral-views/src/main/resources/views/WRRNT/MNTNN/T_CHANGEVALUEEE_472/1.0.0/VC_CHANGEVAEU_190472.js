...
designerEvents.api.changevalue = designerEvents.api.changevalue || designer.dsgEvents();

function VC_CHANGEVAEU_190472(cobisMainModule) {
    cobisMainModule.controllerProvider.register("VC_CHANGEVAEU_190472_MAIN_CTRL", ["$scope", "breadcrumbs",

    function($scope, breadcrumbs) {
        $scope.breadcrumbs = breadcrumbs;
    }]);
    cobisMainModule.controllerProvider.register("VC_CHANGEVAEU_190472_CTRL", ["cobisMessage", cobis.modules.CONTAINER + ".preferencesService", "dsgnrUtils", "designer", "$scope", "$location", "$document", "$parse", "$filter", "$modal", "$q", "$compile", "$timeout",
    ...
    function(cobisMessage, preferencesService, dsgnrUtils, designer, $scope, $location, $document, $parse, $filter, $modal, $q, $compile, $timeout) {
        ...
        $scope.vc = designer.initViewContainer({
            ...
            "${contextPath}/resources/WRRNT/MNTNN/T_CHANGEVALUEEE_472",
        designerEvents.api.changevalue,
        $scope.rowData);
        ...
        if (!$scope.vc.loaded) {
            ...
            $scope.vc.temporarySave = function() {
                ...
                $scope.vc.execute("temporarySave", VC_CHANGEVAEU_190472, data, function() {});
            };
            $scope.vc.temporaryRead = function() {
                if (page.hasTemporaryDataSupport) {
                    ...
                    return $scope.vc.executeService("readTemporaryData", VC_CHANGEVAEU_190472, data).then(

                    function(response) {
                        var result = $scope.vc.processTemporaryResponse(response);
                        if (result) {
                            $scope.vc.execute("deleteTemporaryData", VC_CHANGEVAEU_190472, data, function() {});
            ...
            $scope.vc.viewState.VC_CHANGEVAEU_190472 = {
                style: []
            }
            ...
            //ViewState - Group: Group1177
            $scope.vc.createViewState({
                id: "G_CHANGEVEEA_792420",
                hasId: true,
                componentStyle: [],
                label: 'Group1177',
            ...
            //ViewState - Entity: GenericTransaction, Attribute: newValue
            $scope.vc.createViewState({
                id: "VA_8486ISVFDOHOMXS_773420",
                componentStyle: [],
                label: "WRRNT.LBL_WRRNT_NUEVOVAOO_49519",
                format: "c",
        ...
        $scope._initPage_ProcessRender = function(page) {
            if (page.hasSearchRenderEvent) {
                $scope.vc.render('VC_CHANGEVAEU_190472');
            }
            return page;
        };
...
if (typeof cobisMainModule === "undefined") {
    var cobisMainModule = cobis.createModule("VC_CHANGEVAEU_190472", ["ngResource", "ngRoute", "kendo.directives", "ui.bootstrap", "oc.lazyLoad", "pascalprecht.translate", cobis.modules.CONTAINER, "designerModule"], ["DSGNR", "WRRNT"]);
    ...
    function($controllerProvider, $compileProvider, $filterProvider, $locationProvider, $routeProvider, $provide, $translateProvider, $translatePartialLoaderProvider, $ocLazyLoadProvider) {
        ...
        $routeProvider.when("/VC_CHANGEVAEU_190472", {
            templateUrl: "VC_CHANGEVAEU_190472_FORM.html",
            controller: "VC_CHANGEVAEU_190472_CTRL",
            labelId: "WRRNT.LBL_WRRNT_CAMBIODEE_48627",
            resolve: {
            ...
            redirectTo: function(routeParams, path, search) {
                return "/VC_CHANGEVAEU_190472?" + $.param(search);
            }
        });
        VC_CHANGEVAEU_190472(cobisMainModule);
    }]);
} else {
    VC_CHANGEVAEU_190472(cobisMainModule);
}