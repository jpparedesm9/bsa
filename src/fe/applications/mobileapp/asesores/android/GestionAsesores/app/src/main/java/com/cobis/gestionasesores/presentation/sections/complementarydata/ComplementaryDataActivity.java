...
public class ComplementaryDataActivity extends BaseActivity {
    ...
    public static Intent newIntent(Context context,int personId, Section section) {
        Intent intent = new Intent(context, ComplementaryDataActivity.class);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_complementary_data);

        ComplementaryDataFragment fragment = (ComplementaryDataFragment) getSupportFragmentManager().findFragmentById(R.id.fragment_container);

        if (fragment == null) {
            fragment = ComplementaryDataFragment.newInstance();
            getSupportFragmentManager().beginTransaction().replace(R.id.fragment_container, fragment).commit();
        }

        new ComplementaryDataPresenter(fragment,
