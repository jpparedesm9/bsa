...
var app = cobis.createModule(cobis.modules.ADMCLIENTVIEWER, ["oc.lazyLoad",cobis.modules.ADMCLIENTVIEWER, 'ui.bootstrap', "designerModule", "ngResource"]);    
    ...
    app.controller("adminclientviewer.treeController", ['$scope', '$rootScope', '$filter', '$modal', '$routeParams', 'adminclientviewer.treeServices', '$translate', 'cobisMessage', 'dsgnrCommons', 'designer',
            ...
            $scope.funtionCrudSection = function (dataItem, type, action) {
				...
				dataItem.parent().forEach(function (item){				
					if($scope.isCreating){
						if(item.selected && item.hasChildren){
							sons = item.children.data()
						}
					}else{
						sons.push(item);
					}
			...
			$scope.ctxMnuOpenProduct = function (dataItem,e) {						
				$scope.treeControlProduct=$("#treeControlProduct").data("kendoTreeView");
				$scope.dataItemSelected = $scope.treeControlProduct.dataItem(e.target);						
				$scope.ctxMenu = [];						
					
				if ($scope.dataItemSelected.menuType=='product'||$scope.dataItemSelected.menuType=='customerType') {
					$scope.ctxMenu.push({code:"newProductParentSection",text:$filter('translate')('COMMONS.MENU.MNU_ADD')});
				}
				if ($scope.dataItemSelected.menuType == 'itemProductType'||$scope.dataItemSelected.menuType == 'itemCustomerType') {
					$scope.ctxMenu.push({code:"newProductSection",text:$filter('translate')('COMMONS.MENU.MNU_ADD')});
					$scope.ctxMenu.push({code:"deleteSection",text:$filter('translate')('COMMONS.MENU.MNU_DELETE')});
				}				
				$scope.treeControlProduct.select($scope.treeControlProduct.findByUid($scope.dataItemSelected.uid));
			};
			
			$scope.ctxMnuOpenClient = function (dataItem,e) {						
				$scope.treeControlCustomer=$("#treeControlCustomer").data("kendoTreeView");
				$scope.dataItemSelected = $scope.treeControlCustomer.dataItem(e.target);						
				$scope.ctxMenu = [];						
					
				if ($scope.dataItemSelected.menuType=='customerType') {
					$scope.ctxMenu.push({code:"newCustomerParentSection",text:$filter('translate')('COMMONS.MENU.MNU_ADD')});
				}
				if ($scope.dataItemSelected.menuType == 'itemCustomerType') {
					$scope.ctxMenu.push({code:"newCustomerSection",text:$filter('translate')('COMMONS.MENU.MNU_ADD')});
					$scope.ctxMenu.push({code:"deleteSection",text:$filter('translate')('COMMONS.MENU.MNU_DELETE')});
				}				
				$scope.treeControlCustomer.select($scope.treeControlCustomer.findByUid($scope.dataItemSelected.uid));
			};
			 
			$scope.ctxMnuClickProduct = function(opt){
				if(opt=="newProductParentSection"){
					$scope.onClickNewProductParentSection($scope.dataItemSelected);							
				}
				if(opt=="newProductSection"){
					$scope.onClickNewProductSection($scope.dataItemSelected);
				}
				if(opt=="newCustomerParentSection"){
					$scope.onClickNewCustomerParentSection($scope.dataItemSelected);							
				}
				if(opt=="newCustomerSection"){
					$scope.onClickNewCustomerSection($scope.dataItemSelected);
				}
				if(opt=="deleteSection"){
					$scope.onDeleteSection($scope.dataItemSelected);
				}
			};
