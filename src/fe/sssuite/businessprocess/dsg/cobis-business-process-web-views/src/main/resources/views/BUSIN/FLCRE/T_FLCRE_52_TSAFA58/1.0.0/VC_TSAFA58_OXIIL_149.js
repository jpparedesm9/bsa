...
designerEvents.api.taskexpasigoffcial = designerEvents.api.taskexpasigoffcial || designer.dsgEvents();

function VC_TSAFA58_OXIIL_149(cobisMainModule) {
    cobisMainModule.controllerProvider.register("VC_TSAFA58_OXIIL_149_MAIN_CTRL", ["$scope", "breadcrumbs",

    function($scope, breadcrumbs) {
        $scope.breadcrumbs = breadcrumbs;
    }]);
    cobisMainModule.controllerProvider.register("VC_TSAFA58_OXIIL_149_CTRL", ["cobisMessage", cobis.modules.CONTAINER + ".preferencesService", "dsgnrUtils", "designer", "$scope", "$location", "$document", "$parse", "$filter", "$modal", "$q", "$compile", "$timeout",
    ...
    function(cobisMessage, preferencesService, dsgnrUtils, designer, $scope, $location, $document, $parse, $filter, $modal, $q, $compile, $timeout) {
        ...
        $scope.vc = designer.initViewContainer({
            moduleId: "BUSIN",
            subModuleId: "FLCRE",
            taskId: "T_FLCRE_52_TSAFA58",
            taskVersion: "1.0.0",
            viewContainerId: "VC_TSAFA58_OXIIL_149",
            hasCloseModalEvent: false,
            serverEvents: true
        },
            "${contextPath}/resources/BUSIN/FLCRE/T_FLCRE_52_TSAFA58",
        designerEvents.api.taskexpasigoffcial,
        $scope.rowData);
        ...
        if (!$scope.vc.loaded) {
            ...
            $scope.vc.viewState.VC_TSAFA58_OXIIL_149 = {
                style: []
            }
            //ViewState - Container: FormExprAsigOfficial
            $scope.vc.createViewState({
                id: "VC_TSAFA58_OXIIL_149",
                hasId: true,
                componentStyle: "",
                label: 'FormExprAsigOfficial',
                enabled: designer.constants.mode.All
            });
            //ViewState - Container: Offical
            $scope.vc.createViewState({
                id: "VC_TSAFA58_OFCAL_717",
                hasId: true,
                componentStyle: "",
                label: 'Offical',
                enabled: designer.constants.mode.All
            ...
            //ViewState - Container: Header
            $scope.vc.createViewState({
                id: "VC_TSAFA58_HEADE_338",
                hasId: true,
                componentStyle: "",
                label: 'Header',
                enabled: designer.constants.mode.All
        ...
        $scope._initPage_ProcessRender = function(page) {
            if (page.hasSearchRenderEvent) {
                $scope.vc.render('VC_TSAFA58_OXIIL_149');
            }
            return page;
        };
...
if (typeof cobisMainModule === "undefined") {
    var cobisMainModule = cobis.createModule("VC_TSAFA58_OXIIL_149", ["ngResource", "ngRoute", "kendo.directives", "ui.bootstrap", "oc.lazyLoad", "pascalprecht.translate", cobis.modules.CONTAINER, "designerModule"], ["DSGNR", "BUSIN"]);
    ...
    function($controllerProvider, $compileProvider, $filterProvider, $locationProvider, $routeProvider, $provide, $translateProvider, $translatePartialLoaderProvider, $ocLazyLoadProvider) {
        ...
        $routeProvider.when("/VC_TSAFA58_OXIIL_149", {
            templateUrl: "VC_TSAFA58_OXIIL_149_FORM.html",
            controller: "VC_TSAFA58_OXIIL_149_CTRL",
            label: "FormExprAsigOfficial",
            resolve: {
            ...
            redirectTo: function(routeParams, path, search) {
                return "/VC_TSAFA58_OXIIL_149?" + $.param(search);
            }
        });
        VC_TSAFA58_OXIIL_149(cobisMainModule);
    }]);
} else {
    VC_TSAFA58_OXIIL_149(cobisMainModule);
}