...
designerEvents.api.taskdisbursementformexecution = designerEvents.api.taskdisbursementformexecution || designer.dsgEvents();

function VC_SIURM23_BETIO_527(cobisMainModule) {
    cobisMainModule.controllerProvider.register("VC_SIURM23_BETIO_527_MAIN_CTRL", ["$scope", "breadcrumbs",

    function($scope, breadcrumbs) {
        $scope.breadcrumbs = breadcrumbs;
    }]);
    cobisMainModule.controllerProvider.register("VC_SIURM23_BETIO_527_CTRL", ["cobisMessage", cobis.modules.CONTAINER + ".preferencesService", "dsgnrUtils", "designer", "$scope", "$location", "$document", "$parse", "$filter", "$modal", "$q", "$compile", "$timeout",
    ...
    function(cobisMessage, preferencesService, dsgnrUtils, designer, $scope, $location, $document, $parse, $filter, $modal, $q, $compile, $timeout) {
        ...
        $scope.vc = designer.initViewContainer({
            ...
            taskVersion: "1.0.0",
            viewContainerId: "VC_SIURM23_BETIO_527",
            hasCloseModalEvent: false,
            serverEvents: true
        },
            "${contextPath}/resources/BUSIN/FLCRE/T_FLCRE_80_SIURM23",
        designerEvents.api.taskdisbursementformexecution,
        $scope.rowData);
        ...
        if (!$scope.vc.loaded) {
            ...
            $scope.vc.viewState.VC_SIURM23_BETIO_527 = {
                style: []
            }
            //ViewState - Container: FormDisbursementFormsExecution
            $scope.vc.createViewState({
                id: "VC_SIURM23_BETIO_527",
                hasId: true,
                componentStyle: "",
                label: 'FormDisbursementFormsExecution',
                enabled: designer.constants.mode.All
            });
            //ViewState - Container: GrpTittleHeader
            $scope.vc.createViewState({
                id: "VC_SIURM23_NNOME_794",
                hasId: true,
                componentStyle: "",
            ...
            $scope.vc.createViewState({
                id: "T_FLCRE_80_SIURM23_CANCEL",
                componentStyle: "",
                label: 'Cancel',
                enabled: designer.constants.mode.All,
                visible: designer.constants.mode.All
            });
            //ViewState - Command: Boton Liquidar
            $scope.vc.createViewState({
                id: "CM_SIURM23TOR49",
                componentStyle: "",
                tooltip: "BUSIN.DLB_BUSIN_LIQUIDATE_56797",
                label: "BUSIN.DLB_BUSIN_LIQUIDATE_56797",
                enabled: designer.constants.mode.All,
                visible: designer.constants.mode.All
            });
        ...
        $scope._initPage_ProcessRender = function(page) {
            if (page.hasSearchRenderEvent) {
                $scope.vc.render('VC_SIURM23_BETIO_527');
            }
            return page;
        };
...
if (typeof cobisMainModule === "undefined") {
    var cobisMainModule = cobis.createModule("VC_SIURM23_BETIO_527", ["ngResource", "ngRoute", "kendo.directives", "ui.bootstrap", "oc.lazyLoad", "pascalprecht.translate", cobis.modules.CONTAINER, "designerModule"], ["DSGNR", "BUSIN"]);
    ...
    function($controllerProvider, $compileProvider, $filterProvider, $locationProvider, $routeProvider, $provide, $translateProvider, $translatePartialLoaderProvider, $ocLazyLoadProvider) {
        ...
        $routeProvider.when("/VC_SIURM23_BETIO_527", {
            templateUrl: "VC_SIURM23_BETIO_527_FORM.html",
            controller: "VC_SIURM23_BETIO_527_CTRL",
            label: "FormDisbursementFormsExecution",
            resolve: {
            ...
            redirectTo: function(routeParams, path, search) {
                return "/VC_SIURM23_BETIO_527?" + $.param(search);
            }
        });
        VC_SIURM23_BETIO_527(cobisMainModule);
    }]);
} else {
    VC_SIURM23_BETIO_527(cobisMainModule);
}