...
designerEvents.api.officialactivitycollateralballots = designerEvents.api.officialactivitycollateralballots || designer.dsgEvents();

function VC_ATYEB29_CIATS_707(cobisMainModule) {
    cobisMainModule.controllerProvider.register("VC_ATYEB29_CIATS_707_MAIN_CTRL", ["$scope", "breadcrumbs",

    function($scope, breadcrumbs) {
        $scope.breadcrumbs = breadcrumbs;
    }]);
    cobisMainModule.controllerProvider.register("VC_ATYEB29_CIATS_707_CTRL", ["cobisMessage", cobis.modules.CONTAINER + ".preferencesService", "dsgnrUtils", "designer", "$scope", "$location", "$document", "$parse", "$filter", "$modal", "$q", "$compile", "$timeout",
    ...
    function(cobisMessage, preferencesService, dsgnrUtils, designer, $scope, $location, $document, $parse, $filter, $modal, $q, $compile, $timeout) {
        ...
        $scope.vc = designer.initViewContainer({
            moduleId: "BUSIN",
            subModuleId: "FLCRE",
            taskId: "T_FLCRE_23_ATYEB29",
            taskVersion: "1.0.0",
            viewContainerId: "VC_ATYEB29_CIATS_707",
            hasCloseModalEvent: false,
            serverEvents: true
        },
            "${contextPath}/resources/BUSIN/FLCRE/T_FLCRE_23_ATYEB29",
        designerEvents.api.officialactivitycollateralballots,
        $scope.rowData);
        ...
        if (!$scope.vc.loaded) {
            ...
            $scope.vc.viewState.VC_ATYEB29_CIATS_707 = {
                style: []
            }
            //ViewState - Container: FormOfficialActivityCollateralBallots
            $scope.vc.createViewState({
                id: "VC_ATYEB29_CIATS_707",
                hasId: true,
                componentStyle: "",
                label: 'FormOfficialActivityCollateralBallots',
                enabled: designer.constants.mode.All
            });
            //ViewState - Container: Oficial
            $scope.vc.createViewState({
                id: "VC_ATYEB29_OFIIL_122",
                hasId: true,
                componentStyle: "",
            ...
            }]; //ViewState - Container: AprovedOffitialLoad
            $scope.vc.createViewState({
                id: "VC_ATYEB29_OALAD_811",
                hasId: true,
                componentStyle: "",
                label: 'AprovedOffitialLoad',
                enabled: designer.constants.mode.All
            });
            ...
            $scope.vc.grids.QV_QERIS7170_82.toolbar = [{
                "name": "create",
                "text": "",
                "template": "<button class = 'btn btn-default k-grid-add cb-grid-image-button' " + "ng-if = 'vc.viewState.QV_QERIS7170_82.toolbar.create.visible' " + "ng-disabled = 'vc.viewState.GR_RIOTRDTAVI49_11.disabled?true:false' " + "title=\"{{'DSGNR.SYS_DSGNR_LBLNEW000_00013'|translate}}\"> " + "<span class='glyphicon glyphicon-plus-sign'></span></button>"
            ...
            var unregister = $scope.$watch('vc.afterInitData', function(newValue, oldValue) {
                if (newValue !== oldValue) {
                    unregister();
                    $scope.vc.catalogs.VA_OFICIALOBG2103_OFIL240.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6805_FICE145.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6805_ITCE935.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6805_OINC062.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6805_EDCT992.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6805_RUCE680.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6805_SOET426.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6805_SCOR834.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6805_DSIN268.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6805_IBTO078.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6806_CITE731.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6806_REIT372.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6806_RRQE376.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6806_LIES496.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6806_WNTS030.read();
                    $scope.vc.catalogs.VA_IEWGUARNTK6806_UMRO301.read();
                    ...
                    $scope.vc.catalogs.VA_RIOTRDTAVI4904_TATT517.read();
                    $scope.vc.catalogs.VA_RIOTRDTAVI4904_OUED020.read();
                    $scope.vc.catalogs.VA_RIOTRDTAVI4911_RITT092.read();
                    $scope.vc.catalogs.VA_RIOTRDTAVI4911_CUCY652.read();
                    $scope.vc.catalogs.VA_RIOTRDTAVI4913_CCGR378.read();
                    $scope.vc.catalogs.VA_RIOTRDTAVI4913_ECUE519.read();
                    $scope.vc.catalogs.VA_RIOTRDTAVI4913_NVLX125.read();
                    $scope.vc.catalogs.VA_RIOTRDTAVI4913_CURE278.read();
        ...
        $scope._initPage_ProcessRender = function(page) {
            if (page.hasSearchRenderEvent) {
                $scope.vc.render('VC_ATYEB29_CIATS_707');
            }
            return page;
        };
...
if (typeof cobisMainModule === "undefined") {
    var cobisMainModule = cobis.createModule("VC_ATYEB29_CIATS_707", ["ngResource", "ngRoute", "kendo.directives", "ui.bootstrap", "oc.lazyLoad", "pascalprecht.translate", cobis.modules.CONTAINER, "designerModule"], ["DSGNR", "BUSIN"]);
    ...
    function($controllerProvider, $compileProvider, $filterProvider, $locationProvider, $routeProvider, $provide, $translateProvider, $translatePartialLoaderProvider, $ocLazyLoadProvider) {
        ...
        $routeProvider.when("/VC_ATYEB29_CIATS_707", {
            templateUrl: "VC_ATYEB29_CIATS_707_FORM.html",
            controller: "VC_ATYEB29_CIATS_707_CTRL",
            label: "FormOfficialActivityCollateralBallots",
            resolve: {
            ...
            redirectTo: function(routeParams, path, search) {
                return "/VC_ATYEB29_CIATS_707?" + $.param(search);
            }
        });
        VC_ATYEB29_CIATS_707(cobisMainModule);
    }]);
} else {
    VC_ATYEB29_CIATS_707(cobisMainModule);
}