...
designerEvents.api.taskproductdistributionline = designerEvents.api.taskproductdistributionline || designer.dsgEvents();

function VC_TAKRR10_DIILN_979(cobisMainModule) {
    cobisMainModule.controllerProvider.register("VC_TAKRR10_DIILN_979_MAIN_CTRL", ["$scope", "breadcrumbs",

    function($scope, breadcrumbs) {
        $scope.breadcrumbs = breadcrumbs;
    }]);
    cobisMainModule.controllerProvider.register("VC_TAKRR10_DIILN_979_CTRL", ["cobisMessage", cobis.modules.CONTAINER + ".preferencesService", "dsgnrUtils", "designer", "$scope", "$location", "$document", "$parse", "$filter", "$modal", "$q", "$compile", "$timeout",
    ...
    function(cobisMessage, preferencesService, dsgnrUtils, designer, $scope, $location, $document, $parse, $filter, $modal, $q, $compile, $timeout) {
        ...
        $scope.vc = designer.initViewContainer({
            moduleId: "BUSIN",
            subModuleId: "FLCRE",
            taskId: "T_FLCRE_08_TAKRR10",
            taskVersion: "1.0.0",
            viewContainerId: "VC_TAKRR10_DIILN_979",
            hasCloseModalEvent: false,
            serverEvents: true
        },
            "${contextPath}/resources/BUSIN/FLCRE/T_FLCRE_08_TAKRR10",
        designerEvents.api.taskproductdistributionline,
        $scope.rowData);
        ...
        if (!$scope.vc.loaded) {
            ...
            $scope.vc.viewState.VC_TAKRR10_DIILN_979 = {
                style: []
            }
            //ViewState - Container: FormDistributionLine
            $scope.vc.createViewState({
                id: "VC_TAKRR10_DIILN_979",
                hasId: true,
                componentStyle: "",
                label: 'FormDistributionLine',
                enabled: designer.constants.mode.All
            });
            //ViewState - Container: Grp_HeaderLineView
            $scope.vc.createViewState({
                id: "VC_TAKRR10_UPMBR_028",
                hasId: true,
                componentStyle: "",
                label: 'Grp_HeaderLineView',
                enabled: designer.constants.mode.All
            });
            ...
            //ViewState - Container: Grp_DistributionLine
            $scope.vc.createViewState({
                id: "VC_TAKRR10_RDIRE_658",
                hasId: true,
                componentStyle: "",
                label: 'Grp_DistributionLine',
                enabled: designer.constants.mode.All
            });
            //ViewState - Group: Contenedor Plegable
            $scope.vc.createViewState({
            ...
            }]; //ViewState - Container: Grp_OfficialLoad
            $scope.vc.createViewState({
                id: "VC_TAKRR10_ROFIL_332",
                hasId: true,
                componentStyle: "",
                label: 'Grp_OfficialLoad',
                enabled: designer.constants.mode.All
            });
            ...
            $scope.vc.createViewState({
                id: "T_FLCRE_08_TAKRR10_CANCEL",
                componentStyle: "",
                label: 'Cancel',
                enabled: designer.constants.mode.All,
                visible: designer.constants.mode.All
            });
            //ViewState - Command: CmdSave
            $scope.vc.createViewState({
                id: "CM_TAKRR10DVE80",
                componentStyle: "",
            ...
            var unregister = $scope.$watch('vc.afterInitData', function(newValue, oldValue) {
                if (newValue !== oldValue) {
                    unregister();
                    $scope.vc.catalogs.VA_HEDELNEVIE1304_FICE587.read();
                    $scope.vc.catalogs.VA_HEDELNEVIE1304_ITCE824.read();
                    $scope.vc.catalogs.VA_HEDELNEVIE1304_URQT805.read();
                    $scope.vc.catalogs.VA_HEDELNEVIE1304_NQUE877.read();
                    $scope.vc.catalogs.VA_VIWLNEHADE4804_RTAY467.read();
                    $scope.vc.catalogs.VA_VIWLNEHADE4804_COMT755.read();
                    $scope.vc.catalogs.VA_VIWLNEHADE4804_EPRO969.read();
                    $scope.vc.catalogs.VA_VIWLNEHADE4804_SCTR177.read();
                    $scope.vc.catalogs.VA_VIWLNEHADE4804_RVIC022.read();
        ...
        $scope._initPage_ProcessRender = function(page) {
            if (page.hasSearchRenderEvent) {
                $scope.vc.render('VC_TAKRR10_DIILN_979');
            }
            return page;
        };
...
if (typeof cobisMainModule === "undefined") {
    var cobisMainModule = cobis.createModule("VC_TAKRR10_DIILN_979", ["ngResource", "ngRoute", "kendo.directives", "ui.bootstrap", "oc.lazyLoad", "pascalprecht.translate", cobis.modules.CONTAINER, "designerModule"], ["DSGNR", "BUSIN"]);
    ...
    function($controllerProvider, $compileProvider, $filterProvider, $locationProvider, $routeProvider, $provide, $translateProvider, $translatePartialLoaderProvider, $ocLazyLoadProvider) {
        ...
        $routeProvider.when("/VC_TAKRR10_DIILN_979", {
            templateUrl: "VC_TAKRR10_DIILN_979_FORM.html",
            controller: "VC_TAKRR10_DIILN_979_CTRL",
            label: "FormDistributionLine",
            resolve: {
            ...
            redirectTo: function(routeParams, path, search) {
                return "/VC_TAKRR10_DIILN_979?" + $.param(search);
            }
        });
        VC_TAKRR10_DIILN_979(cobisMainModule);
    }]);
} else {
    VC_TAKRR10_DIILN_979(cobisMainModule);
}