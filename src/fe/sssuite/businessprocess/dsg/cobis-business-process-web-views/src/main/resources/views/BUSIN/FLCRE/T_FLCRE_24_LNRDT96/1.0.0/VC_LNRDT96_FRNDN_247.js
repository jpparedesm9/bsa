...
designerEvents.api.taskrevolvingcreditline = designerEvents.api.taskrevolvingcreditline || designer.dsgEvents();

function VC_LNRDT96_FRNDN_247(cobisMainModule) {
    cobisMainModule.controllerProvider.register("VC_LNRDT96_FRNDN_247_MAIN_CTRL", ["$scope", "breadcrumbs",

    function($scope, breadcrumbs) {
        $scope.breadcrumbs = breadcrumbs;
    }]);
    cobisMainModule.controllerProvider.register("VC_LNRDT96_FRNDN_247_CTRL", ["cobisMessage", cobis.modules.CONTAINER + ".preferencesService", "dsgnrUtils", "designer", "$scope", "$location", "$document", "$parse", "$filter", "$modal", "$q", "$compile", "$timeout",
    ...
    function(cobisMessage, preferencesService, dsgnrUtils, designer, $scope, $location, $document, $parse, $filter, $modal, $q, $compile, $timeout) {
        ...
        $scope.vc = designer.initViewContainer({
            moduleId: "BUSIN",
            subModuleId: "FLCRE",
            taskId: "T_FLCRE_24_LNRDT96",
            taskVersion: "1.0.0",
            viewContainerId: "VC_LNRDT96_FRNDN_247",
            hasCloseModalEvent: false,
            serverEvents: true
        },
            "${contextPath}/resources/BUSIN/FLCRE/T_FLCRE_24_LNRDT96",
        designerEvents.api.taskrevolvingcreditline,
        $scope.rowData);
        ...
        if (!$scope.vc.loaded) {
            ...
            $scope.vc.viewState.VC_LNRDT96_FRNDN_247 = {
                style: []
            }
            //ViewState - Container: FormRevolvingCreditLine
            $scope.vc.createViewState({
                id: "VC_LNRDT96_FRNDN_247",
                hasId: true,
                componentStyle: "",
                label: 'FormRevolvingCreditLine',
                enabled: designer.constants.mode.All
            });
            //ViewState - Container: HeaderLine
            $scope.vc.createViewState({
                id: "VC_LNRDT96_EDEIN_842",
                hasId: true,
                componentStyle: "",
                label: 'HeaderLine',
                enabled: designer.constants.mode.All
            ...
            //ViewState - Container: Distribution
            $scope.vc.createViewState({
                id: "VC_LNRDT96_IUTIO_108",
                hasId: true,
                componentStyle: "",
                label: 'Distribution',
                enabled: designer.constants.mode.All
        ...
        $scope._initPage_ProcessRender = function(page) {
            if (page.hasSearchRenderEvent) {
                $scope.vc.render('VC_LNRDT96_FRNDN_247');
            }
            return page;
        };
...
if (typeof cobisMainModule === "undefined") {
    var cobisMainModule = cobis.createModule("VC_LNRDT96_FRNDN_247", ["ngResource", "ngRoute", "kendo.directives", "ui.bootstrap", "oc.lazyLoad", "pascalprecht.translate", cobis.modules.CONTAINER, "designerModule"], ["DSGNR", "BUSIN"]);
    ...
    function($controllerProvider, $compileProvider, $filterProvider, $locationProvider, $routeProvider, $provide, $translateProvider, $translatePartialLoaderProvider, $ocLazyLoadProvider) {
        ...
        $routeProvider.when("/VC_LNRDT96_FRNDN_247", {
            templateUrl: "VC_LNRDT96_FRNDN_247_FORM.html",
            controller: "VC_LNRDT96_FRNDN_247_CTRL",
            label: "FormRevolvingCreditLine",
            resolve: {
            ...
            redirectTo: function(routeParams, path, search) {
                return "/VC_LNRDT96_FRNDN_247?" + $.param(search);
            }
        });
        VC_LNRDT96_FRNDN_247(cobisMainModule);
    }]);
} else {
    VC_LNRDT96_FRNDN_247(cobisMainModule);
}