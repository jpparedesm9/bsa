...
designerEvents.api.taskofficialanalisysline = designerEvents.api.taskofficialanalisysline || designer.dsgEvents();

function VC_TKOFL79_OICIN_986(cobisMainModule) {
    cobisMainModule.controllerProvider.register("VC_TKOFL79_OICIN_986_MAIN_CTRL", ["$scope", "breadcrumbs",

    function($scope, breadcrumbs) {
        $scope.breadcrumbs = breadcrumbs;
    }]);
    cobisMainModule.controllerProvider.register("VC_TKOFL79_OICIN_986_CTRL", ["cobisMessage", cobis.modules.CONTAINER + ".preferencesService", "dsgnrUtils", "designer", "$scope", "$location", "$document", "$parse", "$filter", "$modal", "$q", "$compile", "$timeout",
    ...
    function(cobisMessage, preferencesService, dsgnrUtils, designer, $scope, $location, $document, $parse, $filter, $modal, $q, $compile, $timeout) {
        ...
        $scope.vc = designer.initViewContainer({
            moduleId: "BUSIN",
            subModuleId: "FLCRE",
            taskId: "T_FLCRE_08_TKOFL79",
            taskVersion: "1.0.0",
            viewContainerId: "VC_TKOFL79_OICIN_986",
            hasCloseModalEvent: false,
            serverEvents: true
        },
            "${contextPath}/resources/BUSIN/FLCRE/T_FLCRE_08_TKOFL79",
        designerEvents.api.taskofficialanalisysline,
        $scope.rowData);
        ...
        if (!$scope.vc.loaded) {
            ...
            $scope.vc.viewState.VC_TKOFL79_OICIN_986 = {
                style: []
            }
            //ViewState - Container: FormOfficialAnalisysLine
            $scope.vc.createViewState({
                id: "VC_TKOFL79_OICIN_986",
                hasId: true,
                componentStyle: "",
                label: 'FormOfficialAnalisysLine',
                enabled: designer.constants.mode.All
            });
            //ViewState - Container: Official
            $scope.vc.createViewState({
                id: "VC_TKOFL79_FCIAL_559",
                hasId: true,
                componentStyle: "",
                label: 'Official',
                enabled: designer.constants.mode.All
            ...
            //ViewState - Container: HeaderLine
            $scope.vc.createViewState({
                id: "VC_TKOFL79_EADER_041",
                hasId: true,
                componentStyle: "",
                label: 'HeaderLine',
                enabled: designer.constants.mode.All
        ...
        $scope._initPage_ProcessRender = function(page) {
            if (page.hasSearchRenderEvent) {
                $scope.vc.render('VC_TKOFL79_OICIN_986');
            }
            return page;
        };
...
if (typeof cobisMainModule === "undefined") {
    var cobisMainModule = cobis.createModule("VC_TKOFL79_OICIN_986", ["ngResource", "ngRoute", "kendo.directives", "ui.bootstrap", "oc.lazyLoad", "pascalprecht.translate", cobis.modules.CONTAINER, "designerModule"], ["DSGNR", "BUSIN"]);
    ...
    function($controllerProvider, $compileProvider, $filterProvider, $locationProvider, $routeProvider, $provide, $translateProvider, $translatePartialLoaderProvider, $ocLazyLoadProvider) {
        ...
        $routeProvider.when("/VC_TKOFL79_OICIN_986", {
            templateUrl: "VC_TKOFL79_OICIN_986_FORM.html",
            controller: "VC_TKOFL79_OICIN_986_CTRL",
            label: "FormOfficialAnalisysLine",
            resolve: {
            ...
            redirectTo: function(routeParams, path, search) {
                return "/VC_TKOFL79_OICIN_986?" + $.param(search);
            }
        });
        VC_TKOFL79_OICIN_986(cobisMainModule);
    }]);
} else {
    VC_TKOFL79_OICIN_986(cobisMainModule);
}