...
task.closeModalEvent.findCustomer = function (args) {
    ...
    if (isGuarantor) {
        ...
        var wPrincipal = true;
        //Determinar si la garantya tiene asociado el cliente principal
...
task.validatePersonalWarranty = function (typeOfGuarantee, viewState) {
    //En la GarantiPersonal se presenta el campo Garante        
...
task.validateDPFWarranty = function (typeOfGuarantee, viewState) {
    //En la GarantPlazo Fijo se presenta el campo Plazo Fijo        
...
task.addPersonalWarrantyList = function (args, isUpdate) {
    //Buscar el id del cliente principal de la garantia
    var customerList = args.commons.api.vc.model.CustomerSearch.data();
    var customerId;
    for (var i = 0; i < customerList.length; i++) {
        if (customerList[i].Principal) {
    ...
    var personalGuarantorData = {
        ...
        CurrentValue: null
        , Currency: null
        , modifyStatus: "N", //se agrega para la historia de modificaci󮠤e garant
        bddStatus: "NO" //se agrega para la historia de modificaci󮠤e garant
    };
    if (isUpdate) {
...
task.addOtherWarrantyList = function (args, isUpdate) {
    //Buscar el id del cliente principal de la garant        
    var customerList = args.commons.api.vc.model.CustomerSearch.data();
    var customerId;
    var customerName;
    for (var i = 0; i < customerList.length; i++) {
