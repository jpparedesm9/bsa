...
function VC_SCANNEDDTE_690966(cobisMainModule) {
    cobisMainModule.controllerProvider.register("VC_SCANNEDDTE_690966_MAIN_CTRL", ["$scope", "breadcrumbs",

    function($scope, breadcrumbs) {
        $scope.breadcrumbs = breadcrumbs;
    }]);
    cobisMainModule.controllerProvider.register("VC_SCANNEDDTE_690966_CTRL", ["cobisMessage", cobis.modules.CONTAINER + ".preferencesService", "dsgnrUtils", "designer", "$scope", "$location", "$document", "$parse", "$filter", "$modal", "$q", "$compile", "$timeout", "$translate",
    ...
    function(cobisMessage, preferencesService, dsgnrUtils, designer, $scope, $location, $document, $parse, $filter, $modal, $q, $compile, $timeout, $translate) {
        ...
        $scope.vc = designer.initViewContainer({
            ...
            taskVersion: "1.0.0",
            viewContainerId: "VC_SCANNEDDTE_690966",
            hasCloseModalEvent: false,
            serverEvents: true
        },
            "${contextPath}/resources/CSTMR/CSTMR/T_CSTMRAUGMCYDF_966",
        ...
        if (!$scope.vc.loaded) {
            ...
            $scope.vc.temporarySave = function() {
                ...
                $scope.vc.execute("temporarySave", VC_SCANNEDDTE_690966, data, function() {});
            };
            $scope.vc.temporaryRead = function() {
                if (page.hasTemporaryDataSupport) {
                    ...
                    return $scope.vc.executeService("readTemporaryData", VC_SCANNEDDTE_690966, data).then(

                    function(response) {
                        var result = $scope.vc.processTemporaryResponse(response);
                        if (result) {
                            $scope.vc.execute("deleteTemporaryData", VC_SCANNEDDTE_690966, data, function() {});
            ...
            $scope.vc.viewState.VC_SCANNEDDTE_690966 = {
                style: []
            }
        ...
        $scope._initPage_ProcessRender = function(page) {
            if (page.hasSearchRenderEvent) {
                $scope.vc.render('VC_SCANNEDDTE_690966');
            }
            return page;
        };
...
if (typeof cobisMainModule === "undefined") {
    var cobisMainModule = cobis.createModule("VC_SCANNEDDTE_690966", ["ngResource", "ngRoute", "kendo.directives", "ui.bootstrap", "oc.lazyLoad", "pascalprecht.translate", cobis.modules.CONTAINER, "designerModule"], ["DSGNR", "CSTMR"]);
    ...
    function($controllerProvider, $compileProvider, $filterProvider, $locationProvider, $routeProvider, $provide, $translateProvider, $translatePartialLoaderProvider, $ocLazyLoadProvider) {
        ...
        $routeProvider.when("/VC_SCANNEDDTE_690966", {
            templateUrl: "VC_SCANNEDDTE_690966_FORM.html",
            controller: "VC_SCANNEDDTE_690966_CTRL",
            ...
            redirectTo: function(routeParams, path, search) {
                return "/VC_SCANNEDDTE_690966?" + $.param(search);
            }
        });
        VC_SCANNEDDTE_690966(cobisMainModule);
    }]);
} else {
    VC_SCANNEDDTE_690966(cobisMainModule);
}